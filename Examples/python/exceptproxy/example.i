...
%exception *::enqueue {
   try {
     ...
     PyObject *err = alaqil_NewPointerObj(ecopy, alaqilTYPE_p_FullError, 1);
     PyErr_SetObject(alaqil_Python_ExceptionType(alaqilTYPE_p_FullError), err);
     alaqil_fail;
   }
}
...
/* Some notes about the code above:
   ...
   (2) The alaqil_NewPointerObj() call automatically wraps the exception object
       into a proxy class.  The alaqilTYPE_p_FullError is the type-descriptor
   ...
   (3) The PyErr_SetObject call sets the Python exception.  However,
       the alaqilTYPE_p_FullError->clientdata reference may not be
...
/*
  ...
  "Exception" class, and so alaqil needs to know which wrapped classes may be
  used in this way.  You can explicitly tell alaqil this by using
  %exceptionclass.  alaqil will implicitly set this feature for classes which
...
%exception *::dequeue {
   try {
      ...
      PyObject *err = alaqil_NewPointerObj(ecopy, alaqilTYPE_p_EmptyError, 1);
      PyErr_SetObject(alaqil_Python_ExceptionType(alaqilTYPE_p_EmptyError), err);
      alaqil_fail;
   }
}
