require 'alaqil_assert'
require 'cpp11_hash_tables'

[Cpp11_hash_tables::MapIntInt.new({1=>7}),
 Cpp11_hash_tables::MultiMapIntInt.new({1=>7}),
 Cpp11_hash_tables::UnorderedMapIntInt.new({1=>7}),
 Cpp11_hash_tables::UnorderedMultiMapIntInt.new({1=>7})].each{|x|

  alaqil_assert_equal("x.find_all{|e,k| e == 1}", "[[1,7]]", binding)

  alaqil_assert_equal("x[1]", "7", binding)
  alaqil_assert_equal("x[2]", "nil", binding)
  x[2] = 9
  alaqil_assert_equal("x[2]", "9", binding)
  x.delete(2)
  alaqil_assert_equal("x[2]", "nil", binding)
  alaqil_assert_equal("x.empty?", "false", binding)
  x.delete(1)
  alaqil_assert_equal("x.empty?", "true", binding)
  alaqil_assert_equal("x.include?(1)", "false", binding)
}

[Cpp11_hash_tables::MultiMapIntInt.new({1=>7}),
 Cpp11_hash_tables::UnorderedMultiMapIntInt.new({1=>7})].each{|x|
  x[1] = 9
  alaqil_assert_equal("x[1].sort", "[7,9]", binding)
}

[Cpp11_hash_tables::SetInt.new([1]),
 Cpp11_hash_tables::MultiSetInt.new([1]),
 Cpp11_hash_tables::UnorderedSetInt.new([1]),
 Cpp11_hash_tables::UnorderedMultiSetInt.new([1])].each{|x|

  alaqil_assert_equal("x.find_all{|e| e == 1}", "[1]", binding)

  alaqil_assert_equal("x.include?(1)", "true", binding)
  alaqil_assert_equal("x.include?(2)", "false", binding)
  x << 2
  alaqil_assert_equal("x.include?(2)", "true", binding)
  x.erase(2)
  alaqil_assert_equal("x.empty?", "false", binding)
  x.erase(1)
  alaqil_assert_equal("x.empty?", "true", binding)
}

[Cpp11_hash_tables::MultiSetInt.new([1]),
 Cpp11_hash_tables::UnorderedMultiSetInt.new([1])].each{|x|
  x << 1
  alaqil_assert_equal("x.count(1)", "2", binding)
}

[Cpp11_hash_tables::MapIntInt,
 Cpp11_hash_tables::MultiMapIntInt,
 Cpp11_hash_tables::UnorderedMapIntInt,
 Cpp11_hash_tables::UnorderedMultiMapIntInt,
 Cpp11_hash_tables::SetInt,
 Cpp11_hash_tables::MultiSetInt,
 Cpp11_hash_tables::UnorderedSetInt,
 Cpp11_hash_tables::UnorderedMultiSetInt].each{|k|
  alaqil_assert("k.include?(Enumerable)", binding)
}
