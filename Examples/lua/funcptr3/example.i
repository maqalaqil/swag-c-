...
#include "example.h"
%}
/* the extra wrappers for lua functions, see alaqil/Lib/lua/lua_fnptr.i for more details */
%include "lua_fnptr.i"
...
int callback(int a, int b, alaqilLUA_FN fn)
{
	alaqilLUA_FN_GET(fn);
...
/* note: this is not so good to just have it as a raw ref
 ...
 a better solution would to be to have a fn which wants a alaqilLUA_FN, then
 checks the type & converts to a alaqilLUA_REF.
*/	
alaqilLUA_REF the_func={0,0};
