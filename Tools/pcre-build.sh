...
pcre_subdir=pcre/pcre-alaqil-install
...
usage() {
  ...
  echo "alaqil build. It does not install PCRE for global use on your system."
  ...
  echo "    directory that you will configure and build alaqil."
  echo "  - Run this script in the same directory that you intend to configure and build alaqil in."
  echo "    This will configure and build PCRE as a static library."
  echo "  - Afterwards run the alaqil configure script which will then find and use the PCRE static"
...
echo ""
echo "The alaqil configure script can now be run, whereupon PCRE will automatically be detected and used from $pcre_install_dir/bin/pcre-config."
