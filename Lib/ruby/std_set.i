...
%define %alaqil_sequence_methods_extra_set(Sequence...)
  %extend {
	...
	rb_raise( rb_eArgError, "no block given" );
      ...
      for ( Sequence::iterator i = $self->begin(); i != $self->end(); ) {
        VALUE r = alaqil::from< Sequence::value_type >(*i);
...
%define %alaqil_set_methods(set...)
  ...
  %alaqil_sequence_methods_common(%arg(set));
  %alaqil_sequence_methods_extra_set(%arg(set));

  %fragment("RubyPairBoolOutputIterator","header",fragment=alaqil_From_frag(bool),fragment="RubySequence_Cont") {}

// Redefine std::set iterator/reverse_iterator typemap
%typemap(out,noblock=1) iterator, reverse_iterator {
  $result = alaqil_NewPointerObj(alaqil::make_set_nonconst_iterator(%static_cast($1,const $type &),
								self),
...
// Redefine std::set std::pair<iterator, bool> typemap
  ...
  std::pair<iterator, bool> {
    ...
    rb_ary_push($result, alaqil_NewPointerObj(alaqil::make_set_nonconst_iterator(%static_cast($1,$type &).first),
                                            alaqil::Iterator::descriptor(),alaqil_POINTER_OWN));
    rb_ary_push($result, alaqil_From(bool)(%static_cast($1,const $type &).second));
   }
