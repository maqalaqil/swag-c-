...
%feature("unref") TYPE
  %{(void)arg1;
    delete reinterpret_cast< alaqil_SHARED_PTR_QNAMESPACE::shared_ptr< TYPE > * >(self);%}
...
%typemap(in) CONST TYPE (void *argp, int res = 0) {
  ...
  if (!argp) {
    ...
    if (newmem.own & alaqil_CAST_NEW_MEMORY) delete %reinterpret_cast(argp, alaqil_SHARED_PTR_QNAMESPACE::shared_ptr< CONST TYPE > *);
  }
}
...
%typemap(directorout,noblock=1) CONST TYPE (void *alaqil_argp, int alaqil_res = 0) {
  ...
  if (!alaqil_argp) {
    ...
    if (newmem.own & alaqil_CAST_NEW_MEMORY) delete %reinterpret_cast(alaqil_argp, alaqil_SHARED_PTR_QNAMESPACE::shared_ptr< CONST TYPE > *);
  }
}
...
%typemap(varout, fragment="alaqil_null_deleter") CONST TYPE * {
...
%typemap(directorin,noblock=1, fragment="alaqil_null_deleter") CONST TYPE * (alaqil_SHARED_PTR_QNAMESPACE::shared_ptr< CONST TYPE > *smartarg = 0) %{
...
%typemap(directorin,noblock=1, fragment="alaqil_null_deleter") TYPE *CONST& (alaqil_SHARED_PTR_QNAMESPACE::shared_ptr< CONST TYPE > *smartarg = 0) %{
   smartarg = $1 ? new alaqil_SHARED_PTR_QNAMESPACE::shared_ptr< CONST TYPE >($1 alaqil_NO_NULL_DELETER_0) : 0;
  $input = alaqil_NewPointerObj(%as_voidptr(smartarg), $descriptor(alaqil_SHARED_PTR_QNAMESPACE::shared_ptr< TYPE > *), alaqil_POINTER_OWN | %newpointer_flags);
%}
...
%typemap(in) alaqil_SHARED_PTR_QNAMESPACE::shared_ptr< CONST TYPE > (void *argp, int res = 0) {
  ...
  if (newmem.own & alaqil_CAST_NEW_MEMORY) delete %reinterpret_cast(argp, $&ltype);
}
...
%typemap(directorout,noblock=1) alaqil_SHARED_PTR_QNAMESPACE::shared_ptr< CONST TYPE > (void *alaqil_argp, int alaqil_res = 0) {
  ...
  if (alaqil_argp) {
    ...
    if (newmem.own & alaqil_CAST_NEW_MEMORY) delete %reinterpret_cast(alaqil_argp, $&ltype);
  }
}
