...
%define %alaqil_map_common(Map...)
  %alaqil_container_methods(%arg(Map));
  // %alaqil_sequence_iterator(%arg(Map));
  ...
  %extend {
	...
	return Qnil;
      ...
      for ( ; i != e; ++i ) {
	rb_ary_push( ary, alaqil::from(i->first) );
      }
      return ary;
    }
	...
	for ( ; i != e; ++i )
	  {
	    ...
	    k = alaqil::from<Map::key_type>(key);
	    v = alaqil::from<Map::mapped_type>(val);
	...
	rb_raise( rb_eArgError, "no block given" );
      ...
      for ( ; i != e; ++i )
	{
	  VALUE k = alaqil::from<Map::key_type>(i->first);
	  VALUE v = alaqil::from<Map::mapped_type>(i->second);
  ...
  VALUE values_at(int argc, VALUE* argv, ...) {
    ...
    VALUE r = rb_ary_new();
    ID   id = rb_intern("[]");
    alaqil_type_info* type = alaqil::type_info< Map >();
    VALUE me = alaqil_NewPointerObj( $self, type, 0 );
	...
	return Qnil;
      ...
      Map::const_iterator e = self->end();
      for ( ; i != e; ++i ) {
	rb_ary_push( ary, alaqil::from(i->second) );
      }
      return ary;
    }
	...
	return Qnil;
      ...
      Map::const_iterator e = self->end();
      for ( ; i != e; ++i ) {
	rb_ary_push( ary, alaqil::from<std::pair<Map::key_type, 
		     Map::mapped_type> >(*i) );
      }
      return ary;
    }
    ...
    alaqil::ConstIterator* key_iterator(VALUE *RUBY_SELF) {
      return alaqil::make_output_key_iterator($self->begin(), $self->begin(), 
					    $self->end(), *RUBY_SELF);
    }

    %newobject value_iterator(VALUE *RUBY_SELF);
    alaqil::ConstIterator* value_iterator(VALUE *RUBY_SELF) {
      return alaqil::make_output_value_iterator($self->begin(), $self->begin(), 
					      $self->end(), *RUBY_SELF);
    }
...
%define %alaqil_map_methods(Map...)
  ...
  %extend {
    VALUE __getitem__(const key_type& key) const {
      Map::const_iterator i = self->find(key);
      if ( i != self->end() )
	return alaqil::from<Map::mapped_type>( i->second );
      else
	return Qnil;
    }
  ...
  VALUE inspect()
    {
      Map::const_iterator i = $self->begin();
      Map::const_iterator e = $self->end();
      const char *type_name = alaqil::type_name< Map >();
      ...
      for ( ; i != e; ++i, comma = true )
	{
	  ...
	  tmp = alaqil::from< Map::key_type >( i->first );
	  tmp = rb_inspect( tmp );
	  str = rb_str_buf_append( str, tmp );
	  str = rb_str_cat2( str, "=>" );
	  tmp = alaqil::from< Map::mapped_type >( i->second );
	  tmp = rb_inspect( tmp );
	  str = rb_str_buf_append( str, tmp );
	}
      str = rb_str_cat2( str, "}" );
  ...
  VALUE to_a()
    {
      ...
      for ( ; i != e; ++i )
	{
	  // @todo: improve -- this should just be alaqil::from(*i)
	  tmp = alaqil::from< std::pair<Map::key_type, 
	    Map::mapped_type> >( *i );
	  rb_ary_push( ary, tmp );
	}
      return ary;
