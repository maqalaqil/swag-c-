...
but instead is an approximate, so that alaqil knows how to wrapper it.
(Eg, all access is by value, not ref, as alaqil turns refs to pointers)
...
template<class T>
std::vector<T>* alaqil_read_number_vector(lua_State* L,int index)
{
	int i=0;
...
template<class T>
int alaqil_write_number_vector(lua_State* L,std::vector<T> *vec)
{
	lua_newtable(L);
...
%define alaqil_TYPEMAP_NUM_VECTOR(T)

// in
%typemap(in) std::vector<T> *INPUT
%{	$1 = alaqil_read_number_vector<T>(L,$input);
	if (!$1) alaqil_fail;%}
...
%{	alaqil_write_number_vector(L,$1); alaqil_arg++; %}
