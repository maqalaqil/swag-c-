...
namespace std {
  ...
  public:
    ...
    %fragment(alaqil_Traits_frag(std::multiset< _Key, _Compare, _Alloc >), "header",
    ...
    %typemap_traits_ptr(alaqil_TYPECHECK_MULTISET, std::multiset< _Key, _Compare, _Alloc >);

    multiset(const _Compare& other);

#ifdef %alaqil_multiset_methods
    // Add alaqil/language extra methods
    %alaqil_multiset_methods(std::multiset< _Key, _Compare, _Alloc >);
#endif
