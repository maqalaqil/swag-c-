...
namespace std {
  ...
  public:
    ...
    %fragment(alaqil_Traits_frag(std::list< _Tp, _Alloc >), "header",
    ...
    %typemap_traits_ptr(alaqil_TYPECHECK_LIST, std::list< _Tp, _Alloc >);

#ifdef %alaqil_list_methods
    // Add alaqil/language extra methods
    %alaqil_list_methods(std::list< _Tp, _Alloc >);
#endif
...
#ifdef %alaqil_list_methods_val
    // Add alaqil/language extra methods
    %alaqil_list_methods_val(std::list< _Tp*, _Alloc >);
#endif
