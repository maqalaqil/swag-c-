...
namespace std {
  ...
  public:
    ...
    %fragment(alaqil_Traits_frag(std::set< _Key, _Compare, _Alloc >), "header",
    ...
    %typemap_traits_ptr(alaqil_TYPECHECK_SET, std::set< _Key, _Compare, _Alloc >);

    set(const _Compare& other);

#ifdef %alaqil_set_methods
    // Add alaqil/language extra methods
    %alaqil_set_methods(std::set< _Key, _Compare, _Alloc >);
#endif
