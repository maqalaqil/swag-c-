...
#ifdef alaqil_EXPORT_ITERATOR_METHODS
%extend {
...
namespace std {
  ...
  public:
    ...
    %traits_alaqiltype(_Key);
    %traits_alaqiltype(_Tp);	    

    %fragment(alaqil_Traits_frag(std::pair< _Key, _Tp >), "header",
	      fragment=alaqil_Traits_frag(_Key),
    ...
    %fragment(alaqil_Traits_frag(std::unordered_map< _Key, _Tp, _Hash, _Pred, _Alloc >), "header",
	      fragment=alaqil_Traits_frag(std::pair< _Key, _Tp >),
    ...
    %typemap_traits_ptr(alaqil_TYPECHECK_MAP, std::unordered_map< _Key, _Tp, _Hash, _Pred, _Alloc >);

#ifdef %alaqil_unordered_map_methods
    // Add alaqil/language extra methods
    %alaqil_unordered_map_methods(std::unordered_map< _Key, _Tp, _Hash, _Pred, _Alloc >);
#endif
