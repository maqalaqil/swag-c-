...
namespace std {
  ...
  public:
    ...
    %fragment(alaqil_Traits_frag(std::queue< _Tp, _Sequence >), "header",
    ...
    %typemap_traits_ptr(alaqil_TYPECHECK_QUEUE, std::queue< _Tp, _Sequence >);
  
#ifdef %alaqil_queue_methods
    // Add alaqil/language extra methods
    %alaqil_queue_methods(std::queue< _Tp, _Sequence >);
#endif
...
#ifdef %alaqil_queue_methods_val
    // Add alaqil/language extra methods
    %alaqil_queue_methods_val(std::queue< _Tp*, _Sequence >);
#endif
