...
//
//#define alaqil_STD_NOMODERN_STL
...
#  if !defined(alaqil_NO_STD_NOITERATOR_TRAITS_STL)
#    define alaqil_STD_NOITERATOR_TRAITS_STL
#  endif
#endif

#if !defined(alaqil_STD_NOITERATOR_TRAITS_STL)
#include <iterator>
#else
namespace std {
...
namespace alaqil {
  ...
  */
  ...
  template <class Type> struct traits_info {
    static alaqil_type_info *type_query(std::string name) {
      name += " *";
      return alaqil_TypeQuery(name.c_str());
    }
    static alaqil_type_info *type_info() {
      static alaqil_type_info *info = type_query(type_name<Type>());
      return info;
    }
  };
  ...
  */
  ...
  template <class Type>
  inline alaqil_type_info *type_info() {
  ...
  */
  template <class Type> struct traits <Type *> {
    ...
    static const char* type_name() {
      static std::string name = make_ptr_name(alaqil::type_name<Type>());
      return name.c_str();
    }
  };
...
/*
  Generate the traits for a alaqiltype
*/

%define %traits_alaqiltype(Type...)
%fragment(alaqil_Traits_frag(Type),"header",fragment="StdTraits") {
  namespace alaqil {
...
%define %typemap_traits(Code,Type...)
  %typemaps_asvalfrom(%arg(Code),
		     %arg(alaqil::asval< Type >),
		     %arg(alaqil::from),
		     %arg(alaqil_Traits_frag(Type)),
		     %arg(alaqil_Traits_frag(Type)),
		     Type);
%enddef
