...
namespace std {
  ...
  public:
    ...
    %fragment(alaqil_Traits_frag(std::array< _Tp, _Nm >), "header",
    ...
    %typemap_traits_ptr(alaqil_TYPECHECK_STDARRAY, std::array< _Tp, _Nm >);

#ifdef %alaqil_array_methods
    // Add alaqil/language extra methods
    %alaqil_array_methods(std::array< _Tp, _Nm >);
#endif
