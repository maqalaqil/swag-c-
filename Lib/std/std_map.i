...
namespace std {
  ...
  public:
    ...
    %fragment(alaqil_Traits_frag(std::map< _Key, _Tp, _Compare, _Alloc >), "header",
    ...
    %typemap_traits_ptr(alaqil_TYPECHECK_MAP, std::map< _Key, _Tp, _Compare, _Alloc >);

    map(const _Compare& other);

#ifdef %alaqil_map_methods
    // Add alaqil/language extra methods
    %alaqil_map_methods(std::map< _Key, _Tp, _Compare, _Alloc >);
#endif
