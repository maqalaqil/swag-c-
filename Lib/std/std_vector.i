...
namespace std {
  ...
  public:
    ...
    %traits_alaqiltype(_Tp);
    %traits_enum(_Tp);

    %fragment(alaqil_Traits_frag(std::vector< _Tp, _Alloc >), "header",
	      fragment=alaqil_Traits_frag(_Tp),
    ...
    %typemap_traits_ptr(alaqil_TYPECHECK_VECTOR, std::vector< _Tp, _Alloc >);

#ifdef %alaqil_vector_methods
    // Add alaqil/language extra methods
    %alaqil_vector_methods(std::vector< _Tp, _Alloc >);
#endif
  ...
  // a 'const alaqilTYPE*&' can be defined
  // ***
...
#ifdef %alaqil_vector_methods_val
    // Add alaqil/language extra methods
    %alaqil_vector_methods_val(std::vector< _Tp*, _Alloc >);
#endif
  ...
  public:
    ...
    %fragment(alaqil_Traits_frag(std::vector< _Tp const*, _Alloc >), "header",
	      fragment=alaqil_Traits_frag(_Tp),
	      fragment="StdVectorTraits") {
      namespace alaqil {
    ...
    %typemap_traits_ptr(alaqil_TYPECHECK_VECTOR, std::vector< _Tp const*, _Alloc >);

#ifdef %alaqil_vector_methods_val
    // Add alaqil/language extra methods
    %alaqil_vector_methods_val(std::vector< _Tp const*, _Alloc >);
#endif
  ...
  public:
    ...
    %traits_alaqiltype(bool);

    %fragment(alaqil_Traits_frag(std::vector<bool, _Alloc >), "header",
	      fragment=alaqil_Traits_frag(bool),
	      fragment="StdVectorTraits") {
      namespace alaqil {
    ...
    %typemap_traits_ptr(alaqil_TYPECHECK_VECTOR, std::vector<bool, _Alloc >);


#ifdef %alaqil_vector_methods_val
    // Add alaqil/language extra methods
    %alaqil_vector_methods_val(std::vector<bool, _Alloc >);
#endif

    %std_vector_methods_val(vector);

#if defined(alaqil_STD_MODERN_STL) && !defined(alaqil_STD_NOMODERN_STL) 
    void flip();
#endif
