 ...
 * typemaps.i.
 *
 * alaqil Typemap library for PHP.
 *
 * This library provides standard typemaps for modifying alaqil's behavior.
...
%{
  if (force$argnum) {  /* pass back arg$argnum through params ($arg) if we can */
    if (!Z_ISREF($arg)) {
      alaqil_PHP_Error(E_WARNING, "Parameter $argnum of $symname wasn't passed by reference");
    } else {
      alaqil_SetPointerZval(*$arg, (void *) ptr$argnum, $*1_descriptor, 1);
    }
  }
%}
