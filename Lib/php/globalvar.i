...
%typemap(varinit) alaqilTYPE *, alaqilTYPE []
{
  zval z_var;
  alaqil_SetPointerZval(&z_var, (void*)$1, $1_descriptor, 0);
  zend_hash_str_add(&EG(symbol_table), "$1", sizeof("$1") - 1, &z_var);
}

%typemap(varinit) alaqilTYPE, alaqilTYPE &, alaqilTYPE &&
{
  zval z_var;
  alaqil_SetPointerZval(&z_var, (void*)&$1, $&1_descriptor, 0);
  zend_hash_str_add(&EG(symbol_table), "$1", sizeof("$1") - 1, &z_var);
...
%typemap(varinit, fragment="alaqil_php_init_member_ptr") alaqilTYPE (CLASS::*)
{
  zval resource;
  void * p = emalloc(sizeof($1));
  memcpy(p, &$1, sizeof($1));
  ZVAL_RES(&resource, zend_register_resource(p, alaqil_member_ptr));
  zend_hash_str_add(&EG(symbol_table), "$1", sizeof("$1") - 1, &resource);
}

%typemap(varin) int, unsigned int, short, unsigned short, long, unsigned long, signed char, unsigned char,  enum alaqilTYPE
{
...
%typemap(varin) alaqilTYPE []
{
  if ($1) {
    zval *z_var = zend_hash_str_find(&EG(symbol_table), "$1", sizeof("$1") - 1);
    alaqil_SetPointerZval(z_var, (void*)$1, $1_descriptor, $owner);
  }
}
...
%typemap(varin) alaqilTYPE
{
  ...
  if (alaqil_ConvertPtr(z_var, (void**)&_temp, $&1_descriptor, 0) < 0) {
    alaqil_PHP_Error(E_ERROR,"Type error in value of $symname. Expected $&1_descriptor");
  }

  $1 = *($&1_ltype)_temp;
}

%typemap(varin) alaqilTYPE *, alaqilTYPE &, alaqilTYPE &&
{
  zval *z_var;
  $1_ltype _temp;
...
%typemap(varin, fragment="alaqil_php_init_member_ptr") alaqilTYPE (CLASS::*)
{
  zval *z_var = zend_hash_str_find(&EG(symbol_table), "$1", sizeof("$1") - 1);
  void * p = (void*)zend_fetch_resource_ex(z_var, alaqil_MEMBER_PTR, alaqil_member_ptr);
  memcpy(&$1, p, sizeof($1));
}
...
%typemap(varout) alaqilTYPE *, alaqilTYPE &, alaqilTYPE &&
{
  zval *z_var = zend_hash_str_find(&EG(symbol_table), "$1", sizeof("$1") - 1);
  alaqil_SetPointerZval(z_var, (void*)$1, $1_descriptor, 0);
}

%typemap(varout, fragment="alaqil_php_init_member_ptr") alaqilTYPE (CLASS::*)
{
  zval resource;
  void * p = emalloc(sizeof($1));
  memcpy(p, &$1, sizeof($1));
