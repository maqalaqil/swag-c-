...
%{
  /* First Check for alaqil wrapped type */
  if (Z_ISNULL($input)) {
      $1 = 0;
  } else if (Z_ISREF($input)) {
      /* Not alaqil wrapped type, so we check if it's a PHP reference type */
      CONVERT_IN(tmp, $*1_ltype, $input);
      $1 = &tmp;
  } else {
      alaqil_PHP_Error(E_ERROR, alaqil_PHP_Arg_Error_Msg($argnum, Expected a reference));
  }
%}
