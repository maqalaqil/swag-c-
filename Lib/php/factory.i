...
if (!dcast) {
  ...
  if (dobj) {
    dcast = 1;
    alaqil_SetPointerZval(return_value, alaqil_as_voidptr(dobj),$descriptor(Type *), $owner);
  }   
}%enddef
...
%typemap(out) Method {
  ...
  if (!dcast) {
    alaqil_SetPointerZval(return_value, alaqil_as_voidptr($1),$descriptor, $owner);
  }
}%enddef
