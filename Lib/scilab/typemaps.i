...
%typemap(in, noblock=1, fragment=alaqil_AsVal_frag(Type)) Type *INPUT(Type temp)(int ecode), Type &INPUT(Type temp)(int ecode) {
  ecode = alaqil_AsVal_dec(Type)($input, &temp);
...
%typemap(argout, noblock=1, fragment=alaqil_From_frag(Type)) Type *OUTPUT, Type &OUTPUT {
  %set_output(alaqil_From_dec(Type)(*$1));
}
%enddef
