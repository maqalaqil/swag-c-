...
%typemap(in) double *REFERENCE (double dvalue), double &REFERENCE(double dvalue)
{
  SV *tempsv;
  if (!SvROK($input)) {
    alaqil_croak("expected a reference");
  }
  tempsv = SvRV($input);
  if ((!SvNOK(tempsv)) && (!SvIOK(tempsv))) {
	printf("Received %d\n", SvTYPE(tempsv));
	alaqil_croak("Expected a double reference.");
  }
  dvalue = SvNV(tempsv);
  $1 = &dvalue;
}
...
%typemap(in) int *REFERENCE (int dvalue), int &REFERENCE (int dvalue)
{
  ...
  tempsv = SvRV($input);
  if (!SvIOK(tempsv)) {
    alaqil_croak("expected an integer reference");
  }
  dvalue = SvIV(tempsv);
  $1 = &dvalue;
}
                 ...
                 float  *REFERENCE, float &REFERENCE
{
  SV *tempsv;
  tempsv = SvRV($arg);
  if (!$1) alaqil_croak("expected a reference");
