 ...
 * std_vector.i
 *
 * alaqil typemaps for std::vector types
 * ----------------------------------------------------------------------------- */
...
namespace std {
        ...
        %typemap(in) vector<T> (std::vector<T>* v) {
            if (alaqil_ConvertPtr($input,(void **) &v, 
                ...
                if (SvTYPE(av) != SVt_PVAV)
                    alaqil_croak("Type error in argument $argnum of $symname. "
                ...
                for (int i=0; i<len; i++) {
                    tv = av_fetch(av, i, 0);
                    if (alaqil_ConvertPtr(*tv, (void **)&obj, 
                                        $descriptor(T *),0) != -1) {
                        $1.push_back(*obj);
                    } else {
                        alaqil_croak("Type error in argument $argnum of "
                                   "$symname. "
                                   "Expected an array of " #T);
                    }
                }
    ...
    template<class T> class vector<T*> {
        %typemap(in) vector<T*> (std::vector<T*>* v) {
	    int res = alaqil_ConvertPtr($input,(void **) &v, $&1_descriptor,0);
            if (alaqil_IsOK(res)){
                $1 = *v;
            } else if (SvROK($input)) {
                AV *av = (AV *)SvRV($input);
		    ...
		    int res = alaqil_ConvertPtr(*tv, &v, $descriptor(T *),0);
                    if (alaqil_IsOK(res)) {
        ...
        %typemap(out) vector<T *> {
            ...
            for (size_t i=0; i<len; i++) {
                T *x = (($1_type &)$1)[i];
                svs[i] = sv_newmortal();
		sv_setsv(svs[i], alaqil_NewPointerObj(x, $descriptor(T *), 0));
            }
            AV *myav = av_make(len, svs);
            delete[] svs;
            $result = newRV_noinc((SV*) myav);
        ...
        %typecheck(alaqil_TYPECHECK_VECTOR) const vector<T *>&,const vector<T *>* {
            {
                /* wrapped vector? */
                std::vector< T *> *v;
		int res = alaqil_ConvertPtr($input,%as_voidptrptr(&v), $1_descriptor,0);
                if (alaqil_IsOK(res)) {
                    $1 = 1;
                } else if (SvROK($input)) {
                    /* native sequence? */
    ...
    specialize_std_vector(float,SvNIOK,alaqilSvToNumber,sv_setnv);
    specialize_std_vector(double,SvNIOK,alaqilSvToNumber,sv_setnv);
    specialize_std_vector(std::string,SvPOK,alaqilSvToString,alaqilSvFromString);
}
