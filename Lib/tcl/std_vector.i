...
Tcl_Obj* alaqilString_FromString(const std::string &s) {
...
int alaqilString_AsString(Tcl_Interp *interp, Tcl_Obj *o, std::string *val) {
    int len;
...
int alaqilInt_As(Tcl_Interp *interp, Tcl_Obj *o, Type *val) {
...
template <typename Type>
int alaqilDouble_As(Tcl_Interp *interp, Tcl_Obj *o, Type *val) {
    int return_val;
    double temp_val;
...
namespace std {
    ...
    template<class T> class vector {
        %typemap(in) vector< T > (std::vector< T > *v) {
            ...
            if (alaqil_ConvertPtr($input, (void **) &v, \
                ...
                $1 = std::vector< T >();
                for (i = 0; i < nitems; i++) {
                    if ((alaqil_ConvertPtr(listobjv[i],(void **) &temp,
                                         $descriptor(T *),0)) != 0) {
                        char message[] = 
                            "list of " #T " expected";
        ...
        %typecheck(alaqil_TYPECHECK_VECTOR) vector< T > {
            ...
            if(alaqil_ConvertPtr($input, (void **) &v, \
                ...
                //check the first value to see if it is of correct type
                    else if ((alaqil_ConvertPtr(listobjv[0],
                                              (void **) &temp, 
                                              $descriptor(T *),0)) != 0)
                        $1 = 0;
                    else
    ...
    specialize_std_vector(char, alaqilInt_As<char>,Tcl_NewIntObj);
    specialize_std_vector(int, Tcl_GetIntFromObj,Tcl_NewIntObj);
    specialize_std_vector(short, alaqilInt_As<short>, Tcl_NewIntObj);
    specialize_std_vector(long, alaqilInt_As<long>, Tcl_NewIntObj);
    specialize_std_vector(unsigned char, 
                          alaqilInt_As<unsigned char>, Tcl_NewIntObj);
    specialize_std_vector(unsigned int, 
                          alaqilInt_As<unsigned int>, Tcl_NewIntObj);
    specialize_std_vector(unsigned short, 
                          alaqilInt_As<unsigned short>, Tcl_NewIntObj);
    specialize_std_vector(unsigned long, 
                          alaqilInt_As<unsigned long>, Tcl_NewIntObj);
    specialize_std_vector(double, Tcl_GetDoubleFromObj, Tcl_NewDoubleObj);
    specialize_std_vector(float, alaqilDouble_As<float>, Tcl_NewDoubleObj);
    specialize_std_vector(std::string, 
                          alaqilString_AsString, alaqilString_FromString);
