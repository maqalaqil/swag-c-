...
namespace std {
    ...
    template<class T> class vector {
        ...
        %typemap(in) const vector<T>& (std::vector<T> temp),
            ...
            if (SCHEME_VECTORP($input)) {
                ...
                Scheme_Object** items = SCHEME_VEC_ELS($input);
                for (unsigned int i=0; i<size; i++) {
                    temp[i] = *((T*) alaqil_MustGetPtr(items[i],
                                                     $descriptor(T *),
                                                     $argnum, 0));
                }
            } else if (SCHEME_NULLP($input)) {
        ...
        %typemap(out) vector<T> {
            ...
            for (unsigned int i=0; i<$1.size(); i++) {
                ...
                els[i] = alaqil_NewPointerObj(x,$descriptor(T *), 1);
            }
        }
        %typecheck(alaqil_TYPECHECK_VECTOR) vector<T> {
            ...
            if (SCHEME_VECTORP($input)) {
                ...
                if (size == 0) {
                    ...
                    T* x;
                    Scheme_Object** items = SCHEME_VEC_ELS($input);
                    if (alaqil_ConvertPtr(items[0],(void**) &x,
                                    $descriptor(T *), 0) != -1)
                        $1 = 1;
                    else
                        $1 = 0;
    ...
    template<> class vector<T> {
        ...
        %typemap(in) const vector<T>& (std::vector<T> temp),
            ...
            if (SCHEME_VECTORP($input)) {
                ...
                $1 = ($1_ltype) alaqil_MustGetPtr($input,$1_descriptor,$argnum - 1, 0);
            }
        }
        %typemap(out) vector<T> {
            $result = scheme_make_vector($1.size(),scheme_undefined);
    ...
    specialize_std_vector(double,SCHEME_REALP,scheme_real_to_double,\
                          scheme_make_double);
    specialize_std_vector(std::string,SCHEME_STRINGP,alaqil_scm_to_string,\
                          alaqil_make_string);
