...
namespace std {
    ...
    template<class T, class U> struct pair {
        ...
        %typemap(out) pair<T,U> {
            T* x = new T($1.first);
            U* y = new U($1.second);
            Scheme_Object* first = alaqil_NewPointerObj(x,$descriptor(T *), 1);
            Scheme_Object* second = alaqil_NewPointerObj(y,$descriptor(U *), 1);
        ...
        %typecheck(alaqil_TYPECHECK_PAIR) pair<T,U> {
            ...
            if (SCHEME_PAIRP($input)) {
                ...
                /* wrapped pair? */
                std::pair<T,U >* p;
                if (alaqil_ConvertPtr($input,(void **) &p,
                                    $&1_descriptor, 0) != -1)
                    $1 = 1;
                else
                    $1 = 0;
    ...
    specialize_std_pair_on_first(bool,SCHEME_BOOLP,
                              SCHEME_TRUEP,alaqil_make_boolean);
    ...
    specialize_std_pair_on_first(std::string,SCHEME_STRINGP,
                              alaqil_scm_to_string,alaqil_make_string);
