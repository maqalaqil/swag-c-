...
%typemap(argout) string & {
  alaqil_result =	caml_list_append(alaqil_result,caml_val_string_len((*$1).c_str(), (*$1).size()));
}

%typemap(directorin) string {
    alaqil_result = caml_val_string_len($1.c_str(), $1.size());
...
%{
  alaqilEXT char **c_charptr_array( const std::vector <std::string > &str_v ) {
