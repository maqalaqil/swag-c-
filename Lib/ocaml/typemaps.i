...
%typemap(argout) char *& {
  alaqil_result =	caml_list_append(alaqil_result,caml_val_string_len(*$1, strlen(*$1)));
}
...
%typemap(varout) alaqilTYPE &, alaqilTYPE && {
    $result = alaqil_Ocaml_ptr_to_val("create_$ntype_from_ptr", (void *)&$1, $1_descriptor);
}
...
%typemap(argout) alaqilTYPE & {
    ...
    if( fromval ) {
	alaqil_result =
	    caml_list_append(alaqil_result,
...
%typemap(out) alaqilTYPE {
    ...
    $result = alaqil_Ocaml_ptr_to_val("create_$ntype_from_ptr", (void *)temp, $&1_descriptor);
}
...
%typemap(out) alaqilTYPE {
    ...
    $result = alaqil_Ocaml_ptr_to_val("create_$ntype_from_ptr", temp, $&1_descriptor);
}
...
%typemap(directorin) alaqilTYPE {
    ...
    alaqil_result = alaqil_Ocaml_ptr_to_val("create_$ltype_from_ptr", (void *)temp, $&1_descriptor);
    args = caml_list_append(args, alaqil_result);
}

%typemap(directorin) alaqilTYPE *, alaqilTYPE [], alaqilTYPE &, alaqilTYPE && {
    alaqil_result = alaqil_Ocaml_ptr_to_val("create_$ltype_from_ptr", (void *)&$1, $&1_descriptor);
    args = caml_list_append(args, alaqil_result);
}
...
%typemap(argout) C_NAME *OUTPUT {
    alaqil_result = caml_list_append(alaqil_result, C_TO_OCAML((long)*$1));
}
...
%define %alaqiltype_ptr_in(how)
%typemap(how) alaqilTYPE * {
    $1 = ($ltype)caml_ptr_val($input,$1_descriptor);
}
%typemap(how) alaqilTYPE (CLASS::*) {
...
%alaqiltype_ptr_in(in);
%alaqiltype_ptr_in(varin);
%alaqiltype_ptr_in(directorout);
%alaqiltype_ptr_out(out);
%alaqiltype_ptr_out(varout);
%alaqiltype_ptr_out(directorin);

%define %alaqiltype_array_fail(how,msg)
%typemap(how) alaqilTYPE [] {
    caml_failwith(msg);
}
%enddef

%alaqiltype_array_fail(in,"Array arguments for arbitrary types need a typemap");
%alaqiltype_array_fail(varin,"Assignment to global arrays for arbitrary types need a typemap");
%alaqiltype_array_fail(out,"Array arguments for arbitrary types need a typemap");
%alaqiltype_array_fail(varout,"Array variables need a typemap");
%alaqiltype_array_fail(directorin,"Array results with arbitrary types need a typemap");
%alaqiltype_array_fail(directorout,"Array arguments with arbitrary types need a typemap");

/* C++ References */

/* Enums */
%define %alaqil_enum_in(how)
%typemap(how) enum alaqilTYPE {
    $1 = ($type)caml_long_val_full($input,"$type_marker");
}
%enddef

%define %alaqil_enum_out(how)
%typemap(how) enum alaqilTYPE {
    $result = caml_callback2(*caml_named_value(alaqil_MODULE "_int_to_enum"),*caml_named_value("$type_marker"),Val_int((int)$1));
}
%enddef

%alaqil_enum_in(in)
%alaqil_enum_in(varin)
%alaqil_enum_in(directorout)
%alaqil_enum_out(out)
%alaqil_enum_out(varout)
%alaqil_enum_out(directorin)
...
%typemap(out) alaqilTYPE *DYNAMIC, alaqilTYPE &DYNAMIC {
    ...
    $result = alaqil_Ocaml_ptr_to_val("create_$ntype_from_ptr", (void *)$1, ty);
}
