...
let _ = Callback.register "alaqil_runmethod" invoke
...
let alaqil_current_type_info = ref C_void
let find_type_info obj = !alaqil_current_type_info 
let _ = Callback.register "alaqil_find_type_info" find_type_info
let set_type_info obj =
  match obj with
    C_ptr _ -> alaqil_current_type_info := obj ;
               obj
    | _ -> raise (Failure "Internal error: passed non pointer to set_type_info")
let _ = Callback.register "alaqil_set_type_info" set_type_info
