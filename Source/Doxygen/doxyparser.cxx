...
DoxygenParser::DoxyCommandEnum DoxygenParser::commandBelongs(const std::string &theCommand) {
  ...
  if (String *const ignore = getIgnoreFeature(theCommand)) {
    ...
    // one given to it by alaqil itself), we may use the value in the future, but
    // for now we only use the attributes.
    if (Strcmp(ignore, "1") != 0) {
      alaqil_warning(WARN_PP_UNEXPECTED_TOKENS, m_fileName.c_str(), m_fileLineNo,
  ...
  if (!endCommand.empty()) {
    ...
    if (String *const contents = getIgnoreFeature(theCommand, "contents")) {
      ...
      if (Strcmp(contents, "parse") == 0) {
        ...
        alaqil_error(m_fileName.c_str(), m_fileLineNo, "Invalid \"doxygen:ignore\" feature \"contents\" attribute \"%s\".\n", Char(contents));
        return;
      }
    }
    ...
    if (Strcmp(range, "line") == 0) {
      ...
      alaqil_error(m_fileName.c_str(), m_fileLineNo, "Invalid \"doxygen:ignore\" feature \"range\" attribute \"%s\".\n", Char(range));
      return;
    }
  }
}
...
void DoxygenParser::printListError(int warningType, const std::string &message) {
  ...
  alaqil_warning(warningType, m_fileName.c_str(), curLine, "%s\n", message.c_str());
}
