...
void JavaDocConverter::fillStaticTables() {
   ...
   * Rule: alaqil type system can't be used to convert C::m to C#m, because in Java it is C.m
  ...
  // (it is internal command of alaqil's parser, contains plain text)
...
string JavaDocConverter::convertLink(string linkObject) {
  ...
  for (size_t i = 0; i < params.size(); i++) {
    // translate c/c++ type string to alaqil's type
    ...
    if (paramStr.find("const") != string::npos)
      alaqilType_add_qualifier(alaqilType, "const");
    ...
    for (int j = (int)params[i].size() - 1; j >= 0; j--) {
      ...
      if (paramStr[j] == ']') {
        ...
        if (j < 0)
          return "";
        alaqilType_add_pointer(alaqilType);
        continue;
      } else if (paramStr[j] == '*')
        alaqilType_add_pointer(alaqilType);
      else if (paramStr[j] == '&')
        alaqilType_add_reference(alaqilType);
        ...
        if (typeNameStart == string::npos)
          ...
          typeNameStart++;
        Append(alaqilType, paramStr.substr(typeNameStart, j - typeNameStart + 1).c_str());
        break;
      }
    }

    // make dummy param list, to lookup typemaps for it
    Parm *dummyParam = NewParm(alaqilType, "", 0);
    alaqil_typemap_attach_parms("jstype", dummyParam, NULL);
    Language::instance()->replaceSpecialVariables(0, Getattr(dummyParam, "tmap:jstype"), dummyParam);

    //alaqil_print(dummyParam, 1);
...
void JavaDocConverter::addError(int warningType, const std::string &message) {
  alaqil_warning(warningType, "", 0, "Doxygen parser warning: %s. \n", message.c_str());
}
