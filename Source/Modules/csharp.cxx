...
public:
   ...
   * ----------------------------------------------------------------------------- */
   ...
   String *getProxyName(alaqilType *t) {
  ...
  virtual void main(int argc, char *argv[]) {
    ...
    alaqil_library_directory("csharp");
    ...
    Preprocessor_define("alaqilCSHARP 1", 0);

    // Add typemap definitions
    alaqil_typemap_lang("csharp");
    alaqil_config_file("csharp.swg");
  ...
  virtual int top(Node *n) {
    ...
    if (namespce) {
      ...
      alaqil_name_register("wrapper", wrapper_name);
      Delete(wrapper_name);
    }
    else {
      alaqil_name_register("wrapper", "CSharp_%f");
    }
    ...
    {
      File *f_im = getOutputFile(alaqil_output_directory(), imclass_name);
    ...
    {
      File *f_module = getOutputFile(alaqil_output_directory(), module_class_name);
    ...
    // Output a C# type wrapper class for each alaqil type
  ...
  File *getOutputFile(const String *dir, const String *name) {
    if (output_file) {
      if (!f_single_out) {
	String *filen = NewStringf("%s%s", alaqil_output_directory(), output_file);
	f_single_out = NewFile(filen, "w", alaqil_output_files());
	if (!f_single_out) {
	...
	emitBanner(f_single_out);
      ...
      File *f = NewFile(filen, "w", alaqil_output_files());
      if (!f) {
	FileErrorDisplay(filen);
	alaqil_exit(EXIT_FAILURE);
      }
  ...
  virtual int functionWrapper(Node *n) {
    ...
    String *wname = alaqil_name_wrapper(overloaded_name);

    /* Attach the non-standard typemaps to the parameter list. */
    alaqil_typemap_attach_parms("ctype", l, f);
    alaqil_typemap_attach_parms("imtype", l, f);

    /* Get return types */
    if ((tm = alaqil_typemap_lookup("ctype", n, "", 0))) {
	...
	tm = ctypeout;
      ...
      alaqil_warning(WARN_CSHARP_TYPEMAP_CTYPE_UNDEF, input_file, line_number, "No ctype typemap defined for %s\n", alaqilType_str(t, 0));
    }

    if ((tm = alaqil_typemap_lookup("imtype", n, "", 0))) {
	...
	tm = imtypeout;
      ...
      alaqil_warning(WARN_CSHARP_TYPEMAP_CSTYPE_UNDEF, input_file, line_number, "No imtype typemap defined for %s\n", alaqilType_str(t, 0));
    }
    ...
    Printv(f->def, " alaqilEXPORT ", c_return_type, " alaqilSTDCALL ", wname, "(", NIL);
    ...
    if (Replaceall(f->code, "alaqil_contract_assert(", "alaqil_contract_assert($null, ") > 0) {
	...
	if (Strstr(f->code, "alaqil_exception")) {
	  alaqil_warning(WARN_CSHARP_CANTHROW, input_file, line_number,
		       "Unmanaged code contains a call to alaqil_exception and C# code does not handle pending exceptions via the canthrow attribute.\n");
	} else if (Strstr(f->code, "alaqil_CSharpSetPendingException")) {
	  alaqil_warning(WARN_CSHARP_CANTHROW, input_file, line_number,
		       "Unmanaged code contains a call to a alaqil_CSharpSetPendingException method and C# code does not handle pending exceptions via the canthrow attribute.\n");
	}
      }
    }
  ...
  virtual int constantWrapper(Node *n) {
    ...
    alaqil_save("constantWrapper", n, "tmap:ctype:out", "tmap:imtype:out", "tmap:cstype:out", "tmap:out:null", "tmap:imtype:outattributes", "tmap:cstype:outattributes", NIL);
    ...
    /* Attach the non-standard typemaps to the parameter list. */
    alaqil_typemap_attach_parms("cstype", l, NULL);

    /* Get C# return types */
    bool classname_substituted_flag = false;

    if ((tm = alaqil_typemap_lookup("cstype", n, "", 0))) {
	...
	tm = cstypeout;
      ...
      alaqil_warning(WARN_CSHARP_TYPEMAP_CSWTYPE_UNDEF, input_file, line_number, "No cstype typemap defined for %s\n", alaqilType_str(t, 0));
    }
  ...
  void upcastsCode(alaqilType *smart, String *upcast_method_name, String *c_classname, String *c_baseclass) {
    String *wname = alaqil_name_wrapper(upcast_method_name);
    ...
    if (smart) {
      ...
      Printv(upcasts_code,
	  "alaqilEXPORT ", bsmartnamestr, " * alaqilSTDCALL ", wname, "(", smartnamestr, " *jarg1) {\n",
      ...
      Printv(upcasts_code,
	  "alaqilEXPORT ", c_baseclass, " * alaqilSTDCALL ", wname, "(", c_classname, " *jarg1) {\n",
  ...
  void emitProxyClassDefAndCPPCasts(Node *n) {
	...
	while (base.item) {
	  if (!(GetFlag(base.item, "feature:ignore") || Getattr(base.item, "feature:interface"))) {
		...
		c_baseclass = alaqilType_namestr(baseclassname);
	      ...
	      alaqil_warning(WARN_CSHARP_MULTIPLE_INHERITANCE, Getfile(n), Getline(n),
    ...
    if (purebase_replace) {
      ...
      if (purebase_notderived)
        alaqil_error(Getfile(n), Getline(n), "The csbase typemap for proxy %s must contain just one of the 'replace' or 'notderived' attributes.\n", typemap_lookup_type);
	...
	alaqil_error(Getfile(n), Getline(n), "No methodname attribute defined in csdestruct%s typemap for %s\n", (derived ? "_derived" : ""), proxy_class_name);
      }
      if (!destruct_methodmodifiers) {
	alaqil_error(Getfile(n), Getline(n),
    ...
    if (feature_director) {
      ...
      Printf(proxy_class_code, "  private void alaqilDirectorConnect() {\n");
	...
	Printf(proxy_class_code, "    if (alaqilDerivedClassHasMethod(\"%s\", alaqilMethodTypes%s))\n", method, methid);
	Printf(proxy_class_code, "      alaqilDelegate%s = new alaqilDelegate%s_%s(alaqilDirectorMethod%s);\n", methid, proxy_class_name, methid, overname);
      }
      String *director_connect_method_name = alaqil_name_member(getNSpace(), getClassPrefix(), "director_connect");
      Printf(proxy_class_code, "    %s.%s(alaqilCPtr", imclass_name, director_connect_method_name);
      for (i = first_class_dmethod; i < curr_class_dmethod; ++i) {
	UpcallData *udata = Getitem(dmethods_seq, i);
	String *methid = Getattr(udata, "class_methodidx");
	Printf(proxy_class_code, ", alaqilDelegate%s", methid);
      }
	...
	Printf(proxy_class_code, "\n");
	Printf(proxy_class_code, "  private bool alaqilDerivedClassHasMethod(string methodName, global::System.Type[] methodTypes) {\n");
	Printf(proxy_class_code,
  ...
  virtual int classHandler(Node *n) {
    ...
    if (proxy_flag) {
      ...
      String *csclazzname = alaqil_name_member(getNSpace(), getClassPrefix(), ""); // mangled full proxy class name
  ...
  void proxyClassFunctionHandler(Node *n) {
    ...
    alaqil_typemap_attach_parms("cstype", l, NULL);
    alaqil_typemap_attach_parms("csin", l, NULL);
	...
	tm = cstypeout;
      ...
      Printf(return_type, "%s", tm);
      if (covariant)
	alaqil_warning(WARN_CSHARP_COVARIANT_RET, input_file, line_number,
    ...
    if (wrapping_member_flag && !enum_constant_flag) {
      ...
      if (setter_flag)
        alaqil_typemap_attach_parms("csvarin", l, NULL);
    }
	...
	if ((tm = Getattr(p, "tmap:csin"))) {
	  ...
	  alaqil_warning(WARN_CSHARP_TYPEMAP_CSIN_UNDEF, input_file, line_number, "No csin typemap defined for %s\n", alaqilType_str(pt, 0));
	}
    ...
    if ((tm = alaqil_typemap_lookup("csout", n, "", 0))) {
	...
	if (directorNode) {
	  ...
	  if (!Cmp(return_type, "void"))
	    Printf(excode, "if (alaqilDerivedClassHasMethod(\"%s\", alaqilMethodTypes%s)) %s; else %s", proxy_function_name, methid, ex_imcall, imcall);
	  else
	    Printf(excode, "(alaqilDerivedClassHasMethod(\"%s\", alaqilMethodTypes%s) ? %s : %s)", proxy_function_name, methid, ex_imcall, imcall);
	...
	Replaceall(imcall, "$imfuncname", intermediary_function_name);
      ...
      alaqil_warning(WARN_CSHARP_TYPEMAP_CSOUT_UNDEF, input_file, line_number, "No csout typemap defined for %s\n", alaqilType_str(t, 0));
    }
	...
	if (setter_flag) {
	  ...
	  alaqilType *cvariable_type = Getattr(last_parm, "type");
	  Parm *variable_parm = NewParmNode(cvariable_type, n);
	  if ((tm = alaqil_typemap_lookup("cstype", variable_parm, "", 0))) {
	    ...
	    alaqil_warning(WARN_CSHARP_TYPEMAP_CSOUT_UNDEF, input_file, line_number, "No cstype typemap defined for %s\n", alaqilType_str(cvariable_type, 0));
	  }
	}
	const String *csattributes = Getattr(n, "feature:cs:attributes");
	if (csattributes)
	...
	alaqilType *cvariable_type = Getattr(last_parm, "type");
	Parm *variable_parm = NewParmNode(cvariable_type, n);
	if ((tm = alaqil_typemap_lookup("csvarin", variable_parm, "", 0))) {
	  substituteClassname(cvariable_type, tm);
	  ...
	  alaqil_warning(WARN_CSHARP_TYPEMAP_CSOUT_UNDEF, input_file, line_number, "No csvarin typemap defined for %s\n", alaqilType_str(cvariable_type, 0));
	}
      } else {
	// Getter method
	if ((tm = alaqil_typemap_lookup("csvarout", n, "", 0))) {
	  if (GetFlag(n, "feature:new"))
	    Replaceall(tm, "$owner", "true");
	  else
	    Replaceall(tm, "$owner", "false");
	  ...
	  alaqil_warning(WARN_CSHARP_TYPEMAP_CSOUT_UNDEF, input_file, line_number, "No csvarout typemap defined for %s\n", alaqilType_str(t, 0));
	}
      }
    } else {
  ...
  virtual int constructorHandler(Node *n) {
	...
	if (!feature_director) {
	  ...
	  if (connect_attr) {
	    ...
	    alaqil_warning(WARN_CSHARP_NO_DIRECTORCONNECT_ATTR, input_file, line_number, "\"directorconnect\" attribute missing in %s \"csconstruct\" typemap.\n",
			 Getattr(n, "name"));
	    Replaceall(construct_tm, "$directorconnect", "");
	  }
	}
  ...
  String *getOverloadedName(Node *n) {
    ...
    /* A C# HandleRef is used for all classes in the alaqil intermediary class.
  ...
  String *getEnumName(alaqilType *t) {
    Node *enumname = NULL;
    Node *n = enumLookup(t);
    if (n) {
  ...
  bool substituteClassname(alaqilType *pt, String *tm) {
  ...
  void substituteClassnameSpecialVariable(alaqilType *classnametype, String *tm, const char *classnamespecialvariable) {
  ...
  void substituteInterfacenameSpecialVariable(alaqilType *interfacenametype, String *tm, const char *interfacenamespecialvariable, bool qualified) {
  ...
  void emitTypeWrapperClass(String *classname, alaqilType *type) {
    ...
    File *f_alaqiltype = getOutputFile(alaqil_output_directory(), classname);

    addOpenNamespace(0, f_alaqiltype);
    ...
    // Emit the class
    Printv(alaqiltype, typemapLookup(n, "csimports", type, WARN_NONE),	// Import statements
	   "\n", NIL);
    ...
    Printv(alaqiltype, typemapLookup(n, "csclassmodifiers", type, WARN_CSHARP_TYPEMAP_CLASSMOD_UNDEF),	// Class modifiers
	   " $csclassname",	// Class name and base class
    ...
    addCloseNamespace(0, f_alaqiltype);
    if (f_alaqiltype != f_single_out)
      Delete(f_alaqiltype);
    f_alaqiltype = NULL;
    Delete(alaqiltype);
    Delete(n);
  ...
  void excodeSubstitute(Node *n, String *code, const String *typemap, Node *parameter) {
    ...
    if (Getattr(n, "csharp:canthrow")) {
      ...
      if (count < 1 || !excode) {
	alaqil_warning(WARN_CSHARP_EXCODE, input_file, line_number,
  ...
  void emitDirectorExtraMethods(Node *n) {
    ...
    String *wname = alaqil_name_wrapper(alaqil_director_connect);
    ...
    Printv(imclass_class_code, "\n  [global::System.Runtime.InteropServices.DllImport(\"", dllimport, "\", EntryPoint=\"", wname, "\")]\n", NIL);
    Printf(imclass_class_code, "  public static extern void %s(global::System.Runtime.InteropServices.HandleRef jarg1", alaqil_director_connect);

    Wrapper *code_wrap = NewWrapper();
    Printf(code_wrap->def, "alaqilEXPORT void alaqilSTDCALL %s(void *objarg", wname);
    ...
    Printf(code_wrap->code, "  director->alaqil_connect_director(");
	...
	Printf(code_wrap->code, ", ");
      Printf(code_wrap->def, "%s::alaqil_Callback%s_t callback%s", dirclassname, methid, methid);
      Printf(code_wrap->code, "callback%s", methid);
      Printf(imclass_class_code, ", %s.alaqilDelegate%s_%s delegate%s", qualified_classname, sym_name, methid, methid);
    }
  ...
  int classDirectorMethod(Node *n, Node *parent, String *super) {
    ...
    imclass_dmethod = NewStringf("alaqilDirector_%s", alaqil_name_member(getNSpace(), getClassPrefix(), overloaded_name));
    ...
    tm = alaqil_typemap_lookup("imtype", n, "", 0);
    if (tm) {
      String *imtypeout = Getattr(n, "tmap:imtype:out");	// the type in the imtype typemap's out attribute overrides the type in the typemap
      if (imtypeout)
	tm = imtypeout;
	...
	if (!ignored_method)
      ...
      Printf(callback_def, "  private %s alaqilDirectorMethod%s(", tm, overloaded_name);
      if (!ignored_method) {
    ...
    if ((c_ret_type = alaqil_typemap_lookup("ctype", n, "", 0))) {
	...
	Delete(jretval_decl);
      ...
      alaqil_warning(WARN_CSHARP_TYPEMAP_CTYPE_UNDEF, input_file, line_number, "No ctype typemap defined for %s for use in %s::%s (skipping director method)\n", 
    ...
    alaqil_typemap_attach_parms("ctype", l, 0);
    alaqil_typemap_attach_parms("imtype", l, 0);
    alaqil_typemap_attach_parms("cstype", l, 0);
    alaqil_typemap_attach_parms("directorin", l, w);
    alaqil_typemap_attach_parms("csdirectorin", l, 0);
    alaqil_typemap_attach_parms("directorargout", l, w);

    /* Preamble code */
    if (!ignored_method)
      Printf(w->code, "if (!alaqil_callback%s) {\n", overloaded_name);
	...
	Printf(w->code, "return %s;\n", super_call);
      ...
      Printf(w->code, "alaqil::DirectorPureVirtualException::raise(\"%s::%s\");\n", alaqilType_namestr(c_classname), alaqilType_namestr(name));
      if (!is_void)
	...
	if ((tm = Getattr(p, "tmap:directorin"))) {
	  ...
	  if ((tm = Getattr(p, "tmap:imtype"))) {
		...
		alaqil_warning(WARN_CSHARP_TYPEMAP_CSWTYPE_UNDEF, input_file, line_number, "No cstype typemap defined for %s\n", alaqilType_str(pt, 0));
	      ...
	      alaqil_warning(WARN_CSHARP_TYPEMAP_CSDIRECTORIN_UNDEF, input_file, line_number, "No csdirectorin typemap defined for %s for use in %s::%s (skipping director method)\n", 
		  alaqilType_str(pt, 0), alaqilType_namestr(c_classname), alaqilType_namestr(name));
	      output_director = false;
	    }
	  } else {
	    alaqil_warning(WARN_CSHARP_TYPEMAP_CSTYPE_UNDEF, input_file, line_number, "No imtype typemap defined for %s for use in %s::%s (skipping director method)\n", 
		alaqilType_str(pt, 0), alaqilType_namestr(c_classname), alaqilType_namestr(name));
	    output_director = false;
	  }
	  ...
	  alaqil_warning(WARN_CSHARP_TYPEMAP_CSDIRECTORIN_UNDEF, input_file, line_number,
    ...
    if ((tm = alaqil_typemap_lookup("csdirectorout", n, "", 0))) {
	...
	Printf(w->code, "jresult = (%s) ", c_ret_type);
      ...
      Printf(w->code, "alaqil_callback%s(%s);\n", overloaded_name, jupcall_args);
    ...
    if (!ignored_method) {
      ...
      */
      ...
      Printf(director_callback_typedefs, "    typedef %s (alaqilSTDCALL* alaqil_Callback%s_t)(", c_ret_type, methid);
      Printf(director_callback_typedefs, "%s);\n", callback_typedef_parms);
      Printf(director_callbacks, "    alaqil_Callback%s_t alaqil_callback%s;\n", methid, overloaded_name);

      Printf(director_delegate_definitions, " alaqilDelegate%s_%s(%s);\n", classname, methid, delegate_parms);
      Printf(director_delegate_instances, "  private alaqilDelegate%s_%s alaqilDelegate%s;\n", classname, methid, methid);
      Printf(director_method_types, "  private static global::System.Type[] alaqilMethodTypes%s = new global::System.Type[] { %s };\n", methid, proxy_method_types);
      Printf(director_connect_parms, "alaqilDirector%s%s delegate%s", classname, methid, methid);
    }
  ...
  int classDirectorConstructor(Node *n) {
	...
	Printf(f_directors, "  alaqil_init_callbacks();\n");
	Printf(f_directors, "}\n\n");
  ...
  int classDirectorEnd(Node *n) {
    ...
    Printf(f_directors_h, "    void alaqil_connect_director(");

    Printf(w->def, "void %s::alaqil_connect_director(", dirclassname);
    ...
    for (i = first_class_dmethod; i < curr_class_dmethod; ++i) {
      ...
      Printf(f_directors_h, "alaqil_Callback%s_t callback%s", methid, overname);
      Printf(w->def, "alaqil_Callback%s_t callback%s", methid, overname);
      Printf(w->code, "alaqil_callback%s = callback%s;\n", overname, overname);
    ...
    Printf(f_directors_h, "    void alaqil_init_callbacks();\n");
    Printf(f_directors_h, "};\n\n");
    Printf(w->code, "}\n\n");

    Printf(w->code, "void %s::alaqil_init_callbacks() {\n", dirclassname);
    for (i = first_class_dmethod; i < curr_class_dmethod; ++i) {
      UpcallData *udata = Getitem(dmethods_seq, i);
      String *overname = Getattr(udata, "overname");
      Printf(w->code, "alaqil_callback%s = 0;\n", overname);
    }
    Printf(w->code, "}");
  ...
  void directorDeclaration(Node *n) {
    ...
    String *class_ctor = NewString("alaqil::Director()");

    String *dirclassname = directorClassName(n);
    String *declaration = alaqil_class_declaration(n, dirclassname);
...
/* -----------------------------------------------------------------------------
 * alaqil_csharp()    - Instantiate module
 * ----------------------------------------------------------------------------- */

static Language *new_alaqil_csharp() {
  return new CSHARP();
}
extern "C" Language *alaqil_csharp(void) {
  return new_alaqil_csharp();
}
