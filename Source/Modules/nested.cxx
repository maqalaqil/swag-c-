...
static String *make_name(Node *n, String *name, alaqilType *decl) {
  ...
  if (!name)
    return 0;
  return alaqil_name_make(n, 0, name, decl, 0);
}
...
static void add_symbols_c(Node *n) {
  ...
  if (!alaqilType_isfunction(decl)) {
    ...
    alaqil_features_get(alaqil_cparse_features(), 0, name, 0, n);
    if (makename) {
    ...
    if (symname) {
      wrn = alaqil_name_warning(n, 0, symname, 0);
    }
  } else {
    String *name = Getattr(n, "name");
    ...
    alaqil_features_get(alaqil_cparse_features(), 0, name, fun, n);

    symname = make_name(n, name, fun);
    wrn = alaqil_name_warning(n, 0, symname, fun);
  ...
  if (GetFlag(n, "feature:ignore")) {
	...
	int redefined = alaqil_need_redefined_warn(n, c, true);
	if (redefined) {
...
static Node *create_insert(Node *n, bool noTypedef = false) {
  ...
  /* Make all alaqil created typedef structs/unions/classes unnamed else
  ...
  {
    /* Remove alaqil directive %constant which may be left in the alaqil created typedefs */
    char *code_ptr = Char(ccode);
    while (code_ptr) {
...
void alaqil_nested_name_unnamed_c_structs(Node *n) {
  ...
  while (c) {
	...
	if (Node *am = Getattr(alaqil_extend_hash(), name)) {
	  alaqil_extend_merge(c, am);
	  alaqil_extend_append_previous(c, am);
	  Delattr(alaqil_extend_hash(), name);
	}
	alaqil_symbol_popscope();

	// process declarations following this type (assign correct new type)
	alaqilType *ty = Copy(name);
	...
	Delete(ty);
	alaqil_symbol_setscope(alaqil_symbol_global_scope());
	add_symbols_c(c);
    ...
    // process children
    alaqil_nested_name_unnamed_c_structs(c);
    c = next;
  }
}
...
void alaqil_nested_process_classes(Node *n) {
  ...
  while (c) {
	...
	if (!checkAttribute(c, "access", "public"))
      ...
      alaqil_nested_process_classes(c);
    }
    c = next;
  }
