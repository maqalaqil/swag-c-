 ...
 * ocaml.cxx
 *
 * Ocaml language module for alaqil.
 * ----------------------------------------------------------------------------- */
...
static alaqilType *name_qualifier_type = 0;
...
public:
  ...
  void PrintIncludeArg() {
    Printv(stdout, alaqil_LIB, alaqil_FILE_DELIMITER, ocaml_path, "\n", NIL);
  }
  ...
  virtual void main(int argc, char *argv[]) {
    ...
    alaqil_library_directory(ocaml_path);
	...
	if (strcmp(argv[i], "-help") == 0) {
	  ...
	  if (argv[i + 1]) {
	    Printf(stderr, "alaqil: warning: -suffix option deprecated.  alaqil 3.0.4 and later provide a -cppext option which should be used instead.\n");
    ...
    Preprocessor_define("alaqilOCAML 1", 0);
    // Set name of typemaps

    alaqil_typemap_lang("ocaml");

    // Read in default typemaps */
    alaqil_config_file("ocaml.i");
    allow_overloading();
  ...
  String *alaqil_director_declaration(Node *n) {
    ...
    String *directorname = NewStringf("alaqilDirector_%s", classname);
  ...
  virtual int top(Node *n) {
    ...
    alaqil_register_filebyname("mli", f_mlibody);
    alaqil_register_filebyname("ml", f_mlbody);
    alaqil_register_filebyname("mlitail", f_mlitail);
    alaqil_register_filebyname("mltail", f_mltail);
    alaqil_register_filebyname("director", f_directors);
    alaqil_register_filebyname("director_h", f_directors_h);
    alaqil_register_filebyname("classtemplate", f_classtemplate);
    alaqil_register_filebyname("class_ctors", f_class_ctors);

    if (old_variable_names) {
      alaqil_name_register("set", "%n%v__set__");
      alaqil_name_register("get", "%n%v__get__");
    }
    ...
    Printf(f_runtime, "#define alaqil_MODULE \"%s\"\n", module);
    /* Module name */
    ...
    Printf(f_enumtypes_type, "open alaqil\n" "type c_enum_type = [ \n  `unknown\n");
    ...
    String *mlfilen = NewStringf("%s%s", alaqil_output_directory(), mlfile);
    if ((f_mlout = NewFile(mlfilen, "w", alaqil_output_files())) == 0) {
      FileErrorDisplay(mlfilen);
      alaqil_exit(EXIT_FAILURE);
    }
    String *mlifilen = NewStringf("%s%s", alaqil_output_directory(), mlifile);
    if ((f_mliout = NewFile(mlifilen, "w", alaqil_output_files())) == 0) {
    ...
    Printf(f_mlibody, "val enum_to_int : c_enum_type -> c_obj -> alaqil.c_obj\n");
    ...
    Printf(f_init, "#define alaqil_init f_%s_init\n" "%s" "}\n", module, init_func_def);
  ...
  void throw_unhandled_ocaml_type_error(alaqilType *d, const char *types) {
    alaqil_warning(WARN_TYPEMAP_UNDEF, input_file, line_number, "Unable to handle type %s (%s).\n", alaqilType_str(d, 0), types);
  }
  ...
  void oc_alaqilType_del_reference(alaqilType *t) {
    char *c = Char(t);
    if (strncmp(c, "q(", 2) == 0) {
  ...
  void oc_alaqilType_del_array(alaqilType *t) {
    char *c = Char(t);
    if (strncmp(c, "q(", 2) == 0) {
      Delete(alaqilType_pop(t));
      c = Char(t);
  ...
  int
   is_a_reference(alaqilType *t) {
    return alaqilType_isreference(alaqilType_typedef_resolve_all(t));
  }

  int
   is_an_array(alaqilType *t) {
    return alaqilType_isarray(alaqilType_typedef_resolve_all(t));
  }
  ...
  virtual int membervariableHandler(Node *n) {
    ...
    String *mname = alaqil_name_member(NSPACE_TODO, classname, symname);
  ...
  virtual int functionWrapper(Node *n) {
    ...
    Printv(f->def, "alaqilEXT CAML_VALUE ", wname, " (", NIL);
    ...
    Wrapper_add_local(f, "ret", "alaqil_CAMLlocal2(alaqil_result,rv)");
    d = alaqilType_typedef_qualified(d);
    ...
    Printf(f->code, "alaqil_result = Val_unit;\n");
	...
	p = Getattr(p, "tmap:in:next");
      ...
      pt = alaqilType_typedef_qualified(pt);
    ...
    if ((tm = alaqil_typemap_lookup_out("out", n, alaqil_cresult_name(), f, actioncode))) {
      Replaceall(tm, "$source", "alaqil_result");
    ...
    if ((tm = alaqil_typemap_lookup("alaqil_result", n, alaqil_cresult_name(), 0))) {
      Replaceall(tm, "$source", alaqil_cresult_name());
      Printv(f->code, tm, "\n", NIL);
    }
    // Wrap things up (in a manner of speaking)

    Printv(f->code, tab4, "alaqil_result = caml_list_append(alaqil_result,rv);\n", NIL);
    Printv(f->code, tab4, "CAMLreturn(alaqil_result);\n", NIL);
    Printv(f->code, "}\n", NIL);
	...
	String *dispatch = alaqil_overload_dispatch(n,
						  "free(argv);\n" "CAMLreturn(%s(args));\n",
						  &maxargs);
	...
	Printv(df->def,
	       "alaqilEXT CAML_VALUE ", wname, "(CAML_VALUE args) {\n" "  CAMLparam1(args);\n" "  int i;\n" "  int argc = caml_list_length(args);\n", NIL);
	Printv(df->code,
    ...
    if (expose_func) {
      ...
      Printf(f_mlbody, "let %s arg = match %s_f (%s(fnhelper arg)) with\n", mangled_name, mangled_name,
	     in_constructor && alaqil_directorclass(getCurrentClass()) ? "director_core_helper " : "");
  ...
  virtual int variableWrapper(Node *n) {
    ...
    Printf(f->def, "alaqilEXT CAML_VALUE %s(CAML_VALUE args) {\n", var_name);
    // Printv(f->def, "#define FUNC_NAME \"", proc_name, "\"", NIL);

    Wrapper_add_local(f, "args", "CAMLparam1(args)");
    Wrapper_add_local(f, "alaqil_result", "alaqil_CAMLlocal1(alaqil_result)");
    Printf(f->code, "alaqil_result = Val_unit;\n");
	...
	emit_action_code(n, f->code, tm);
      } else if ((tm = alaqil_typemap_lookup("in", n, name, 0))) {
	Replaceall(tm, "$source", "args");
	Replaceall(tm, "$target", name);
	Replaceall(tm, "$input", "args");
	emit_action_code(n, f->code, tm);
    ...
    if ((tm = alaqil_typemap_lookup("varout", n, name, 0))) {
      ...
      emit_action_code(n, f->code, tm);
    } else if ((tm = alaqil_typemap_lookup("out", n, name, 0))) {
      Replaceall(tm, "$source", name);
      Replaceall(tm, "$target", "alaqil_result");
      Replaceall(tm, "$result", "alaqil_result");
      emit_action_code(n, f->code, tm);
    ...
    if (GetFlag(n, "feature:immutable")) {
      Printf(f_mlbody, "external _%s : c_obj -> alaqil.c_obj = \"%s\" \n", mname, var_name);
      Printf(f_mlibody, "val _%s : c_obj -> alaqil.c_obj\n", iname);
      if (const_enum) {
  ...
  virtual int constantWrapper(Node *n) {
    ...
    alaqilType *qname = Getattr(n, "qualified:name");
    ...
    if (!name) {
      ...
      Insert(name, 0, "_alaqil_wrap_");
      Setattr(n, "feature:symname", name);
    }
    // See if there's a typemap

    // Create variable and assign it a value
    Printf(f_header, "static %s = %s;\n", alaqilType_str(type, name), value);
  ...
  void Multiwrite(String *s) {
	...
	File *fout = alaqil_filebyname(stream_name);
	if (fout) {
	  next += strlen("*)");
   ...
   * I correctly incorporate alaqil's typechecking, this isn't a big deal.
   *
   ...
   * alaqil-generated object that does not contain type info, since the
   * existence of the object means that alaqil knows the type.
   *
  ...
  int classHandler(Node *n) {
    ...
    if (sizeof_feature) {
      Printf(f_wrappers,
	     "alaqilEXT CAML_VALUE _wrap_%s_sizeof( CAML_VALUE args ) {\n"
  ...
  String *normalizeTemplatedClassName(String *name) {
    String *name_normalized = alaqilType_typedef_resolve_all(name);
    bool took_action;
	...
	alaqilType_del_pointer(name_normalized);
	took_action = true;
      }

      if (is_a_reference(name_normalized)) {
	oc_alaqilType_del_reference(name_normalized);
	took_action = true;
      }

      if (is_an_array(name_normalized)) {
	oc_alaqilType_del_array(name_normalized);
	took_action = true;
      }
    } while (took_action);

    return alaqilType_str(name_normalized, 0);
  }
  ...
  alaqilType *fully_qualified_enum_type(Node *n) {
    Node *parent = 0;
  ...
  int enumvalueDeclaration(Node *n) {
    ...
    if (const_enum && qtype && name && !Getattr(seen_enumvalues, name)) {
      ...
      Setattr(n, "qualified:name", alaqilType_namestr(qtype));

      String *evname = alaqilType_manglestr(qtype);
      Insert(evname, 0, "alaqil_ENUM_");
  ...
  int enumDeclaration(Node *n) {
    ...
    if (name) {
      ...
      if (!seen_enum) {
         ...
         * The problem I'm having is that alaqil isn't very precise about
         * typedef name resolution.  My opinion is that alaqilType_typedef
    ...
    if (const_enum) {
      ...
      Printf(f_enum_to_int, "| `Int x -> alaqil.C_int x\n" "| _ -> raise (LabelNotFromThisEnum v))\n");
    }
  ...
  int classDirectorMethod(Node *n, Node *parent, String *super) {
	...
	Printf(w->code, "alaqil::DirectorPureVirtualException::raise(\"Attempted to invoke pure virtual method %s::%s\");\n", alaqilType_namestr(c_classname),
      ...
      Wrapper_add_local(w, "alaqil_result", "alaqil_CAMLlocal2(alaqil_result, args)");
	...
	p = nextSibling(p);
      ...
      Printv(w->code, "alaqil_result = Val_unit;\n", 0);
      ...
      Printv(w->code,
	     "alaqil_result = caml_alaqil_alloc(1,C_list);\n" "alaqil_Store_field(alaqil_result,0,args);\n" "args = alaqil_result;\n" "alaqil_result = Val_unit;\n", 0);
      Printf(w->code, "static CAML_VALUE *alaqil_ocaml_func_val = NULL;\n" "if (!alaqil_ocaml_func_val) {\n");
      Printf(w->code, "  alaqil_ocaml_func_val = caml_named_value(\"alaqil_runmethod\");\n  }\n");
      Printf(w->code, "alaqil_result = caml_callback3(*alaqil_ocaml_func_val,alaqil_get_self(),caml_copy_string(\"%s\"),args);\n", Getattr(n, "name"));
      /* exception handling */
	...
	Printf(w->code, "}\n");
      ...
      tm = alaqil_typemap_lookup("directorout", n, "c_result", w);
      if (tm != 0) {
	Replaceall(tm, "$input", "alaqil_result");
	/* TODO check this */
  ...
  int classDirectorDefaultConstructor(Node *n) {
    ...
    {
      Wrapper *w = NewWrapper();
      Printf(w->def, "alaqilDirector_%s::alaqilDirector_%s(CAML_VALUE self) : alaqil::Director(self) { }", classname, classname);
      Wrapper_print(w, f_directors);
      DelWrapper(w);
    }
    Printf(f_directors_h, "    alaqilDirector_%s(CAML_VALUE self);\n", classname);
    Delete(classname);
    Setattr(n, "parms", q);
   ...
   * be generated in the typemaps, and also at alaqil time to be the same
   * string.  The problem that arises is that alaqil variously generates
   * enum e_name_tag
   * e_name_tag
   * e_typedef_name
   * for
  ...
  String *runtimeCode() {
    String *s = alaqil_include_sys("ocaml.swg");
    if (!s) {
  ...
  String *defaultExternalRuntimeFilename() {
    return NewString("alaqilocamlrun.h");
  }
};

/* -------------------------------------------------------------------------
 * alaqil_ocaml()    - Instantiate module
 * ------------------------------------------------------------------------- */

static Language *new_alaqil_ocaml() {
  return new OCAML();
}
extern "C" Language *alaqil_ocaml(void) {
  return new_alaqil_ocaml();
}
