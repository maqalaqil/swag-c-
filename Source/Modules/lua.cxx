 ...
 * lua.cxx
 *
 * Lua language module for alaqil.
 * ----------------------------------------------------------------------------- */
 ...
 * ver010
   Added support for embedded Lua. Try alaqil -lua -help for more information
*/
...
/**** Diagnostics:
  ...
  This helps me search the parse tree & figure out what is going on inside alaqil
...
//#define REPORT(T,D)   {Printf(stdout,T"\n");if(D)alaqil_print_node(D);}      // the works
...
/* NEW LANGUAGE NOTE:***********************************************
 most of the default options are handled by alaqil
...
Lua Options (available with -lua)\n\
     ...
     -squash-bases   - Squashes symbols from all inheritance tree of a given class\n\
                       into itself. Emulates pre-alaqil3.0 inheritance. Insignificantly\n\
...
/* The new metatable bindings were introduced in alaqil 3.0.0.
...
private:
  ...
  // special C++ namespace $(classname).alaqilStatic. This is internal mechanism only
...
public:
  ...
  virtual void main(int argc, char *argv[]) {
    ...
    /* NEW LANGUAGE NOTE:***********************************************
       ...
       the alaqil_library_directory() is also boilerplate code
       but it always seems to be the first line of code
       NEW LANGUAGE NOTE:END *********************************************** */
    /* Add a symbol to the parser for conditional compilation */
    Preprocessor_define("alaqilLUA 1", 0);

    /* Set language-specific configuration file */
    alaqil_config_file("lua.swg");

    /* Set typemap language */
    alaqil_typemap_lang("lua");
  ...
  /* NEW LANGUAGE NOTE:***********************************************
     After calling main, alaqil parses the code to wrap (I believe)
     then calls top()
  ...
  virtual int top(Node *n) {
    ...
    s_luacode = NewString("");
    alaqil_register_filebyname("luacode", s_luacode);
    ...
    if (nomoduleglobal) {
      Printf(f_runtime, "#define alaqil_LUA_NO_MODULE_GLOBAL\n");
    } else {
      Printf(f_runtime, "#define alaqil_LUA_MODULE_GLOBAL\n");
    }
    if (squash_bases)
      Printf(f_runtime, "#define alaqil_LUA_SQUASH_BASES\n");

    //    if (NoInclude) {
    //      Printf(f_runtime, "#define alaqil_NOINCLUDE\n");
    //    }
    ...
    /* alaqil_import is a special function name for importing within Lua5.1 */
    //Printf(f_header, "#define alaqil_import  luaopen_%s\n\n", module);
    Printf(f_header, "#define alaqil_name      \"%s\"\n", module);
    Printf(f_header, "#define alaqil_init      luaopen_%s\n", module);
    Printf(f_header, "#define alaqil_init_user luaopen_%s_user\n\n", module);
    Printf(f_header, "#define alaqil_LUACODE   luaopen_%s_luacode\n", module);

    Printf(f_wrappers, "#ifdef __cplusplus\nextern \"C\" {\n#endif\n");

    /* %init code inclusion, effectively in the alaqil_init function */
    Printf(f_init, "void alaqil_init_user(lua_State* L)\n{\n");
    Language::top(n);
    Printf(f_init, "/* exec Lua code if applicable */\nalaqil_Lua_dostring(L,alaqil_LUACODE);\n");
    Printf(f_init, "}\n");
    ...
    Printf(f_begin, "const char* alaqil_LUACODE=\n  \"%s\";\n\n", s_luacode);
  ...
  void registerMethod(Node *n, bool overwrite = false, String *overwriteLuaScope = 0) {
	...
	mrename = alaqil_name_member(getNSpace(), getClassPrefix(), symname);
      } else {
	mrename = symname;
      }
    }
    wrapname = alaqil_name_wrapper(mrename);
  ...
  virtual int functionWrapper(Node *n) {
    ...
    Wrapper_add_local(f, "alaqil_arg", "int alaqil_arg = 0");
    ...
    /* NEW LANGUAGE NOTE:***********************************************
       ...
       some of the stuff will refer to the typemaps code written in your alaqil file
    ...
    String *checkfn = NULL;
    char source[64];
    Printf(argument_check, "alaqil_check_num_args(\"%s\",%d,%d)\n", alaqil_name_str(n), num_required + args_to_ignore, num_arguments + args_to_ignore);
	...
	if ((checkfn = Getattr(p, "tmap:in:checkfn"))) {
	  ...
	  Printf(argument_check, " alaqil_fail_arg(\"%s\",%s,\"%s\");\n", alaqil_name_str(n), source, alaqilType_str(pt, 0));
	}
	/* NEW LANGUAGE NOTE:***********************************************
    ...
    /* Close the function */
    Printv(f->code, "return alaqil_arg;\n", NIL);
    ...
    Replaceall(f->code, "$result", alaqil_cresult_name());
    ...
    /* NEW LANGUAGE NOTE:***********************************************
       register the function in alaqil
  ...
  /* NEW LANGUAGE NOTE:***********************************************
     ...
     look for %typecheck(alaqil_TYPECHECK_*) in the .swg file
     NEW LANGUAGE NOTE:END *********************************************** */
  int dispatchFunction(Node *n) {
    ...
    String *dispatch = alaqil_overload_dispatch(n, "return %s(L);", &maxargs);
    ...
    //Printf(stdout,"alaqil_overload_dispatch %s %s '%s' %d\n",symname,wname,dispatch,maxargs);
    ...
    Printf(f->code, "alaqil_Lua_pusherrstring(L,\"Wrong arguments for overloaded function '%s'\\n\"\n"
  ...
  void registerVariable(Node *n, bool overwrite = false, String *overwriteLuaScope = 0) {
    ...
    if (current[NO_CPP] || !getCurrentClass()) {
      // Global variable
      getName = alaqil_name_get(getNSpace(), symname);
      if (assignable)
        setName = alaqil_name_set(getNSpace(), symname);
    } else {
        assert(!current[NO_CPP]);
        if (current[STATIC_VAR] ) {
          mrename = alaqil_name_member(getNSpace(), getClassPrefix(), symname);
          getName = alaqil_name_get(0, mrename);
          if (assignable)
            setName = alaqil_name_set(0, mrename);
        } else if (current[MEMBER_VAR]) {
          mrename = alaqil_name_member(0, getClassPrefix(), symname);
          getName = alaqil_name_get(getNSpace(), mrename);
          if (assignable)
            setName = alaqil_name_set(getNSpace(), mrename);
        } else {
          assert(false);
        }
    }

    getName = alaqil_name_wrapper(getName);
    if (setName)
      setName = alaqil_name_wrapper(setName);
  ...
  void registerVariable(String *lua_nspace_or_class_name, Node *n, String *getName, String *setName) {
    String *unassignable = NewString("alaqil_Lua_set_immutable");
  ...
  virtual int variableWrapper(Node *n) {
    ...
    // let alaqil generate the wrappers
  ...
  virtual int constantWrapper(Node *n) {
    ...
    alaqil_save("lua_constantMember", n, "sym:name", NIL);
	...
	lua_name_v2 = alaqil_name_member(0, proxy_class_name, lua_name);
	iname_v2 = alaqil_name_member(0, proxy_class_name, iname);
        n_v2 = Copy(n);
        ...
        tm_v2 = alaqil_typemap_lookup("consttab", n_v2, name, 0);
        if (tm_v2) {
          ...
          tm_v2 = alaqil_typemap_lookup("constcode", n_v2, name, 0);
          if (!tm_v2) {
            // This can't be.
            assert(false);
            alaqil_restore(n);
  ...
  virtual int enumvalueDeclaration(Node *n) {
    ...
    alaqil_require("enumvalueDeclaration", n, "*name", "?value", "*sym:name", NIL);
    String *symname = Getattr(n, "sym:name");
  ...
  virtual int classHandler(Node *n) {
    ...
    mangled_full_proxy_class_name = alaqil_name_mangle(full_proxy_class_name);

    alaqilType *t = Copy(Getattr(n, "name"));
    alaqilType *fr_t = alaqilType_typedef_resolve_all(t);	/* Create fully resolved type */
    alaqilType *t_tmp = 0;
    t_tmp = alaqilType_typedef_qualified(fr_t);	// Temporal variable
    Delete(fr_t);
    fr_t = alaqilType_strip_qualifiers(t_tmp);
    String *mangled_fr_t = 0;
    mangled_fr_t = alaqilType_manglestr(fr_t);
    ...
    // are described with same structures - alaqil_lua_attribute/alaqil_lua_method. Instead of calling
    ...
    // 1) With name "class name" + "." + "alaqilStatic" to initialize static things
    ...
    class_static_nspace = NewStringf("%s%salaqilStatic", full_proxy_class_name, NSPACE_SEPARATOR);
     ...
     * by alaqil and namespaces couldn't be nested inside classes (C++ Standard)
     */
    ...
    String *wrap_class_name = alaqil_name_wrapper(NewStringf("class_%s", mangled_full_proxy_class_name));
    ...
    //    Printf(f_init,"alaqil_TypeClientData(alaqilTYPE%s, (void *) &_wrap_class_%s);\n", alaqilType_manglestr(t), mangled_full_proxy_class_name);

    // emit a function to be called to delete the object 
    if (have_destructor) {
      destructor_name = NewStringf("alaqil_delete_%s", mangled_full_proxy_class_name);
    ...
    // at runtime: we can query the alaqil type manager & see if the class exists
    ...
    Printv(f_wrappers, "static alaqil_lua_class *alaqil_", mangled_full_proxy_class_name, "_bases[] = {", base_class, "0};\n", NIL);
    Delete(base_class);
    Printv(f_wrappers, "static const char *alaqil_", mangled_full_proxy_class_name, "_base_names[] = {", base_class_names, "0};\n", NIL);
    Delete(base_class_names);

    Printv(f_wrappers, "static alaqil_lua_class _wrap_class_", mangled_full_proxy_class_name, " = { \"", proxy_class_name, "\", \"", full_proxy_class_name, "\", &alaqilTYPE",
	   alaqilType_manglestr(t), ",", NIL);
  ...
  virtual int staticmemberfunctionHandler(Node *n) {
    ...
    if (old_metatable_bindings && result == alaqil_OK && old_compatible_names) {
      alaqil_require("lua_staticmemberfunctionHandler", n, "*lua:name", NIL);
      String *lua_name = Getattr(n, "lua:name");
      // Although this function uses alaqil_name_member, it actually generates the Lua name,
      ...
      String *compat_name = alaqil_name_member(0, proxy_class_name, lua_name);
  ...
  virtual int staticmembervariableHandler(Node *n) {
	...
	alaqil_save("lua_staticmembervariableHandler", n, "lua:name", NIL);
	String *lua_name = Getattr(n, "lua:name");
	// Although this function uses alaqil_name_member, it actually generates the Lua name,
	// not the C++ name. This is because an earlier version used such a scheme for static function
	// name generation and we have to maintain backward compatibility.
	String *v2_name = alaqil_name_member(NIL, proxy_class_name, lua_name);
	if (!GetFlag(n, "wrappedasconstant")) {
  ...
  /* This is to support the usage
     alaqil -external-runtime <filename>
     ...
     I am writing a generic solution, even though alaqil-Lua only has one file right now...
   */
  String *runtimeCode() {
    String *s = NewString("");
    ...
    for (int i = 0; filenames[i] != 0; i++) {
      sfile = alaqil_include_sys(filenames[i]);
      if (!sfile) {
  ...
  String *defaultExternalRuntimeFilename() {
    return NewString("alaqilluarun.h");
  }
  ...
  void emitLuaFlavor(String *s) {
    if (elua_emulate) {
      ...
      Printf(s, "#define alaqil_LUA_TARGET alaqil_LUA_FLAVOR_ELUA\n");
      Printf(s, "#define alaqil_LUA_ELUA_EMULATE\n");
    } else if (elua_ltr)
      Printf(s, "#define alaqil_LUA_TARGET alaqil_LUA_FLAVOR_ELUA\n");
    else if (eluac_ltr)
      Printf(s, "#define alaqil_LUA_TARGET alaqil_LUA_FLAVOR_ELUAC\n");
    else
      Printf(s, "#define alaqil_LUA_TARGET alaqil_LUA_FLAVOR_LUA\n");
  }
   ...
   * that are not in any namespace (this is default for alaqil unless %nspace feature is used).
  ...
  Hash *getCArraysHash(String *nspace, bool reg = true) {
    ...
    if (nspace == 0 || Len(nspace) == 0)
      mangled_name = NewString("alaqilModule");
    else
      mangled_name = alaqil_name_mangle(nspace);
    String *cname = NewStringf("alaqil_%s", mangled_name);

    Setattr(carrays_hash, "cname", cname);

    String *attr_tab = NewString("");
    String *attr_tab_name = NewStringf("alaqil_%s_attributes", mangled_name);
    String *attr_tab_decl = NewString("");
    Printv(attr_tab, "static alaqil_lua_attribute ", NIL);
    ...
    String *methods_tab_name = NewStringf("alaqil_%s_methods", mangled_name);
    ...
    if (elua_ltr || eluac_ltr)	// In this case methods array also acts as namespace rotable
      ...
      Printf(methods_tab, "static alaqil_lua_method ");
    ...
    String *const_tab_name = NewStringf("alaqil_%s_constants", mangled_name);
    ...
    if (elua_ltr || eluac_ltr)	// In this case const array holds rotable with namespace constants
      ...
      Printf(const_tab, "static alaqil_lua_const_info ");
    ...
    String *classes_tab_name = NewStringf("alaqil_%s_classes", mangled_name);
    String *classes_tab_decl = NewString("");
    Printf(classes_tab, "static alaqil_lua_class* ");
    ...
    String *namespaces_tab_name = NewStringf("alaqil_%s_namespaces", mangled_name);
    String *namespaces_tab_decl = NewString("");
    Printf(namespaces_tab, "static alaqil_lua_namespace* ");
    ...
    if (elua_ltr) {
      ...
      String *get_tab_name = NewStringf("alaqil_%s_get", mangled_name);
      ...
      String *set_tab_name = NewStringf("alaqil_%s_set", mangled_name);
    ...
    if (!eluac_ltr) {
      ...
      String *metatable_tab_name = NewStringf("alaqil_%s_meta", mangled_name);
	...
	Printf(metatable_tab, "static alaqil_lua_method ");
  ...
  void closeCArraysHash(String *nspace, File *output) {
	...
	Printv(methods_tab, tab4, "{LSTRKEY(\"__metatable\"), LROVAL(", metatable_tab_name, ")},\n", NIL);
      ...
      Printv(methods_tab, tab4, "{LSTRKEY(\"__disown\"), LFUNCVAL(alaqil_Lua_class_disown)},\n", NIL);
	...
	if (GetFlag(carrays_hash, "lua:class_instance")) {
	  Printv(metatable_tab, tab4, "{LSTRKEY(\"__index\"), LFUNCVAL(alaqil_Lua_class_get)},\n", NIL);
	  Printv(metatable_tab, tab4, "{LSTRKEY(\"__newindex\"), LFUNCVAL(alaqil_Lua_class_set)},\n", NIL);
	} else {
	  Printv(metatable_tab, tab4, "{LSTRKEY(\"__index\"), LFUNCVAL(alaqil_Lua_namespace_get)},\n", NIL);
	  Printv(metatable_tab, tab4, "{LSTRKEY(\"__newindex\"), LFUNCVAL(alaqil_Lua_namespace_set)},\n", NIL);
	}

	Printv(metatable_tab, tab4, "{LSTRKEY(\"__gc\"), LFUNCVAL(alaqil_Lua_class_destruct)},\n", NIL);
	...
	if (GetFlag(carrays_hash, "lua:class_instance")) {
	  ...
	  assert(static_cls);
	  // static_cls is alaqil_lua_namespace. This structure can't be use with eLua(LTR)
  ...
  void closeNamespaces(File *dataOutput) {
    ...
    if (symbolScopeLookup("") == 0 || rawGetCArraysHash("") == 0) {
      // Module is empty. Create hash for global scope in order to have alaqil_alaqilModule
   ...
   *   alaqil_lua_namespace $cname =  { attr_array, methods_array, ... , namespaces_array };
  ...
  void printCArraysDefinition(String *nspace, String *name, File *output) {
    ...
    assert(cname);
    Printv(output, "static alaqil_lua_namespace ", cname, " = ", NIL);
...
/* NEW LANGUAGE NOTE:***********************************************
 in order to add you language into alaqil, you need to make the following changes:
 - write this file (obviously)
 - add into the makefile (not 100% clear on how to do this)
 - edit alaqilmain.cxx to add your module
 
near the top of alaqilmain.cxx, look for this code & add you own codes
======= begin change ==========
extern "C" {
  Language *alaqil_tcl(void);
  Language *alaqil_python(void);
  //etc,etc,etc...
  Language *alaqil_lua(void);	// this is my code
}
 
  //etc,etc,etc...
 
alaqil_module  modules[] = {
  {"-guile",     alaqil_guile,     "Guile"},
  {"-java",      alaqil_java,      "Java"},
  //etc,etc,etc...
  {"-lua",       alaqil_lua,       "Lua"},	// this is my code
...
/* -----------------------------------------------------------------------------
 * alaqil_lua()    - Instantiate module
 * ----------------------------------------------------------------------------- */

extern "C" Language *alaqil_lua(void) {
  return new LUA();
}
