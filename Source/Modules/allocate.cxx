 ...
 * Doc/Manual/alaqilPlus.html for details.
 * ----------------------------------------------------------------------------- */
...
extern "C" {
  ...
  static int check_implemented(Node *n) {
	...
	if (alaqilType_isfunction(decl)) {
	  alaqilType *decl1 = alaqilType_typedef_resolve_all(decl);
	  alaqilType *decl2 = alaqilType_pop_function(decl1);
...
class Allocate:public Dispatcher {
  ...
  int function_is_defined_in_bases(Node *n, Node *bases) {
    ...
    String *resolved_decl = alaqilType_typedef_resolve_all(this_decl);
  ...
  int function_is_defined_in_bases_seek(Node *n, Node *b, Node *base, String *this_decl, String *name, String *this_type, String *resolved_decl) {
    ...
    alaqilType *base_type = Getattr(base, "type");
    if (base_decl && base_type) {
      if (checkAttribute(base, "name", name) && !GetFlag(b, "feature:ignore") /* whole class is ignored */ ) {
	if (alaqilType_isfunction(resolved_decl) && alaqilType_isfunction(base_decl)) {
	  ...
	  if (returntype_match && decl_match) {
	    ...
	    if (!returntype_match) {
	      ...
	      if (!returntype_match) {
		covariant_returntype = alaqilType_issubtype(this_returntype, base_returntype) ? true : false;
		...
		alaqilType *base_fn = NewString("");
		alaqilType *this_fn = NewString("");
		alaqilType_add_function(base_fn, base_parms);
		alaqilType_add_function(this_fn, this_parms);
		base_fn = alaqilType_typedef_resolve_all(base_fn);
		this_fn = alaqilType_typedef_resolve_all(this_fn);
		if (Strcmp(base_fn, this_fn) == 0) {
		  // Finally check that the qualifiers match
		  int base_qualifier = alaqilType_isqualifier(resolved_decl);
		  int this_qualifier = alaqilType_isqualifier(base_decl);
	  ...
	  if (decl_match && returntype_match) {
		...
		if (both_have_public_access || both_have_protected_access) {
	      ...
	      alaqilType *most_base_covariant_type = Getattr(base, "covariant");
  ...
  String *function_return_type(Node *n, bool resolve = true) {
    ...
    if (resolve) {
      ...
      ty = alaqilType_typedef_resolve_all(unresolved);
      Delete(unresolved);
    }
    return ty;
  }
  ...
  int is_abstract_inherit(Node *n, Node *base = 0, int first = 0) {
    ...
    if (!base) {
      ...
      Symtab *oldtab = alaqil_symbol_setscope(stab);
      int ret = is_abstract_inherit(n, n, 1);
      alaqil_symbol_setscope(oldtab);
      return ret;
    }
	...
	if (base_decl)
	  base_decl = alaqilType_typedef_resolve_all(base_decl);
	if (alaqilType_isfunction(base_decl))
	  search_decl = alaqilType_pop_function(base_decl);
	Node *dn = alaqil_symbol_clookup_local_check(name, 0, check_implemented);
	Delete(search_decl);
	Delete(base_decl);
  ...
  List *smart_pointer_methods(Node *cls, List *methods, int isconst, String *classname = 0) {
	...
	if (!GetFlag(c, "feature:ignore")) {
	  ...
	  if (!((Cmp(storage, "typedef") == 0))
	    ...
	    Node *e = alaqil_symbol_clookup_local(name, 0);
	    if (e && is_public(e) && !GetFlag(e, "feature:ignore") && (Cmp(symname, Getattr(e, "sym:name")) == 0)) {
	      alaqil_warning(WARN_LANG_DEREF_SHADOW, Getfile(e), Getline(e), "Declaration of '%s' shadows declaration accessible via operator->(),\n", name);
	      alaqil_warning(WARN_LANG_DEREF_SHADOW, Getfile(c), Getline(c), "previous declaration of '%s'.\n", name);
	    } else {
		...
		while (cc) {
		  ...
		  if (isconst) {
		    alaqilType *decl = Getattr(cp, "decl");
		    if (decl) {
		      if (alaqilType_isfunction(decl)) {	/* If method, we only add if it's a const method */
			if (alaqilType_isconst(decl)) {
			  Append(methods, cp);
			}
		      } else {
			Append(methods, cp);
  ...
  void mark_exception_classes(ParmList *p) {
    while (p) {
      ...
      if (alaqilType_isreference(t) || alaqilType_ispointer(t) || alaqilType_isarray(t)) {
	Delete(alaqilType_pop(t));
      }
      Node *c = alaqil_symbol_clookup(t, 0);
      if (c) {
  ...
  void process_exceptions(Node *n) {
    ...
    if (scatchlist) {
      catchlist = alaqil_cparse_parms(scatchlist, n);
      if (catchlist) {
...
Allocate():
  ...
  virtual int classDeclaration(Node *n) {
    Symtab *symtab = alaqil_symbol_current();
    alaqil_symbol_setscope(Getattr(n, "symtab"));
	...
	if (!GetFlag(n, "feature:notabstract")) {
	  ...
	  if (na) {
	    alaqil_warning(WARN_TYPE_ABSTRACT, Getfile(n), Getline(n),
			 "Class '%s' might be abstract, " "no constructors generated,\n", alaqilType_namestr(Getattr(n, "name")));
	    alaqil_warning(WARN_TYPE_ABSTRACT, Getfile(na), Getline(na), "Method %s might not be implemented.\n", alaqil_name_decl(na));
    ...
    if (!Getattr(n, "allocate:smartpointer")) {
      Node *sp = alaqil_symbol_clookup("operator ->", 0);
      if (sp) {
	/* Look for parent */
    ...
    alaqil_interface_propagate_methods(n);
  ...
  virtual int cDeclaration(Node *n) {
	...
	if ((Strcmp(name, "operator ->") == 0) && (!GetFlag(n, "feature:ignore"))) {
	  ...
	  while (sn) {
	    if (!Getattr(sn, "parms")) {
	      alaqilType *type = alaqilType_typedef_resolve_all(Getattr(sn, "type"));
	      alaqilType_push(type, Getattr(sn, "decl"));
	      Delete(alaqilType_pop_function(type));
	      alaqilType *base = alaqilType_base(type);
	      Node *sc = alaqil_symbol_clookup(base, 0);
	      if ((sc) && (Strcmp(nodeType(sc), "class") == 0)) {
		if (alaqilType_check_decl(type, "p.")) {
		  /* Need to check if type is a const pointer */
		  int isconst = 0;
		  Delete(alaqilType_pop(type));
		  ...
		  /* Hmmm.  The return value is not a pointer.  If the type is a value
		     ...
		     can be found */
		  if ((alaqilType_check_decl(type, "")) || (alaqilType_check_decl(type, "r."))) {
		    Node *nn = alaqil_symbol_clookup("operator ->", Getattr(sc, "symtab"));
		    if (nn) {
		      Delete(base);
		      Delete(type);
		      sn = nn;
  ...
  virtual int constructorDeclaration(Node *n) {
    ...
    if (parms && (ParmList_numrequired(parms) == 1)) {
      ...
      alaqilType *type = Getattr(inclass, "name");
      String *tn = NewStringf("r.q(const).%s", type);
      String *cc = alaqilType_typedef_resolve_all(tn);
      alaqilType *rt = alaqilType_typedef_resolve_all(Getattr(parms, "type"));
      if (alaqilType_istemplate(type)) {
	String *tmp = alaqil_symbol_template_deftype(cc, 0);
	Delete(cc);
	cc = tmp;
	tmp = alaqil_symbol_template_deftype(rt, 0);
	Delete(rt);
	rt = tmp;
      }
      if (Strcmp(cc, rt) == 0) {
	...
	if (Strcmp(cc, Getattr(parms, "type")) == 0) {
	  ...
	  String *ty = alaqilType_strip_qualifiers(Getattr(parms, "type"));
	  if (Strcmp(cc, ty) == 0) {
	    copy_constructor = 1;
	  }
	  Delete(ty);
