...
class D : public Language {
  ...
  // The name of the proxy module which exposes the (alaqil) module contents as a
  // D module.
   ...
   * alaqil types data.
   */
  // Collects information about encountered types alaqil does not know about (e.g.
...
public:
  ...
  virtual int top(Node *n) {
    ...
    dmodule_directory = Copy(alaqil_output_directory());
    if (Len(package) > 0) {
      String *package_directory = Copy(package);
      Replaceall(package_directory, ".", alaqil_FILE_DELIMITER);
    ...
    alaqil_name_register("wrapper", "D_%f");
    ...
    // TODO: Add support for »static« linking.
    {
      ...
      File *im_d_file = NewFile(filen, "w", alaqil_output_files());
      if (!im_d_file) {
    ...
    // Generate the main D proxy module.
    {
      ...
      File *proxy_d_file = NewFile(filen, "w", alaqil_output_files());
      if (!proxy_d_file) {
	FileErrorDisplay(filen);
	alaqil_exit(EXIT_FAILURE);
      }
      ...
      // Write a D type wrapper class for each alaqil type to the proxy module code.
      for (Iterator alaqil_type = First(unknown_types); alaqil_type.key; alaqil_type = Next(alaqil_type)) {
	writeTypeWrapperClass(alaqil_type.key, alaqil_type.item);
      }
    ...
    for (Iterator it = First(nspace_proxy_dmodules); it.key; it = Next(it)) {
      ...
      File *file = NewFile(filename, "w", alaqil_output_files());
      if (!file) {
  ...
  virtual int enumDeclaration(Node *n) {
	...
	File *class_file = NewFile(filename, "w", alaqil_output_files());
	if (!class_file) {
	  FileErrorDisplay(filename);
	  alaqil_exit(EXIT_FAILURE);
	}
  ...
  virtual int memberfunctionHandler(Node *n) {
    ...
    String *intermediary_function_name =
      alaqil_name_member(getNSpace(), proxy_class_name, overloaded_name);
    ...
    if (split_proxy_dmodule &&
      ...
      alaqil_warning(WARN_D_NAME_COLLISION, input_file, line_number,
  ...
  virtual int constructorHandler(Node *n) {
    ...
    Printf(helper_code, "static private %s alaqilConstruct%s(",
    ...
    alaqil_typemap_attach_parms("dtype", l, NULL);
    alaqil_typemap_attach_parms("din", l, NULL);
	...
	alaqil_warning(WARN_D_TYPEMAP_DTYPE_UNDEF, input_file, line_number,
	...
	alaqil_warning(WARN_D_TYPEMAP_DIN_UNDEF, input_file, line_number,
    ...
    if (construct_tm) {
      const bool use_director = (parentNode(n) && alaqil_directorclass(n));
      if (!use_director) {
	...
	if (connect_attr) {
	  ...
	  alaqil_warning(WARN_D_NO_DIRECTORCONNECT_ATTR, input_file, line_number,
	...
	Printv(helper_code, "\n", terminator_code, NIL);
      ...
      Printf(helper_code, "\n}\n");
      String *helper_name = NewStringf("%s.alaqilConstruct%s(%s)",
  ...
  virtual int destructorHandler(Node *n) {
    ...
    Printv(destructor_call, im_dmodule_fq_name, ".", alaqil_name_destroy(getNSpace(),symname), "(cast(void*)alaqilCPtr)", NIL);
    const String *methodmods = Getattr(n, "feature:d:methodmodifiers");
    if (methodmods)
  ...
  virtual int classHandler(Node *n) {
    ...
    if (split_proxy_dmodule) {
      ...
      class_file = NewFile(filename, "w", alaqil_output_files());
      Delete(output_directory);
      if (!class_file) {
	FileErrorDisplay(filename);
	alaqil_exit(EXIT_FAILURE);
    ...
    String *dclazzname = alaqil_name_member(getNSpace(), proxy_class_name, "");
  ...
  virtual int constantWrapper(Node *n) {
    ...
    if (GetFlag(n, "feature:d:manifestconst") != 1) {
      ...
      alaqil_save("constantWrapper", n, "tmap:ctype:out", "tmap:imtype:out", "tmap:dtype:out", "tmap:out:null", "tmap:imtype:outattributes", "tmap:dtype:outattributes", NIL);
  ...
  virtual int functionWrapper(Node *n) {
	...
	tm = ctypeout;
      ...
      alaqil_warning(WARN_D_TYPEMAP_CTYPE_UNDEF, input_file, line_number,
	...
	tm = imtypeout;
      ...
      alaqil_warning(WARN_D_TYPEMAP_IMTYPE_UNDEF, input_file, line_number, "No imtype typemap defined for %s\n", alaqilType_str(t, 0));
    }
    ...
    Printv(f->def, " alaqilEXPORT ", c_return_type, " ", wname, "(", NIL);
	...
	alaqil_warning(WARN_D_TYPEMAP_CTYPE_UNDEF, input_file, line_number, "No ctype typemap defined for %s\n", alaqilType_str(pt, 0));
      }
	...
	if (Strstr(f->code, "alaqil_exception")) {
	  alaqil_warning(WARN_D_CANTHROW_MISSING, input_file, line_number,
	  "C code contains a call to alaqil_exception and D code does not handle pending exceptions via the canthrow attribute.\n");
	} else if (Strstr(f->code, "alaqil_DSetPendingException")) {
	  alaqil_warning(WARN_D_CANTHROW_MISSING, input_file, line_number,
	  "C code contains a call to a alaqil_DSetPendingException method and D code does not handle pending exceptions via the canthrow attribute.\n");
	}
      }
    }
  ...
  virtual int classDirectorInit(Node *n) {
    ...
    // class alaqilDirector_myclass : public myclass, public alaqil::Director {
    ...
    Printf(f_directors_h,
      "%s : public %s, public alaqil::Director {\n", declaration, base);
    Printf(f_directors_h, "\npublic:\n");
    ...
    // Stash for later.
    Setattr(n, "director:ctor", NewString("alaqil::Director()"));
  ...
  virtual int classDirectorMethod(Node *n, Node *parent, String *super) {
    ...
    imclass_dmethod = NewStringf("alaqilDirector_%s", alaqil_name_member(getNSpace(), classname, overloaded_name));
	...
	tm = imtypeout;
      ...
      Printf(callback_def, "\nprivate extern(C) %s alaqilDirectorCallback_%s_%s(void* dObject", tm, classname, overloaded_name);
      Printv(proxy_callback_return_type, tm, NIL);
    } else {
      alaqil_warning(WARN_D_TYPEMAP_IMTYPE_UNDEF, input_file, line_number,
    ...
    alaqil_typemap_attach_parms("dtype", l, 0);
    alaqil_typemap_attach_parms("directorin", l, w);
    alaqil_typemap_attach_parms("ddirectorin", l, 0);
    alaqil_typemap_attach_parms("directorargout", l, w);

    // Preamble code.
    if (!ignored_method)
      Printf(w->code, "if (!alaqil_callback_%s) {\n", overloaded_name);
	...
	if ((tm = Getattr(p, "tmap:directorin"))) {
	  ...
	  if ((tm = lookupDTypemap(p, "imtype", true))) {
		...
		alaqil_warning(WARN_D_TYPEMAP_DTYPE_UNDEF, input_file, line_number,
	      ...
	      alaqil_warning(WARN_D_TYPEMAP_DDIRECTORIN_UNDEF, input_file, line_number,
	...
	Printf(w->code, "jresult = (%s) ", c_ret_type);
      ...
      Printf(w->code, "alaqil_callback_%s(d_object%s);\n", overloaded_name, dcallback_call_args);
	...
	dp_return_type = NewString("");
      ...
      Printf(director_callback_typedefs, "    typedef %s (* alaqil_Callback%s_t)", c_ret_type, methid);
      Printf(director_callback_typedefs, "(void *dobj%s);\n", callback_typedef_parms);
      Printf(director_callback_pointers, "    alaqil_Callback%s_t alaqil_callback_%s;\n", methid, overloaded_name);
  ...
  virtual int classDirectorEnd(Node *n) {
    ...
    Printf(f_directors_h, "    void alaqil_connect_director(void* dobj");

    Printf(w->def, "void %s::alaqil_connect_director(void* dobj", director_classname);
    ...
    for (i = first_class_dmethod; i < curr_class_dmethod; ++i) {
      ...
      Printf(w->def, ", alaqil_Callback%s_t callback_%s", methid, overname);
      Printf(w->code, "alaqil_callback_%s = callback_%s;\n", overname, overname);
    }
    ...
    for (i = first_class_dmethod; i < curr_class_dmethod; ++i) {
      UpcallData *udata = Getitem(dmethods_seq, i);
      String *overname = Getattr(udata, "overname");
      Printf(w->code, "alaqil_callback_%s = 0;\n", overname);
    }
    Printf(w->code, "}");
...
private:
  ...
  void writeImDModuleFunction(const_String_or_char_ptr d_name,
    ...
    Printf(im_dmodule_code, "alaqilExternC!(%s function%s) %s;\n", return_type,
      parameters, d_name);
  ...
  void writeProxyClassFunction(Node *n) {
    ...
    if (wrapping_member_flag) {
      ...
      const String *setter_name = alaqil_name_set(getNSpace(),
        alaqil_name_member(0, proxy_class_name, variable_name));
    ...
    if (!static_flag) {
      Printf(imcall, "cast(void*)alaqilCPtr");
    }
	...
	Replaceall(tm, "$owner", "false");
      ...
      if (explicit_n && alaqil_directorclass(getCurrentClass())) {
	String *ex_overloaded_name = getOverloadedName(explicit_n);
	String *ex_intermediary_function_name = alaqil_name_member(getNSpace(), proxy_class_name, ex_overloaded_name);
	...
	if (!Cmp(return_type, "void"))
	  Printf(excode, "if (alaqilIsMethodOverridden!(%s delegate(%s), %s function(%s), %s)()) %s; else %s",
	    return_type, proxy_param_types, return_type, proxy_param_types, proxy_function_name, ex_imcall, imcall);
	else
	  Printf(excode, "((alaqilIsMethodOverridden!(%s delegate(%s), %s function(%s), %s)()) ? %s : %s)",
	    return_type, proxy_param_types, return_type, proxy_param_types, proxy_function_name, ex_imcall, imcall);
	...
	Replaceall(imcall, "$imfuncname", intermediary_function_name);
      ...
      alaqil_warning(WARN_D_TYPEMAP_DOUT_UNDEF, input_file, line_number,
	...
	while (base.item) {
	  if (!GetFlag(base.item, "feature:ignore")) {
		...
		c_baseclass = alaqilType_namestr(baseclassname);
	      ...
	      alaqil_warning(WARN_D_MULTIPLE_INHERITANCE, Getfile(n), Getline(n),
		  "Base %s of class %s ignored: multiple inheritance is not supported in D.\n", alaqilType_namestr(baseclassname), alaqilType_namestr(proxyclassname));
	    }
	  }
	  base = Next(base);
	}
  ...
  void writeClassUpcast(Node *n, const String* d_class_name, String* c_class_name, String* c_base_name) {
    ...
    String *upcast_name = alaqil_name_member(getNSpace(), d_class_name, (smart != 0 ? "SmartPtrUpcast" : "Upcast"));
    String *upcast_wrapper_name = alaqil_name_wrapper(upcast_name);
    ...
    if (smart) {
      ...
      alaqilType *rclassname = alaqilType_typedef_resolve_all(c_class_name);
      alaqilType *rbaseclass = alaqilType_typedef_resolve_all(c_base_name);
      ...
      Printv(upcasts_code,
	"alaqilEXPORT ", bsmartnamestr, " * ", upcast_wrapper_name,
	...
	"\n", NIL);
      ...
      Printv(upcasts_code,
	"alaqilEXPORT ", c_base_name, " * ", upcast_wrapper_name,
   ...
   * --------------------------------------------------------------------------- */
  void writeTypeWrapperClass(String *classname, alaqilType *type) {
    Node *n = NewHash();
  ...
  void writeDirectorConnectProxy(Node* classNode) {
    ...
    String *connect_name = alaqil_name_member(getNSpace(),
      proxy_class_name, "director_connect");
    Printf(proxy_class_body_code, "\nprivate void alaqilDirectorConnect() {\n");
    ...
    for (i = first_class_dmethod; i < curr_class_dmethod; ++i) {
      ...
      Printf(proxy_class_body_code, "  if (alaqilIsMethodOverridden!(%s delegate(%s), %s function(%s), %s)()) {\n", return_type, param_list, return_type, param_list, method);
      Printf(proxy_class_body_code, "    callback%s = &alaqilDirectorCallback_%s_%s;\n", methid, proxy_class_name, overloaded_name);
      Printf(proxy_class_body_code, "  }\n\n");
    }
    Printf(proxy_class_body_code, "  %s.%s(cast(void*)alaqilCPtr, cast(void*)this", im_dmodule_fq_name, connect_name);
    for (i = first_class_dmethod; i < curr_class_dmethod; ++i) {
      UpcallData *udata = Getitem(dmethods_seq, i);
      String *methid = Getattr(udata, "class_methodidx");
    ...
    if (first_class_dmethod < curr_class_dmethod) {
      Printf(proxy_class_body_code, "\n");
      Printf(proxy_class_body_code, "private bool alaqilIsMethodOverridden(DelegateType, FunctionType, alias fn)() %s{\n", (d_version > 1) ? "const " : "");
      Printf(proxy_class_body_code, "  DelegateType dg = &fn;\n");
      Printf(proxy_class_body_code, "  return dg.funcptr != alaqilNonVirtualAddressOf!(FunctionType, fn);\n");
      Printf(proxy_class_body_code, "}\n");
      Printf(proxy_class_body_code, "\n");
      Printf(proxy_class_body_code, "private static Function alaqilNonVirtualAddressOf(Function, alias fn)() {\n");
  ...
  void writeDirectorConnectWrapper(Node *n) {
    ...
    Replaceall(wrapper_loader_bind_code, "$symbol", alaqil_name_wrapper(connect_name));

    Printf(im_dmodule_code, "extern(C) void function(void* cObject, void* dObject");

    code_wrap = NewWrapper();
    Printf(code_wrap->def, "alaqilEXPORT void D_%s(void *objarg, void *dobj", connect_name);

    Printf(code_wrap->code, "  %s *obj = (%s *)objarg;\n", norm_name, norm_name);
    Printf(code_wrap->code, "  %s *director = static_cast<%s *>(obj);\n", dirClassName, dirClassName);

    Printf(code_wrap->code, "  director->alaqil_connect_director(dobj");
   ...
   * both C and D.
   * --------------------------------------------------------------------------- */
  String *getPrimitiveDptype(Node *node, alaqilType *type) {
    alaqilType *stripped_type = alaqilType_typedef_resolve_all(type);

    // A reference can only be the »outermost element« of a type.
    bool mutable_ref = false;
    if (alaqilType_isreference(stripped_type)) {
      alaqilType_del_reference(stripped_type);

      if (alaqilType_isconst(stripped_type)) {
	alaqilType_del_qualifier(stripped_type);
      } else {
	mutable_ref = true;
      }
    }
    ...
    while (alaqilType_ispointer(stripped_type)) {
      ++indirection_count;
      alaqilType_del_pointer(stripped_type);
    }

    // Now that we got rid of the pointers, see if we are dealing with a
    // primitive type.
    String *dtype = 0;
    if (alaqilType_isfunction(stripped_type) && indirection_count > 0) {
      // type was a function pointer, split it up.
      alaqilType_add_pointer(stripped_type);
      --indirection_count;

      alaqilType *return_type = Copy(stripped_type);
      alaqilType *params_type = alaqilType_functionpointer_decompose(return_type);
	...
	return 0;
      ...
      List *parms = alaqilType_parmlist(params_type);
	...
	for (Iterator it = First(param_dtypes); it.item; it = Next(it)) {
      ...
      dtype = NewStringf("%s.alaqilExternC!(%s function(%s))", im_dmodule_fq_name,
  ...
  const String *lookupCodeTypemap(Node *n, const_String_or_char_ptr tmap_method,
    alaqilType *type, int warning, Node *typemap_attributes = 0) const {
	...
	alaqil_warning(warning, Getfile(n), Getline(n),
   ...
   * alaqil_typemap_lookup is used otherwise.
   * --------------------------------------------------------------------------- */
  String *lookupDTypemap(Node *n, const_String_or_char_ptr method, bool attached = false) {
    ...
    if (attached) {
      ...
      result = alaqil_typemap_lookup(method, n, "", 0);
    }
   ...
   * classes/structs/unions alaqil knows about. Also substitutes the enumeration
   ...
   * --------------------------------------------------------------------------- */
  bool replaceClassname(String *tm, alaqilType *pt) {
   ...
   * --------------------------------------------------------------------------- */
  void replaceClassnameVariable(String *target, const char *variable, alaqilType *type) {
	...
	// alaqil does not know anything about the type (after resolving typedefs).
	// Just mangle the type name string like $descriptor(type) would do.
	String *descriptor = NewStringf("alaqilTYPE%s", alaqilType_manglestr(type));
  ...
  void replaceExcode(Node *n, String *code, const String *typemap, Node *parameter) const {
    ...
    if (Getattr(n, "d:canthrow")) {
      ...
      if (count < 1 || !excode) {
	alaqil_warning(WARN_D_EXCODE_MISSING, input_file, line_number,
  ...
  void replaceImportTypeMacros(String *target) const {
	...
	alaqil_error(Getfile(target), Getline(target), "Syntax error in: %s\n", current_macro);
   ...
   * --------------------------------------------------------------------------- */
  String *createProxyName(alaqilType *t) {
  ...
  bool wrapMemberFunctionAsDConst(Node *n) const {
    ...
    return GetFlag(n, "memberget") || alaqilType_isconst(Getattr(n, "decl"));
  }
  ...
  bool areAllOverloadsOverridden(Node *n) const {
    ...
    for (Node *tmp = firstSibling(base_function); tmp; tmp = Getattr(tmp, "sym:nextSibling")) {
	  ...
	  !(alaqil_director_mode() && alaqil_director_protected_mode() && alaqil_all_protected_mode())) {
  ...
  size_t overridingOverloadCount(Node *n) const {
    ...
    do {
      ...
      // if it is not marked as »override« by the alaqil parser.
...
static Language *new_alaqil_d() {
  return new D();
}

/* -----------------------------------------------------------------------------
 * alaqil_d()    - Instantiate module
 * ----------------------------------------------------------------------------- */
extern "C" Language *alaqil_d(void) {
  return new_alaqil_d();
}
