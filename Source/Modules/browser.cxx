...
static void display(FILE *f, Node *n) {
  ...
  Printf(f, "<HTML><HEAD><TITLE>alaqil-%s</TITLE></HEAD><BODY BGCOLOR=\"#ffffff\">\n", alaqil_package_version());
  Printf(f, "<b>alaqil-%s</b><br>\n", alaqil_package_version());
...
void symbol_handler(FILE *f) {
  ...
  {
    String *q = alaqil_symbol_qualifiedscopename(sym);
    if (!Len(q)) {
  ...
  if (name) {
    Node *n = alaqil_symbol_clookup(name, sym);
    Printf(f, "Symbol '%s':\n", name);
    Printf(f, "<blockquote>\n");
    if (!n) {
...
void alaqil_browser(Node *top, int port) {
#ifdef alaqil_SWILL
  int sport;
  browser_exit = 0;
  ...
  Printf(stderr, "alaqil: Tree browser listening on port %d\n", sport);
