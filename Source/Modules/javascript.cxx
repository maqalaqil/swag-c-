 ...
 * javascript.cxx
 *
 * Javascript language module for alaqil.
 * ----------------------------------------------------------------------------- */
 ...
 * The implementation delegates to alaqil Hash DOHs and provides
...
JSEmitter *alaqil_javascript_create_JSCEmitter();
JSEmitter *alaqil_javascript_create_V8Emitter();
JSEmitter *alaqil_javascript_create_NodeJSEmitter();

/**********************************************************************
 * JAVASCRIPT: alaqil module implementation
 **********************************************************************/
...
void JAVASCRIPT::main(int argc, char *argv[]) {
  // Set javascript subdirectory in alaqil library
  alaqil_library_directory("javascript");
  ...
  case JSEmitter::V8:
    {
      emitter = alaqil_javascript_create_V8Emitter();
      Preprocessor_define("alaqil_JAVASCRIPT_V8 1", 0);
      alaqil_library_directory("javascript/v8");
  ...
  case JSEmitter::JavascriptCore:
    {
      emitter = alaqil_javascript_create_JSCEmitter();
      Preprocessor_define("alaqil_JAVASCRIPT_JSC 1", 0);
      alaqil_library_directory("javascript/jsc");
      break;
    }
  ...
  default:
    {
      Printf(stderr, "alaqil Javascript: Unknown engine. Please specify one of '-jsc', '-v8' or '-node'.\n");
      alaqil_exit(-1);
      break;
    }
  }
  ...
  Preprocessor_define("alaqilJAVASCRIPT 1", 0);

  // Add typemap definitions
  alaqil_typemap_lang("javascript");

  // Set configuration file
  alaqil_config_file("javascript.swg");

  allow_overloading();
}

/* -----------------------------------------------------------------------------
 * alaqil_javascript()    - Instantiate module
 * ----------------------------------------------------------------------------- */

static Language *new_alaqil_javascript() {
  return new JAVASCRIPT();
}

extern "C" Language *alaqil_javascript(void) {
  return new_alaqil_javascript();
}
...
int JSEmitter::enterClass(Node *n) {
  ...
  state.clazz(NAME_MANGLED, alaqilType_manglestr(mangled_name));
  Delete(mangled_name);

  state.clazz(TYPE, NewString(Getattr(n, "classtype")));

  String *type = alaqilType_manglestr(Getattr(n, "classtypeobj"));
...
int JSEmitter::enterVariable(Node *n) {
  ...
  if (Equal(Getattr(n, "view"), "memberconstantHandler")) {
    ...
    state.variable(NAME, alaqil_scopename_last(Getattr(n, "sym:name")));
  }
...
int JSEmitter::emitCtor(Node *n) {
  ...
  String *wrap_name = alaqil_name_wrapper(Getattr(n, "sym:name"));
  if (is_overloaded) {
  ...
  Printf(wrapper->locals, "%sresult;", alaqilType_str(Getattr(n, "type"), 0));
...
int JSEmitter::emitDtor(Node *n) {
  ...
  alaqilType *type = state.clazz(TYPE);
  String *p_classtype = alaqilType_add_pointer(state.clazz(TYPE));
  String *ctype = alaqilType_lstr(p_classtype, "");
   ...
   * the alaqil function was not being generated. More specifically:
   struct MyData {
   %extend {
   ~MyData() {
   FreeData($self);
   ...
   alaqilINTERN void delete_MyData(struct MyData *self){
   FreeData(self);
   }

   I don't understand fully why it wasn't being generated. It just seems to happen in the Lua generator.
   There is a comment about staticmemberfunctionHandler having an inconsistency and I tracked down dome of the alaqilINTERN void delete_*
  ...
  /* The next challenge is to generate the correct finalize function for JavaScriptCore to call.
     ...
     {
     alaqil_PRV_DATA* t = (alaqil_PRV_DATA*)JSObjectGetPrivate(thisObject);
     if(t && t->alaqilCMemOwn) free ((${type}*)t->alaqilCObject);
     if(t) free(t);
     }
     %}

     But for the above example case of %extend to define a destructor on a struct, we need to override the system to not call
     free ((${type}*)t->alaqilCObject);
     ...
     alaqil_PRV_DATA* t = (alaqil_PRV_DATA*)JSObjectGetPrivate(thisObject);
     if(t && t->alaqilCMemOwn) {
     ${type}* arg1 = (${type}*)t->alaqilCObject;
     ${destructor_action}
     }
     if(t) free(t);
     ...
     I think there is a deeper underlying alaqil issue because I don't think it should be char*. However, it doesn't really matter for free.
...
int JSEmitter::emitConstant(Node *n) {
  ...
  // a lot of alaqil internal constants were emitted
  ...
  if (alaqilType_type(type) == T_MPOINTER) {
    ...
    String *str = alaqilType_str(type, mpointer_wname);
...
int JSEmitter::emitFunction(Node *n, bool is_member, bool is_static) {
  ...
  String *iname = Getattr(n, "sym:name");
  String *wrap_name = alaqil_name_wrapper(iname);
  if (is_overloaded) {
...
int JSEmitter::emitFunctionDispatcher(Node *n, bool /*is_member */ ) {
  ...
  Node *methodclass = alaqil_methodclass(n);
  ...
  String *final_wrap_name = alaqil_name_wrapper(new_string);
...
void JSEmitter::marshalOutput(Node *n, ParmList *params, Wrapper *wrapper, String *actioncode, const String *cresult, bool emitReturnVariable) {
  ...
  tm = alaqil_typemap_lookup_out("out", n, cresult, wrapper, actioncode);
  bool should_own = GetFlag(n, "feature:new") != 0;

  if (tm) {
    Replaceall(tm, "$objecttype", alaqil_scopename_last(alaqilType_str(alaqilType_strip_qualifiers(type), 0)));
...
// For namespace functions Language::getNSpace() does not give a valid result
  ...
  if (nspace == NULL) {
    ...
    nspace = alaqil_scopename_prefix(Getattr(n, "name"));
  }
  ...
  // replace "." with "::" that we can use alaqil_scopename_last
  Replaceall(scope, ".", "::");
...
int JSEmitter::createNamespace(String *scope) {
  ...
  String *parent_scope = alaqil_scopename_prefix(scope);
...
Hash *JSEmitter::createNamespaceEntry(const char *_name, const char *parent, const char *parent_mangled) {
  ...
  Setattr(entry, NAME, alaqil_scopename_last(name));
  Setattr(entry, NAME_MANGLED, alaqil_name_mangle(name));
...
int JSCEmitter::initialize(Node *n) {
  ...
  /* Initialize I/O */
  f_wrap_cpp = NewFile(outfile, "w", alaqil_output_files());
  if (!f_wrap_cpp) {
  ...
  alaqil_register_filebyname("begin", f_wrap_cpp);
...
int JSCEmitter::exitFunction(Node *n) {
  ...
  if (is_overloaded) {
    if (!Getattr(n, "sym:nextSibling")) {
      //state.function(WRAPPER_NAME, alaqil_name_wrapper(Getattr(n, "name")));
      // create dispatcher
...
int JSCEmitter::exitClass(Node *n) {
  ...
  if (base_class != NULL) {
	...
	.replace("$jsbaseclassmangled", alaqilType_manglestr(Getattr(base_class, "name")))
  ...
  /* Note: this makes sure that there is a alaqil_type added for this class */
  alaqilType_remember_clientdata(state.clazz(TYPE_MANGLED), NewString("0"));
...
int V8Emitter::initialize(Node *n) {
  ...
  // note: this is necessary for built-in generation of alaqil runtime code
  alaqil_register_filebyname("begin", f_wrap_cpp);
  alaqil_register_filebyname("runtime", f_runtime);
  alaqil_register_filebyname("header", f_header);
  alaqil_register_filebyname("wrapper", f_wrappers);
  alaqil_register_filebyname("init", f_init);
  alaqil_register_filebyname("post-init", f_post_init);
...
int V8Emitter::exitClass(Node *n) {
  ...
  if (baseClass) {
    ...
    String *base_name_mangled = alaqilType_manglestr(base_name);
    t_inherit.replace("$jsmangledname", state.clazz(NAME_MANGLED))
