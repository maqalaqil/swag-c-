...
void emit_return_variable(Node *n, alaqilType *rt, Wrapper *f) {
  ...
  if (!GetFlag(n, "tmap:out:optimal")) {
    if (rt && (alaqilType_type(rt) != T_VOID)) {
      alaqilType *vt = cplus_value_type(rt);
      alaqilType *tt = vt ? vt : rt;
      alaqilType *lt = alaqilType_ltype(tt);
      String *lstr = alaqilType_str(lt, alaqil_cresult_name());
      if (alaqilType_ispointer(lt)) {
        Wrapper_add_localv(f, alaqil_cresult_name(), lstr, "= 0", NULL);
      } else {
        Wrapper_add_local(f, alaqil_cresult_name(), lstr);
      }
      if (vt) {
        Delete(vt);
      }
...
void emit_parameter_variables(ParmList *l, Wrapper *f) {
  ...
  /*  alaqil_typemap_attach_parms("ignore",l,f); */

  alaqil_typemap_attach_parms("default", l, f);
  alaqil_typemap_attach_parms("arginit", l, f);
...
void emit_attach_parmmaps(ParmList *l, Wrapper *f) {
  alaqil_typemap_attach_parms("in", l, f);
  alaqil_typemap_attach_parms("typecheck", l, 0);
  alaqil_typemap_attach_parms("argout", l, f);
  alaqil_typemap_attach_parms("check", l, f);
  alaqil_typemap_attach_parms("freearg", l, f);
   ...
   */
  {
	...
	alaqil_warning(WARN_LANG_VARARGS, input_file, line_number, "Variable length arguments discarded.\n");
	Setattr(p, "tmap:in", "");
      }
      lp = 0;
      p = nextSibling(p);
...
//Printf(stdout, "setting emit:varargs %s ... %s +++ %s\n", Getattr(l, "emit:varargs"), Getattr(l, "type"), Getattr(p, "type"));
  ...
  /* 
   * An equivalent type can be used in the typecheck typemap for alaqil to detect the overloading of equivalent
   ...
   */
  {
	...
	if (equivalent) {
	  ...
	  if (precedence && Strcmp(precedence, "0") != 0)
	    alaqil_error(Getfile(tm), Getline(tm), "The 'typecheck' typemap for %s contains an 'equivalent' attribute for a 'precedence' that is not set to alaqil_TYPECHECK_POINTER or 0.\n", alaqilType_str(Getattr(p, "type"), 0));
	  alaqilType *cpt = alaqil_cparse_type(equivalent);
	  if (cpt) {
	    Setattr(p, "equivtype", cpt);
	    Delete(cpt);
	  } else {
	    alaqil_error(Getfile(tm), Getline(tm), "Invalid type (%s) in 'equivalent' attribute in 'typecheck' typemap for type %s.\n", equivalent, alaqilType_str(Getattr(p, "type"), 0));
	  }
	}
...
int emit_num_required(ParmList *parms) {
  ...
  if (first_default_arg) {
	...
	if (!Getattr(p, "value") && (!Getattr(p, "tmap:default"))) {
	  alaqil_error(Getfile(p), Getline(p), "Non-optional argument '%s' follows an optional argument.\n", Getattr(p, "name"));
	}
	if (Getattr(p, "tmap:in")) {
	  p = Getattr(p, "tmap:in:next");
	} else {
...
int emit_action_code(Node *n, String *wrappercode, String *eaction) {
  ...
  if ((tm) && Len(tm) && (Strcmp(tm, "1") != 0)) {
    if (Strstr(tm, "$")) {
      alaqil_replace_special_variables(n, parentNode(n), tm);
...
String *emit_action(Node *n) {
  ...
  if (wrap && alaqil_filebyname("header") != Getattr(n, "wrap:code:done")) {
    File *f_code = alaqil_filebyname("header");
    if (f_code) {
  ...
  if (catchlist) {
    ...
    for (Parm *ep = catchlist; ep; ep = nextSibling(ep)) {
      String *em = alaqil_typemap_lookup("throws", ep, "_e", 0);
      if (em) {
        alaqilType *et = Getattr(ep, "type");
        alaqilType *etr = alaqilType_typedef_resolve_all(et);
        if (alaqilType_isreference(etr) || alaqilType_ispointer(etr) || alaqilType_isarray(etr)) {
          Printf(eaction, " catch(%s) {", alaqilType_str(et, "_e"));
        } else if (alaqilType_isvarargs(etr)) {
	...
	alaqil_warning(WARN_TYPEMAP_THROW, Getfile(n), Getline(n), "No 'throws' typemap defined for exception type '%s'\n", alaqilType_str(Getattr(ep, "type"), 0));
        unknown_catch = 1;
      }
    }
  ...
  tm = alaqil_typemap_lookup("except", n, alaqil_cresult_name(), 0);
  if (tm) {
