 ...
 * guile.cxx
 *
 * Guile language module for alaqil.
 * ----------------------------------------------------------------------------- */
...
Guile Options (available with -guile)\n\
     ...
     -package <name>         - Set the path of the module to <name>\n\
                               (default NULL)\n\
     -prefix <name>          - Use <name> as prefix [default \"galaqil_\"]\n\
...
static String *prefix = NewString("galaqil_");
static char *module = 0;
static String *package = 0;
static enum {
  GUILE_LSTYLE_SIMPLE,		// call `alaqil_init()'
...
public:
  ...
  virtual void main(int argc, char *argv[]) {
    int i;
     ...
     alaqil_library_directory("guile");
     alaqil_typemap_lang("guile");
	...
	if (strcmp(argv[i], "-help") == 0) {
	  ...
	  if (argv[i + 1]) {
	    procdoc = NewFile(argv[i + 1], "w", alaqil_output_files());
	    if (!procdoc) {
	  ...
	  exportprimitive = 1;
	  // should use alaqil_warning() here?
	  alaqil_mark_arg(i);
	}
      }
    }
    ...
    Preprocessor_define("alaqilGUILE 1", 0);
    /* Read in default typemaps */
    alaqil_config_file("guile_scm.swg");
    allow_overloading();
  ...
  virtual int top(Node *n) {
    ...
    scmtext = NewString("");
    alaqil_register_filebyname("scheme", scmtext);
    exported_symbols = NewString("");
    goopstext = NewString("");
    alaqil_register_filebyname("goops", goopstext);
    goopscode = NewString("");
    ...
    case GUILE_LSTYLE_SIMPLE:
      /* Simple linkage; we have to export the alaqil_init function. The user can
         rename the function by a #define. */
      Printf(f_runtime, "#define alaqil_GUILE_INIT_STATIC extern\n");
      break;
    default:
      /* Other linkage; we make the alaqil_init function static */
      Printf(f_runtime, "#define alaqil_GUILE_INIT_STATIC static\n");
      break;
    }
    ...
    Printf(f_runtime, "alaqil_GUILE_INIT_STATIC void\nalaqil_init (void);\n");
    if (CPlusPlus) {
      Printf(f_runtime, "\n}\n");
    }
  ...
  void emit_linkage(String *module_name) {
	...
	Replaceall(mod, "/", " ");
	Printf(f_init, "    scm_register_module_xxx (\"%s\", (void *) alaqil_init);\n", mod);
	Printf(f_init, "    return SCM_UNSPECIFIED;\n");
	Delete(mod);
      }
      Printf(f_init, "}\n");
    ...
    case GUILE_LSTYLE_MODULE:
      ...
      Printf(f_init, "static void alaqil_init_helper(void *data)\n");
      Printf(f_init, "{\n    alaqil_init();\n");
	...
	Printf(f_init, "      alaqil_init_helper, NULL);\n");
	Printf(f_init, "    return SCM_UNSPECIFIED;\n");
	Delete(mod);
      }
      Printf(f_init, "}\n");
	...
	Printv(primitive_name, "-", primsuffix, NIL);
      ...
      String *fname = NewStringf("%s%s.scm",
				 alaqil_output_directory(),
				 primitive_name);
      Delete(primitive_name);
      File *scmstubfile = NewFile(fname, "w", alaqil_output_files());
      if (!scmstubfile) {
	...
	alaqil_exit(EXIT_FAILURE);
      ...
      alaqil_banner_target_lang(scmstubfile, ";;;");
      Printf(scmstubfile, "\n");
    ...
    if (goops) {
      ...
      String *fname = NewStringf("%s%s.scm", alaqil_output_directory(),
				 module_name);
      File *goopsfile = NewFile(fname, "w", alaqil_output_files());
      if (!goopsfile) {
	FileErrorDisplay(fname);
	alaqil_exit(EXIT_FAILURE);
      }
      Delete(fname);
      alaqil_banner_target_lang(goopsfile, ";;;");
      Printf(goopsfile, "\n");
      Printf(goopsfile, "(define-module (%s))\n", mod);
      Printf(goopsfile, "%s\n", goopstext);
      Printf(goopsfile, "(use-modules (oop goops) (alaqil common))\n");
      if (primRenamer) {
  ...
  bool handle_documentation_typemap(String *output,
    ...
    Replaceall(tm, "$type", alaqilType_str(pt, 0));
  ...
  virtual int functionWrapper(Node *n) {
    ...
    Wrapper_add_local(f, "galaqil_result", "SCM galaqil_result");
    Wrapper_add_local(f, "galaqil_list_p", "alaqilUNUSED int galaqil_list_p = 0");
	...
	alaqilType *pb = alaqilType_typedef_resolve_all(alaqilType_base(pt));
	alaqilType *pn = Getattr(p, "name");
	String *argname;
	scheme_argnum++;
	...
	if (goops) {
	  if (i < numreq) {
	    if (strcmp("void", Char(pt)) != 0) {
	      Node *class_node = alaqil_symbol_clookup_check(pb, Getattr(n, "sym:symtab"),
							   has_classname);
    ...
    if (exporting_destructor) {
      ...
      String *tm = NewString("alaqil_Guile_MarkPointerDestroyed($input);");
    ...
    if ((tm = alaqil_typemap_lookup_out("out", n, alaqil_cresult_name(), f, actioncode))) {
      Replaceall(tm, "$result", "galaqil_result");
      Replaceall(tm, "$target", "galaqil_result");
    ...
    Printv(f->code, "return galaqil_result;\n", NIL);
	...
	Printf(f_wrappers, "alaqil_Guile_GetArgs (arg, rest, %d, %d, \"%s\");\n", numreq, numargs - numreq, proc_name);
	...
	Printv(f_wrappers, "}\n", NIL);
	/* Register it */
	Printf(f_init, "scm_c_define_gsubr(\"%s\", 0, 0, 1, (alaqil_guile_proc) %s_rest);\n", proc_name, wname);
	...
	if (only_setters && !goops)
	  Printf(f_init, "scm_c_make_gsubr(\"%s\", %d, %d, 0, (alaqil_guile_proc) %s);\n", proc_name, numreq, numargs - numreq, wname);
	else
	  Printf(f_init, "scm_c_define_gsubr(\"%s\", %d, %d, 0, (alaqil_guile_proc) %s);\n", proc_name, numreq, numargs - numreq, wname);
	...
	if (exporting_destructor) {
	  Printf(f_init, "((alaqil_guile_clientdata *)(alaqilTYPE%s->clientdata))->destroy = (guile_destructor) %s;\n", alaqiltype_ptr, wname);
	  //Printf(f_init, "alaqil_TypeClientData(alaqilTYPE%s, (void *) %s);\n", alaqiltype_ptr, wname);
	}
	Printf(f_init, "scm_c_define_gsubr(\"%s\", %d, %d, 0, (alaqil_guile_proc) %s);\n", proc_name, numreq, numargs - numreq, wname);
      }
	...
	Printf(df->code, "int argc = alaqil_Guile_GetArgs (argv, rest, %d, %d, \"%s\");\n", 0, maxargs, proc_name);
	Printv(df->code, dispatch, "\n", NIL);
	Printf(df->code, "scm_misc_error(\"%s\", \"No matching method for generic function `%s'\", SCM_EOL);\n", proc_name, iname);
	Printf(df->code, "#undef FUNC_NAME\n");
	Printv(df->code, "}\n", NIL);
	Wrapper_print(df, f_wrappers);
	Printf(f_init, "scm_c_define_gsubr(\"%s\", 0, 0, 1, (alaqil_guile_proc) %s);\n", proc_name, dname);
	DelWrapper(df);
	Delete(dispatch);
	Delete(dname);
      }
  ...
  virtual int variableWrapper(Node *n) {
    ...
    if (1 || (alaqilType_type(t) != T_USER) || (is_a_pointer(t))) {
	...
	if (!goops && GetFlag(n, "feature:constasvar")) {
	  ...
	  if (scmstub) {
	    ...
	    Printf(f_init, "scm_c_define_gsubr(\"%s\", 0, %d, 0, (alaqil_guile_proc) %s);\n", proc_name, !GetFlag(n, "feature:immutable"), var_name);
	...
	Printf(f_init, "{ SCM p = scm_c_define_gsubr(\"%s\", 0, 1, 0, (alaqil_guile_proc) %s);\n", proc_name, var_name);
	Printf(f_init, "scm_c_define");
	...
	String *class_name = alaqilType_typedef_resolve_all(alaqilType_base(t));
  ...
  virtual int constantWrapper(Node *n) {
    ...
    if ((alaqilType_type(nctype) == T_USER) && (!is_a_pointer(nctype))) {
    ...
    if ((tm = alaqil_typemap_lookup("constant", n, name, 0))) {
      ...
      Printf(f_header, "static %s = (%s)(%s);\n", alaqilType_str(type, var_name), alaqilType_str(type, 0), value);
    }
    {
  ...
  virtual int classHandler(Node *n) {
    ...
    Printf(f_runtime, "static alaqil_guile_clientdata _alaqil_guile_clientdata%s = { NULL, SCM_EOL };\n", mangled_classname);

    Printv(f_init, "alaqil_TypeClientData(alaqilTYPE", alaqiltype_ptr, ", (void *) &_alaqil_guile_clientdata", mangled_classname, ");\n", NIL);
    ...
    Printv(goopscode, "  #:metaclass <alaqil-metaclass>\n", NIL);
    ...
    if (goops) {
      ...
      Printf(funcName, "_alaqil_guile_setgoopsclass");
      ...
      Printv(f_wrappers, "  ((alaqil_guile_clientdata *)(alaqilTYPE", alaqiltype_ptr, "->clientdata))->goops_class = cl;\n", NIL);
      Printf(f_wrappers, "  return SCM_UNSPECIFIED;\n");
      Printf(f_wrappers, "}\n#undef FUNC_NAME\n\n");

      Printf(f_init, "scm_c_define_gsubr(\"%s\", 1, 0, 0, (alaqil_guile_proc) %s);\n", guileFuncName, funcName);
...
#     undef store_pragma
  ...
  String *runtimeCode() {
    String *s;
    s = alaqil_include_sys("guile_scm_run.swg");
    if (!s) {
  ...
  String *defaultExternalRuntimeFilename() {
    return NewString("alaqilguilerun.h");
  }
};

/* -----------------------------------------------------------------------------
 * alaqil_guile()    - Instantiate module
 * ----------------------------------------------------------------------------- */

static Language *new_alaqil_guile() {
  return new GUILE();
}
extern "C" Language *alaqil_guile(void) {
  return new_alaqil_guile();
}
