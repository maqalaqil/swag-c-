 ...
 * allegrocl.cxx
 *
 * ALLEGROCL language module for alaqil.
 * ----------------------------------------------------------------------------- */
...
Allegro CL Options (available with -allegrocl)\n\
     ...
     -isolate        - Define all alaqil helper functions in a package unique to this\n\
                       module. Avoids redefinition warnings when loading multiple\n\
                       alaqilged modules into the same running Allegro CL image.\n\
";

static File *f_cl = 0;
String *f_clhead = NewString("");
String *f_clwrap = NewString("(alaqil-in-package ())\n\n");
static File *f_begin;
...
static String *alaqil_package = 0;
...
static bool unique_alaqil_package = false;

static alaqilType *fwdref_ffi_type = NewString("__alaqilACL_FwdReference");
...
String *class_from_class_or_class_ref(String *type) {
  alaqilType *stripped = alaqilType_strip_qualifiers(type);
  if (alaqilType_isclass(stripped))
    return stripped;

  if (alaqilType_ispointer(stripped) || alaqilType_isreference(stripped)) {
    // Printf(stderr,"It is a pointer/reference. Is it a class?\n");
    alaqilType_pop(stripped);
    if (alaqilType_isclass(stripped)) {
      return stripped;
    }
  }
...
static String *strip_namespaces(String *str) {
  return alaqil_scopename_last(str);
}
...
void add_forward_referenced_type(Node *n, int overwrite = 0) {
  ...
  if (!val || overwrite) {
    ...
    String *mangled_lname_gen = NewStringf("#.(alaqil-insert-id \"%s\" %s :type :class)", name, ns);

    Setattr(defined_foreign_ltypes, Copy(k), mangled_lname_gen);
    //    Printf(f_cl, ";; forward reference stub\n"
    //           "(alaqil-def-foreign-class \"%s\" (ff:foreign-pointer) (:class ))\n\n"
    //     , name);
...
void add_defined_foreign_type(Node *n, int overwrite = 0, String *k = 0,
    ...
    if (templated) {
	 ...
	 Both modes:
         ...
         non-typedef'd structs, the classHand mapping will be
           ...
           struct __xxx -> (alaqil-insert-id "__xxx")
       */
      // alaqil_print_node(n);
  ...
  String *tname = alaqilType_istemplate_templateprefix(name);
  if (tname) {
    String *temp = strip_namespaces(tname);
    name = NewStringf("%s%s%s", temp, alaqilType_templateargs(name), alaqilType_templatesuffix(name));
    Delete(temp);
    Delete(tname);
  }
  ...
  if (!val || overwrite || is_fwd_ref) {
...
#endif
    String *mangled_name_gen = NewStringf("#.(alaqil-insert-id \"%s\" %s :type :type)", name, ns_list);
    String *mangled_lname_gen = NewStringf("#.(alaqil-insert-id \"%s\" %s :type :class)", name, ns_list);
	...
	add_linked_type(n);
      ...
      // Printf(stderr,"*** 1-a\n");
      if (alaqilType_istemplate(k)) {
	alaqilType *resolved = alaqilType_typedef_resolve_all(k);
	// Printf(stderr,"*** 1-b\n");
	...
	if (!Strcmp(nodeType(n), "cdecl") && !Strcmp(Getattr(n, "storage"), "typedef")) {
	  alaqilType *type = alaqilType_strip_qualifiers(Getattr(n, "type"));
#ifdef ALLEGROCL_CLASS_DEBUG
	  Printf(stderr, "Examining typedef '%s' for class references. (%d)\n", type, alaqilType_isclass(type));
#endif
	  if (alaqilType_isclass(type)) {
		...
		&& defined_type && !Strcmp(defined_type, defined_key_type)) {
	      ...
	      alaqilType *lookup_type = alaqilType_istemplate(type) ? alaqilType_typedef_resolve_all(type) : Copy(type);
		...
		String *mangled_new_name = NewStringf("#.(alaqil-insert-id \"%s\" %s)", symname, ns_list);
		String *mangled_new_lname = NewStringf("#.(alaqil-insert-id \"%s\" %s :type :class)", symname, ns_list);
	  ...
	  String *resolved = alaqilType_typedef_resolve_all(Getattr(n, "name"));
    ...
    if (!CPlusPlus || Strcmp(Getattr(n,"kind"),"typedef")) {
       alaqil_warning(WARN_TYPE_REDEFINED, Getfile(n), Getline(n),
...
void note_implicit_template_instantiation(alaqilType *t) {
  // the namespace of the implicit instantiation is not necessarily
  // current_namespace. Attempt to cull this from the type.
#ifdef ALLEGROCL_CLASS_DEBUG
  Printf(stderr, "culling namespace of '%s' from '%s'\n", t, alaqilType_templateprefix(t));
#endif
  alaqilType *type = Copy(t);
  alaqilType *tok = alaqilType_pop(type);
  String *implicit_ns = alaqilType_istemplate(tok) ? alaqil_scopename_prefix(alaqilType_templateprefix(tok)) : 0;
  add_defined_foreign_type(0, 0, t, t, implicit_ns ? implicit_ns : current_namespace);

  Delete(type);
}

String *get_ffi_type(Node *n, alaqilType *ty, const_String_or_char_ptr name) {
  ...
  if (found_type) {
    ...
    const String *tm = alaqil_typemap_lookup("ffitype", node, name, 0);
    Delete(node);
...
/* walk type and return string containing lisp version.
   recursive. */
String *internal_compose_foreign_type(Node *n, alaqilType *ty) {
  ...
  while (Len(ty) != 0) {
    tok = alaqilType_pop(ty);

    if (alaqilType_isfunction(tok)) {
      // Generate Function wrapper
      Printf(ffiType, "(:function ");
      // walk parm list
      List *pl = alaqilType_parmlist(tok);

      Printf(ffiType, "(");	// start parm list
      for (Iterator i = First(pl); i.item; i = Next(i)) {
	alaqilType *f_arg = alaqilType_strip_qualifiers(i.item);
	...
	Delete(f_arg);
      ...
      break;
    } else if (alaqilType_ispointer(tok) || alaqilType_isreference(tok)) {
      Printf(ffiType, "(* %s)", internal_compose_foreign_type(n, ty));
    } else if (alaqilType_isarray(tok)) {
      Printf(ffiType, "(:array %s", internal_compose_foreign_type(n, ty));
      String *atype = NewString("int");
      String *dim = convert_literal(alaqilType_array_getdim(tok, 0), atype);
      Delete(atype);
      if (is_integer(dim)) {
	Printf(ffiType, " %s)", dim);
      } else {
	Printf(ffiType, " #| %s |#)", alaqilType_array_getdim(tok, 0));
      }
    } else if (alaqilType_ismemberpointer(tok)) {
      // temp
      Printf(ffiType, "(* %s)", internal_compose_foreign_type(n, ty));
    } else {
	...
	alaqilType *resolved_type = alaqilType_typedef_resolve_all(tok);
	...
	if (!res) {
	  ...
	  // alaqil_warning(WARN_TYPE_UNDEFINED_CLASS, Getfile(tok), Getline(tok), "Unable to find definition of '%s', assuming forward reference.\n", tok);
...
*/
  ...
  alaqilType *temp = alaqilType_strip_qualifiers(ty);
...
static String *mangle_name(Node *n, char const *prefix = "ACL", String *ns = current_namespace) {
  ...
  String *mangled_name = alaqil_name_mangle(pre_mangled_name);
...
String *infix_to_prefix(String *val, char split_op, const String *op, String *type) {
  ...
  if (((split_op == '+') || (split_op == '-')) && Len(ored) == 2 &&
      (alaqilType_type(type) == T_FLOAT || alaqilType_type(type) == T_DOUBLE || alaqilType_type(type) == T_LONGDOUBLE)) {
...
String *convert_literal(String *literal, String *type, bool try_to_split) {
  ...
  if (try_to_split && alaqilType_type(type) != T_STRING) {
  ...
  if (alaqilType_type(type) == T_FLOAT || alaqilType_type(type) == T_DOUBLE || alaqilType_type(type) == T_LONGDOUBLE) {
	...
	*num_end = '\0';
	...
	num = 0;
      ...
      String *id = NewStringf("#.(alaqil-insert-id \"%s\" %s :type :constant)",
			      num, ns);
      Delete(num);
      num = id;
    }
    ...
    res = NewStringf("#.(alaqil-insert-id \"%s\" %s :type :constant)", num, ns);
    Delete(num);
    Delete(trimmed);
    Delete(ns);
    return res;
...
void emit_stub_class(Node *n) {
  ...
  Printf(f_clhead, "(alaqil-def-foreign-stub \"%s\")\n", name);
...
void emit_synonym(Node *synonym) {
  ...
  if (CPlusPlus && !Strcmp(nodeType(synonym), "cdecl")) {
    ...
    syn_ltype = NewStringf("#.(alaqil-insert-id \"%s\" %s :type :class)", strip_namespaces(real_name), synonym_ns);
    syn_type = NewStringf("#.(alaqil-insert-id \"%s\" %s :type :type)", strip_namespaces(real_name), synonym_ns);
  } else {
  ...
  if( of_ltype )
      Printf(f_clhead, "(alaqil-def-synonym-type %s\n   %s\n   %s)\n", syn_ltype, of_ltype, syn_type);
...
void emit_full_class(Node *n) {
  ...
  for (c = firstChild(n); c; c = nextSibling(c)) {
	...
	childDecl = NewString("");
      ...
      if (!alaqilType_isfunction(childType)) {
...
#endif
	Printf(slotdefs, "(#.(alaqil-insert-id \"%s\" %s :type :slot :class \"%s\") %s)", cname, ns, name, compose_foreign_type(n, childType));
	Delete(ns);
	if (access && Strcmp(access, "public"))
  ...
  update_package_if_needed(n, f_clhead);
  Printf(f_clhead, "(alaqil-def-foreign-class \"%s\"\n %s\n  (:%s\n%s))\n\n", name, supers, kind, slotdefs);
...
void emit_typedef(Node *n) {
  ...
  if (in_class) {
    ...
    String *tname = alaqilType_istemplate_templateprefix(class_name);
    if (tname) {
      String *temp = strip_namespaces(tname);
      class_name = NewStringf("%s%s%s", temp, alaqilType_templateargs(class_name), alaqilType_templatesuffix(class_name));
      Delete(temp);
      Delete(tname);
    }
...
//  Printf(f_clhead, ";; %s\n", alaqilType_typedef_resolve_all(lisp_type));
  Printf(f_clhead, "(alaqil-def-foreign-type \"%s\"\n  %s)\n", name, lisp_type);
...
void emit_enum_type_no_wrap(Node *n) {
  ...
  alaqilType *tmp = NewStringf("enum %s", unnamed ? unnamed : name);

  Node *node = NewHash();
  Setattr(node, "type", tmp);
  Setfile(node, Getfile(n));
  Setline(node, Getline(n));
  const String *enumtype = alaqil_typemap_lookup("ffitype", node, "", 0);
  Delete(node);
  ...
  if (name) {
    ...
    Printf(f_clhead, "(alaqil-def-foreign-type \"%s\" %s)\n", name, enumtype);
    Delete(ns);
	...
	if (converted_val) {
	  Printf(f_clhead, "(alaqil-defconstant \"%s\" %s)\n", valname, converted_val);
	  Delete(converted_val);
	} else {
	  alaqil_warning(WARN_LANG_DISCARD_CONST, Getfile(n), Getline(n), "Unable to parse enum value '%s'. Setting to NIL\n", val);
	  Printf(f_clhead, "(alaqil-defconstant \"%s\" nil #| %s |#)\n", valname, val);
	}
      }
    }
  }
...
void emit_enum_type(Node *n) {
  ...
  if (name) {
    ...
    for(c = firstChild(n); c; c=nextSibling(c)) {
      ...
      Printf(f_clhead, "(alaqil-defvar \"%s\" \"%s\" :type :constant :ftype :signed-long)\n", Getattr(c, "sym:name"), mangled_name);
      Delete(mangled_name);
    }
  }
#ifdef ALLEGROCL_WRAP_DEBUG
...
void emit_default_linked_type(Node *n) {
  ...
  if (!Strcmp(nodeType(n), "classforward")) {
    ...
    Printf(f_clhead, "(alaqil-def-foreign-class \"%s\" (ff:foreign-pointer) (:class ))\n\n", Getattr(n, "sym:name"));
...
extern "C" Language *alaqil_allegrocl(void) {
...
void ALLEGROCL::main(int argc, char *argv[]) {
  ...
  Preprocessor_define("alaqilALLEGROCL 1", 0);
  alaqil_library_directory("allegrocl");
  alaqil_config_file("allegrocl.swg");
...
int ALLEGROCL::top(Node *n) {
  ...
  alaqil_package = unique_alaqil_package ? NewStringf("alaqil.%s", module_name) : NewString("alaqil");

  Printf(cl_filename, "%s%s.cl", alaqil_output_directory(), module_name);

  f_cl = NewFile(cl_filename, "w", alaqil_output_files());
  if (!f_cl) {
  ...
  if (Generate_Wrapper) {
    f_begin = NewFile(cxx_filename, "w", alaqil_output_files());
    if (!f_begin) {
      Delete(f_cl);
      Printf(stderr, "Unable to open %s for writing\n", cxx_filename);
      alaqil_exit(EXIT_FAILURE);
  ...
  alaqil_register_filebyname("header", f_cxx_header);
  alaqil_register_filebyname("wrapper", f_cxx_wrapper);
  alaqil_register_filebyname("begin", f_begin);
  alaqil_register_filebyname("runtime", f_runtime);
  alaqil_register_filebyname("lisp", f_clwrap);
  alaqil_register_filebyname("lisphead", f_cl);

  alaqil_banner(f_begin);

  Printf(f_runtime, "\n\n#ifndef alaqilALLEGROCL\n#define alaqilALLEGROCL\n#endif\n\n");
  ...
  Printf(f_cl, "\n"
	 ...
	 "  (:export #:*alaqil-identifier-converter* #:*alaqil-module-name*\n"
	 "           #:*void* #:*alaqil-export-list*))\n"
	 "(in-package :%s)\n\n"
	 "(eval-when (:compile-toplevel :load-toplevel :execute)\n"
	 "  (defparameter *alaqil-identifier-converter* '%s)\n"
	 "  (defparameter *alaqil-module-name* :%s))\n\n", alaqil_package, alaqil_package, identifier_converter, module_name);
  Printf(f_cl, "(defpackage :%s\n" "  (:use :common-lisp :%s :ff :excl))\n\n", module_name, alaqil_package);
  ...
  Printf(f_clwrap, "\n(cl::in-package :%s)\n", alaqil_package);
  Printf(f_clwrap, "\n(macrolet ((alaqil-do-export ()\n");
  Printf(f_clwrap, "                 `(dolist (s ',*alaqil-export-list*)\n");
  Printf(f_clwrap, "                    (apply #'export s))))\n");
  Printf(f_clwrap, "   (alaqil-do-export))\n");
  Printf(f_clwrap, "\n(setq *alaqil-export-list* nil)\n");
...
String *get_lisp_type(Node *n, alaqilType *ty, const_String_or_char_ptr name) {
  Node *node = NewHash();
  Setattr(node, "type", ty);
...
String *id_converter_type(alaqilType const *type) {
  ...
  if (alaqilType_ispointer(t)) {
    ...
    String *klass = alaqilType_parm(t);
    alaqilType_pop(t);
    ...
    String *parmlist_str = alaqilType_parm(t);
    List *parms = alaqilType_parmlist(parmlist_str);

    for (Iterator i = First(parms); i.item;) {
      String *parm = id_converter_type((alaqilType *) i.item);
    ...
    String *qualifiers_str = Copy(alaqilType_parm(t));	// ?!
    ...
    Delete(qualifiee);
  } else if (alaqilType_istemplate(t)) {
...
String *dispatching_type(Node *n, Parm *p) {
  ...
  String *parsed = Getattr(p, "type");	//alaqil_cparse_type(Getattr(p,"tmap:ctype"));
  String *cl_t = alaqilType_typedef_resolve_all(parsed);

  Node *node = NewHash();
  Setattr(node, "type", parsed);
  Setfile(node, Getfile(n));
  Setline(node, Getline(n));
  const String *tm = alaqil_typemap_lookup("lispclass", node, Getattr(p, "name"), 0);
  Delete(node);
  ...
  //  if (!result && alaqilType_ispointer(cl_t)) {
  //    alaqilType_pop(cl_t);
...
int ALLEGROCL::emit_dispatch_defun(Node *n) {
...
#endif
  List *overloads = alaqil_overload_rank(n, true);
  ...
  Printf(f_clwrap, "(alaqil-dispatcher (\"%s\" %s :arities (", Getattr(overloaded_from, "allegrocl:dispatcher:name"), id_args);
...
int ALLEGROCL::emit_defun(Node *n, File *fcl) {
  ...
  alaqil_typemap_attach_parms("lin", pl, wrap);
  // alaqil_typemap_attach_parms("ffitype", pl, wrap);
  alaqil_typemap_lookup("lout", n, alaqil_cresult_name(), 0);

  alaqilType *result_type = alaqil_cparse_type(Getattr(n, "tmap:ctype"));
  ...
  if (Generate_Wrapper) {
	...
	// Printf(stderr, "   set a:d:name='%s', a:d:i-c='%s'\n", Getattr(n,"allegrocl:dispatcher:name"), Getattr(n,"allegrocl:dispatcher:include-class"));
      ...
      Printf(fcl, "(alaqil-defmethod (\"%s\" \"%s\"%s)\n", funcname, mangled_name, extra_parms);
    } else
      Printf(fcl, "(alaqil-defun (\"%s\" \"%s\"%s)\n", funcname, mangled_name, extra_parms);
    Delete(extra_parms);
  }
  // Just C
  else {
    Printf(fcl, "(alaqil-defun (\"%s\" \"%s\")\n", funcname, Generate_Wrapper ? mangled_name : funcname);
  }
...
/*  } else if (any_varargs(pl)) {
    ...
    for (p = pl; p; p = nextSibling(p), argnum++, largnum++) {
      ...
      alaqilType *argtype = alaqil_cparse_type(Getattr(p, "tmap:ctype"));
      alaqilType *parmtype = Getattr(p,"type");
	...
	Printf(fcl, "\n   ");
      ...
      if (!alaqilType_isvarargs(parmtype)) {
	...
	// wrapper code...
	Setattr(p, "lname", NewStringf("alaqil_arg%d", largnum));
  ...
  alaqilType *parsed = alaqil_cparse_type(Getattr(n, "tmap:ctype"));
  //  alaqilType *cl_t = alaqilType_typedef_resolve_all(parsed);
  alaqilType *cl_t = class_from_class_or_class_ref(parsed);
  String *out_ffitype = compose_foreign_type(n, parsed);
  String *deref_out_ffitype;
  String *out_temp = Copy(parsed);

  if (alaqilType_ispointer(out_temp)) {
    alaqilType_pop(out_temp);
  ...
  Printf(stderr, "for output wrapping %s: type=%s, ctype=%s\n", Getattr(n, "name"),
	 Getattr(n, "type"), alaqil_cparse_type(Getattr(n, "tmap:ctype")));
#endif
  ...
  Replaceall(wrap->code, "$body", NewStringf("(alaqil-ff-call%s)", wrap->locals));
  ...
  /////////////////////////////////////////////////////
  Printf(fcl, "  (:returning (%s %s)", compose_foreign_type(n, result_type), get_lisp_type(n, Getattr(n, "type"), alaqil_cresult_name()));
...
int ALLEGROCL::functionWrapper(Node *n) {
  ...
  String *raw_return_type = alaqil_typemap_lookup("ctype", n, "", 0);
  alaqilType *return_type = alaqil_cparse_type(raw_return_type);
  alaqilType *resolved = alaqilType_typedef_resolve_all(return_type);
  ...
  if (!is_void_return) {
     String *lresult_init =
	     NewStringf("= (%s)0",
			alaqilType_str(alaqilType_strip_qualifiers(return_type),0));
     Wrapper_add_localv(f, "lresult",
			alaqilType_lstr(alaqilType_ltype(return_type), "lresult"),
			lresult_init, NIL);
     Delete(lresult_init);
  }
  ...
  alaqil_typemap_attach_parms("ctype", parms, f);
  alaqil_typemap_attach_parms("lin", parms, f);
  ...
  if (overloaded) {
    ...
    // basically alaqil_overload_check(n), but with script_lang_wrapping
    // set to true.
    Delete(alaqil_overload_rank(n, true));
  ...
  for (i = 0, p = parms; i < num_arguments; i++) {
    ...
    alaqilType *c_parm_type = alaqil_cparse_type(Getattr(p, "tmap:ctype"));
    String *arg = NewStringf("l%s", Getattr(p, "lname"));

    // Emit parameter declaration
    if (gencomma)
      Printf(name_and_parms, ", ");
    String *parm_decl = alaqilType_str(c_parm_type, arg);
  ...
  String *signature = alaqilType_str(return_type, name_and_parms);
...
int ALLEGROCL::namespaceDeclaration(Node *n) {
  ...
  */
  ...
  if (!GetInt(defined_namespace_packages, current_namespace)) {
    ...
    Printf(f_clhead, "(alaqil-defpackage %s)\n", lispy_namespace);
...
int ALLEGROCL::constructorHandler(Node *n) {
  ...
  // Let alaqil generate a global forwarding function.
...
int ALLEGROCL::constantWrapper(Node *n) {
  ...
  if (Generate_Wrapper) {
    ...
    if (alaqilType_type(const_type) == T_STRING) {
      const_val = NewStringf("\"%s\"", raw_const);
    } else if (alaqilType_type(const_type) == T_CHAR) {
      const_val = NewStringf("'%s'", raw_const);
    } else {
      const_val = Copy(raw_const);
    }

    alaqilType_add_qualifier(const_type, "const");

    String *ppcname = NewStringf("ACLppc_%s", Getattr(n, "sym:name"));
    // Printf(f_runtime, "static const %s = %s;\n", alaqilType_lstr(const_type, ppcname), const_val);
  ...
  if (converted_value) {
    Printf(f_clwrap, "(alaqil-defconstant \"%s\" %s)\n", name, converted_value);
  } else {
    alaqil_warning(WARN_LANG_DISCARD_CONST, Getfile(n), Getline(n), "Unable to parse constant value '%s'. Setting to NIL\n", value);
    Printf(f_clwrap, "(alaqil-defconstant \"%s\" nil #| %s |#)\n", name, value);
  }
...
int ALLEGROCL::globalvariableHandler(Node *n) {
  ...
  if (alaqilType_isclass(rtype)) {
    alaqilType_add_pointer(type);
    alaqilType_add_pointer(rtype);
  }

  Printf(f_clwrap, "(alaqil-defvar \"%s\" \"%s\" :type %s)\n",
	 Getattr(n, "sym:name"), Getattr(n, "sym:name"), ((alaqilType_isconst(type)) ? ":constant" : ":variable"));
...
int ALLEGROCL::variableWrapper(Node *n) {
  ...
  // Let alaqil generate a get/set function pair.
  if (Generate_Wrapper)
  ...
  ctype = alaqilType_str(type, 0);

  // EXPORT <alaqilType_str> <mangled_name>;
  // <alaqilType_str> <mangled_name> = <name>;
  Printf(f_runtime, "EXPORT %s %s;\n%s %s = %s%s;\n", ctype, mangled_name, ctype, mangled_name, (pointer_added ? "&" : ""), name);

  Printf(f_cl, "(alaqil-defvar \"%s\" :type %s)\n", mangled_name, ((alaqilType_isconst(type)) ? ":constant" : ":variable"));
...
int ALLEGROCL::typedefHandler(Node *n) {
  ...
  alaqilType *typedef_type = Getattr(n,"type");
...
#ifdef ALLEGROCL_TYPE_DEBUG
  Printf(stderr, "** lookup='%s'(%p), typedef_type='%s', strcmp = '%d' strstr = '%d'\n", lookup, lookup, typedef_type, Strcmp(typedef_type,"void"), Strstr(ff_type,"__alaqilACL_FwdReference"));
#endif

  if(lookup || (!lookup && Strcmp(typedef_type,"void")) ||
     (!lookup && Strstr(ff_type,"__alaqilACL_FwdReference"))) {
...
int ALLEGROCL::cppClassHandler(Node *n) {
  ...
  /* Template instantiation.
     Careful. 
     alaqil does not create instantiations of templated classes whenever
  ...
  for (c = firstChild(n); c; c = nextSibling(c)) {
    ...
    alaqilType *childType = NewStringf("%s%s", Getattr(c, "decl"),
				     Getattr(c, "type"));
#ifdef ALLEGROCL_CLASS_DEBUG
    Printf(stderr, "looking at child '%p' of type '%s' '%d'\n", c, childType, alaqilType_isfunction(childType));
    // alaqil_print_node(c);
#endif
    if (!alaqilType_isfunction(childType))
...
int ALLEGROCL::emit_one(Node *n) {
  ...
  if (Cmp(nodeType(n), "namespace") != 0 && Cmp(current_package, current_namespace) != 0) {
    String *lispy_namespace = listify_namespace(current_namespace);
    Printf(f_clwrap, "(alaqil-in-package %s)\n", lispy_namespace);
    Delete(lispy_namespace);
    Delete(current_package);
...
int ALLEGROCL::enumvalueDeclaration(Node *n) {
  ...
  /* print this when in C mode? make this a command-line arg? */
  if (Generate_Wrapper) {
	  alaqilType *enum_type = Copy(Getattr(n,"type"));
	  String *mangled_name =
		  mangle_name(n, "ACL_ENUM",
			      in_class ? Getattr(in_class,"name") :
			      current_namespace);
	  
	  alaqilType_add_qualifier(enum_type,"const");

	  String *enum_decl = alaqilType_str(enum_type, mangled_name);
...
int ALLEGROCL::templateDeclaration(Node *n) {
  ...
  if (!Strcmp(type, "cdecl")) {
    alaqilType *ty = NewStringf("%s%s", Getattr(n, "decl"),
			      Getattr(n, "type"));
    Delete(ty);
  }
