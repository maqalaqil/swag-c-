 ...
 * java.cxx
 *
 * Java language module for alaqil.
 * ----------------------------------------------------------------------------- */
...
public:
  ...
  void constructIntermediateClassName(Node *n) {
    ...
    if (nspace && !package) {
      ...
      alaqil_warning(WARN_JAVA_NSPACE_WITHOUT_PACKAGE, Getfile(n), Getline(n),
   ...
   * ----------------------------------------------------------------------------- */
   ...
   String *getProxyName(alaqilType *t, bool jnidescriptor = false) {
  ...
  virtual void main(int argc, char *argv[]) {
    ...
    Preprocessor_define("alaqilJAVA 1", 0);

    // Add typemap definitions
    alaqil_typemap_lang("java");
    alaqil_config_file("java.swg");

    allow_overloading();
    alaqil_interface_feature_enable();
  }
  ...
  virtual int top(Node *n) {
    ...
    alaqil_name_register("wrapper", Char(wrapper_name));
    if (old_variable_names) {
      alaqil_name_register("set", "set_%n%v");
      alaqil_name_register("get", "get_%n%v");
    }
    ...
    // Generate the intermediary class
    {
      ...
      File *f_im = NewFile(filen, "w", alaqil_output_files());
      if (!f_im) {
	...
	Printv(f_im, "\n", imclass_directors, NIL);
      ...
      if (n_dmethods > 0) {
	Putc('\n', f_im);
	Printf(f_im, "  private final static native void alaqil_module_init();\n");
	Printf(f_im, "  static {\n");
	Printf(f_im, "    alaqil_module_init();\n");
	Printf(f_im, "  }\n");
      }
    ...
    {
      String *filen = NewStringf("%s%s.java", alaqil_output_directory(), module_class_name);
      File *f_module = NewFile(filen, "w", alaqil_output_files());
      if (!f_module) {
	FileErrorDisplay(filen);
	alaqil_exit(EXIT_FAILURE);
      }
    ...
    if (Len(module_class_constants_code) != 0) {
      String *filen = NewStringf("%s%s.java", alaqil_output_directory(), constants_interface_name);
      File *f_module = NewFile(filen, "w", alaqil_output_files());
      if (!f_module) {
	FileErrorDisplay(filen);
	alaqil_exit(EXIT_FAILURE);
    ...
    // Output a Java type wrapper class for each alaqil type
    for (Iterator alaqil_type = First(alaqil_types_hash); alaqil_type.key; alaqil_type = Next(alaqil_type)) {
      emitTypeWrapperClass(alaqil_type.key, alaqil_type.item);
    }
    ...
    for (it1 = First(filenames_list); it1.item; it1 = Next(it1)) {
      String *item1_lower = alaqil_string_lower(it1.item);
      for (it2 = Next(it1); it2.item; it2 = Next(it2)) {
	String *item2_lower = alaqil_string_lower(it2.item);
	if (it1.item && it2.item) {
	  if (Strcmp(item1_lower, item2_lower) == 0) {
	    alaqil_warning(WARN_LANG_PORTABILITY_FILENAME, input_file, line_number,
  ...
  virtual int nativeWrapper(Node *n) {
    ...
    if (Getattr(n, "type")) {
      alaqil_save("nativeWrapper", n, "name", NIL);
      ...
      alaqil_error(input_file, line_number, "No return type for %%native method %s.\n", Getattr(n, "wrap:name"));
    }
  ...
  virtual int functionWrapper(Node *n) {
    ...
    String *wname = alaqil_name_wrapper(jniname);

    Delete(jniname);

    /* Attach the non-standard typemaps to the parameter list. */
    alaqil_typemap_attach_parms("jni", l, f);
    alaqil_typemap_attach_parms("jtype", l, f);
    alaqil_typemap_attach_parms("jstype", l, f);

    /* Get return types */
    if ((tm = alaqil_typemap_lookup("jni", n, "", 0))) {
      Printf(c_return_type, "%s", tm);
    } else {
      alaqil_warning(WARN_JAVA_TYPEMAP_JNI_UNDEF, input_file, line_number, "No jni typemap defined for %s\n", alaqilType_str(t, 0));
    }

    if ((tm = alaqil_typemap_lookup("jtype", n, "", 0))) {
      Printf(im_return_type, "%s", tm);
    } else {
      alaqil_warning(WARN_JAVA_TYPEMAP_JTYPE_UNDEF, input_file, line_number, "No jtype typemap defined for %s\n", alaqilType_str(t, 0));
    }
    ...
    Printv(f->def, "alaqilEXPORT ", c_return_type, " JNICALL ", wname, "(JNIEnv *jenv, jclass jcls", NIL);
    ...
    if ((throw_parm_list = Getattr(n, "catchlist"))) {
      alaqil_typemap_attach_parms("throws", throw_parm_list, f);
    ...
    Replaceall(f->code, "alaqil_contract_assert(", "alaqil_contract_assert($null, ");
    ...
    if (proxy_flag && wrapping_member_flag && !enum_constant_flag) {
      ...
      bool getter_flag = Cmp(symname, alaqil_name_set(getNSpace(), alaqil_name_member(0, getClassPrefix(), variable_name))) != 0;
  ...
  virtual int enumDeclaration(Node *n) {
	...
	if (proxy_flag && is_wrapping_class()) {
	  ...
	  File *f_enum = NewFile(filen, "w", alaqil_output_files());
	  if (!f_enum) {
	    FileErrorDisplay(filen);
	    alaqil_exit(EXIT_FAILURE);
	  }
  ...
  virtual int constantWrapper(Node *n) {
    String *symname = Getattr(n, "sym:name");
    alaqilType *t = Getattr(n, "type");
    alaqilType *valuetype = Getattr(n, "valuetype");
    ParmList *l = Getattr(n, "parms");
    String *tm;
    String *return_type = NewString("");
    String *constants_code = NewString("");
    alaqil_save("constantWrapper", n, "value", NIL);
    ...
    /* Adjust the enum type for the alaqil_typemap_lookup.
    ...
    /* Attach the non-standard typemaps to the parameter list. */
    alaqil_typemap_attach_parms("jstype", l, NULL);

    /* Get Java return types */
    bool classname_substituted_flag = false;

    if ((tm = alaqil_typemap_lookup("jstype", n, "", 0))) {
      classname_substituted_flag = substituteClassname(t, tm);
      Printf(return_type, "%s", tm);
    } else {
      alaqil_warning(WARN_JAVA_TYPEMAP_JSTYPE_UNDEF, input_file, line_number, "No jstype typemap defined for %s\n", alaqilType_str(t, 0));
    }

    // Add the stripped quotes back in
    String *new_value = NewString("");
    if (alaqilType_type(t) == T_STRING) {
      Printf(new_value, "\"%s\"", Copy(Getattr(n, "value")));
      Setattr(n, "value", new_value);
    } else if (alaqilType_type(t) == T_CHAR) {
      Printf(new_value, "\'%s\'", Copy(Getattr(n, "value")));
      Setattr(n, "value", new_value);
    }
	...
	if (alaqilType_isenum(t)) {
	  ...
	  Printf(constants_code, "%s.alaqilToEnum(%s.%s());\n", return_type, full_imclass_name, alaqil_name_get(getNSpace(), symname));
	} else {
	  // This handles function pointers using the %constant directive
	  Printf(constants_code, "new %s(%s.%s(), false);\n", return_type, full_imclass_name ? full_imclass_name : imclass_name, alaqil_name_get(getNSpace(), symname));
	}
      } else {
	Printf(constants_code, "%s.%s();\n", full_imclass_name ? full_imclass_name : imclass_name, alaqil_name_get(getNSpace(), symname));
      }
	...
	if (alaqilType_type(valuetype) == T_CHAR)
  ...
  virtual int insertDirective(Node *n) {
    ...
    if (!ImportMode && (Cmp(section, "proxycode") == 0)) {
      if (proxy_class_code) {
	alaqil_typemap_replace_embedded_typemap(code, n);
  ...
  virtual int pragmaDirective(Node *n) {
	...
	if (Strcmp(code, "jniclassbase") == 0) {
	  ...
	  alaqil_name_unregister("wrapper");
	  alaqil_name_register("wrapper", Char(wrapper_name));
	  ...
	  alaqil_error(input_file, line_number, "Deprecated pragma. Please use the moduleimports pragma.\n");
	} else if (Strcmp(code, "moduleinterface") == 0) {
	  alaqil_error(input_file, line_number, "Deprecated pragma. Please use the moduleinterfaces pragma.\n");
	} else if (Strcmp(code, "modulemethodmodifiers") == 0) {
	  alaqil_error(input_file, line_number, "Deprecated pragma. Please use %%javamethodmodifiers.\n");
	} else if (Strcmp(code, "allshadowimport") == 0) {
	  alaqil_error(input_file, line_number, "Deprecated pragma. Please use %%typemap(javaimports).\n");
	} else if (Strcmp(code, "allshadowcode") == 0) {
	  alaqil_error(input_file, line_number, "Deprecated pragma. Please use %%typemap(javacode).\n");
	} else if (Strcmp(code, "allshadowbase") == 0) {
	  alaqil_error(input_file, line_number, "Deprecated pragma. Please use %%typemap(javabase).\n");
	} else if (Strcmp(code, "allshadowinterface") == 0) {
	  alaqil_error(input_file, line_number, "Deprecated pragma. Please use %%typemap(javainterfaces).\n");
	} else if (Strcmp(code, "allshadowclassmodifiers") == 0) {
	  alaqil_error(input_file, line_number, "Deprecated pragma. Please use %%typemap(javaclassmodifiers).\n");
	} else if (proxy_flag) {
	  if (Strcmp(code, "shadowcode") == 0) {
	    ...
	    alaqil_error(input_file, line_number, "Unrecognized pragma.\n");
	  }
	} else {
	  alaqil_error(input_file, line_number, "Unrecognized pragma.\n");
	}
  ...
  String *getInterfaceName(alaqilType *t, bool qualified) {
  ...
  void addInterfaceNameAndUpcasts(alaqilType *smart, String *interface_list, String *interface_upcasts, Hash *base_list, String *c_classname) {
    List *keys = Keys(base_list);
    for (Iterator it = First(keys); it.item; it = Next(it)) {
      Node *base = Getattr(base_list, it.item);
      String *c_baseclass = alaqilType_namestr(Getattr(base, "name"));
	...
	cptr_method_name = NewStringf("%s_GetInterfaceCPtr", interface_name);
      ...
      String *upcast_method_name = alaqil_name_member(getNSpace(), proxy_class_name, cptr_method_name);
  ...
  void upcastsCode(alaqilType *smart, String *upcast_method_name, String *c_classname, String *c_baseclass) {
    String *jniname = makeValidJniName(upcast_method_name);
    String *wname = alaqil_name_wrapper(jniname);
    Printf(imclass_cppcasts_code, "  public final static native long %s(long jarg1);\n", upcast_method_name);
    if (smart) {
      alaqilType *bsmart = Copy(smart);
      alaqilType *rclassname = alaqilType_typedef_resolve_all(c_classname);
      alaqilType *rbaseclass = alaqilType_typedef_resolve_all(c_baseclass);
      Replaceall(bsmart, rclassname, rbaseclass);
      Delete(rclassname);
      Delete(rbaseclass);
      String *smartnamestr = alaqilType_namestr(smart);
      String *bsmartnamestr = alaqilType_namestr(bsmart);
      Printv(upcasts_code,
	  "alaqilEXPORT jlong JNICALL ", wname, "(JNIEnv *jenv, jclass jcls, jlong jarg1) {\n",
	  "    jlong baseptr = 0;\n"
	  "    ", smartnamestr, " *argp1;\n"
	  "    (void)jenv;\n"
	  "    (void)jcls;\n"
  ...
  void emitProxyClassDefAndCPPCasts(Node *n) {
    String *c_classname = alaqilType_namestr(Getattr(n, "name"));
    String *c_baseclass = NULL;
    String *baseclass = NULL;
    String *c_baseclassname = NULL;
    String *interface_list = NewStringEmpty();
    String *interface_upcasts = NewStringEmpty();
    alaqilType *typemap_lookup_type = Getattr(n, "classtypeobj");
    bool feature_director = alaqil_directorclass(n) ? true : false;
	...
	while (base.item) {
	  if (!(GetFlag(base.item, "feature:ignore") || Getattr(base.item, "feature:interface"))) {
	    ...
	    if (!c_baseclassname) {
	      ...
	      if (baseclass)
		c_baseclass = alaqilType_namestr(baseclassname);
	    } else {
	      /* Warn about multiple inheritance for additional base class(es) */
	      String *proxyclassname = Getattr(n, "classtypeobj");
	      alaqil_warning(WARN_JAVA_MULTIPLE_INHERITANCE, Getfile(n), Getline(n),
		  "Warning for %s, base %s ignored. Multiple inheritance is not supported in Java.\n", alaqilType_namestr(proxyclassname), alaqilType_namestr(baseclassname));
	    }
	  }
	  base = Next(base);
	}
    ...
    if (purebase_replace) {
      ...
      if (purebase_notderived)
        alaqil_error(Getfile(n), Getline(n), "The javabase typemap for proxy %s must contain just one of the 'replace' or 'notderived' attributes.\n", typemap_lookup_type);
	...
	alaqil_error(Getfile(n), Getline(n), "No methodname attribute defined in javadestruct%s typemap for %s\n", (derived ? "_derived" : ""), proxy_class_name);
      }
      if (!destruct_methodmodifiers) {
	alaqil_error(Getfile(n), Getline(n), "No methodmodifiers attribute defined in javadestruct%s typemap for %s.\n", (derived ? "_derived" : ""), proxy_class_name);
      }
    }
    ...
    /* Also insert the alaqilTakeOwnership and alaqilReleaseOwnership methods */
    if (feature_director) {
      String *destruct_jnicall, *release_jnicall, *take_jnicall;
      String *changeown_method_name = alaqil_name_member(getNSpace(), getClassPrefix(), "change_ownership");

      destruct_jnicall = NewStringf("%s()", destruct_methodname);
      release_jnicall = NewStringf("%s.%s(this, alaqilCPtr, false)", full_imclass_name, changeown_method_name);
      take_jnicall = NewStringf("%s.%s(this, alaqilCPtr, true)", full_imclass_name, changeown_method_name);
    ...
    if (derived) {
      String *upcast_method_name = alaqil_name_member(getNSpace(), getClassPrefix(), smart != 0 ? "alaqilSmartPtrUpcast" : "alaqilUpcast");
      upcastsCode(smart, upcast_method_name, c_classname, c_baseclass);
      Delete(upcast_method_name);
    }
  ...
  * ---------------------------------------------------------------------- */
  ...
  virtual int classHandler(Node *n) {
	...
	f_proxy = NewFile(filen, "w", alaqil_output_files());
	if (!f_proxy) {
	  FileErrorDisplay(filen);
	  alaqil_exit(EXIT_FAILURE);
	}
	...
	f_interface = NewFile(filen, "w", alaqil_output_files());
	if (!f_interface) {
	  FileErrorDisplay(filen);
	  alaqil_exit(EXIT_FAILURE);
	}
    ...
    if (proxy_flag) {
      ...
      String *javaclazzname = alaqil_name_member(getNSpace(), getClassPrefix(), ""); // mangled full proxy class name
	...
	alaqil_offset_string(proxy_class_def, nesting_depth);
	Append(old_proxy_class_code, proxy_class_def);
	alaqil_offset_string(proxy_class_code, nesting_depth);
	...
	if (!has_outerclass)
	  ...
	  alaqil_offset_string(proxy_class_constants_code, nesting_depth);
  ...
  virtual int memberfunctionHandler(Node *n) {
    ...
    if (proxy_flag) {
      String *overloaded_name = getOverloadedName(n);
      String *intermediary_function_name = alaqil_name_member(getNSpace(), getClassPrefix(), overloaded_name);
  ...
  void proxyClassFunctionHandler(Node *n) {
    ...
    if (l) {
      if (alaqilType_type(Getattr(l, "type")) == T_VOID) {
	l = nextSibling(l);
      }
    }

    /* Attach the non-standard typemaps to the parameter list */
    alaqil_typemap_attach_parms("in", l, NULL);
    alaqil_typemap_attach_parms("jtype", l, NULL);
    alaqil_typemap_attach_parms("jstype", l, NULL);
    alaqil_typemap_attach_parms("javain", l, NULL);
    ...
    if ((tm = alaqil_typemap_lookup("jstype", n, "", 0))) {
      ...
      alaqilType *covariant = Getattr(n, "covariant");
      substituteClassname(covariant ? covariant : t, tm);
      Printf(return_type, "%s", tm);
      if (covariant)
	alaqil_warning(WARN_JAVA_COVARIANT_RET, input_file, line_number,
		     "Covariant return types not supported in Java. Proxy method will return %s.\n", alaqilType_str(covariant, 0));
    } else {
      alaqil_warning(WARN_JAVA_TYPEMAP_JSTYPE_UNDEF, input_file, line_number, "No jstype typemap defined for %s\n", alaqilType_str(t, 0));
    }
    ...
    if (wrapping_member_flag && !enum_constant_flag) {
      ...
      setter_flag = (Cmp(Getattr(n, "sym:name"), alaqil_name_set(getNSpace(), alaqil_name_member(0, getClassPrefix(), variable_name))) == 0);
    }
    ...
    if (!static_flag) {
      Printf(imcall, "alaqilCPtr");

      String *this_type = Copy(getClassType());
      String *name = NewString("jself");
      String *qualifier = Getattr(n, "qualifier");
      if (qualifier)
	alaqilType_push(this_type, qualifier);
      alaqilType_add_pointer(this_type);
      Parm *this_parm = NewParm(this_type, name, n);
      alaqil_typemap_attach_parms("jtype", this_parm, NULL);
      alaqil_typemap_attach_parms("jstype", this_parm, NULL);
	...
	if ((tm = Getattr(p, "tmap:javain"))) {
	  ...
	  alaqil_warning(WARN_JAVA_TYPEMAP_JAVAIN_UNDEF, input_file, line_number, "No javain typemap defined for %s\n", alaqilType_str(pt, 0));
	}
    ...
    if ((tm = alaqil_typemap_lookup("javaout", n, "", 0))) {
	...
	String *ex_intermediary_function_name = alaqil_name_member(getNSpace(), getClassPrefix(), ex_overloaded_name);
	...
	Replaceall(imcall, "$imfuncname", intermediary_function_name);
      ...
      alaqil_warning(WARN_JAVA_TYPEMAP_JAVAOUT_UNDEF, input_file, line_number, "No javaout typemap defined for %s\n", alaqilType_str(t, 0));
    }
  ...
  virtual int constructorHandler(Node *n) {
    ...
    String *post_code = NewString("");
    String *im_return_type = NewString("");
    bool feature_director = (parentNode(n) && alaqil_directorclass(n));
    ...
    if (proxy_flag) {
      String *overloaded_name = getOverloadedName(n);
      String *mangled_overname = alaqil_name_construct(getNSpace(), overloaded_name);
      String *imcall = NewString("");
	...
	Delete(doxygen_comments);
      ...
      Printf(helper_code, "  static private %s alaqilConstruct%s(", im_return_type, proxy_class_name);
	...
	if (!feature_director) {
	  ...
	  if (connect_attr) {
	    ...
	    alaqil_warning(WARN_JAVA_NO_DIRECTORCONNECT_ATTR, input_file, line_number, "\"directorconnect\" attribute missing in %s \"javaconstruct\" typemap.\n",
			 Getattr(n, "name"));
	    Replaceall(construct_tm, "$directorconnect", "");
	  }
	}
	...
	Printv(function_code, " ", construct_tm, "\n", NIL);
      ...
      if (is_pre_code || is_post_code) {
        ...
        Printf(helper_code, "\n  }\n");
        String *helper_name = NewStringf("%s.alaqilConstruct%s(%s)", proxy_class_name, proxy_class_name, helper_args);
  ...
  virtual int destructorHandler(Node *n) {
    ...
    if (proxy_flag) {
      Printv(destructor_call, full_imclass_name, ".", alaqil_name_destroy(getNSpace(), symname), "(alaqilCPtr)", NIL);
  ...
  String *getOverloadedName(Node *n) {
     ...
     * a Java long is used for all classes in the alaqil intermediary class.
  ...
  void moduleClassFunctionHandler(Node *n) {
    ...
    if (proxy_flag && global_variable_flag) {
      // Capitalize the first letter in the variable to create a JavaBean type getter/setter function name
      func_name = NewString("");
      setter_flag = (Cmp(Getattr(n, "sym:name"), alaqil_name_set(getNSpace(), variable_name)) == 0);
      if (setter_flag)
  ...
  virtual void replaceSpecialVariables(String *method, String *tm, Parm *parm) {
    (void)method;
    alaqilType *type = Getattr(parm, "type");
  ...
  String *enumValue(Node *n) {
	...
	if (!getCurrentClass() || !cparse_cplusplus || !proxy_flag) {
	  ...
	  constantWrapper(n);
	  value = NewStringf("%s.%s()", full_imclass_name ? full_imclass_name : imclass_name, alaqil_name_get(getNSpace(), symname));
	} else {
	  memberconstantHandler(n);
	  value = NewStringf("%s.%s()", full_imclass_name ? full_imclass_name : imclass_name, alaqil_name_get(getNSpace(), alaqil_name_member(0, getEnumClassPrefix(), symname)));
	}
	Delete(newsymname);
      }
    }
  ...
  String *getEnumName(alaqilType *t, bool jnidescriptor) {
    Node *enumname = NULL;
    Node *n = enumLookup(t);
    if (n) {
	...
	if (symname) {
	  ...
	  String *scopename_prefix = alaqil_scopename_prefix(Getattr(n, "name"));
	  String *proxyname = 0;
	  if (scopename_prefix) {
   ...
   * that alaqil knows about. Also substitutes enums with enum name.
  ...
  bool substituteClassname(alaqilType *pt, String *tm, bool jnidescriptor = false) {
    bool substitution_performed = false;
    alaqilType *type = Copy(alaqilType_typedef_resolve_all(pt));
    alaqilType *strippedtype = alaqilType_strip_qualifiers(type);

    if (Strstr(tm, "$javaclassname")) {
      alaqilType *classnametype = Copy(strippedtype);
    ...
    if (Strstr(tm, "$*javaclassname")) {
      alaqilType *classnametype = Copy(strippedtype);
      Delete(alaqilType_pop(classnametype));
    ...
    if (Strstr(tm, "$&javaclassname")) {
      alaqilType *classnametype = Copy(strippedtype);
      alaqilType_add_pointer(classnametype);
      substituteClassnameSpecialVariable(classnametype, tm, "$&javaclassname", jnidescriptor);
      substitution_performed = true;
      Delete(classnametype);
    }
    if (Strstr(tm, "$javainterfacename")) {
      alaqilType *interfacenametype = Copy(strippedtype);
    ...
    if (Strstr(tm, "$*javainterfacename")) {
      alaqilType *interfacenametype = Copy(strippedtype);
      Delete(alaqilType_pop(interfacenametype));
    ...
    if (Strstr(tm, "$&javainterfacename")) {
      alaqilType *interfacenametype = Copy(strippedtype);
      alaqilType_add_pointer(interfacenametype);
      substituteInterfacenameSpecialVariable(interfacenametype, tm, "$&javainterfacename", jnidescriptor, true);
      substitution_performed = true;
      Delete(interfacenametype);
    }
  ...
  void substituteClassnameSpecialVariable(alaqilType *classnametype, String *tm, const char *classnamespecialvariable, bool jnidescriptor) {
    String *replacementname;

    if (alaqilType_isenum(classnametype)) {
	...
	if (anonymous_enum) {
	  ...
	  replacementname = NewStringf("alaqilTYPE%s", alaqilType_manglestr(classnametype));
	  Replace(replacementname, "enum ", "", DOH_REPLACE_ANY);
	  Setattr(alaqil_types_hash, replacementname, classnametype);
	}
      }
    } else {
	...
	// use $descriptor if alaqil does not know anything about this type. Note that any typedefs are resolved.
	replacementname = NewStringf("alaqilTYPE%s", alaqilType_manglestr(classnametype));
  ...
  void substituteInterfacenameSpecialVariable(alaqilType *interfacenametype, String *tm, const char *interfacenamespecialvariable, bool jnidescriptor, bool qualified) {
  ...
  void emitTypeWrapperClass(String *classname, alaqilType *type) {
    Node *n = NewHash();
    Setfile(n, input_file);
    Setline(n, line_number);

    String *alaqiltype = NewString("");
    String *filen = NewStringf("%s%s.java", alaqil_output_directory(), classname);
    File *f_alaqiltype = NewFile(filen, "w", alaqil_output_files());
    if (!f_alaqiltype) {
      FileErrorDisplay(filen);
      alaqil_exit(EXIT_FAILURE);
    }
    ...
    emitBanner(f_alaqiltype);

    if (package)
      Printf(f_alaqiltype, "package %s;\n", package);
    ...
    // Emit the class
    Printv(alaqiltype, typemapLookup(n, "javaimports", type, WARN_NONE),	// Import statements
    ...
    Replaceall(alaqiltype, "$javaclassname", classname);
    Replaceall(alaqiltype, "$module", module_class_name);
    Replaceall(alaqiltype, "$imclassname", imclass_name);

    // For unknown enums
    Replaceall(alaqiltype, "$static ", "");
    Replaceall(alaqiltype, "$enumvalues", "");

    Printv(f_alaqiltype, alaqiltype, NIL);
   ...
   * return is never NULL, unlike alaqil_typemap_lookup()
   * ----------------------------------------------------------------------------- */

  const String *typemapLookup(Node *n, const_String_or_char_ptr tmap_method, alaqilType *type, int warning, Node *typemap_attributes = 0) {
    Node *node = !typemap_attributes ? NewHash() : typemap_attributes;
    Setattr(node, "type", type);
    Setfile(node, Getfile(n));
    Setline(node, Getline(n));
    const String *tm = alaqil_typemap_lookup(tmap_method, node, "", 0);
    if (!tm) {
      tm = empty_string;
      if (warning != WARN_NONE)
	alaqil_warning(warning, Getfile(n), Getline(n), "No %s typemap defined for %s\n", tmap_method, alaqilType_str(type, 0));
    }
  ...
  void addThrows(Node *n, const String *attribute, Node *parameter) {
	...
	for (Iterator cls = First(temp_classes_list); cls.item; cls = Next(cls)) {
	  ...
	  if (Len(exception_class) > 0) {
	    ...
	    alaqilType *pt = Getattr(parameter, "type");
  ...
  String *prematureGarbageCollectionPreventionParameter(alaqilType *t, Parm *p) {
    String *pgcpp_java_type = 0;
    String *jtype = NewString(Getattr(p, "tmap:jtype"));

    // Strip C comments
    String *stripped_jtype = alaqil_strip_c_comments(jtype);
    if (stripped_jtype) {
      Delete(jtype);
	  ...
	  String *interface_name = getInterfaceName(t, true);
          ...
          if (!pgcpp_java_type) {
            ...
            if (jstype) {
              ...
              if (classes) {
                // Strip C comments
                String *stripped_jstype = alaqil_strip_c_comments(jstype);
                if (stripped_jstype) {
                  Delete(jstype);
  ...
  String *outputDirectory(String *nspace) {
    String *output_directory = Copy(alaqil_output_directory());
    if (nspace) {
      String *nspace_subdirectory = Copy(nspace);
      Replaceall(nspace_subdirectory, ".", alaqil_FILE_DELIMITER);
      String *newdir_error = alaqil_new_subdirectory(output_directory, nspace_subdirectory);
      if (newdir_error) {
	Printf(stderr, "%s\n", newdir_error);
	Delete(newdir_error);
	alaqil_exit(EXIT_FAILURE);
      }
      Printv(output_directory, nspace_subdirectory, alaqil_FILE_DELIMITER, 0);
  ...
  void emitDirectorUpcalls() {
    if (n_dmethods) {
      ...
      String *alaqil_module_init = NewString("alaqil_module_init");
      String *alaqil_module_init_jni = makeValidJniName(alaqil_module_init);
	...
	Putc('\n', dmethod_data);
      ...
      Printf(f_runtime, "namespace alaqil {\n");
      ...
      Printf(w->def, "alaqilEXPORT void JNICALL Java_%s%s_%s(JNIEnv *jenv, jclass jcls) {", jnipackage, jni_imclass_name, alaqil_module_init_jni);
      ...
      Printf(w->code, "alaqil::jclass_%s = (jclass) jenv->NewGlobalRef(jcls);\n", imclass_name);
      Printf(w->code, "if (!alaqil::jclass_%s) return;\n", imclass_name);
      Printf(w->code, "for (i = 0; i < (int) (sizeof(methods)/sizeof(methods[0])); ++i) {\n");
      Printf(w->code, "  alaqil::director_method_ids[i] = jenv->GetStaticMethodID(jcls, methods[i].method, methods[i].signature);\n");
      Printf(w->code, "  if (!alaqil::director_method_ids[i]) return;\n");
      Printf(w->code, "}\n");
      ...
      Delete(dmethod_data);
      Delete(alaqil_module_init_jni);
      Delete(alaqil_module_init);
  ...
  void emitDirectorExtraMethods(Node *n) {
    if (!alaqil_directorclass(n))
      return;

    // Output the director connect method:
    String *jni_imclass_name = makeValidJniName(imclass_name);
    String *norm_name = alaqilType_namestr(Getattr(n, "name"));
    String *alaqil_director_connect = alaqil_name_member(getNSpace(), getClassPrefix(), "director_connect");
    String *alaqil_director_connect_jni = makeValidJniName(alaqil_director_connect);
    ...
    Printf(imclass_class_code, "  public final static native void %s(%s obj, long cptr, boolean mem_own, boolean weak_global);\n",
	   alaqil_director_connect, full_proxy_class_name);

    code_wrap = NewWrapper();
    Printf(code_wrap->def,
	   "alaqilEXPORT void JNICALL Java_%s%s_%s(JNIEnv *jenv, jclass jcls, jobject jself, jlong objarg, jboolean jalaqil_mem_own, "
	   "jboolean jweak_global) {\n", jnipackage, jni_imclass_name, alaqil_director_connect_jni);
    ...
    Printf(code_wrap->code, "  director->alaqil_connect_director(jenv, jself, jenv->GetObjectClass(jself), "
	   "(jalaqil_mem_own == JNI_TRUE), (jweak_global == JNI_TRUE));\n");
    Printf(code_wrap->code, "}\n");

    Wrapper_print(code_wrap, f_wrappers);
    DelWrapper(code_wrap);

    Delete(alaqil_director_connect_jni);
    Delete(alaqil_director_connect);

    // Output the alaqilReleaseOwnership, alaqilTakeOwnership methods:
    String *changeown_method_name = alaqil_name_member(getNSpace(), getClassPrefix(), "change_ownership");
    ...
    code_wrap = NewWrapper();
    Printf(code_wrap->def,
	   "alaqilEXPORT void JNICALL Java_%s%s_%s(JNIEnv *jenv, jclass jcls, jobject jself, jlong objarg, jboolean jtake_or_release) {\n",
    ...
    Printf(code_wrap->code, "    director->alaqil_java_change_ownership(jenv, jself, jtake_or_release ? true : false);\n");
    Printf(code_wrap->code, "  }\n");
    Printf(code_wrap->code, "}\n");
  ...
  void emitCodeTypemap(Node *n, bool derived, alaqilType *lookup_type, const String *typemap, const String *methodname, const String *jnicall) {
    const String *tm = NULL;
	...
	alaqil_error(input_file, line_number, "No %s method name attribute for %s\n", lookup_tmname, proxy_class_name);
      }
    } else {
      alaqil_error(input_file, line_number, "No %s typemap for %s\n", lookup_tmname, proxy_class_name);
    }
  ...
  void substitutePackagePath(String *text, Parm *p) {
    ...
    if (p)
      pkg_path = alaqil_typemap_lookup("javapackage", p, "", 0);
  ...
  int classDirectorMethod(Node *n, Node *parent, String *super) {
    ...
    String *name = Getattr(n, "name");
    String *symname = Getattr(n, "sym:name");
    alaqilType *returntype = Getattr(n, "type");
    String *overloaded_name = getOverloadedName(n);
    ...
    alaqilType *c_ret_type = NULL;
    String *jupcall_args = NewString("alaqiljobj");
    ...
    imclass_dmethod = NewStringf("%s", alaqil_name_member(getNSpace(), dirclassname, overloaded_name));

    qualified_return = alaqilType_rcaststr(returntype, "c_result");
	...
	if (!(alaqilType_ispointer(returntype) || alaqilType_isreference(returntype))) {
	  ...
	  String *base_typename = alaqilType_base(returntype);
	  String *resolved_typename = alaqilType_typedef_resolve_all(base_typename);
	  Symtab *symtab = Getattr(n, "sym:symtab");
	  Node *typenode = alaqil_symbol_clookup(resolved_typename, symtab);

	  if (alaqilType_ispointer(returntype) || (typenode && Getattr(typenode, "abstracts"))) {
	    ...
	    alaqilType *noref_type = alaqilType_del_reference(Copy(returntype));
	    String *noref_ltype = alaqilType_lstr(noref_type, 0);
	    String *return_ltype = alaqilType_lstr(returntype, 0);
	    ...
	    Printf(w->code, "result_default = alaqilValueInit< %s >();\n", noref_ltype);
	...
	if (!vt) {
	  Wrapper_add_localv(w, "c_result", alaqilType_lstr(returntype, "c_result"), NIL);
	} else {
	  Wrapper_add_localv(w, "c_result", alaqilType_lstr(vt, "c_result"), NIL);
	  Delete(vt);
	}
      }
    }
    ...
    tm = alaqil_typemap_lookup("jtype", n, "", 0);
    if (tm) {
    ...
    String *cdesc = NULL;
    alaqilType *covariant = Getattr(n, "covariant");
    alaqilType *adjustedreturntype = covariant ? covariant : returntype;
    Parm *adjustedreturntypeparm = NewParmNode(adjustedreturntype, n);

    if (alaqil_typemap_lookup("directorin", adjustedreturntypeparm, "", 0)
	&& (cdesc = Getattr(adjustedreturntypeparm, "tmap:directorin:descriptor"))) {
      ...
      alaqil_warning(WARN_TYPEMAP_DIRECTORIN_UNDEF, input_file, line_number, "No or improper directorin typemap defined for %s for use in %s::%s (skipping director method)\n", 
    ...
    /* Get the JNI field descriptor for this return type, add the JNI field descriptor
       to jniret_desc */
    if ((c_ret_type = alaqil_typemap_lookup("jni", n, "", 0))) {
	...
	Delete(jretval_decl);
      ...
      String *jdesc = NULL;
      if (alaqil_typemap_lookup("directorin", tp, "", 0)
	...
	alaqil_warning(WARN_TYPEMAP_DIRECTORIN_UNDEF, input_file, line_number,
		     ...
		     alaqilType_str(c_ret_type, 0), alaqilType_namestr(c_classname), alaqilType_namestr(name));
	output_director = false;
      }

      Delete(tp);
    } else {
      alaqil_warning(WARN_JAVA_TYPEMAP_JNI_UNDEF, input_file, line_number, "No jni typemap defined for %s for use in %s::%s (skipping director method)\n", 
	  alaqilType_str(returntype, 0), alaqilType_namestr(c_classname), alaqilType_namestr(name));
      output_director = false;
    }
    ...
    /* Attach the standard typemaps */
    alaqil_typemap_attach_parms("out", l, 0);
    alaqil_typemap_attach_parms("jni", l, 0);
    alaqil_typemap_attach_parms("jtype", l, 0);
    alaqil_typemap_attach_parms("directorin", l, w);
    alaqil_typemap_attach_parms("javadirectorin", l, 0);
    alaqil_typemap_attach_parms("directorargout", l, w);

    if (!ignored_method) {
      /* Add Java environment pointer to wrapper */
      String *jenvstr = NewString("jenv");
      String *jobjstr = NewString("alaqiljobj");

      Wrapper_add_localv(w, "alaqiljnienv", "JNIEnvWrapper", "alaqiljnienv(this)", NIL, NIL);
      Wrapper_add_localv(w, jenvstr, "JNIEnv *", jenvstr, "= alaqiljnienv.getJNIEnv()", NIL);
      Wrapper_add_localv(w, jobjstr, "jobject", jobjstr, "= (jobject) NULL", NIL);
      Delete(jenvstr);
      Delete(jobjstr);

      /* Preamble code */
      Printf(w->code, "if (!alaqil_override[%d]) {\n", classmeth_off);
    }
	...
	Printf(w->code, "return %s;\n", super_call);
      ...
      Printf(w->code, "alaqil_JavaThrowException(JNIEnvWrapper(this).getJNIEnv(), alaqil_JavaDirectorPureVirtual, ");
    ...
    if (!ignored_method) {
      Printf(w->code, "}\n");
      Printf(w->code, "alaqiljobj = alaqil_get_self(jenv);\n");
      Printf(w->code, "if (alaqiljobj && jenv->IsSameObject(alaqiljobj, NULL) == JNI_FALSE) {\n");
    }
    ...
    if ((tm = alaqil_typemap_lookup("directorin", tp, "", 0))
	&& (jdesc = Getattr(tp, "tmap:directorin:descriptor"))) {
	...
	if (!ignored_method)
	  Wrapper_add_localv(w, arg, c_decl, (!(alaqilType_ispointer(pt) || alaqilType_isreference(pt)) ? "" : "= 0"), NIL);

	/* Add input marshalling code and update JNI field descriptor */
	if ((desc_tm = alaqil_typemap_lookup("directorin", tp, "", 0))
	    && (jdesc = Getattr(tp, "tmap:directorin:descriptor"))
	  ...
	  if ((tm = Getattr(p, "tmap:jtype"))) {
		...
		Printv(imcall_args, ln, NIL);
	      ...
	      alaqil_warning(WARN_JAVA_TYPEMAP_JAVADIRECTORIN_UNDEF, input_file, line_number, "No javadirectorin typemap defined for %s for use in %s::%s (skipping director method)\n", 
		  alaqilType_str(pt, 0), alaqilType_namestr(c_classname), alaqilType_namestr(name));
	      output_director = false;
	    }
	  } else {
	    alaqil_warning(WARN_JAVA_TYPEMAP_JTYPE_UNDEF, input_file, line_number, "No jtype typemap defined for %s for use in %s::%s (skipping director method)\n", 
		alaqilType_str(pt, 0), alaqilType_namestr(c_classname), alaqilType_namestr(name));
	    output_director = false;
	  }
	  ...
	  if (!desc_tm) {
	    alaqil_warning(WARN_JAVA_TYPEMAP_JAVADIRECTORIN_UNDEF, input_file, line_number,
			 "No or improper directorin typemap defined for %s for use in %s::%s (skipping director method)\n", 
			 alaqilType_str(c_param_type, 0), alaqilType_namestr(c_classname), alaqilType_namestr(name));
	    p = nextSibling(p);
	  } else if (!jdesc) {
	    alaqil_warning(WARN_JAVA_TYPEMAP_DIRECTORIN_NODESC, input_file, line_number,
    ...
    if (catches_list) {
      alaqil_typemap_attach_parms("throws", catches_list, 0);
      alaqil_typemap_attach_parms("directorthrows", catches_list, 0);
	...
	alaqil_typemap_attach_parms("directorthrows", throw_parm_list, 0);
      }
      for (p = throw_parm_list; p; p = nextSibling(p)) {
	if (Getattr(p, "tmap:throws")) {
    ...
    if (!is_void) {
      if ((tm = alaqil_typemap_lookup("javadirectorout", n, "", 0))) {
	...
	Printf(callback_code, "    return %s;\n", tm);
      ...
      if ((tm = alaqil_typemap_lookup("out", n, "", 0)))
        addThrows(n, "tmap:out", n);
	...
	Printf(w->code, "jresult = (%s) ", c_ret_type);
      ...
      Printf(w->code, "jenv->%s(alaqil::jclass_%s, alaqil::director_method_ids[%s], %s);\n", methop, imclass_name, methid, jupcall_args);
	...
	if ((tm = alaqil_typemap_lookup("directorout", n, result_str, w))) {
	...
	if ((tm = Getattr(p, "tmap:directorargout"))) {
      ...
      Printf(w->code, "} else {\n");
      Printf(w->code, "alaqil_JavaThrowException(jenv, alaqil_JavaNullPointerException, \"null upcall object in %s::%s \");\n",
	     alaqilType_namestr(c_classname), alaqilType_namestr(name));
      Printf(w->code, "}\n");

      Printf(w->code, "if (alaqiljobj) jenv->DeleteLocalRef(alaqiljobj);\n");
    ...
    if (status == alaqil_OK && output_director) {
      if (!is_void) {
  ...
  void directorExceptHandler(Node *n, ParmList *throw_parm_list, Wrapper *w) {
    ...
    if (!directorexcept) {
      ...
      Printf(directorexcept, "  alaqil::DirectorException::raise(jenv, $error);\n");
	...
	for (Parm *p = throw_parm_list; p; p = nextSibling(p)) {
	  ...
	  if (tm) {
	    ...
	    alaqil_warning(WARN_TYPEMAP_DIRECTORTHROWS_UNDEF, Getfile(n), Getline(n), "No directorthrows typemap defined for %s\n", alaqilType_str(t, 0));
	  }
	}
	...
	Delete(directorthrowshandlers_code);
      ...
      Replaceall(directorexcept, "$error", "alaqilerror");
      Printf(w->code, "    %s\n", directorexcept);
    }
  ...
  void directorPrefixArgs(Node *n) {
    ...
    alaqilType_add_pointer(jenv_type);
  ...
  int classDirectorConstructor(Node *n) {
	...
	String *target = alaqil_method_decl(0, decl, dirclassname, parms, 0);
	String *call = alaqil_csuperclass_call(0, basetype, superparms);
	String *classtype = alaqilType_namestr(Getattr(n, "name"));
	...
	Delete(call);
      ...
      /* constructor header */
      {
	String *target = alaqil_method_decl(0, decl, dirclassname, parms, 1);
  ...
  int classDirectorInit(Node *n) {
    ...
    Printf(f_directors_h, "\npublic:\n");
    Printf(f_directors_h, "    void alaqil_connect_director(JNIEnv *jenv, jobject jself, jclass jcls, bool alaqil_mem_own, bool weak_global);\n");
  ...
  int classDirectorDestructor(Node *n) {
    ...
    String *classname = alaqil_class_name(current_class);
    ...
    Printv(w->code, "  alaqil_disconnect_director_self(\"", disconn_methodname, "\");\n", "}\n", NIL);
  ...
  int classDirectorEnd(Node *n) {
    ...
    Printf(w->def, "void %s::alaqil_connect_director(JNIEnv *jenv, jobject jself, jclass jcls, bool alaqil_mem_own, bool weak_global) {", director_classname);
    ...
    Printf(w->code, "if (alaqil_set_self(jenv, jself, alaqil_mem_own, weak_global)) {\n");
    ...
    if (n_methods) {
      /* Emit the alaqil_overrides() method and the alaqil_override array */
      Printf(f_directors_h, "public:\n");
      Printf(f_directors_h, "    bool alaqil_overrides(int n) {\n");
      Printf(f_directors_h, "      return (n < %d ? alaqil_override[n] : false);\n", n_methods);
      Printf(f_directors_h, "    }\n");
      Printf(f_directors_h, "protected:\n");
      Printf(f_directors_h, "    alaqil::BoolArray<%d> alaqil_override;\n", n_methods);
      ...
      if (GetFlag(n, "feature:director:assumeoverride")) {
        Printf(w->code, "  alaqil_override[i] = derived;\n");
      } else {
        Printf(w->code, "  alaqil_override[i] = false;\n");
        Printf(w->code, "  if (derived) {\n");
        Printf(w->code, "    jmethodID methid = jenv->GetMethodID(jcls, methods[i].mname, methods[i].mdesc);\n");
        Printf(w->code, "    alaqil_override[i] = (methid != methods[i].base_methid);\n");
  ...
  void directorDeclaration(Node *n) {
    ...
    String *class_ctor = NewString("alaqil::Director(jenv)");

    String *directorname = directorClassName(n);
    String *declaration = alaqil_class_declaration(n, directorname);

    Printf(declaration, " : public %s, public alaqil::Director", base);
...
/* -----------------------------------------------------------------------------
 * alaqil_java()    - Instantiate module
 * ----------------------------------------------------------------------------- */

static Language *new_alaqil_java() {
  return new JAVA();
}
extern "C" Language *alaqil_java(void) {
  return new_alaqil_java();
}
