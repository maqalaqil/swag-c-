 ...
 * cffi.cxx
 *
 * cffi language module for alaqil.
 * ----------------------------------------------------------------------------- */
...
CFFI Options (available with -cffi)\n\
     ...
     -[no]alaqil-lisp    - Turn on or off generation of code for helper lisp\n\
                         macro, functions, etc. which alaqil uses while\n\
...
void CFFI::main(int argc, char *argv[]) {
  ...
  Preprocessor_define("alaqilCFFI 1", 0);
  alaqil_library_directory("cffi");
  alaqil_config_file("cffi.swg");
  generate_typedef_flag = 0;
  no_alaqil_lisp = false;
  CWrap = false;
  for (i = 1; i < argc; i++) {
    if (!Strcmp(argv[i], "-help")) {
      ...
      CWrap = false;
      alaqil_mark_arg(i);
    } else if (!strcmp(argv[i], "-alaqil-lisp")) {
      no_alaqil_lisp = false;
      alaqil_mark_arg(i);
    } else if (!strcmp(argv[i], "-noalaqil-lisp")) {
      no_alaqil_lisp = true;
      alaqil_mark_arg(i);
    }
...
int CFFI::top(Node *n) {
  ...
  Printf(lisp_filename, "%s%s.lisp", alaqil_output_directory(), module);

  File *f_lisp = NewFile(lisp_filename, "w", alaqil_output_files());
  if (!f_lisp) {
  ...
  if (CPlusPlus || CWrap) {
    ...
    Printf(clos_filename, "%s%s-clos.lisp", alaqil_output_directory(), module);
    f_clos = NewFile(clos_filename, "w", alaqil_output_files());
    if (!f_clos) {
      Delete(f_lisp);
      Printf(stderr, "Unable to open %s for writing\n", cxx_filename);
      alaqil_exit(EXIT_FAILURE);
  ...
  alaqil_register_filebyname("lisphead", f_clhead);
  if (!no_alaqil_lisp)
    alaqil_register_filebyname("alaqillisp", f_cl);
  else
    alaqil_register_filebyname("alaqillisp", f_null);

  alaqil_banner(f_begin);

  Printf(f_runtime, "\n\n#ifndef alaqilCFFI\n#define alaqilCFFI\n#endif\n\n");

  alaqil_banner_target_lang(f_lisp, ";;;");
...
void CFFI::emit_defmethod(Node *n) {
  ...
  for (Parm *p = pl; p; p = nextSibling(p), argnum++) {
    ...
    String *ffitype = alaqil_typemap_lookup("lispclass", p, "", 0);
...
int CFFI::functionWrapper(Node *n) {
  ...
  String *result_convert = alaqil_typemap_lookup_out("out", n, alaqil_cresult_name(), f, actioncode);
  if (result_convert) {
...
void CFFI::emit_defun(Node *n, String *name) {
  ...
  String *ffitype = alaqil_typemap_lookup("cout", n, ":pointer", 0);
  ...
  for (Parm *p = pl; p; p = nextSibling(p), argnum++) {
    ...
    ffitype = alaqil_typemap_lookup("cin", p, "", 0);
...
int CFFI::variableWrapper(Node *n) {
  ...
  String *lisp_type = alaqil_typemap_lookup("cin", n, "", 0);
...
int CFFI::typedefHandler(Node *n) {
  if (generate_typedef_flag && strncmp(Char(Getattr(n, "type")), "enum", 4)) {
    ...
    Printf(f_cl, "\n(cffi:defctype %s %s)\n", lisp_name, alaqil_typemap_lookup("cin", n, "", 0));
    emit_export(n, lisp_name);
  }
...
int CFFI::enumDeclaration(Node *n) {
  ...
  if (name && Len(name) != 0) {
    ...
    alaqil_typemap_register("cin", pattern, lisp_name, NULL, NULL);
    alaqil_typemap_register("cout", pattern, lisp_name, NULL, NULL);
    Delete(pattern);
...
void CFFI::emit_class(Node *n) {
  ...
  alaqil_typemap_register("lispclass", pattern, lisp_name, NULL, NULL);
  alaqilType_add_pointer(Getattr(pattern, "type"));
  alaqil_typemap_register("lispclass", pattern, lisp_name, NULL, NULL);
  alaqilType_add_qualifier(Getattr(pattern, "type"), "const");
  alaqil_typemap_register("lispclass", pattern, lisp_name, NULL, NULL);
  alaqilType_del_pointer(Getattr(pattern, "type"));
  alaqilType_add_reference(Getattr(pattern, "type"));
  alaqil_typemap_register("lispclass", pattern, lisp_name, NULL, NULL);
  ...
  childDecl = NewString("");
      ...
      alaqilType *childType = NewStringf("%s%s", childDecl,
               Getattr(c, "type"));
...
#endif
  Printf(slotdefs, "(#.(alaqil-insert-id \"%s\" %s :type :slot :class \"%s\") %s)", cname, ns, name, childType);  //compose_foreign_type(childType)
  Delete(ns);
...
void CFFI::emit_struct_union(Node *n, bool un = false) {
  ...
  for (Node *c = firstChild(n); c; c = nextSibling(c)) {
    ...
    if (Strcmp(nodeType(c), "cdecl")) {
      ...
      alaqilType *childType = NewStringf("%s%s", Getattr(c, "decl"), Getattr(c, "type"));

      Node *node = NewHash();
      Setattr(node, "type", childType);
      Setfile(node, Getfile(n));
      Setline(node, Getline(n));
      const String *tm = alaqil_typemap_lookup("cin", node, "", 0);
	...
	slot_name = NewStringf("t_var");
      ...
      if (alaqilType_isarray(childType) && alaqilType_array_ndim(childType) == 1) {
          String *dim = alaqilType_array_getdim(childType, 0);
...
String *CFFI::lispify_name(Node *n, String *ty, const char *flag, bool kw) {
  ...
  if (intern_func) {
    if (Strcmp(intern_func, "1") == 0)
      intern_func = NewStringf("alaqil-lispify");
...
String *CFFI::infix_to_prefix(String *val, char split_op, const String *op, String *type) {
  ...
  //        (alaqilType_type(type) == T_FLOAT || alaqilType_type(type) == T_DOUBLE ||
  //    alaqilType_type(type) == T_LONGDOUBLE) ) {
...
String *CFFI::convert_literal(String *literal, String *type, bool try_to_split) {
  ...
  if (alaqilType_type(type) == T_FLOAT || alaqilType_type(type) == T_DOUBLE || alaqilType_type(type) == T_LONGDOUBLE) {
    ...
    Delete(num);
    return result;
  } else if (alaqilType_type(type) == T_INT || alaqilType_type(type) == T_UINT) {
...
extern "C" Language *alaqil_cffi(void) {
  return new CFFI();
}
