 ...
 * modula3.cxx
 *
 * Modula3 language module for alaqil.
 * ----------------------------------------------------------------------------- */
...
/*
  Report:
   - It's not a good concept to use member variables or global variables
     ...
     E.g. For alaqil this means: Generating accessor functions for member variables
   ...
   - alaqilTYPE const * - typemap is ignored, whereas
     alaqilTYPE *       - typemap is invoked, why?
     Had it been (const alaqilTYPE *) instead?
   - enumeration items should definitely be equipped
     ...
     by 'make' or by alaqil and the resulting output is fed back to alaqil.
   ...
   - Can one interpret $n_basetype as the identifier matched with alaqilTYPE ?

  alaqil's odds:
   - arguments of type (Node *) for alaqil functions
     should be most often better (const Node *):
     alaqil_symbol_qualified, Getattr, nodeType, parentNode
   - unique identifier style instead of
     NewString, Getattr, firstChild
   - 'class'.name is qualified,
     'enum'.name and 'enumitem'.name is not
   - alaqil_symbol_qualified() returns NIL for enumeration nodes

   - Is there a function that creates a C representation of a alaqil type string?
...
MODULA3():
  ...
  String *getMappedType(Node *p, const char *map) {
    ...
    if (tm == NIL) {
      alaqil_warning(WARN_MODULA3_TYPEMAP_TYPE_UNDEF, input_file, line_number,
		   "No '%s' typemap defined for type '%s'\n", map, alaqilType_str(Getattr(p, "type"), 0));
    }
    Delete(mapattr);
    return tm;
  }
   ...
   * getMappedTypeNew()
   *
   * Similar to getMappedType but uses alaqil_type_lookup_new.
   * ----------------------------------------------------------------------------- */

  String *getMappedTypeNew(Node *n, const char *map, const char *lname = "", bool warn = true) {
    String *tm = alaqil_typemap_lookup(map, n, lname, 0);
    if ((tm == NIL) && warn) {
      alaqil_warning(WARN_MODULA3_TYPEMAP_TYPE_UNDEF, input_file, line_number,
		   "No '%s' typemap defined for type '%s'\n", map, alaqilType_str(Getattr(n, "type"), 0));
    }
    return tm;
  }
...
//printf("dir for %s: %s\n", Char(Getattr(p,"name")), Char(dir));
  ...
  File *openWriteFile(String *name) {
    File *file = NewFile(name, "w", alaqil_output_files());
    if (!file) {
  ...
  long aToL(const String *value) {
    ...
    if (*endptr != 0) {
      alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "The string <%s> does not denote a numeric value.\n", value);
    }
    return numvalue;
  }
...
//printf("evaluation: return %ld\n", numvalue);
  ...
  virtual void main(int argc, char *argv[]) {
    ...
    alaqil_library_directory("modula3");
    ...
    Preprocessor_define("alaqilMODULA3 1", 0);

    // Add typemap definitions
    alaqil_typemap_lang("modula3");
    alaqil_config_file("modula3.swg");
...
/* This file was generated from %s\n\
   by alaqil with option -generaterename. */\n\
\n", input_file);
    scanRename(file, n);
    Delete(file);
    return alaqil_OK;
  ...
  int generateTypemapTop(Node *n) {
    ...
    Printf(file, "\
/* This file was generated from %s\n\
   by alaqil with option -generatetypemap. */\n\
\n", input_file);
    scanTypemap(file, n);
    Delete(file);
    return alaqil_OK;
  ...
  int generateM3Top(Node *n) {
    ...
    alaqil_register_filebyname("m3rawintf", m3raw_intf.f);
    alaqil_register_filebyname("m3rawimpl", m3raw_impl.f);
    alaqil_register_filebyname("m3wrapintf", m3wrap_intf.f);
    alaqil_register_filebyname("m3wrapimpl", m3wrap_impl.f);
    alaqil_register_filebyname("m3makefile", m3makefile);
    ...
    Printf(f_runtime, "\n\n#ifndef alaqilMODULA3\n#define alaqilMODULA3\n#endif\n\n");

    alaqil_name_register("wrapper", "Modula3_%f");
    ...
    // Generate m3makefile
    // This will be unnecessary if alaqil is invoked from Quake.
    {
      File *file = openWriteFile(NewStringf("%sm3makefile", alaqil_output_directory()));
    ...
    {
      File *file = openWriteFile(NewStringf("%s%s.i3", alaqil_output_directory(), m3raw_name));
    ...
    {
      File *file = openWriteFile(NewStringf("%s%s.m3", alaqil_output_directory(), m3raw_name));
    ...
    {
      File *file = openWriteFile(NewStringf("%s%s.i3", alaqil_output_directory(), m3wrap_name));
    ...
    {
      File *file = openWriteFile(NewStringf("%s%s.m3", alaqil_output_directory(), m3wrap_name));
    ...
    // Output a Modula 3 type wrapper class for each alaqil type
    for (Iterator alaqil_type = First(alaqil_types_hash); alaqil_type.item != NIL; alaqil_type = Next(alaqil_type)) {
  ...
  virtual int functionWrapper(Node *n) {
	...
	alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "Function type <%s> unknown.\n", Char(funcType));
#endif
      }
...
// a Java relict
  ...
  virtual int emitCWrapper(Node *n, const String *wname) {
    ...
    if (!native_function_flag) {
      String *tm = alaqil_typemap_lookup("ret", n, alaqil_cresult_name(), 0);
      if (tm != NIL) {
	Replaceall(tm, "$source", alaqil_cresult_name());	/* deprecated */
	Printf(f->code, "%s\n", tm);
      }
  ...
  virtual int emitM3RawPrototype(Node *n, const String *cname, const String *m3name) {
    ...
    /* Attach the non-standard typemaps to the parameter list. */
    alaqil_typemap_attach_parms("m3rawinmode", l, NULL);
    alaqil_typemap_attach_parms("m3rawintype", l, NULL);
  ...
  long getConstNumeric(Node *n) {
    ...
    long numvalue;
    if (constnumeric == NIL) {
      alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "Feature 'constnumeric' is necessary to obtain value of %s.\n", name);
      return 0;
    } else if (!strToL(constnumeric, numvalue)) {
      alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number,
  ...
  void generateSetConstant(Node *n, String *name) {
    ...
    if (!isIntType) {
      ...
      if (enumeration == NIL) {
	alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "There is no enumeration <%s> as needed for the set.\n", setname);
	isIntType = true;
      } else {
	...
	if (isIntType) {
	  ...
	  if (bitname == NIL) {
	    alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "Enumeration <%s> has no value <%s>.\n", setname, bitval);
	  } else {
	    if (gencomma) {
	      Printv(m3wrap_intf.f, ",", NIL);
	    }
...
*/
	if (Strcmp(code, "imclassbase") == 0) {
	  ...
	  alaqil_warning(WARN_MODULA3_UNKNOWN_PRAGMA, input_file, line_number, "Unrecognized pragma <%s>.\n", code);
	}
	Delete(strvalue);
      }
    }
  ...
  void Setfeature(Node *n, const char *feature, const String *value, bool warn = false) {
    ...
    if ((Setattr(n, attr, value) != 0) && warn) {
      alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "Feature <%s> of %s did already exist.\n", feature, Getattr(n, "name"));
    }
    Delete(attr);
  }
  ...
  bool convertInt(long in, long &out, const String *mode) {
    if ((mode == NIL) || (Strcmp(mode, "int:int") == 0) || (Strcmp(mode, "set:set") == 0)) {
      ...
      alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "Unknown integer conversion method <%s>.\n", mode);
      return false;
    }
  }
...
//printf("constnumeric: %s\n", Char(value));
	    ...
	    if (convertInt(numvalue, numvalue, conv)) {
	      ...
	      if (oldname != NIL) {
		alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "The value <%s> is already assigned to <%s>.\n", value, oldname);
	      }
//printf("items %p, set %s = %s\n", items, Char(newvalue), Char(m3name));
  ...
  void tagConstants(Node *first, String *parentEnum, const const_id_pattern & pat, const String *pragma, List *convdesc) {
	...
	String *newprefix = NIL;
	{
	  ...
	  for (; nameit.item != NIL; nameit = Next(nameit)) {
	    ...
	    if (Strcmp(tag, "srcstyle") == 0) {
	      ...
	      alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "Unknown name conversion tag <%s> with value <%s>.\n", tag, data);
	    }
	    Delete(nameassign);
	  }
	  Delete(namedesc);
	...
	if (srcstyle && Strcmp(srcstyle, "underscore") == 0) {
	  ...
	  if (srcstyle != NIL) {
	    alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "Unknown C identifier style <%s>.\n", srcstyle);
	  }
	  newname = Copy(name);
	}
	...
	if (Strcmp(pragma, "enumitem") == 0) {
	  if (Len(relationdesc) != 1) {
	    alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "Expected <enumeration>, got <%s>.\n", relationstr);
	  }
	  ...
	  if (Len(relationdesc) != 1) {
	    alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "Expected <ordinal type>, got <%s>.\n", relationstr);
	  }
	  ...
	  if (Len(relationdesc) != 2) {
	    alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "Expected <set type,base type>, got <%s>.\n", relationstr);
	  }
	...
	//printf("explore enum %s, qualification %s\n", Char(name), Char(alaqil_symbol_qualified(n)));
  ...
  void scanForConstPragmas(Node *n) {
	...
	if (Strcmp(lang, "modula3") == 0) {
	  ...
	  if (cpt != cpt_none) {
		...
		alaqil_warning(WARN_MODULA3_BAD_ENUMERATION, input_file, line_number, "Unknown identification tag <%s> with value <%s>.\n", tag, data);
	      }
  ...
  void emitProxyClassDefAndCPPCasts(Node *n) {
	...
	if (!GetFlag(base.item, "feature:ignore")) {
	  ...
	  if (!c_baseclassname) {
	    ...
	    alaqil_warning(WARN_MODULA3_MULTIPLE_INHERITANCE, Getfile(n), Getline(n),
    ...
    if (!destruct_methodname) {
      alaqil_error(Getfile(n), Getline(n), "No methodname attribute defined in m3destruct%s typemap for %s\n", (derived ? "_derived" : ""), proxy_class_name);
    }
    ...
    if (derived) {
      ...
      Printv(upcasts_code,
	     "alaqilEXPORT long Modula3_$imclazznameTo$imbaseclass",
	     "(long objectRef) {\n",
  ...
  virtual int classHandler(Node *n) {
	...
	alaqil_exit(EXIT_FAILURE);
      ...
      String *filen = NewStringf("%s%s.m3", alaqil_output_directory(), proxy_class_name);
	...
	alaqil_warning(WARN_MODULA3_TYPECONSTRUCTOR_UNKNOWN, input_file, line_number, "Unknown type constructor %s\n", kind);
      }
    }
  ...
  virtual int memberfunctionHandler(Node *n) {
...

    {
      ...
      alaqil_typemap_attach_parms("m3wrapinmode", pl, NULL);
      alaqil_typemap_attach_parms("m3wrapinname", pl, NULL);
      alaqil_typemap_attach_parms("m3wrapintype", pl, NULL);
      alaqil_typemap_attach_parms("m3wrapindefault", pl, NULL);
...
*/
    ...
    if (proxy_flag) {
      ...
      String *intermediary_function_name = alaqil_name_member(NSPACE_TODO, proxy_class_name, overloaded_name);
  ...
  void proxyClassFunctionHandler(Node *n) {
    ...
    alaqil_typemap_attach_parms("m3wraptype", l, NULL);
    alaqil_typemap_attach_parms("m3in", l, NULL);
    ...
    if (proxy_flag && wrapping_member_flag && !enum_constant_flag) {
      // Properties
      setter_flag = (Cmp(Getattr(n, "sym:name"), alaqil_name_set(NSPACE_TODO, alaqil_name_member(NSPACE_TODO, proxy_class_name, variable_name)))
		     == 0);
    }
  ...
  virtual int constructorHandler(Node *n) {
    ...
    if (proxy_flag) {
      ...
      Printv(imcall, " : this(", m3raw_name, ".", alaqil_name_construct(NSPACE_TODO, overloaded_name), "(", NIL);
  ...
  virtual int destructorHandler(Node *n) {
    ...
    if (proxy_flag) {
      Printv(destructor_call, m3raw_name, ".", alaqil_name_destroy(NSPACE_TODO, symname), "(alaqilCPtr)", NIL);
    }
    return alaqil_OK;
  }
  ...
  void emitM3Wrapper(Node *n, const String *func_name) {
    ...
    /* Attach the non-standard typemaps to the parameter list */
    alaqil_typemap_attach_parms("m3wrapargvar", l, NULL);
    alaqil_typemap_attach_parms("m3wrapargconst", l, NULL);
    alaqil_typemap_attach_parms("m3wrapargraw", l, NULL);
    alaqil_typemap_attach_parms("m3wrapargdir", l, NULL);
    alaqil_typemap_attach_parms("m3wrapinmode", l, NULL);
    alaqil_typemap_attach_parms("m3wrapinname", l, NULL);
    alaqil_typemap_attach_parms("m3wrapintype", l, NULL);
    alaqil_typemap_attach_parms("m3wrapindefault", l, NULL);
    alaqil_typemap_attach_parms("m3wrapinconv", l, NULL);
    alaqil_typemap_attach_parms("m3wrapincheck", l, NULL);
    alaqil_typemap_attach_parms("m3wrapoutname", l, NULL);
    alaqil_typemap_attach_parms("m3wrapouttype", l, NULL);
    alaqil_typemap_attach_parms("m3wrapoutconv", l, NULL);
    alaqil_typemap_attach_parms("m3wrapoutcheck", l, NULL);
    ...
    alaqil_typemap_attach_parms("m3wrapfreearg", l, NULL);

/*
    alaqil_typemap_attach_parms("m3wrapargvar:throws", l, NULL);
    alaqil_typemap_attach_parms("m3wrapargraw:throws", l, NULL);
    alaqil_typemap_attach_parms("m3wrapinconv:throws", l, NULL);
    alaqil_typemap_attach_parms("m3wrapincheck:throws", l, NULL);
    alaqil_typemap_attach_parms("m3wrapoutconv:throws", l, NULL);
    alaqil_typemap_attach_parms("m3wrapoutcheck:throws", l, NULL);

    attachMappedType(n, "m3wrapretvar:throws");
    attachMappedType(n, "m3wrapretconv:throws");
    attachMappedType(n, "m3wrapretcheck:throws");

    alaqil_typemap_attach_parms("m3wrapfreearg:throws", l, NULL);
*/
	...
	if (tm != NIL) {
      ...
      if (multiretval) {
	Printv(result_name, alaqil_cresult_name(), NIL);
    ...
    if (proxy_flag && global_variable_flag) {
      setter_flag = (Cmp(Getattr(n, "sym:name"), alaqil_name_set(NSPACE_TODO, variable_name)) == 0);
      // Properties
      if (setter_flag) {
	// Setter method
  ...
  bool substituteClassname(alaqilType *pt, String *tm) {
	...
	Replaceall(tm, "$m3classname", classname);
      } else {			// use $descriptor if alaqil does not know anything about this type. Note that any typedefs are resolved.
	String *descriptor = NULL;
	alaqilType *type = Copy(alaqilType_typedef_resolve_all(pt));

	if (Strstr(tm, "$&m3classname")) {
	  alaqilType_add_pointer(type);
	  descriptor = NewStringf("alaqilTYPE%s", alaqilType_manglestr(type));
	...
	Setattr(alaqil_types_hash, descriptor, type);
	Delete(descriptor);
	Delete(type);
      }
  ...
  void attachParameterNames(Node *n, const char *tmid, const char *nameid, const char *fmt) {
	...
	alaqil_warning(WARN_MODULA3_DOUBLE_ID, input_file, line_number, "Argument '%s' twice.\n", newname);
      }
...
/* not used any longer
    - try alaqilType_str if required again */
#if 0
  /* -----------------------------------------------------------------------------
   * createCSignature()
   *
  ...
  void emitTypeWrapperClass(String *classname, alaqilType *type) {
    ...
    String *filen = NewStringf("%s%s.m3", alaqil_output_directory(), classname);
    ...
    // Emit the class
    Printv(alaqiltype, typemapLookup(n, "m3imports", type, WARN_NONE),	// Import statements
  ...
  void addImports(Hash *imports_hash, const String *typemap, Node *node) {
	...
	for (Iterator imp = First(import_list); imp.item != NIL; imp = Next(imp)) {
	  ...
	  if (Len(import_pair) == 1) {
	    ...
	    alaqil_warning(WARN_MODULA3_BAD_IMPORT, input_file, line_number,
			 "Malformed import '%s' for typemap '%s' defined for type '%s'\n", imp, typemap, alaqilType_str(Getattr(node, "type"), 0));
	  }
	  Delete(import_pair);
	}
      }
...
/* -----------------------------------------------------------------------------
 * alaqil_modula3()    - Instantiate module
 * ----------------------------------------------------------------------------- */

extern "C" Language *alaqil_modula3(void) {
  return new MODULA3();
}
