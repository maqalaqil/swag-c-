 ...
 * of alaqil, and the README file for authors - http://www.alaqil.org/release.html.
 *
 * go.cxx
 *
 * Go language module for alaqil.
 * ----------------------------------------------------------------------------- */
...
static inline void _rotl(alaqil_uint64 *v, int bits) {
  assert(bits <= 32);
...
// dst ^= src
static inline void _xor(alaqil_uint64 *dst, alaqil_uint64 *src) {
  dst->lo ^= src->lo;
  dst->hi ^= src->hi;
}

// dst += src
static inline void _add(alaqil_uint64 *dst, alaqil_uint64 *src) {
  dst->lo += src->lo;
...
static void siphash(alaqil_uint64 *out, const char *inc, unsigned long inlen) {
  /* "somepseudorandomlygeneratedbytes" */
  alaqil_uint64 v0 = {0x736f6d65UL, 0x70736575UL};
  alaqil_uint64 v1 = {0x646f7261UL, 0x6e646f6dUL};
  alaqil_uint64 v2 = {0x6c796765UL, 0x6e657261UL};
  alaqil_uint64 v3 = {0x74656462UL, 0x79746573UL};
  alaqil_uint64 b;
  /* hard-coded k. */
  alaqil_uint64 k0 = {0x07060504UL, 0x03020100UL};
  alaqil_uint64 k1 = {0x0F0E0D0CUL, 0x0B0A0908UL};
  int i;
...
class GO:public Language {
  ...
  // been defined.  The index is a alaqilType.
...
public:
  GO():package(NULL),
    ...
    director_language = 1;
    director_prot_ctor_code = NewString("_alaqil_gopanic(\"accessing abstract class or protected constructor\");");
  }
...
private:
  ...
  virtual void main(int argc, char *argv[]) {
    ...
    Preprocessor_define("alaqilGO 1", 0);

    if (cgo_flag) {
      Preprocessor_define("alaqilGO_CGO 1", 0);
    }

    if (gccgo_flag) {
      Preprocessor_define("alaqilGO_GCCGO 1", 0);
    }
    ...
    if (intgo_type_size == 0 && !display_help) {
      Printf(stderr, "alaqil -go: -intgosize option required but not specified\n");
      alaqil_exit(EXIT_FAILURE);
    }

    if (intgo_type_size == 32) {
      Preprocessor_define("alaqilGO_INTGO_SIZE 32", 0);
    } else if (intgo_type_size == 64) {
      Preprocessor_define("alaqilGO_INTGO_SIZE 64", 0);
    } else {
      Preprocessor_define("alaqilGO_INTGO_SIZE 0", 0);
    }

    // Add typemap definitions.
    alaqil_typemap_lang("go");
    alaqil_config_file("go.swg");
  ...
  virtual int top(Node *n) {
    ...
    String *alaqil_filename = Getattr(n, "infile");
    String *c_filename = Getattr(n, "outfile");
    String *c_filename_h = Getattr(n, "outfile_h");

    String *go_filename = NewString("");
    Printf(go_filename, "%s%s.go", alaqil_output_directory(), module);

    String *gc_filename = NULL;
    if (!gccgo_flag) {
      gc_filename = NewString("");
      Printf(gc_filename, "%s%s_gc.c", alaqil_output_directory(), module);
    }

    // Generate a unique ID based on a hash of the alaqil input.
    alaqil_uint64 hash = {0, 0};
    FILE *alaqil_input = alaqil_open(alaqil_filename);
    if (alaqil_input == NULL) {
      FileErrorDisplay(alaqil_filename);
      alaqil_exit(EXIT_FAILURE);
    }
    String *alaqil_input_content = alaqil_read_file(alaqil_input);
    siphash(&hash, Char(alaqil_input_content), Len(alaqil_input_content));
    Delete(alaqil_input_content);
    fclose(alaqil_input);
    unique_id = NewString("");
    ...
    f_c_begin = NewFile(c_filename, "w", alaqil_output_files());
    if (!f_c_begin) {
	...
	alaqil_exit(EXIT_FAILURE);
      ...
      f_c_directors_h = NewFile(c_filename_h, "w", alaqil_output_files());
      if (!f_c_directors_h) {
    ...
    f_go_begin = NewFile(go_filename, "w", alaqil_output_files());
    if (!f_go_begin) {
    ...
    if (!gccgo_flag && !cgo_flag) {
      f_gc_begin = NewFile(gc_filename, "w", alaqil_output_files());
      if (!f_gc_begin) {
    ...
    alaqil_register_filebyname("begin", f_c_begin);
    alaqil_register_filebyname("runtime", f_c_runtime);
    alaqil_register_filebyname("header", f_c_header);
    alaqil_register_filebyname("wrapper", f_c_wrappers);
    alaqil_register_filebyname("init", f_c_init);
    alaqil_register_filebyname("director", f_c_directors);
    alaqil_register_filebyname("director_h", f_c_directors_h);
    alaqil_register_filebyname("go_begin", f_go_begin);
    alaqil_register_filebyname("go_imports", f_go_imports);
    alaqil_register_filebyname("go_runtime", f_go_runtime);
    alaqil_register_filebyname("go_header", f_go_header);
    alaqil_register_filebyname("go_wrapper", f_go_wrappers);
    alaqil_register_filebyname("go_director", f_go_directors);
    if (!gccgo_flag && !cgo_flag) {
      alaqil_register_filebyname("gc_begin", f_gc_begin);
      alaqil_register_filebyname("gc_runtime", f_gc_runtime);
      alaqil_register_filebyname("gc_header", f_gc_header);
      alaqil_register_filebyname("gc_wrapper", f_gc_wrappers);
    }
    if (cgo_flag) {
      alaqil_register_filebyname("cgo_comment", f_cgo_comment);
      alaqil_register_filebyname("cgo_comment_typedefs", f_cgo_comment_typedefs);
    }

    alaqil_banner(f_c_begin);
    if (CPlusPlus) {
      Printf(f_c_begin, "\n// source: %s\n\n", alaqil_filename);
    } else {
      Printf(f_c_begin, "\n/* source: %s */\n\n", alaqil_filename);
    ...
    Printf(f_c_runtime, "#define alaqilMODULE %s\n", module);
    if (gccgo_flag) {
      Printf(f_c_runtime, "#define alaqilGO_PREFIX %s\n", go_prefix);
    }

    if (directorsEnabled()) {
      Printf(f_c_runtime, "#define alaqil_DIRECTORS\n");

      alaqil_banner(f_c_directors_h);
      Printf(f_c_directors_h, "\n// source: %s\n\n", alaqil_filename);

      Printf(f_c_directors_h, "#ifndef alaqil_%s_WRAP_H_\n", module);
      Printf(f_c_directors_h, "#define alaqil_%s_WRAP_H_\n\n", module);
      Printf(f_c_directors_h, "class alaqil_memory;\n\n");

      Printf(f_c_directors, "\n// C++ director class methods.\n");
      String *filename = alaqil_file_filename(c_filename_h);
      Printf(f_c_directors, "#include \"%s\"\n\n", filename);
      Delete(filename);
    }

    alaqil_banner(f_go_begin);
    Printf(f_go_begin, "\n// source: %s\n", alaqil_filename);

    if (!gccgo_flag && !cgo_flag && soname) {
      alaqil_banner(f_gc_begin);
      Printf(f_gc_begin, "\n/* source: %s */\n\n", alaqil_filename);
    ...
    if (cgo_flag) {
      ...
      // intgo also.
      Printv(f_cgo_comment_typedefs, "#define intgo alaqil_intgo\n", NULL);
      Printv(f_cgo_comment_typedefs, "typedef void *alaqil_voidp;\n", NULL);
    }
    ...
    if (gccgo_flag && !cgo_flag) {
      Printf(f_go_runtime, "func alaqilCgocall()\n");
      Printf(f_go_runtime, "func alaqilCgocallDone()\n");
      Printf(f_go_runtime, "func alaqilCgocallBack()\n");
      Printf(f_go_runtime, "func alaqilCgocallBackDone()\n\n");
    }
    ...
    // Set up the hash table for types not defined by alaqil.
    ...
    if (directorsEnabled()) {
      ...
      alaqil_insert_file("director_common.swg", f_c_runtime);
      alaqil_insert_file("director.swg", f_c_runtime);
    }

    Delete(go_imports);

    // Write out definitions for the types not defined by alaqil.
	...
	if (!Getattr(defined_types, cp)) {
	  ...
	  Printv(f_go_wrappers, "\talaqilcptr() uintptr;\n", NULL);
	  Printv(f_go_wrappers, "}\n", NULL);
	  Printv(f_go_wrappers, "func (p ", cp, ") alaqilcptr() uintptr {\n", NULL);
   ...
   * importDirective()
   *
   * Handle a alaqil import statement by generating a Go import
   * statement.
   * ------------------------------------------------------------ */
  ...
  virtual int functionWrapper(Node *n) {
    ...
    if (alaqil_methodclass(n) && alaqil_directorclass(n)
    ...
    alaqilType *result = Getattr(n, "type");

    // For some reason alaqil changs the "type" value during the call to
	...
	return alaqil_NOWRAP;
      ...
      if (alaqil_methodclass(n) && alaqil_directorclass(n)) {
	// The core alaqil code skips the first parameter when
	...
	String *call = alaqil_cppconstructor_call(getClassType(),
						Getattr(n, "parms"));
	alaqilType *type = Copy(getClassType());
    ...
    String *wname = alaqil_name_wrapper(name);
    if (overname) {
   ...
   * wname: The alaqil wrapped name--the name of the C function.
  ...
  int makeWrappers(Node *n, String *name, String *go_name, String *overname, String *wname, List *base, ParmList *parms, alaqilType *result, bool is_static) {
  ...
  int makeCgoWrappers(Node *n, String *go_name, String *overname, String *wname, List *base, ParmList *parms, alaqilType *result, bool is_static) {
    alaqil_save("makeCgoWrappers", n, "emit:cgotype", "emit:cgotypestruct", NULL);
  ...
  int cgoGoWrapper(const cgoWrapperInfo *info) {
	...
	Printv(f_go_wrappers, "_alaqil_base", NULL);
      } else {
	...
	if (pi >= required_count) {
	  Printv(parm_print, "_alaqil_args ...interface{}", NULL);
	  break;
	}
    ...
    if (info->is_constructor) {
      String *cl = exportedName(class_name);
      Printv(parm_print, " (_alaqil_ret ", cl, ")", NULL);
      Delete(cl);
    } else {
      if (alaqilType_type(info->result) != T_VOID) {
	String *tm = goType(info->n, info->result);
	Printv(parm_print, " (_alaqil_ret ", tm, ")", NULL);
	Delete(tm);
      }
    }
	...
	Printf(f_go_wrappers, "\tif len(_alaqil_args) > %d {\n", i - required_count);
	Printf(f_go_wrappers, "\t\t%s = _alaqil_args[%d].(%s)\n", Getattr(p, "lname"), i - required_count, tm);
	Printv(f_go_wrappers, "\t}\n", NULL);
	Delete(tm);
	p = nextParm(p);
      }
	...
	ret_type = goImType(info->n, info->result);
      ...
      Printv(f_go_wrappers, "\tvar alaqil_r ", ret_type, "\n", NULL);
	...
	memcpy_ret = true;
      ...
      if (memcpy_ret) {
	Printv(call, "alaqil_r_p := ", NULL);
      } else {
	Printv(call, "alaqil_r = (", ret_type, ")(", NULL);
      }
    ...
    if (parm_count > required_count) {
      Printv(call, "C.alaqil_intgo(len(_alaqil_args))", NULL);
      ++args;
    }
	...
	Printv(call, ", ", NULL);
      ...
      ++args;
      Printv(call, "C.uintptr_t(_alaqil_base)", NULL);
    }
	...
	Printv(call, ", ", NULL);
      ...
      String *ivar = NewStringf("_alaqil_i_%d", i);
	...
	if ((i == 0 && info->is_destructor) || ((i > 0 || !info->receiver || info->base || info->is_constructor) && goTypeIsInterface(p, pt))) {
	  Printv(f_go_wrappers, ".alaqilcptr()", NULL);
	}
	Printv(f_go_wrappers, "\n", NULL);
    ...
    if (alaqilType_type(info->result) != T_VOID && !memcpy_ret) {
    ...
    if (memcpy_ret) {
      Printv(f_go_wrappers, "\talaqil_r = *(*", ret_type, ")(unsafe.Pointer(&alaqil_r_p))\n", NULL);
    }
    if (ret_type) {
      Delete(ret_type);
    }
    ...
    if (alaqilType_type(info->result) != T_VOID) {
      ...
      alaqil_save("cgoGoWrapper", info->n, "type", "tmap:goout", NULL);
      Setattr(info->n, "type", info->result);

      String *goout = goTypemapLookup("goout", info->n, "alaqil_r");
      if (goout == NULL) {
	Printv(f_go_wrappers, "\treturn alaqil_r\n", NULL);
      } else {
	String *tm = goType(info->n, info->result);
	Printv(f_go_wrappers, "\tvar alaqil_r_1 ", tm, "\n", NULL);
	goout = Copy(goout);
	Replaceall(goout, "$input", "alaqil_r");
	Replaceall(goout, "$result", "alaqil_r_1");
	Printv(f_go_wrappers, goout, "\n", NULL);
	Printv(f_go_wrappers, "\treturn alaqil_r_1\n", NULL);
      }

      alaqil_restore(info->n);
    }
  ...
  int cgoCommentWrapper(const cgoWrapperInfo *info) {
    ...
    if (parm_count > required_count) {
      Printv(f_cgo_comment, "intgo _alaqil_args", NULL);
      ++args;
    }
	...
	Printv(f_cgo_comment, ", ", NULL);
      ...
      ++args;
      Printv(f_cgo_comment, "uintptr_t _alaqil_base", NULL);
    }
  ...
  int cgoGccWrapper(const cgoWrapperInfo *info) {
    ...
    alaqil_save("cgoGccWrapper", info->n, "parms", NULL);

    ParmList *parms = info->parms;

    Parm *base_parm = NULL;
    if (info->base && !isStatic(info->n)) {
      alaqilType *base_type = Copy(getClassType());
      alaqilType_add_pointer(base_type);
    ...
    if (parm_count > required_count) {
      Printv(fnname, "intgo _alaqil_optargc", NULL);
      ++args;
    }
	...
	Printv(fnname, ", ", NULL);
      ...
      alaqilType *pt = Copy(Getattr(p, "type"));
      if (alaqilType_isarray(pt)) {
	alaqilType_del_array(pt);
	alaqilType_add_pointer(pt);
      }
      String *pn = NewStringf("_alaqil_go_%d", i);
    ...
    if (alaqilType_type(info->result) == T_VOID) {
      ...
      String *ln = NewString("_alaqil_go_result");
      ct = gcCTypeForGoValue(info->n, info->result, ln);
      Wrapper_add_local(f, "_alaqil_go_result", ct);
      Delete(ct);
      Delete(ln);
    }
	...
	if (i < required_count) {
	  ...
	  Printf(f->code, "\tif (_alaqil_optargc > %d) {\n", i - required_count);
	  Printv(f->code, "\t\t", tm, "\n", NULL);
	  Printv(f->code, "\t}\n", NULL);
	}
	Delete(tm);
    ...
    if (alaqilType_type(info->result) != T_VOID) {
      Printv(f->code, "\treturn _alaqil_go_result;\n", NULL);
    }
  ...
  int goFunctionWrapper(Node *n, String *name, String *go_name, String *overname, String *wname, List *base, ParmList *parms, alaqilType *result, bool is_static) {
    ...
    alaqil_save("cgoGoWrapper", n, "type", "tmap:goout", NULL);
    Setattr(n, "type", result);

    String *goout = goTypemapLookup("goout", n, "alaqil_r");
	...
	if (alaqilType_type(result) != T_VOID) {
	...
	Printv(f_go_wrappers, " {\n", NULL);
	if (arg) {
	  Printv(f_go_wrappers, "\t_alaqil_p := uintptr(unsafe.Pointer(&base))\n", NULL);
	} else {
	  Printv(f_go_wrappers, "\tvar _alaqil_p uintptr\n", NULL);
	}
	Printv(f_go_wrappers, "\t_cgo_runtime_cgocall(", wname, ", _alaqil_p)\n", NULL);
	...
	for (; i < parm_count; ++i) {
      ...
      bool need_return_var = alaqilType_type(result) != T_VOID && ((gccgo_flag && is_constructor) || has_goout);
      if (need_return_var) {
	Printv(f_go_wrappers, "\tvar alaqil_r ", NULL);
	if (is_constructor) {
	  String *cl = exportedName(class_name);
	...
	Printv(call, "\tdefer alaqilCgocallDone()\n", NULL);
	Printv(call, "\talaqilCgocall()\n", NULL);
      }

      Printv(call, "\t", NULL);
      if (alaqilType_type(result) != T_VOID) {
	if (need_return_var) {
	  Printv(call, "alaqil_r = ", NULL);
	} else {
      ...
      if (parm_count > required_count) {
	Printv(call, "len(_alaqil_args)", NULL);
      }
	...
	if (goin == NULL) {
	  ...
	  if ((i == 0 && is_destructor) || ((i > 0 || !receiver || base || is_constructor) && goTypeIsInterface(p, pt))) {
	    Printv(call, ".alaqilcptr()", NULL);
	  }
	  Setattr(p, "emit:goinput", ln);
	} else {
	  String *ivar = NewString("");
	  Printf(ivar, "_alaqil_i_%d", i);
	  String *itm = goImType(p, pt);
	  Printv(f_go_wrappers, "\tvar ", ivar, " ", itm, "\n", NULL);
	  goin = Copy(goin);
	  Replaceall(goin, "$input", ln);
	...
	if (paramNeedsEscape(p)) {
	  Printv(f_go_wrappers, "\tif alaqil_escape_always_false {\n", NULL);
	  Printv(f_go_wrappers, "\t\talaqil_escape_val = ", Getattr(p, "emit:goinput"), "\n", NULL);
	  Printv(f_go_wrappers, "\t}\n", NULL);
	}
	...
	Printv(f_go_wrappers, "\t_alaqil_p := uintptr(unsafe.Pointer(&", first, "))\n", NULL);
      }
      Printv(f_go_wrappers, "\t_cgo_runtime_cgocall(", wname, ", _alaqil_p)\n", NULL);
      Printv(f_go_wrappers, "\treturn\n", NULL);
      Printv(f_go_wrappers, "}", NULL);
  ...
  Wrapper* initGoTypemaps(ParmList *parms) {
    ...
    for (int i = 0; i < parm_count; ++i) {
      p = getParm(p);
      alaqil_cparm_name(p, i);
      p = nextParm(p);
    }

    alaqil_typemap_attach_parms("default", parms, dummy);
    alaqil_typemap_attach_parms("gotype", parms, dummy);
    alaqil_typemap_attach_parms("goin", parms, dummy);
    alaqil_typemap_attach_parms("goargout", parms, dummy);
    alaqil_typemap_attach_parms("imtype", parms, dummy);
   ...
   * executing the alaqil wrapper code.
   * ---------------------------------------------------------------------- */

  int gccFunctionWrapper(Node *n, List *base, String *wname, ParmList *parms, alaqilType *result) {
    Wrapper *f = NewWrapper();

    alaqil_save("gccFunctionWrapper", n, "parms", NULL);
    ...
    String *alaqilargs = NewString("\tstruct alaqilargs {\n");

    if (parm_count > required_count) {
      needs_alaqilargs = true;
      Printv(alaqilargs, "\t\tintgo _alaqil_optargc;\n", NULL);
    }
    ...
    for (int i = 0; i < parm_count; ++i) {
      ...
      Printv(alaqilargs, "\t\t\t", ct, ";\n", NULL);
      needs_alaqilargs = true;
      Delete(ct);

      String *gn = NewStringf("_alaqil_go_%d", i);
    ...
    if (alaqilType_type(result) != T_VOID) {
      Printv(alaqilargs, "\t\tlong : 0;\n", NULL);
      String *ln = NewString(alaqil_cresult_name());
      ...
      ln = NewString("_alaqil_go_result");
      ct = gcCTypeForGoValue(n, result, ln);
      Wrapper_add_local(f, "_alaqil_go_result", ct);
      Delete(ct);
      Delete(ln);
    }
    Printv(alaqilargs, "\t} alaqilSTRUCTPACKED *alaqil_a = (struct alaqilargs *) alaqil_v;\n", NULL);
	...
	if (i < required_count) {
	  ...
	  Printf(f->code, "\tif (alaqil_a->_alaqil_optargc > %d) {\n", i - required_count);
	  Printv(f->code, "\t\t", tm, "\n", NULL);
	  Printv(f->code, "\t}\n", NULL);
	}
	Delete(tm);
    ...
    if (needs_alaqilargs)
    {
      Printv(f->locals, alaqilargs, NULL);
    }
  ...
  int gccgoFunctionWrapper(Node *n, List *base, String *wname, ParmList *parms, alaqilType *result) {
    Wrapper *f = NewWrapper();

    alaqil_save("gccgoFunctionWrapper", n, "parms", NULL);
    ...
    for (int i = 0; i < parm_count; ++i) {
      ...
      String *tm = Getattr(p, "tmap:in");
      if (!tm) {
	alaqil_warning(WARN_TYPEMAP_IN_UNDEF, input_file, line_number,
  ...
  void emitGoAction(Node *n, List *base, ParmList *parms, alaqilType *result, Wrapper *f) {
    if (!gccgo_flag && !cgo_flag && alaqilType_type(result) != T_VOID) {
      Wrapper_add_local(f, "alaqil_stktop", "char *alaqil_stktop");
      Printv(f->code, "\talaqil_stktop = _alaqil_topofstack();\n", NULL);
    }
    String *actioncode;
    if (!base || isStatic(n)) {
    ...
    alaqil_save("emitGoAction", n, "type", "tmap:out", NULL);

    Setattr(n, "type", result);

    String *tm = alaqil_typemap_lookup_out("out", n, alaqil_cresult_name(), f, actioncode);
    if (!tm) {
      alaqil_warning(WARN_TYPEMAP_OUT_UNDEF, input_file, line_number, "Unable to use return type %s\n", alaqilType_str(result, 0));
    } else {
      Replaceall(tm, "$result", "_alaqil_go_result");
    ...
    if (!gccgo_flag && !cgo_flag && alaqilType_type(result) != T_VOID) {
      ...
      // have been copied.  We need to adjust alaqil_a accordingly here.
      // This is what cgo does.
      Printv(f->code, "\talaqil_a = (struct alaqilargs*)((char*)alaqil_a + (_alaqil_topofstack() - alaqil_stktop));\n", NULL);
      Printv(f->code, "\talaqil_a->", alaqil_cresult_name(), " = ", "_alaqil_go_result;\n", NULL);
    }
  ...
  void cleanupFunction(Node *n, Wrapper *f, ParmList *parms) {
    ...
    if (GetFlag(n, "feature:new")) {
      String *tm = alaqil_typemap_lookup("newfree", n, alaqil_cresult_name(), 0);
      if (tm) {
  ...
  virtual int constantWrapper(Node *n) {
    ...
    if (!alaqilType_issimple(type) && alaqilType_type(type) != T_STRING) {
    ...
    String *copy = NULL;
    if (alaqilType_type(type) == T_BOOL) {
      if (Cmp(value, "true") != 0 && Cmp(value, "false") != 0) {
	return goComplexConstant(n, type);
      }
    } else if (alaqilType_type(type) == T_STRING || alaqilType_type(type) == T_CHAR) {
    ...
    } else if (alaqilType_type(type) == T_CHAR) {
      Printv(f_go_wrappers, "'", value, "'", NULL);
    } else {
      Printv(f_go_wrappers, value, NULL);
    }
  ...
  virtual int enumvalueDeclaration(Node *n) {
    ...
    alaqil_require("enumvalueDeclaration", n, "*sym:name", NIL);
    ...
    if (GetFlag(parent, "scopedenum")) {
      ...
      symname = alaqil_name_member(0, Getattr(parent, "sym:name"), symname);
  ...
  int goComplexConstant(Node *n, alaqilType *type) {
    String *symname = Getattr(n, "sym:name");
    if (!symname) {
    ...
    if (rawval && Len(rawval)) {
      // Based on alaqil_VargetToFunction
      String *nname = NewStringf("(%s)", rawval);
      String *call;
      if (alaqilType_isclass(type)) {
	call = NewStringf("%s", nname);
      } else {
	call = alaqilType_lcaststr(type, nname);
      }
      String *cres = alaqil_cresult(type, alaqil_cresult_name(), call);
      Setattr(n, "wrap:action", cres);
      Delete(nname);
      ...
      Printv(get, alaqil_cresult_name(), " = ", NULL);
    ...
    String *go_name = NewString("_alaqil_get");
    if (class_name) {
    ...
    String *wname = alaqil_name_wrapper(sname);
    Append(wname, unique_id);
    Setattr(n, "wrap:name", wname);
  ...
  virtual int classHandler(Node *n) {
    ...
    // value.
    Printv(f_go_wrappers, "func (p ", go_type_name, ") alaqilcptr() uintptr {\n", NULL);
    Printv(f_go_wrappers, "\treturn (uintptr)(p)\n", NULL);
    Printv(f_go_wrappers, "}\n\n", NULL);

    // A method used as a marker for the class, to avoid invalid
    // interface conversions when using multiple inheritance.
    Printv(f_go_wrappers, "func (p ", go_type_name, ") alaqilIs", go_name, "() {\n", NULL);
    Printv(f_go_wrappers, "}\n\n", NULL);
    ...
    Printv(f_go_wrappers, "\talaqilcptr() uintptr\n", NULL);
    Printv(f_go_wrappers, "\talaqilIs", go_name, "()\n", NULL);
  ...
  int addBase(Node *n, Node *base, List *bases, Hash *local) {
	...
	continue;
      ...
      alaqilType_push(ty, Getattr(ni, "decl"));
      String *fullty = alaqilType_typedef_resolve_all(ty);
      bool is_function = alaqilType_isfunction(fullty) ? true : false;
      Delete(ty);
      Delete(fullty);
  ...
  int goBaseMethod(Node *method_class, List *bases, Node *method) {
    ...
    alaqilType_push(result, Getattr(method, "decl"));
    if (alaqilType_isqualifier(result)) {
      Delete(alaqilType_pop(result));
    }
    Delete(alaqilType_pop_function(result));

    // If the base method is imported, wrap:action may not be set.
    alaqil_save("goBaseMethod", method, "wrap:name", "wrap:action", "parms", NULL);
    Setattr(method, "wrap:name", wname);
    if (!Getattr(method, "wrap:action")) {
      if (!is_static) {
	alaqil_MethodToFunction(method, getNSpace(), getClassType(), (Getattr(method, "template") ? SmartPointer : Extend | SmartPointer), NULL, false);
	...
	String *call = alaqil_cfunction_call(Getattr(method, "name"), Getattr(method, "parms"));
	Setattr(method, "wrap:action", alaqil_cresult(Getattr(method, "type"), alaqil_cresult_name(), call));
      }
    }

    int r = makeWrappers(method, name, go_name, overname, wname, bases, Getattr(method, "parms"), result, is_static);

    alaqil_restore(method);
  ...
  int goBaseVariable(Node *var_class, List *bases, Node *var) {
    ...
    alaqil_save("goBaseVariable", var, "type", "wrap:action", NULL);

    // For a pointer type we apparently have to wrap in the decl.
    alaqilType *var_type = NewString(Getattr(var, "type"));
    alaqilType_push(var_type, Getattr(var, "decl"));
    Setattr(var, "type", var_type);

    alaqilType *vt = Copy(var_type);
    ...
    // Copied from alaqil_wrapped_member_var_type.
    if (alaqilType_isclass(vt)) {
      if (flags & CWRAP_NATURAL_VAR) {
	if (CPlusPlus) {
	  if (!alaqilType_isconst(vt)) {
	    alaqilType_add_qualifier(vt, "const");
	  }
	  alaqilType_add_reference(vt);
	}
      } else {
	alaqilType_add_pointer(vt);
      }
    }

    String *mname = alaqil_name_member(getNSpace(), Getattr(var_class, "sym:name"), var_name);
	...
	if (Strncmp(ki.key, "tmap:", 5) == 0) {
      ...
      alaqil_save("goBaseVariableSet", var, "name", "sym:name", "type", NULL);
      ...
      alaqil_MembersetToFunction(var, class_name, flags);

      String *wname = alaqil_name_wrapper(mname_set);
      Append(wname, unique_id);
    ...
    alaqil_MembergetToFunction(var, class_name, flags);
    ...
    String *wname = alaqil_name_wrapper(mname_get);
    Append(wname, unique_id);
  ...
  void addFirstBaseInterface(Node *n, Hash *parents, List *bases) {
    ...
    if (!GetFlag(b.item, "feature:ignore")) {
      ...
      Printv(f_go_wrappers, "func (p ", go_type_name, ") alaqilIs", go_base_name, "() {\n", NULL);
      Printv(f_go_wrappers, "}\n\n", NULL);

      Printv(interfaces, "\talaqilIs", go_base_name, "()\n", NULL);

      Printv(f_go_wrappers, "func (p ", go_type_name, ") alaqilGet", go_base_name, "() ", go_base_type, " {\n", NULL);
      Printv(f_go_wrappers, "\treturn ", go_base_type_name, "(p.alaqilcptr())\n", NULL);
      Printv(f_go_wrappers, "}\n\n", NULL);

      Printv(interfaces, "\talaqilGet", go_base_name, "() ", go_base_type, "\n", NULL);
  ...
  int addExtraBaseInterfaces(Node *n, Hash *parents, List *bases) {
	...
	continue;
      ...
      alaqil_save("addExtraBaseInterface", n, "wrap:action", "wrap:name", "wrap:parms", NULL);

      alaqilType *type = Copy(Getattr(n, "classtypeobj"));
      alaqilType_add_pointer(type);
      Parm *parm = NewParm(type, "self", n);
      Setattr(n, "wrap:parms", parm);

      String *pn = alaqil_cparm_name(parm, 0);
      String *action = NewString("");
      Printv(action, alaqil_cresult_name(), " = (", Getattr(b.item, "classtype"), "*)", pn, ";", NULL);
      Delete(pn);

      Setattr(n, "wrap:action", action);

      String *name = Copy(class_name);
      Append(name, "_alaqilGet");
      Append(name, go_base_name);

      String *go_name = NewString("alaqilGet");
      ...
      alaqilType *result = Copy(Getattr(b.item, "classtypeobj"));
      alaqilType_add_pointer(result);
  ...
  void addParentExtraBaseInterfaces(Node *n, Hash *parents, Node *base, bool is_base_first, String *sofar) {
    ...
    String *sf = NewString("");
    Printv(sf, sofar, ".alaqilGet", go_this_base_name, "()", NULL);
	...
	Printv(f_go_wrappers, "func (p ", go_type_name, ") alaqilGet", go_base_name, "() ", go_base_name, " {\n", NULL);
	Printv(f_go_wrappers, "\treturn p", sf, ".alaqilGet", go_base_name, "()\n", NULL);
	Printv(f_go_wrappers, "}\n\n", NULL);

	Printv(interfaces, "\talaqilGet", go_base_name, "() ", go_base_name, "\n", NULL);
  ...
  int classDirectorInit(Node *n) {
    ...
    String *director_struct_name = NewString("_alaqil_Director");
    Append(director_struct_name, go_name);

    String *cxx_director_name = NewString("alaqilDirector_");
    ...
    Printv(f_go_wrappers, "func (p *", director_struct_name, ") alaqilcptr() uintptr {\n", NULL);
    Printv(f_go_wrappers, "\treturn p.", go_type_name, ".alaqilcptr()\n", NULL);
    Printv(f_go_wrappers, "}\n\n", NULL);

    Printv(f_go_wrappers, "func (p *", director_struct_name, ") alaqilIs", go_name, "() {\n", NULL);
    Printv(f_go_wrappers, "}\n\n", NULL);
  ...
  int classDirectorConstructor(Node *n) {
    ...
    String *fn_name = NewString("_alaqil_NewDirector");
    Append(fn_name, cn);
    ...
    String *wname = alaqil_name_wrapper(fn_name);
    ...
    alaqil_typemap_attach_parms("gotype", parms, NULL);
    alaqil_typemap_attach_parms("goin", parms, NULL);
    alaqil_typemap_attach_parms("goargout", parms, NULL);
    alaqil_typemap_attach_parms("imtype", parms, NULL);
    int parm_count = emit_num_arguments(parms);
    ...
    alaqilType *first_type = NewString("int");
    Parm *first_parm = NewParm(first_type, "alaqil_p", n);
	...
	Printv(f_go_wrappers, "func ", fn_with_over_name, "(_alaqil_director int", NULL);
	...
	Printv(f_go_wrappers, ") (_alaqil_ret ", go_type_name, ")", NULL);

	if (!gccgo_flag) {
	  Printv(f_go_wrappers, " {\n", NULL);
	  Printv(f_go_wrappers, "\t_alaqil_p := uintptr(unsafe.Pointer(&_alaqil_director))\n", NULL);
	  Printv(f_go_wrappers, "\t_cgo_runtime_cgocall(", wname, ", _alaqil_p)\n", NULL);
	  Printv(f_go_wrappers, "\treturn\n", NULL);
	  Printv(f_go_wrappers, "}", NULL);
	}
	...
	p = nextParm(p);
      ...
      if (gccgo_flag && !cgo_flag) {
	Printv(f_go_wrappers, "\tdefer alaqilCgocallDone()\n", NULL);
	Printv(f_go_wrappers, "\talaqilCgocall()\n", NULL);
      }
      ...
      if (cgo_flag) {
	Printv(call, go_type_name, "(C.", wname, "(C.int(alaqilDirectorAdd(p))", NULL);
      } else {
	Printv(call, fn_with_over_name, "(alaqilDirectorAdd(p)", NULL);
      }
	...
	Printv(call, ")", NULL);
      ...
      alaqilType *result = Copy(Getattr(parentNode(n), "classtypeobj"));
      alaqilType_add_pointer(result);

      alaqil_save("classDirectorConstructor", n, "wrap:name", "wrap:action", NULL);

      String *dwname = alaqil_name_wrapper(name);
      Append(dwname, unique_id);
      Setattr(n, "wrap:name", dwname);

      String *action = NewString("");
      Printv(action, alaqil_cresult_name(), " = new alaqilDirector_", class_name, "(", NULL);
      String *pname = alaqil_cparm_name(NULL, 0);
      Printv(action, pname, NULL);
      Delete(pname);
      p = parms;
      for (int i = 0; i < parm_count; ++i) {
	p = getParm(p);
	String *pname = alaqil_cparm_name(NULL, i + 1);
	Printv(action, ", ", NULL);
	if (alaqilType_isreference(Getattr(p, "type"))) {
	  Printv(action, "*", NULL);
	}
	Printv(action, pname, NULL);
	Delete(pname);
    ...
    String *decl = alaqil_method_decl(NULL, Getattr(n, "decl"), cxx_director_name, first_parm, 0);
    Printv(f_c_directors_h, "  ", decl, ";\n", NULL);
    Delete(decl);

    decl = alaqil_method_decl(NULL, Getattr(n, "decl"), cxx_director_name, first_parm, 0);
    ...
    Printv(f_c_directors, "      go_val(alaqil_p), alaqil_mem(0)\n", NULL);
    Printv(f_c_directors, "{ }\n\n", NULL);
  ...
  int classDirectorDestructor(Node *n) {
    ...
    if (!is_ignored) {
      ...
      String *wname = alaqil_name_wrapper(fnname);
      Append(wname, unique_id);

      Setattr(n, "wrap:name", fnname);

      alaqil_DestructorToFunction(n, getNSpace(), getClassType(), CPlusPlus, Extend);
    ...
    String *go_name = NewString("alaqilgo_DeleteDirector_");
    Append(go_name, class_name);
    ...
    Printv(f_c_directors_h, "  virtual ~alaqilDirector_", class_name, "()", NULL);
    ...
    Printv(director_sig, "alaqilDirector_", class_name, "::~alaqilDirector_", class_name, "()", NULL);
    ...
    Printv(f_c_directors, "  delete alaqil_mem;\n", NULL);
  ...
  void makeDirectorDestructorWrapper(String *go_name, String *director_struct_name, String *director_sig) {
    ...
    if (gccgo_flag) {
      Printv(f_go_wrappers, "\talaqilCgocallBack()\n", NULL);
      Printv(f_go_wrappers, "\tdefer alaqilCgocallBackDone()\n", NULL);
    }
    Printv(f_go_wrappers, "\talaqilDirectorLookup(c).(*", director_struct_name, ").", class_receiver, " = 0\n", NULL);
    Printv(f_go_wrappers, "\talaqilDirectorDelete(c)\n", NULL);
    Printv(f_go_wrappers, "}\n\n", NULL);

    String *wname = NewString("_alaqilgo_wrap_DeleteDirector_");
    ...
    if (!gccgo_flag) {
      Printv(f_c_directors, "  struct { intgo p; } alaqilSTRUCTPACKED a;\n", NULL);
  ...
  int classDirectorMethod(Node *n, Node *parent, String *super) {
	...
	for (Node *on = Getattr(n, "sym:overloaded"); on; on = Getattr(on, "sym:nextSibling")) {
	  // alaqil_overload_rank expects wrap:name and wrap:parms to be
	  // set.
	  String *wn = alaqil_name_wrapper(Getattr(on, "sym:name"));
  ...
  int oneClassDirectorMethod(Node *n, Node *parent, String *super) {
    ...
    String *interface_name = NewString("_alaqil_DirectorInterface");
    ...
    Replace(callback_name, "_alaqil", "alaqil", DOH_REPLACE_FIRST);
    if (overname) {
    ...
    String *upcall_wname = alaqil_name_wrapper(upcall_name);
    if (overname) {
    ...
    alaqil_director_parms_fixup(parms);

    alaqil_typemap_attach_parms("directorin", parms, w);
    alaqil_typemap_attach_parms("directorargout", parms, w);
    alaqil_typemap_attach_parms("godirectorin", parms, w);
    alaqil_typemap_attach_parms("goin", parms, dummy);
    alaqil_typemap_attach_parms("goargout", parms, dummy);
	...
	if (cgo_flag) {
	  ...
	  Printv(f_go_wrappers, "func ", upcall_gc_name, "(_alaqil_ptr ", go_type_name, NULL);
	  ...
	  if (alaqilType_type(result) != T_VOID) {
	    String *tm = goWrapperType(n, result, true);
	    Printv(f_go_wrappers, " (_alaqil_ret ", tm, ")", NULL);
	    Delete(tm);
	  }

	  if (!gccgo_flag) {
	    Printv(f_go_wrappers, " {\n", NULL);
	    Printv(f_go_wrappers, "\t_alaqil_p := uintptr(unsafe.Pointer(&_alaqil_ptr))\n", NULL);
	    Printv(f_go_wrappers, "\t_cgo_runtime_cgocall(", upcall_wname, ", _alaqil_p)\n", NULL);
	    Printv(f_go_wrappers, "\treturn\n", NULL);
	    Printv(f_go_wrappers, "}", NULL);
	  }
      ...
      Printv(f_go_wrappers, "func (alaqil_p *", director_struct_name, ") ", go_with_over_name, "(", NULL);
	...
	Delete(tm);
      ...
      Printv(f_go_wrappers, "\tif alaqil_g, alaqil_ok := alaqil_p.v.(", interface_name, "); alaqil_ok {\n", NULL);
      Printv(f_go_wrappers, "\t\t", NULL);
      if (alaqilType_type(result) != T_VOID) {
	Printv(f_go_wrappers, "return ", NULL);
      }
      Printv(f_go_wrappers, "alaqil_g.", go_with_over_name, "(", NULL);
	...
	if (alaqilType_type(result) != T_VOID) {
	  ...
	  Printv(f_go_wrappers, "\tvar alaqil_r ", ret_type, "\n", NULL);
	  goout = goTypemapLookup("goout", n, "alaqil_r");
	  if (goout) {
	    has_goout = true;
	  }
	...
	Printv(call, "alaqil_p.", go_type_name, NULL);
	if (cgo_flag) {
	  Printv(call, ")", NULL);
	}
	...
	if (cgo_flag) {
	  ...
	  if (alaqilType_type(result) != T_VOID && !memcpy_ret) {
	    // Close the type conversion of the return value.
	    Printv(call, ")", NULL);
	  }
	}
	...
	if (wt) {
      ...
      if (!GetFlag(n, "abstract")) {
	String *upcall_method_name = NewString("_alaqil_upcall_");
	Append(upcall_method_name, name);
	if (overname) {
	  Append(upcall_method_name, overname);
	}
	alaqilType *rtype = Getattr(n, "classDirectorMethods:type");
	String *upcall_decl = alaqil_method_decl(rtype, Getattr(n, "decl"), upcall_method_name, parms, 0);
	...
	String *super_call = alaqil_method_call(super, parms);
	...
	alaqilType *first_type = NULL;
	Parm *first_parm = parms;
	if (!is_static) {
	  first_type = NewString("alaqilDirector_");
	  Append(first_type, class_name);
	  alaqilType_add_pointer(first_type);
	  first_parm = NewParm(first_type, "p", n);
	  set_nextSibling(first_parm, parms);
	}

	alaqil_save("classDirectorMethod", n, "wrap:name", "wrap:action", NULL);

	Setattr(n, "wrap:name", upcall_wname);

	String *action = NewString("");
	if (alaqilType_type(result) != T_VOID) {
	  Printv(action, alaqil_cresult_name(), " = (", alaqilType_lstr(result, 0), ")", NULL);
	  if (alaqilType_isreference(result)) {
	    Printv(action, "&", NULL);
	  }
	}
	Printv(action, alaqil_cparm_name(NULL, 0), "->", upcall_method_name, "(", NULL);

	p = parms;
	int i = 0;
	while (p != NULL) {
	  if (alaqilType_type(Getattr(p, "type")) != T_VOID) {
	    String *pname = alaqil_cparm_name(NULL, i + 1);
	    if (i > 0) {
	      Printv(action, ", ", NULL);
	    }
	...
	Printv(f_go_wrappers, "//export ", callback_name, "\n", NULL);
      ...
      Printv(f_go_wrappers, "func ", callback_name, "(alaqil_c int", NULL);
	...
	Printv(f_go_wrappers, "(alaqil_result ", result_wrapper, ") ", NULL);
      }
      Printv(f_go_wrappers, "{\n", NULL);
	...
	if (alaqilType_type(result) != T_VOID) {
	  ...
	  Printv(f_go_wrappers, "\n", NULL);
	  goout = goTypemapLookup("godirectorout", n, "alaqil_r");
	}
	...
	Printv(call, "alaqil_p.", go_with_over_name, "(", NULL);
	...
	Printv(f_go_wrappers, "\talaqil_p := alaqilDirectorLookup(alaqil_c).(*", director_struct_name, ")\n", NULL);
    ...
    if (!is_ignored || is_pure_virtual) {
      ...
      String *decl = alaqil_method_decl(rtype, Getattr(n, "decl"), Getattr(n, "name"), parms, 0);
      Printv(f_c_directors_h, "  virtual ", decl, NULL);
      Delete(decl);

      String *qname = NewString("");
      Printv(qname, "alaqilDirector_", class_name, "::", Getattr(n, "name"), NULL);
      decl = alaqil_method_decl(rtype, Getattr(n, "decl"), qname, parms, 0);
	...
	Delete(throws);
      ...
      if (alaqilType_type(result) != T_VOID) {
	if (!alaqilType_isclass(result)) {
	  if (!(alaqilType_ispointer(result) || alaqilType_isreference(result))) {
	    String *construct_result = NewStringf("= alaqilValueInit< %s >()", alaqilType_lstr(result, 0));
	    Wrapper_add_localv(w, "c_result", alaqilType_lstr(result, "c_result"), construct_result, NIL);
	    Delete(construct_result);
	  } else {
	    Wrapper_add_localv(w, "c_result", alaqilType_lstr(result, "c_result"), "= 0", NIL);
	  }
	} else {
	  String *cres = alaqilType_lstr(result, "c_result");
	  Printf(w->code, "%s;\n", cres);
	  Delete(cres);
	}
      }
	...
	Printv(w->code, "  _alaqil_gopanic(\"call to pure virtual function ", Getattr(parent, "sym:name"), name, "\");\n", NULL);
	if (alaqilType_type(result) != T_VOID) {
	  String *retstr = alaqilType_rcaststr(result, "c_result");
  ...
  void makeDirectorMethodWrapper(Node *n, Wrapper *w, String *callback_name) {
    ...
    String *callback_wname = alaqil_name_wrapper(callback_name);
	...
	String *rname = NewString(alaqil_cresult_name());
	...
	Delete(rname);
      ...
      Printv(w->code, "  } alaqilSTRUCTPACKED alaqil_a;\n", NULL);
      Printv(w->code, "  alaqil_a.go_val = go_val;\n", NULL);
	...
	if (!tm) {
	  alaqil_warning(WARN_TYPEMAP_DIRECTORIN_UNDEF, input_file,
		       line_number, "Unable to use type %s as director method argument\n", alaqilType_str(Getattr(p, "type"), 0));
	} else {
	  tm = Copy(tm);
	  String *ln = Getattr(p, "lname");
	  String *input = NewString("");
	  Printv(input, "alaqil_a.", ln, NULL);
	...
	p = Getattr(p, "tmap:directorin:next");
      ...
      Printv(w->code, "  crosscall2(", callback_wname, ", &alaqil_a, (int) sizeof alaqil_a);\n", NULL);
	...
	String *tm = alaqil_typemap_lookup("directorout", n, result_str, NULL);
	if (!tm) {
	  alaqil_warning(WARN_TYPEMAP_DIRECTOROUT_UNDEF, input_file, line_number,
		       "Unable to use type %s as director method result\n", alaqilType_str(result, 0));
	} else {
	  static const String *alaqil_a_result = NewStringf("alaqil_a.%s", alaqil_cresult_name());
	  Replaceall(tm, "$input", alaqil_a_result);
	...
	Delete(result_str);
      ...
      if (alaqilType_type(result) != T_VOID) {
	String *r = NewString(alaqil_cresult_name());
	...
	p = Getattr(p, "tmap:directorin:next");
      ...
      Printv(w->code, "  ", NULL);
      if (alaqilType_type(result) != T_VOID) {
	Printv(w->code, alaqil_cresult_name(), " = ", NULL);
      }
  ...
  int classDirectorEnd(Node *n) {
    ...
    Printv(f_c_directors_h, "  alaqil_memory *alaqil_mem;\n", NULL);
    Printv(f_c_directors_h, "};\n\n", NULL);
  ...
  String *buildThrow(Node *n) {
    ...
    if (throw_parm_list) {
      alaqil_typemap_attach_parms("throws", throw_parm_list, NULL);
    }
    bool first = true;
	...
	String *s = alaqilType_str(Getattr(p, "type"), 0);
	Printv(ret, s, NULL);
	Delete(s);
      }
    }
  ...
  int makeDispatchFunction(Node *n, String *go_name, String *receiver, bool is_static, alaqilType *director_struct, bool is_upcall) {
    ...
    List *dispatch = alaqil_overload_rank(n, false);
    int nfunc = Len(dispatch);

    alaqilType *all_result;
    bool mismatch;
    if (is_constructor) {
      assert(!is_upcall);
      if (!is_director) {
	...
	alaqilType *result = Getattr(ni, "go:type");
	assert(result);
    ...
    } else if (all_result && alaqilType_type(all_result) != T_VOID) {
      if (is_director && is_constructor) {
	Printv(f_go_wrappers, " ", receiver, NULL);
	if (add_to_interface) {
	...
	while (pj) {
	  ...
	  alaqilType *tm = goOverloadType(pj, Getattr(pj, "type"));
	  ...
	  for (int k = 0; k < Len(coll) && !emitcheck; ++k) {
		...
		break;
	      ...
	      if (l == j) {
		alaqilType *tml = goOverloadType(pl, Getattr(pl, "type"));
		if (Cmp(tm, tml) != 0) {
		  emitcheck = true;
		}
		Delete(tml);
	...
	result = NULL;
      ...
      if (result && alaqilType_type(result) != T_VOID && (!all_result || alaqilType_type(all_result) != T_VOID)) {
	...
	alaqilType *tm = goWrapperType(pi, Getattr(pi, "type"), true);
	...
	alaqilType *tm = goType(p, Getattr(p, "type"));
	...
	if (!all_result || alaqilType_type(all_result) != T_VOID) {
	  Printv(end, " 0", NULL);
	}
      }
  ...
  bool checkFunctionVisibility(Node *n, Node *parent) {
    ...
    if (dirprot_mode() && alaqil_directorclass(parent))
      return true;
  ...
  String *exportedName(String *name) {
    ...
    String *ret = alaqil_name_mangle(copy);
    Delete(copy);
    return ret;
  }
   ...
   * simply returns a copy of the name.  This undoes alaqil's way of
  ...
  String *buildGoName(String *name, bool is_static, bool is_friend) {
    ...
    Delete(c2);
    Delete(c3);
    String *ret = alaqil_name_mangle(nw);
    Delete(nw);
    return ret;
  }
  ...
  String *buildGoWrapperName(String *name, String *overname) {
    String *s1 = NewString("_alaqil_wrap_");
    Append(s1, name);
    String *s2 = alaqil_name_mangle(s1);
    Delete(s1);
    if (overname) {
      Append(s2, overname);
    }
  ...
  bool checkNameConflict(String* name, Node* n, const_String_or_char_ptr scope) {
	...
	n2 = Getattr(lk, "name");
      ...
      alaqil_warning(WARN_GO_NAME_CONFLICT, input_file, line_number,
  ...
  bool checkIgnoredType(Node *n, String *go_name, alaqilType *type) {
    if (hasGoTypemap(n, type)) {
      return true;
    }

    alaqilType *t = alaqilType_typedef_resolve_all(type);
    ...
    } else if (alaqilType_ispointer(t) || alaqilType_isarray(t) || alaqilType_isqualifier(t) || alaqilType_isreference(t)) {
      alaqilType *r = Copy(t);
      if (alaqilType_ispointer(r)) {
	alaqilType_del_pointer(r);
      } else if (alaqilType_isarray(r)) {
	alaqilType_del_array(r);
      } else if (alaqilType_isqualifier(r)) {
	alaqilType_del_qualifier(r);
      } else {
	alaqilType_del_reference(r);
      }
    ...
    if (is_conflict) {
      String *s = alaqilType_str(t, NULL);
      alaqil_warning(WARN_GO_NAME_CONFLICT, input_file, line_number,
   ...
   * goType()
   *
   * Given a alaqil type, return a string for the type in Go.
   * ---------------------------------------------------------------------- */

  String *goType(Node *n, alaqilType *type) {
   ...
   * goImType()
   *
   * Given a alaqil type, return a string for the intermediate Go type
   * to pass to C/C++.  This is like goType except that it looks for
   * an imtype typemap entry first.
   * ---------------------------------------------------------------------- */

  String *goImType(Node *n, alaqilType *type) {
  ...
  String *goTypeWithInfo(Node *n, alaqilType *type, bool use_imtype, bool *p_is_interface) {
    if (p_is_interface) {
	...
	if (!ret) {
	  ret = alaqil_typemap_lookup("imtype", n, "", NULL);
	}
      } else {
	Parm *p = NewParm(type, "goImType", n);
	ret = alaqil_typemap_lookup("imtype", p, "", NULL);
	Delete(p);
      }
    }
    if (!ret) {
	...
	if (!ret) {
	  ret = alaqil_typemap_lookup("gotype", n, "", NULL);
	}
      } else {
	Parm *p = NewParm(type, "goType", n);
	ret = alaqil_typemap_lookup("gotype", p, "", NULL);
	Delete(p);
      }
    }
    ...
    } else if (alaqilType_isfunctionpointer(t) || alaqilType_isfunction(t)) {
      ret = NewString("_alaqil_fnptr");
    } else if (alaqilType_ismemberpointer(t)) {
      ret = NewString("_alaqil_memberptr");
    } else if (alaqilType_issimple(t)) {
      Node *cn = classLookup(t);
      if (cn) {
	...
	// alaqil does not know about this type.
	ret = exportedName(t);
	Setattr(undefined_types, t, t);
      }
      if (p_is_interface) {
	*p_is_interface = true;
    ...
    } else if (alaqilType_ispointer(t) || alaqilType_isarray(t)) {
      alaqilType *r = Copy(t);
      if (alaqilType_ispointer(r)) {
	alaqilType_del_pointer(r);
      } else {
	...
	if (is_interface) {
	  alaqilType *c = Copy(r);
	  if (alaqilType_isqualifier(c)) {
	    alaqilType_del_qualifier(c);
	    if (alaqilType_ispointer(c) || alaqilType_isarray(c)) {
	...
	if (is_interface) {
      ...
      Delete(r);
    } else if (alaqilType_isreference(t)) {
      alaqilType *r = Copy(t);
      alaqilType_del_reference(r);
      ...
      if (alaqilType_isqualifier(r)) {
	String *q = alaqilType_parm(r);
	if (Strcmp(q, "const") == 0) {
	  alaqilType *c = Copy(r);
	  alaqilType_del_qualifier(c);
	  if (alaqilType_ispointer(c)) {
	    add_pointer = false;
	  }
	  Delete(c);
	}
      ...
      if (add_pointer) {
	alaqilType_add_pointer(r);
      }
      ret = goTypeWithInfo(n, r, false, p_is_interface);
      Delete(r);
    } else if (alaqilType_isqualifier(t)) {
      alaqilType *r = Copy(t);
      alaqilType_del_qualifier(r);
      ret = goTypeWithInfo(n, r, false, p_is_interface);
      Delete(r);
    ...
    if (!ret) {
      alaqil_warning(WARN_LANG_NATIVE_UNIMPL, input_file, line_number, "No Go typemap defined for %s\n", alaqilType_str(type, 0));
      ret = NewString("uintptr");
    }
   ...
   * cgoTypeForGoValue()
   *
   * Given a alaqil type, return a string for the C type to use for the
  ...
  String *cgoTypeForGoValue(Node *n, alaqilType *type, bool *c_struct_type) {
    *c_struct_type = false;
    ...
    String *t = Copy(type);
    if (alaqilType_isarray(t)) {
      alaqilType_del_array(t);
    ...
    static int count;
    ++count;
    ct = NewStringf("alaqil_type_%d", count);
	...
	// We #define intgo to alaqil_intgo for the cgo comment.
	Replace(gct, "intgo", "alaqil_intgo", DOH_REPLACE_FIRST);
      }
      p = Strstr(gct, ct);
      if (p != NULL && p > (char*)Char(gct) && p[-1] == ' ' && p[Len(ct)] == '\0') {
  ...
  String *goWrapperType(Node *n, alaqilType *type, bool is_result) {
    bool is_interface;
	...
	alaqilType *ty = alaqilType_typedef_resolve_all(type);
	while (true) {
	  if (alaqilType_ispointer(ty)) {
	    alaqilType_del_pointer(ty);
	  } else if (alaqilType_isarray(ty)) {
	    alaqilType_del_array(ty);
	  } else if (alaqilType_isreference(ty)) {
	    alaqilType_del_reference(ty);
	  } else if (alaqilType_isqualifier(ty)) {
	    alaqilType_del_qualifier(ty);
	  } else {
	    break;
	  }
	}
	assert(alaqilType_issimple(ty));
   ...
   * And if that interface type represents a C++ type that alaqil does
  ...
  String *goOverloadType(Node *n, alaqilType *type) {
    alaqilType *ty = alaqilType_typedef_resolve_all(type);
    while (true) {
      if (alaqilType_ispointer(ty)) {
	alaqilType_del_pointer(ty);
  ...
  String *goCPointerType(alaqilType *type, bool add_to_hash) {
    alaqilType *ty = alaqilType_typedef_resolve_all(type);
	...
	ret = NewString("");
	Printv(ret, getModuleName(Getattr(cnmod, "name")), ".alaqilcptr", ex, NULL);
      }
    }
    Delete(ty);
    Delete(ex);
  ...
  String *gcCTypeForGoValue(Node *n, alaqilType *type, String *name) {
    bool is_interface;
    ...
    bool is_function = Strcmp(gt, "_alaqil_fnptr") == 0;
    bool is_member = Strcmp(gt, "_alaqil_memberptr") == 0;
    ...
    if (is_string) {
      ...
      alaqilType *t = alaqilType_typedef_resolve_all(type);
      if (alaqilType_ispointer(t)) {
	alaqilType_del_pointer(t);
      }
      if (alaqilType_isreference(t)) {
	alaqilType_del_reference(t);
      }
      alaqilType_add_pointer(t);
      ret = alaqilType_lstr(t, name);
      Delete(t);
      Delete(tail);
      return ret;
    } else {
      alaqilType *t = alaqilType_typedef_resolve_all(type);
      if (!has_typemap && alaqilType_isreference(t)) {
	...
	if (alaqilType_isqualifier(t)) {
	  ...
	  if (Strcmp(q, "const") == 0) {
	    alaqilType_del_qualifier(t);
	    if (hasGoTypemap(n, t) || alaqilType_ispointer(t)) {
	      if (is_int) {
		ret = NewString("intgo ");
		Append(ret, name);
	      } else if (is_int64) {
	...
	alaqilType *tstripped = alaqilType_strip_qualifiers(t);
	if (alaqilType_isenum(tstripped))
	  is_int = true;
	Delete(tstripped);
      }
	...
	Delete(tail);
	return alaqilType_lstr(type, name);
      }
    }

    Append(ret, tail);
    if (!has_typemap && alaqilType_isreference(type)) {
      Append(ret, "* ");
    }
    Append(ret, name);
    Delete(tail);
  ...
  String *gccgoCTypeForGoValue(Node *n, alaqilType *type, String *name) {
  ...
  bool goTypeIsInterface(Node *n, alaqilType *type) {
    bool is_interface;
  ...
  bool hasGoTypemap(Node *n, alaqilType *type) {
    Parm *p = NewParm(type, "test", n);
    alaqilType *tm = alaqil_typemap_lookup("gotype", p, "", NULL);
    Delete(p);
  ...
  bool isStatic(Node *n) {
    ...
    return (storage && (alaqil_storage_isstatic(n) || Strcmp(storage, "friend") == 0) && (!SmartPointer || !Getattr(n, "allocate:smartpointeraccess")));
  }
  ...
  String *goTypemapLookup(const char *name, Node *node, const char *lname) {
    String *ret = alaqil_typemap_lookup(name, node, lname, NULL);
    if (ret != NULL && Len(ret) == 0) {
      ret = NULL;
    }
    return ret;
...
/* -----------------------------------------------------------------------------
 * alaqil_go()    - Instantiate module
 * ----------------------------------------------------------------------------- */

static Language *new_alaqil_go() {
  return new GO();
}
extern "C" Language *alaqil_go(void) {
  return new_alaqil_go();
}
