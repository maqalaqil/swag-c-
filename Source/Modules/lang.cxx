...
extern "C" {
  int alaqil_director_mode() {
    return director_mode;
  }
  int alaqil_director_protected_mode() {
    return director_protected_mode;
  }
  int alaqil_all_protected_mode() {
...
int Dispatcher::emit_one(Node *n) {
  ...
  if (!tag) {
    /* Printf(stderr,"alaqil: Fatal internal error. Malformed parse tree
       node!\n"); */
    return alaqil_OK;
  }
   ...
   * =============================================================== */
    ...
    alaqil_error(input_file, line_number, "Unrecognized parse tree node type '%s'\n", tag);
    ret = alaqil_ERROR;
  }
  if (wrn)
    alaqil_warnfilter(wrn, 0);
  return ret;
}
...
directors(0) {
  ...
  /* Default director constructor code, passed to alaqil_ConstructorToFunction */
...
Language::~Language() {
  ...
  String *Language::directorClassName(Node *n) {
    ...
    if (Len(nspace) > 0)
      dirclassname = NewStringf("alaqilDirector_%s_%s", nspace, classname);
    else
      dirclassname = NewStringf("alaqilDirector_%s", classname);
...
int Language::emit_one(Node *n) {
  ...
  /*
     ...
     if (symtab) {
     symtab = alaqil_symbol_setscope(symtab);
     }
   */
 ...
 * alaqil_pragma()
 *
 * Handle alaqil pragma directives.  
 * -------------------------------------------------------------------------- */

void alaqil_pragma(char *lang, char *name, char *value) {
  if (strcmp(lang, "alaqil") == 0) {
    if ((strcmp(name, "make_default") == 0) || ((strcmp(name, "makedefault") == 0))) {
      ...
      if (!s) {
	alaqil_error(input_file, line_number, "Bad value for attributefunction. Expected \"fmtget:fmtset\".\n");
      } else {
	*s = 0;
...
int Language::use_naturalvar_mode(Node *n) const {
  ...
  if (!explicitly_off && !nvar) {
    ...
    alaqilType *fullty = alaqilType_typedef_resolve_all(ty);
    if (alaqilType_isclass(fullty)) {
      alaqilType *tys = alaqilType_strip_qualifiers(fullty);
      if (!CPlusPlus) {
	Replaceall(tys, "struct ", "");
	Replaceall(tys, "union ", "");
	Replaceall(tys, "class ", "");
      }
      Node *typenode = alaqil_symbol_clookup(tys, 0);
      if (typenode) {
...
int Language::applyDirective(Node *n) {
  ...
  while (c) {
    ...
    if (ParmList_len(pattern) != ParmList_len(apattern)) {
      alaqil_error(input_file, line_number, "Can't apply (%s) to (%s).  Number of arguments don't match.\n", ParmList_str(pattern), ParmList_str(apattern));
    } else {
      if (!alaqil_typemap_apply(pattern, apattern)) {
	alaqil_warning(WARN_TYPEMAP_APPLY_UNDEF, input_file, line_number, "Can't apply (%s). No typemaps are defined.\n", ParmList_str(pattern));
      }
    }
    c = nextSibling(c);
  }
...
int Language::clearDirective(Node *n) {
  ...
  for (p = firstChild(n); p; p = nextSibling(p)) {
    ...
    alaqil_typemap_clear_apply(pattern);
  }
  return alaqil_OK;
}
...
int Language::constantDirective(Node *n) {
  ...
  if (!ImportMode) {
    alaqil_require("constantDirective", n, "name", "?value", NIL);
...
int Language::insertDirective(Node *n) {
  ...
  if ((!ImportMode) || Getattr(n, "generated")) {
    ...
    File *f = 0;
    if (!section) {		/* %{ ... %} */
      f = alaqil_filebyname("header");
    } else {
      f = alaqil_filebyname(section);
    }
    if (f) {
      Printf(f, "%s\n", code);
    } else {
      alaqil_error(input_file, line_number, "Unknown target '%s' for %%insert directive.\n", section);
    }
    return alaqil_OK;
  } else {
    return alaqil_NOWRAP;
...
int Language::pragmaDirective(Node *n) {
  ...
  if (!ImportMode) {
    ...
    String *name = Getattr(n, "name");
    String *value = Getattr(n, "value");
    alaqil_pragma(Char(lan), Char(name), Char(value));
...
int Language::typemapDirective(Node *n) {
  ...
  if (code && (Strstr(code, "$source") || (Strstr(code, "$target")))) {
    alaqil_warning(WARN_TYPEMAP_SOURCETARGET, Getfile(n), Getline(n), "Deprecated typemap feature ($source/$target).\n");
    if (!namewarn) {
      alaqil_warning(WARN_TYPEMAP_SOURCETARGET, Getfile(n), Getline(n), "The use of $source and $target in a typemap declaration is deprecated.\n\
...
Doc/Manual/Typemaps.html for complete details.\n");
  ...
  if (Strcmp(method, "except") == 0) {
    alaqil_warning(WARN_DEPRECATED_EXCEPT_TM, Getfile(n), Getline(n), "%%typemap(except) is deprecated. Use the %%exception directive.\n");
  }
  ...
  if (Strcmp(method, "in") == 0) {
	...
	if (!multiinput && (GetInt(k, "value") > 1)) {
	  alaqil_error(Getfile(n), Getline(n), "Multiple-input typemaps (numinputs > 1) not supported by this target language module.\n");
	  return alaqil_ERROR;
	}
	break;
      }
  ...
  if (Strcmp(method, "ignore") == 0) {
    alaqil_warning(WARN_DEPRECATED_IGNORE_TM, Getfile(n), Getline(n), "%%typemap(ignore) has been replaced by %%typemap(in,numinputs=0).\n");
  ...
  if (code) {
    ...
    alaqil_cparse_replace_descriptor(code);
  }
  ...
  while (items) {
    ...
    if (code) {
      alaqil_typemap_register(method, pattern, code, parms, kwargs);
    } else {
      alaqil_typemap_clear(method, pattern);
    }
...
int Language::typemapcopyDirective(Node *n) {
  ...
  while (items) {
    ...
    if (nsrc != ParmList_len(npattern)) {
      alaqil_error(input_file, line_number, "Can't copy typemap. Number of types differ.\n");
    } else {
      if (alaqil_typemap_copy(method, pattern, npattern) < 0) {
	alaqil_error(input_file, line_number, "Can't copy typemap (%s) %s = %s\n", method, ParmList_str(pattern), ParmList_str(npattern));
      }
    }
    items = nextSibling(items);
  }
...
int Language::cDeclaration(Node *n) {
  ...
  if (Getattr(n, "feature:onlychildren")) {
	...
	alaqil_warning(WARN_LANG_TEMPLATE_METHOD_IGNORE, input_file, line_number,
  ...
  if (Cmp(storage, "typedef") == 0) {
    alaqil_save("cDeclaration", n, "type", NIL);
    alaqilType *t = Copy(type);
    if (t) {
      alaqilType_push(t, decl);
      Setattr(n, "type", t);
      typedefHandler(n);
    }
    alaqil_restore(n);
  ...
  over = alaqil_symbol_isoverloaded(n);
  if (!overloading) {
    if (over)
      over = first_nontemplate(over);
    if (over && (over != n)) {
      alaqil_warning(WARN_LANG_OVERLOAD_DECL, input_file, line_number, "Overloaded declaration ignored.  %s\n", alaqil_name_decl(n));
      alaqil_warning(WARN_LANG_OVERLOAD_DECL, Getfile(over), Getline(over), "Previous declaration is %s\n", alaqil_name_decl(over));
      return alaqil_NOWRAP;
    }
  }

  if (!validIdentifier(symname)) {
    alaqil_warning(WARN_LANG_IDENTIFIER, input_file, line_number, "Can't wrap '%s' unless renamed to a valid identifier.\n", alaqilType_namestr(symname));
    return alaqil_NOWRAP;
  }

  ty = NewString(type);
  alaqilType_push(ty, decl);
  fullty = alaqilType_typedef_resolve_all(ty);
  if (alaqilType_isfunction(fullty)) {
    if (!alaqilType_isfunction(ty)) {
      Delete(ty);
      ty = fullty;
      fullty = 0;
      ParmList *parms = alaqilType_function_parms(ty, n);
      Setattr(n, "parms", parms);
    }
    /* Transform the node into a 'function' node and emit */
    if (!CurrentClass) {
      f_header = alaqil_filebyname("header");

      if (AddExtern) {
	if (f_header) {
	  if (alaqil_storage_isextern(n) || (ForceExtern && !storage)) {
	    /* we don't need the 'extern' part in the C/C++ declaration,
	       and it produces some problems when namespace and SUN
	       Studio is used.

	       Printf(f_header,"extern %s", alaqilType_str(ty,name));
	       ...
	       calling conventions - alaqil doesn't understand Windows (non standard) calling
	       conventions in the first place, so can't regenerate them.
	     */
	    String *str = alaqilType_str(ty, name);
	    ...
	    Printf(f_header, ";\n");
	  } else if (alaqil_storage_isexternc(n)) {
    ...
    if (alaqilType_isqualifier(ty)) {
      alaqilType *qual = alaqilType_pop(ty);
      Setattr(n, "qualifier", qual);
      Delete(qual);
    }
    Delete(alaqilType_pop_function(ty));
    DohIncref(type);
    Setattr(n, "type", ty);
    ...
    Delattr(n, "decl");
    if (!CurrentClass) {
      if (alaqil_storage_isextern(n) || ForceExtern) {
	if (AddExtern) {
	  f_header = alaqil_filebyname("header");
	  if (f_header) {
	    String *str = alaqilType_str(ty, name);
    ...
    if (!alaqilType_ismutable(ty)) {
      SetFlag(n, "feature:immutable");
    }
    /* If an array and elements are const, then read-only */
    if (alaqilType_isarray(ty)) {
      alaqilType *tya = alaqilType_array_type(ty);
      if (alaqilType_isconst(tya)) {
	SetFlag(n, "feature:immutable");
      }
      Delete(tya);
    }
...
int Language::functionHandler(Node *n) {
  ...
  int isstatic = CurrentClass && alaqil_storage_isstatic(n) && !(SmartPointer && Getattr(n, "allocate:smartpointeraccess"));
  ...
  if (!CurrentClass) {
	...
	if (virtual_but_not_pure_virtual) {
	  ...
	  Printv(new_symname, "alaqilExplicit", suffix, NIL);
...
int Language::globalfunctionHandler(Node *n) {
  ...
  alaqil_require("globalfunctionHandler", n, "name", "sym:name", "type", "?parms", NIL);
  ...
  String *call = alaqil_cfunction_call(extendname ? extendname : name, parms);
...
int Language::callbackfunctionHandler(Node *n) {
  alaqil_require("callbackfunctionHandler", n, "name", "*sym:name", "*type", "?value", NIL);
  String *type = Getattr(n, "type");
  String *name = Getattr(n, "name");
  String *parms = Getattr(n, "parms");
  String *cbname = Getattr(n, "feature:callback:name");
  String *calltype = NewStringf("(%s (*)(%s))(%s)", alaqilType_str(type, 0), ParmList_str(parms), alaqilType_namestr(name));
  alaqilType *cbty = Copy(type);
  alaqilType_add_function(cbty, parms);
  alaqilType_add_pointer(cbty);
...
int Language::memberfunctionHandler(Node *n) {
  ...
  alaqil_require("memberfunctionHandler", n, "*name", "*sym:name", "*type", "?parms", "?value", NIL);
  ...
  if (cb) {
    ...
    alaqilType *cbty = Copy(type);
    alaqilType_add_function(cbty, parms);
    alaqilType_add_memberpointer(cbty, ClassName);
    ...
    Setattr(n, "feature:callback:name", alaqil_name_member(NSpace, ClassPrefix, cbname));
  ...
  String *fname = alaqil_name_member(NSpace, ClassPrefix, symname);
  ...
  if (!is_public(n) && (is_member_director(CurrentClass, n) || GetFlag(n, "explicitcall") || isNonVirtualProtectedAccess(n))) {
    ...
    if (qualifier)
      alaqilType_push(director_type, qualifier);
    alaqilType_add_pointer(director_type);
  }
  ...
  alaqil_MethodToFunction(n, NSpace, ClassType, flags, director_type, is_member_director(CurrentClass, n));
...
int Language::staticmemberfunctionHandler(Node *n) {
  ...
  alaqil_require("staticmemberfunctionHandler", n, "*name", "*sym:name", "*type", NIL);
  alaqil_save("staticmemberfunctionHandler", n, "storage", NIL);
  String *name = Getattr(n, "name");
  String *symname = Getattr(n, "sym:name");
  alaqilType *type = Getattr(n, "type");
  ParmList *parms = Getattr(n, "parms");
  ...
  if (!Extend) {
    ...
    String *mname = alaqil_name_mangle(ClassName);
    cname = alaqil_name_member(NSpace, mname, name);
    Delete(mname);
  }
  mrename = alaqil_name_member(NSpace, ClassPrefix, symname);

  if (Extend) {
    String *code = Getattr(n, "code");
    String *defaultargs = Getattr(n, "defaultargs");
    String *mangled = alaqil_name_mangle(mrename);
    Delete(mrename);
    mrename = mangled;
    ...
    if (!defaultargs && code) {
      ...
      String *mangled_cname = alaqil_name_mangle(cname);
      alaqil_add_extension_code(n, mangled_cname, parms, type, code, CPlusPlus, 0);
...
int Language::variableHandler(Node *n) {
  ...
  if (!CurrentClass) {
    ...
    alaqil_save("variableHandler", n, "feature:immutable", NIL);
    if (SmartPointer) {
    ...
    if (alaqil_storage_isstatic(n) && !(SmartPointer && Getattr(n, "allocate:smartpointeraccess"))) {
...
int Language::membervariableHandler(Node *n) {
  ...
  alaqil_require("membervariableHandler", n, "*name", "*sym:name", "*type", NIL);
  alaqil_save("membervariableHandler", n, "parms", NIL);
  ...
  if (!AttributeFunctionGet) {
    String *mname = alaqil_name_member(0, ClassPrefix, symname);
    String *mrename_get = alaqil_name_get(NSpace, mname);
    String *mrename_set = alaqil_name_set(NSpace, mname);
    Delete(mname);
	...
	if (SmartPointer) {
	  ...
	  String *pname = isNonVirtualProtectedAccess(n) ? NewString("darg") : alaqil_cparm_name(0, 0);
	  target = NewStringf("%s->%s", pname, name);
	  Delete(pname);
	}
	...
	tm = alaqil_typemap_lookup("memberin", nin, target, 0);
	Delete(nin);
      }
      ...
      alaqil_MembersetToFunction(n, ClassType, flags);
    ...
    /* Emit get function */
    {
      ...
      if (isNonVirtualProtectedAccess(n))
        flags = flags | CWRAP_ALL_PROTECTED_ACCESS;
      alaqil_MembergetToFunction(n, ClassType, flags);
    ...
    if (!Extend) {
      ActionFunc = Copy(alaqil_cmemberget_call(name, type));
      cpp_member_func(Char(gname), Char(gname), type, 0);
      Delete(ActionFunc);
    } else {
      String *cname = alaqil_name_get(NSpace, name);
    ...
    if (!GetFlag(n, "feature:immutable")) {
      ...
      vty = NewString("void");
      if (!Extend) {
	ActionFunc = Copy(alaqil_cmemberset_call(name, type));
	cpp_member_func(Char(gname), Char(gname), vty, p);
	Delete(ActionFunc);
      } else {
	String *cname = alaqil_name_set(NSpace, name);
...
int Language::staticmembervariableHandler(Node *n) {
  alaqil_require("staticmembervariableHandler", n, "*name", "*sym:name", "*type", "?value", NIL);
  String *value = Getattr(n, "value");
  ...
  if (!value || !Getattr(n, "hasconsttype")) {
    ...
    mrename = alaqil_name_member(0, ClassPrefix, symname);
    ...
    if (Extend) {
      ...
      String *value = alaqilType_namestr(cname);
      Setattr(n, "value", value);
    }

    alaqilType *t1 = alaqilType_typedef_resolve_all(Getattr(n, "type"));
    alaqilType *t2 = alaqilType_strip_qualifiers(t1);
    Setattr(n, "type", t2);
    Delete(t1);
    Delete(t2);
...
int Language::enumvalueDeclaration(Node *n) {
  ...
  alaqil_require("enumvalueDeclaration", n, "*name", "*sym:name", "?value", NIL);
  String *value = Getattr(n, "value");
  String *name = Getattr(n, "name");
  String *tmpValue;
  ...
  if (GetFlag(parent, "scopedenum")) {
    String *symname = alaqil_name_member(0, Getattr(parent, "sym:name"), Getattr(n, "sym:name"));
    Setattr(n, "sym:name", symname);
    Delete(symname);
  }
...
int Language::memberconstantHandler(Node *n) {
  ...
  alaqil_require("memberconstantHandler", n, "*name", "*sym:name", "value", NIL);
  ...
  String *mrename = alaqil_name_member(0, EnumClassPrefix, enumvalue_symname ? enumvalue_symname : symname);
  Setattr(n, "sym:name", mrename);
...
int Language::typedefHandler(Node *n) {
  ...
  alaqilType *decl = Getattr(n, "decl");
  if (!alaqilType_ispointer(decl) && !alaqilType_isreference(decl)) {
    alaqilType *pname = Copy(name);
    alaqilType_add_pointer(pname);
    alaqilType_remember(pname);
    Delete(pname);
  }
  return alaqil_OK;
}
...
static String *vtable_method_id(Node *n) {
  ...
  String *local_decl = alaqilType_typedef_resolve_all(decl);
  String *tmp = alaqilType_pop_function(local_decl);
  Delete(local_decl);
  local_decl = tmp;
...
int Language::unrollVirtualMethods(Node *n, Node *parent, List *vm, int default_director, int &virtual_destructor, int protectedbase) {
  ...
  for (ni = Getattr(n, "firstChild"); ni; ni = nextSibling(ni)) {
    ...
    if ((Cmp(nodeType, "cdecl") == 0) || is_destructor) {
      ...
      if (alaqilType_isfunction(decl) && (((!protectedbase || dirprot_mode()) && is_public(nn)) || need_nonpublic_member(nn))) {
	...
	alaqilType *ty = NewString(Getattr(m, "type"));
	alaqilType_push(ty, decl);
	if (alaqilType_isqualifier(ty)) {
	  Delete(alaqilType_pop(ty));
	}
	Delete(alaqilType_pop_function(ty));
	...
	alaqil_features_get(alaqil_cparse_features(), 0, mname, Getattr(m, "decl"), m);
...
int Language::classDirectorDisown(Node *n) {
  ...
  String *mrename;
  String *symname = Getattr(n, "sym:name");
  mrename = alaqil_name_disown(NSpace, symname);
  ...
  Printv(action, "{\n", "alaqil::Director *director = alaqil_DIRECTOR_CAST(arg1);\n", "if (director) director->alaqil_disown();\n", "}\n", NULL);
...
int Language::classDirectorConstructors(Node *n) {
  ...
  if (!constructor) {
    if (!default_ctor) {
      ...
      alaqil_warning(WARN_LANG_DIRECTOR_ABSTRACT, Getfile(n), Getline(n), "Director class '%s' can't be constructed\n", alaqilType_namestr(Getattr(n, "name")));
      return alaqil_OK;
    }
  ...
  /* this is just to support old java behavior, ie, the default
     ...
     (mmatus) This is very strange, since alaqil compiled with gcc3.2.3
...
int Language::classDirectorMethods(Node *n) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (!Cmp(type, "destructor")) {
      ...
      alaqil_require("classDirectorMethods", method, "*type", NIL);
      assert(Getattr(method, "returntype"));
      Setattr(method, "type", Getattr(method, "returntype"));
      if (classDirectorMethod(method, n, fqdname) == alaqil_OK)
...
int Language::classDirectorDestructor(Node *n) {
   ...
   */
  File *f_directors = alaqil_filebyname("director");
  File *f_directors_h = alaqil_filebyname("director_h");
  if (Getattr(n, "throw")) {
...
int Language::classDirector(Node *n) {
  ...
  for (ni = Getattr(n, "firstChild"); ni; ni = nextSibling(ni)) {
    ...
    if (Cmp(nodeType, "destructor") == 0 && GetFlag(ni, "final")) {
      String *classtype = Getattr(n, "classtype");
      alaqil_WARN_NODE_BEGIN(ni);
      alaqil_warning(WARN_LANG_DIRECTOR_FINAL, input_file, line_number, "Destructor %s is final, %s cannot be a director class.\n", alaqil_name_decl(ni), classtype);
      alaqil_WARN_NODE_END(ni);
  ...
  if (virtual_destructor || Len(vtable) > 0) {
    if (!virtual_destructor) {
      String *classtype = Getattr(n, "classtype");
      alaqil_warning(WARN_LANG_DIRECTOR_VDESTRUCT, input_file, line_number, "Director base class %s has no virtual destructor.\n", classtype);
    }
...
static void addCopyConstructor(Node *n) {
  ...
  alaqilType *type = Copy(cname);
  String *name = alaqil_scopename_last(cname);
  ...
  if (Getattr(n, "allocate:has_constructor")) {
	...
	String *clast = alaqil_scopename_last(cname);
  ...
  String *symname = alaqil_name_make(cn, cname, name, decl, oldname);
  if (Strcmp(symname, "$ignore") != 0) {
    ...
    Symtab *oldscope = alaqil_symbol_setscope(Getattr(n, "symtab"));
    Node *on = alaqil_symbol_add(symname, cn);
    alaqil_features_get(alaqil_cparse_features(), alaqil_symbol_qualifiedscopename(0), name, decl, cn);
    alaqil_symbol_setscope(oldscope);
...
static void addDestructor(Node *n) {
  ...
  Insert(name, 0, "~");
  String *decl = NewString("f().");
  String *symname = alaqil_name_make(cn, cname, name, decl, 0);
  if (Strcmp(symname, "$ignore") != 0) {
    ...
    Symtab *oldscope = alaqil_symbol_setscope(Getattr(n, "symtab"));
    Node *nonstandard_destructor = Equal(possible_nonstandard_symname, symname) ? 0 : alaqil_symbol_clookup(possible_nonstandard_symname, 0);
    Node *on = alaqil_symbol_add(symname, cn);
    alaqil_features_get(alaqil_cparse_features(), alaqil_symbol_qualifiedscopename(0), name, decl, cn);
    alaqil_symbol_setscope(oldscope);

    if (on == cn) {
      // alaqil accepts a non-standard named destructor in %extend that uses a typedef for the destructor name
...
int Language::classDeclaration(Node *n) {
  ...
  if (cplus_mode != PUBLIC)
    return alaqil_NOWRAP;
  if (!name) {
    alaqil_warning(WARN_LANG_CLASS_UNNAMED, input_file, line_number, "Can't generate wrappers for unnamed struct/class.\n");
    return alaqil_NOWRAP;
  }

  /* Check symbol name for template.   If not renamed. Issue a warning */
  if (!validIdentifier(symname)) {
    alaqil_warning(WARN_LANG_IDENTIFIER, input_file, line_number, "Can't wrap class %s unless renamed to a valid identifier.\n", alaqilType_namestr(symname));
    return alaqil_NOWRAP;
  }
  ...
  Setattr(n, "classtype", alaqilType_namestr(ClassType));
...
int Language::classHandler(Node *n) {
  ...
  if (Getattr(n, "template"))
    Extend = 0;
  bool hasDirector = alaqil_directorclass(n) ? true : false;
  ...
  if (hasDirector) {
	...
	alaqilType *type = Getattr(method, "nodeType");
	...
	if (!symbolLookup(wrapname, "") && (!is_public(method))) {
	   ...
	   * If the method is overloaded, then alaqil_overload_dispatch() incorrectly generates a call to the base wrapper, _wrap_xxx method
...
int Language::constructorDeclaration(Node *n) {
  ...
  Node *over = alaqil_symbol_isoverloaded(n);
  if (over && !Getattr(CurrentClass, "sym:cleanconstructor")) {
    int dirclass = alaqil_directorclass(CurrentClass);
    Node *nn = over;
    while (nn) {
      if (!is_public(nn)) {
  ...
  if ((cplus_mode != PUBLIC)) {
    ...
    if (!alaqil_directorclass(CurrentClass) || !need_nonpublic_ctor(n))
      return alaqil_NOWRAP;
  }

  /* Name adjustment for %name */
  alaqil_save("constructorDeclaration", n, "sym:name", NIL);
  ...
  if (!Abstract) {
	...
	if (over != n) {
	  alaqil_warning(WARN_LANG_OVERLOAD_CONSTRUCT, input_file, line_number,
		       "Overloaded constructor ignored.  %s\n", alaqil_name_decl(n));
	  alaqil_warning(WARN_LANG_OVERLOAD_CONSTRUCT, Getfile(over), Getline(over),
      ...
      String *scope = alaqil_scopename_check(ClassName) ? alaqil_scopename_prefix(ClassName) : 0;
      String *actual_name = scope ? NewStringf("%s::%s", scope, name) : NewString(name);
      Delete(scope);
      if (!Equal(actual_name, expected_name) && !alaqilType_istemplate(expected_name) && !alaqilType_istemplate(actual_name)) {
	...
	if (Extend) {
	  ...
	  alaqilType *name_resolved = alaqilType_typedef_resolve_all(actual_name);
	  alaqilType *expected_name_resolved = alaqilType_typedef_resolve_all(expected_name);
	  ...
	  if (!illegal_name)
	    alaqil_warning(WARN_LANG_EXTEND_CONSTRUCTOR, input_file, line_number, "Use of an illegal constructor name '%s' in %%extend is deprecated, the constructor name should be '%s'.\n", 
		alaqilType_str(alaqil_scopename_last(actual_name), 0), alaqilType_str(alaqil_scopename_last(expected_name), 0));
	  Delete(name_resolved);
	  Delete(expected_name_resolved);
	}
	if (illegal_name) {
	  alaqil_warning(WARN_LANG_RETURN_TYPE, input_file, line_number, "Function %s must have a return type. Ignored.\n", alaqil_name_decl(n));
	  alaqil_restore(n);
	  return alaqil_NOWRAP;
	}
      }
...
int Language::constructorHandler(Node *n) {
  alaqil_require("constructorHandler", n, "?name", "*sym:name", "?type", "?parms", NIL);
  String *symname = Getattr(n, "sym:name");
  String *mrename = alaqil_name_construct(NSpace, symname);
  String *nodeType = Getattr(n, "nodeType");
  ...
  alaqil_ConstructorToFunction(n, NSpace, ClassType, none_comparison, director_ctor, CPlusPlus, flags, DirectorClassName);
  Setattr(n, "sym:name", mrename);
  functionWrapper(n);
  Delete(mrename);
  alaqil_restore(n);
  if (abstracts)
    Setattr(alaqil_methodclass(n), "abstracts", abstracts);
  return alaqil_OK;
}
...
int Language::copyconstructorHandler(Node *n) {
  alaqil_require("copyconstructorHandler", n, "?name", "*sym:name", "?type", "?parms", NIL);
  String *symname = Getattr(n, "sym:name");
  String *mrename = alaqil_name_copyconstructor(NSpace, symname);
  List *abstracts = 0;
  String *director_ctor = get_director_ctor_code(n, director_ctor_code,
						 director_prot_ctor_code,
						 abstracts);
  alaqil_ConstructorToFunction(n, NSpace, ClassType, none_comparison, director_ctor, CPlusPlus, Getattr(n, "template") ? 0 : Extend, DirectorClassName);
  Setattr(n, "sym:name", mrename);
  functionWrapper(n);
  Delete(mrename);
  alaqil_restore(n);
...
int Language::destructorDeclaration(Node *n) {
  ...
  alaqil_save("destructorDeclaration", n, "name", "sym:name", NIL);
  ...
  if (!Equal(actual_name, expected_name) && !(Getattr(n, "template"))) {
	...
	if (Strncmp(name_resolved, "struct ", 7) == 0)
      ...
      illegal_name = !Equal(name_resolved, expected_name_resolved);
      if (!illegal_name)
	alaqil_warning(WARN_LANG_EXTEND_DESTRUCTOR, input_file, line_number, "Use of an illegal destructor name '%s' in %%extend is deprecated, the destructor name should be '%s'.\n", 
	    alaqilType_str(alaqil_scopename_last(actual_name), 0), alaqilType_str(alaqil_scopename_last(expected_name), 0));
      Delete(name_resolved);
      Delete(expected_name_resolved);
    }
    ...
    if (illegal_name) {
      alaqil_warning(WARN_LANG_ILLEGAL_DESTRUCTOR, input_file, line_number, "Illegal destructor name %s. Ignored.\n", alaqil_name_decl(n));
      alaqil_restore(n);
      return alaqil_NOWRAP;
    }
  }
...
int Language::destructorHandler(Node *n) {
  alaqil_require("destructorHandler", n, "?name", "*sym:name", NIL);
  alaqil_save("destructorHandler", n, "type", "parms", NIL);
  ...
  mrename = alaqil_name_destroy(NSpace, csymname);

  alaqil_DestructorToFunction(n, NSpace, ClassType, CPlusPlus, Extend);
  Setattr(n, "sym:name", mrename);
  functionWrapper(n);
  Delete(mrename);
  alaqil_restore(n);
...
int Language::constantWrapper(Node *n) {
  ...
  alaqilType *type = Getattr(n, "type");
  String *value = Getattr(n, "value");
  String *str = alaqilType_str(type, name);
...
int Language::variableWrapper(Node *n) {
  alaqil_require("variableWrapper", n, "*name", "*sym:name", "*type", "?parms", "?varset", "?varget", NIL);
  String *symname = Getattr(n, "sym:name");
  alaqilType *type = Getattr(n, "type");
  String *name = Getattr(n, "name");
  ...
  if (assignable) {
    int make_set_wrapper = 1;
    String *tm = alaqil_typemap_lookup("globalin", n, name, 0);

    alaqil_VarsetToFunction(n, flags);
    String *sname = alaqil_name_set(NSpace, symname);
  ...
  alaqil_VargetToFunction(n, flags);
  String *gname = alaqil_name_get(NSpace, symname);
...
int Language::functionWrapper(Node *n) {
  ...
  Printf(stdout, "functionWrapper   : %s\n", alaqilType_str(type, NewStringf("%s(%s)", name, ParmList_str_defaultargs(parms))));
...
int Language::addSymbol(const String *s, const Node *n, const_String_or_char_ptr scope) {
  ...
  if (!symbols) {
	...
	alaqil_error(input_file, line_number, "'%s' is multiply defined in the generated target language module in scope '%s'.\n", s, scope);
      else
	alaqil_error(input_file, line_number, "'%s' is multiply defined in the generated target language module.\n", s);
      alaqil_error(Getfile(c), Getline(c), "Previous declaration of '%s'\n", s);
      return 0;
    }
  }
  Setattr(symbols, s, n);
...
int Language::addInterfaceSymbol(const String *interface_name, Node *n, const_String_or_char_ptr scope) {
  if (interface_name) {
    ...
    if (existing_symbol) {
      ...
      alaqil_error(input_file, line_number, "The interface feature name '%s' for proxy class '%s' is already defined in the generated target language module in scope '%s'.\n",
	  interface_name, proxy_class_name, scope);
      alaqil_error(Getfile(existing_symbol), Getline(existing_symbol), "Previous declaration of '%s'\n", interface_name);
      return 0;
    }
...
Node *Language::classLookup(const alaqilType *s) {
  ...
  if (!n) {
    Symtab *stab = 0;
    alaqilType *ty1 = alaqilType_typedef_resolve_all(s);
    alaqilType *ty2 = alaqilType_strip_qualifiers(ty1);

    String *base = alaqilType_base(ty2);
    ...
    String *prefix = alaqilType_prefix(ty2);

    /* Do a symbol table search on the base type */
    while (!n) {
      Hash *nstab;
      n = alaqil_symbol_clookup(base, stab);
      if (!n)
	break;
...
Node *Language::enumLookup(alaqilType *s) {
  ...
  if (!n) {
    Symtab *stab = 0;
    alaqilType *lt = alaqilType_ltype(s);
    alaqilType *ty1 = alaqilType_typedef_resolve_all(lt);
    alaqilType *ty2 = alaqilType_strip_qualifiers(ty1);
...
String *Language::makeParameterName(Node *n, Parm *p, int arg_num, bool setter) const {
  ...
  if (!pn || count > 1) {
    ...
    arg = alaqil_name_make(p, 0, pn, 0, 0);
  }
...
int Language::abstractClassTest(Node *n) {
  ...
  if (vtable) {
	...
	alaqil_warning(WARN_LANG_DIRECTOR_ABSTRACT, Getfile(n), Getline(n),
		     "Director class '%s' is abstract, abstract method '%s' is not accessible, maybe due to multiple inheritance or 'nodirector' feature\n",
		     alaqilType_namestr(Getattr(n, "name")), Getattr(dirabstract, "name"));
      } else {
	alaqil_warning(WARN_LANG_DIRECTOR_ABSTRACT, Getfile(n), Getline(n),
...
int Language::is_assignable(Node *n) {
  ...
  alaqilType *type = Getattr(n, "type");
  Node *cn = 0;
  alaqilType *ftd = alaqilType_typedef_resolve_all(type);
  alaqilType *td = alaqilType_strip_qualifiers(ftd);
  if (alaqilType_type(td) == T_USER) {
    cn = alaqil_symbol_clookup(td, 0);
    if (cn) {
