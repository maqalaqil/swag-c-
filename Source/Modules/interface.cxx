...
static void collect_interface_base_classes(Node *n) {
  if (Getattr(n, "feature:interface")) {
	...
	if (!GetFlag(base.item, "feature:ignore")) {
	  if (!Getattr(base.item, "feature:interface")) {
	    alaqil_error(Getfile(n), Getline(n), "Base class '%s' of '%s' is not similarly marked as an interface.\n", alaqilType_namestr(Getattr(base.item, "name")), alaqilType_namestr(Getattr(n, "name")));
...
static void process_interface_name(Node *n) {
  if (Getattr(n, "feature:interface")) {
    ...
    if (!Len(interface_name)) {
      alaqil_error(Getfile(n), Getline(n), "The interface feature for '%s' is missing the name attribute.\n", alaqilType_namestr(Getattr(n, "name")));
      alaqil_exit(EXIT_FAILURE);
    }
...
/* -----------------------------------------------------------------------------
 * alaqil_interface_propagate_methods()
 *
...
void alaqil_interface_propagate_methods(Node *n) {
  if (interface_feature_enabled) {
	...
	continue;
      ...
      String *this_decl_resolved = alaqilType_typedef_resolve_all(this_decl);
      bool identically_overloaded_method = false; // true when a base class' method is implemented in n
      if (alaqilType_isfunction(this_decl_resolved)) {
	...
	for (Node *child = firstChild(n); child; child = nextSibling(child)) {
	  ...
	  if (Cmp(nodeType(child), "cdecl") == 0) {
	    if (checkAttribute(child, "name", name)) {
	      String *decl = alaqilType_typedef_resolve_all(Getattr(child, "decl"));
