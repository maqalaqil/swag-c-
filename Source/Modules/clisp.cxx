 ...
 * clisp.cxx
 *
 * clisp language module for alaqil.
 * ----------------------------------------------------------------------------- */
...
public:
  ...
  List *entries;
private:
  String *get_ffi_type(Node *n, alaqilType *ty);
...
void CLISP::main(int argc, char *argv[]) {
  ...
  Preprocessor_define("alaqilCLISP 1", 0);
  alaqil_library_directory("clisp");
  alaqil_config_file("clisp.swg");
...
int CLISP::top(Node *n) {
  ...
  output_filename = NewStringf("%s%s.lisp", alaqil_output_directory(), module);
  ...
  alaqil_register_filebyname("wrapper", f_null);

  String *header = NewString("");

  alaqil_banner_target_lang(header, ";;");
...
int CLISP::functionWrapper(Node *n) {
  ...
  if (!extern_all_flag && (!storage || (!alaqil_storage_isextern(n) && !alaqil_storage_isexternc(n))))
    return alaqil_OK;
...
String *CLISP::get_ffi_type(Node *n, alaqilType *ty) {
  Node *node = NewHash();
  Setattr(node, "type", ty);
  Setfile(node, Getfile(n));
  Setline(node, Getline(n));
  const String *tm = alaqil_typemap_lookup("in", node, "", 0);
  Delete(node);

  if (tm) {
    return NewString(tm);
  } else if (alaqilType_ispointer(ty)) {
    alaqilType *cp = Copy(ty);
    alaqilType_del_pointer(cp);
    String *inner_type = get_ffi_type(n, cp);

    if (alaqilType_isfunction(cp)) {
      return inner_type;
    }

    alaqilType *base = alaqilType_base(ty);
    String *base_name = alaqilType_str(base, 0);
    ...
    alaqilType *cp = Copy(ty);
    String *array_dim = alaqilType_array_getdim(ty, 0);

    if (!Strcmp(array_dim, "")) {	//dimension less array convert to pointer
      Delete(array_dim);
      alaqilType_del_array(cp);
      alaqilType_add_pointer(cp);
      String *str = get_ffi_type(n, cp);
      Delete(cp);
      return str;
    } else {
      alaqilType_pop_arrays(cp);
      String *inner_type = get_ffi_type(n, cp);
      Delete(cp);

      int ndim = alaqilType_array_ndim(ty);
      String *dimension;
      if (ndim == 1) {
	dimension = array_dim;
      } else {
	...
	for (int i = 1; i < ndim; i++) {
	  array_dim = alaqilType_array_getdim(ty, i);
	  Append(dimension, " ");
  ...
  } else if (alaqilType_isfunction(ty)) {
    alaqilType *cp = Copy(ty);
    alaqilType *fn = alaqilType_pop_function(cp);
    String *args = NewString("");
    ParmList *pl = alaqilType_function_parms(fn, n);
    if (ParmList_len(pl) != 0) {
  ...
  String *str = alaqilType_str(ty, 0);
  if (str) {
...
extern "C" Language *alaqil_clisp(void) {
  return new CLISP();
}
