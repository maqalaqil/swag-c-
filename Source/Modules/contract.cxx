 ...
 * contract.cxx
 *
 * Support for Wrap by Contract in alaqil.
 * ----------------------------------------------------------------------------- */
...
void inherit_contracts(Node *c, Node *n, Hash *contracts, Hash *messages) {
  ...
  if (local_decl) {
    local_decl = alaqilType_typedef_resolve_all(local_decl);
  } else {
    return;
  }
  /* Width first search */
...
String *Contracts::make_expression(String *s, Node *n) {
  ...
  for (ei = First(list_assert); ei.item; ei = Next(ei)) {
    expr = ei.item;
    if (Len(expr)) {
      Replaceid(expr, Getattr(n, "name"), alaqil_cresult_name());
      if (Len(str_assert))
	Append(str_assert, "&&");
...
int Contracts::emit_contract(Node *n, int method) {
  ...
  if ((c = Getattr(contracts, "require:"))) {
    Setattr(n, "contract:preassert", NewStringf("alaqil_contract_assert(%s, \"Contract violation: require: %s\");\n", c, Getattr(messages, "require:")));
  }
  if ((c = Getattr(contracts, "ensure:"))) {
    Setattr(n, "contract:postassert", NewStringf("alaqil_contract_assert(%s, \"Contract violation: ensure: %s\");\n", c, Getattr(messages, "ensure:")));
  }
  return alaqil_OK;
}
...
int Contracts::cDeclaration(Node *n) {
  ...
  if (Getattr(n, "feature:contract"))
    ret = emit_contract(n, InClass && !alaqil_storage_isstatic(n));
  return ret;
}
