 ...
 * alaqil directives.
 *
 * - alaqil directives such as %include, %extern, and %import are handled
...
static String *cpp_include(const_String_or_char_ptr fn, int sysfile) {
  String *s = sysfile ? alaqil_include_sys(fn) : alaqil_include(fn);
  if (s && single_include) {
  ...
  if (!s) {
    if (ignore_missing) {
      alaqil_warning(WARN_PP_MISSING_FILE, Getfile(fn), Getline(fn), "Unable to find '%s'\n", fn);
    } else {
      alaqil_error(Getfile(fn), Getline(fn), "Unable to find '%s'\n", fn);
    }
  } else {
    String *lf;
    Seek(s, 0, SEEK_SET);
...
static String *kpp_alaqilmacro = 0;
...
void Preprocessor_init(void) {
  ...
  kpp_alaqilmacro = NewString("alaqilmacro");
 ...
 * Preprocessor_define()
 *
 * Defines a new C preprocessor symbol.   alaqilmacro specifies whether or not the macro has
 * alaqil macro semantics.
 * ----------------------------------------------------------------------------- */
...
String *Macro_vararg_name(const_String_or_char_ptr str, const_String_or_char_ptr line) {
  ...
  if (strcmp(dots, "...") != 0) {
    alaqil_error(Getfile(line), Getline(line), "Illegal macro argument name '%s'\n", str);
    Delete(argname);
    return NULL;
  }
  if (dots == s) {
...
Hash *Preprocessor_define(const_String_or_char_ptr _str, int alaqilmacro) {
  ...
  while ((c = Getc(str)) != EOF) {
	...
	if (c == ')')
      ...
      if (c != ')') {
	alaqil_error(Getfile(argstr), Getline(argstr), "Missing \')\' in macro parameters\n");
	goto macro_error;
      }
      break;
  ...
  if (argstr) {
	...
	if (varargname) {
	  Delete(varargname);
	  alaqil_error(Getfile(argstr), Getline(argstr), "Variable length macro argument must be last parameter\n");
	} else {
	...
	Delete(argname);
	alaqil_error(Getfile(argstr), Getline(argstr), "Illegal character in macro argument name\n");
	goto macro_error;
      }
    }
    if (Len(argname)) {
  ...
  Setfile(macro, file);
  if (alaqilmacro) {
    Setattr(macro, kpp_alaqilmacro, "1");
  }
  symbols = Getattr(cpp, kpp_symbols);
  if ((m1 = Getattr(symbols, macroname))) {
    if (!Checkattr(m1, kpp_value, macrovalue)) {
      alaqil_error(Getfile(macroname), Getline(macroname), "Macro '%s' redefined,\n", macroname);
      alaqil_error(Getfile(m1), Getline(m1), "previous definition of '%s'.\n", macroname);
      goto macro_error;
    }
  } else {
...
unterm:
  if (ismacro)
    alaqil_error(Getfile(args), Getline(args), "Unterminated call invoking macro '%s'\n", macro_name);
  else
    alaqil_error(Getfile(args), Getline(args), "Unterminated call to '%s'\n", macro_name);
  return args;
}
...
static String *get_filename(String *str, int *sysfile) {
  ...
  *sysfile = 0;
  ...
  alaqil_filename_unescape(fn);
  alaqil_filename_correct(fn);
  Seek(fn, 0, SEEK_SET);
  return fn;
}
...
static String *expand_macro(String *name, List *args, String *line_file) {
  ...
  if (args && (!margs || Len(margs) != Len(args))) {
    if (margs && Len(margs) > (1 + isvarargs))
      alaqil_error(macro_start_file, macro_start_line, "Macro '%s' expects %d arguments\n", name, Len(margs) - isvarargs);
    else if (margs && Len(margs) == (1 + isvarargs))
      alaqil_error(macro_start_file, macro_start_line, "Macro '%s' expects 1 argument\n", name);
    else
      alaqil_error(macro_start_file, macro_start_line, "Macro '%s' expects no arguments\n", name);
    macro_level--;
    return 0;
  }
  ...
  if (args && margs) {
	...
	String *marg = alaqil_string_mangle(arg);
	Clear(temp);
  ...
  if (Getattr(macro, kpp_alaqilmacro)) {
    String *g;
    ...
    if (strchr(Char(g), '\n')) {
      Printf(f, "/*@alaqil:%s,%d,%s@*/%s/*@alaqil@*/", Getfile(macro), Getline(macro), name, g);
#if 0
      Printf(f, "/*@alaqil:%s@*/%s/*@alaqil@*/", name, g);
#endif
    } else {
      Append(f, g);
    }
...
static DOH *Preprocessor_replace(DOH *s) {
  ...
  while ((c = Getc(s)) != EOF) {
	...
	/* See if this is the special "defined" operator */
	  ...
	  alaqil_warning(998, Getfile(s), Getline(s), "Found: %s preprocessor directive.\n", id);
	  Append(ns, id);
	} else if (Equal("#ifdef", id) || Equal("#ifndef", id)) {
	  alaqil_warning(998, Getfile(s), Getline(s), "The %s preprocessor directive does not work in macros, try #if instead.\n", id);
	  Append(ns, id);
	  */
  ...
  if (state == 2 || state == 4) {
    /* See if this is the special "defined" operator */
    if (Equal(kpp_defined, id)) {
      alaqil_error(Getfile(s), Getline(s), "No arguments given to defined()\n");
    } else if (Equal(kpp_LINE, id)) {
      Printf(ns, "%d", macro_level > 0 ? macro_start_line : Getline(s));
    } else if (Equal(kpp_FILE, id)) {
      String *fn = Copy(macro_level > 0 ? macro_start_file : Getfile(s));
...
static int checkpp_id(DOH *s) {
  ...
  while ((c = Scanner_token(scan))) {
    hastok = 1;
    if ((c == alaqil_TOKEN_ID) || (c == alaqil_TOKEN_LBRACE) || (c == alaqil_TOKEN_RBRACE))
      return 1;
  }
  if (!hastok)
    return 1;
...
/*
  ...
  alaqilIMPORTED (when %importing a file).
 */
static void push_imported() {
  if (imported_depth == 0) {
    Preprocessor_define("alaqilIMPORTED 1", 0);
  }
  ++imported_depth;
}
...
static void pop_imported() {
  --imported_depth;
  if (imported_depth == 0) {
    Preprocessor_undef("alaqilIMPORTED");
  }
}
...
String *Preprocessor_parse(String *s) {
  ...
  while ((c = Getc(s)) != EOF) {
	...
	if (skip_tochar(s, '\"', chunk) < 0) {
	  alaqil_error(Getfile(s), start_line, "Unterminated string constant\n");
	}
      } else if (c == '\'') {
	start_line = Getline(s);
	if (skip_tochar(s, '\'', chunk) < 0) {
	  alaqil_error(Getfile(s), start_line, "Unterminated character constant\n");
	}
      } else if (c == '/')
	state = 30;		/* Comment */
      break;
	...
	if (allow) {
	  ...
	  if ((m) && !(Getattr(m, kpp_args))) {
	    ...
	    copy_location(m, v);
	    if (Len(v)) {
	      alaqil_error_silent(1);
	      v1 = Preprocessor_replace(v);
	      alaqil_error_silent(0);
	...
	if (allow) {
	  ...
	  if (Len(value) > 0) {
	    ...
	    alaqil_error(Getfile(s), Getline(id), "Missing identifier for #ifdef.\n");
	    allow = 0;
	  }
	  mask = 1;
	}
	...
	if (allow) {
	  ...
	  if (Len(value) > 0) {
	    ...
	    alaqil_error(Getfile(s), Getline(id), "Missing identifier for #ifndef.\n");
	    allow = 0;
	  }
	  mask = 1;
	}
	...
	if (level <= 0) {
	  alaqil_error(Getfile(s), Getline(id), "Misplaced #else.\n");
	} else {
	  cond_lines[level - 1] = Getline(id);
	  if (Len(value) != 0)
	    alaqil_warning(WARN_PP_UNEXPECTED_TOKENS, Getfile(s), Getline(id), "Unexpected tokens after #else directive.\n");
	  if (allow) {
	    allow = 0;
	    mask = 0;
	...
	level--;
	if (level < 0) {
	  alaqil_error(Getfile(id), Getline(id), "Extraneous #endif.\n");
	  level = 0;
	} else {
	  if (level < start_level) {
	    if (Len(value) != 0)
	      alaqil_warning(WARN_PP_UNEXPECTED_TOKENS, Getfile(s), Getline(id), "Unexpected tokens after #endif directive.\n");
	    allow = 1;
	    start_level--;
	  }
	}
	...
	if (allow) {
	  ...
	  if (Len(sval) > 0) {
	    ...
	    if (e) {
	      ...
	      Seek(value, 0, SEEK_SET);
	      alaqil_warning(WARN_PP_EVALUATION, Getfile(value), Getline(value), "Could not evaluate expression '%s'\n", value);
	      if (msg)
		alaqil_warning(WARN_PP_EVALUATION, Getfile(value), Getline(value), "Error: '%s'\n", msg);
	      allow = 0;
	    } else {
	      if (val == 0)
		allow = 0;
	    ...
	    alaqil_error(Getfile(s), Getline(id), "Missing expression for #if.\n");
	    allow = 0;
	  }
	  expand_defined_operator = 0;
	  mask = 1;
	...
	if (level == 0) {
	  alaqil_error(Getfile(s), Getline(id), "Misplaced #elif.\n");
	} else {
	  cond_lines[level - 1] = Getline(id);
	  if (allow) {
	    allow = 0;
		...
		if (val)
	      ...
	      alaqil_error(Getfile(s), Getline(id), "Missing expression for #elif.\n");
	      allow = 0;
	    }
	    expand_defined_operator = 0;
	  }
	...
	if (allow) {
	  alaqil_warning(WARN_PP_CPP_WARNING, Getfile(s), Getline(id), "CPP #warning, \"%s\".\n", value);
	}
      } else if (Equal(id, kpp_error)) {
	if (allow) {
	  if (error_as_warning) {
	    alaqil_warning(WARN_PP_CPP_ERROR, Getfile(s), Getline(id), "CPP #error \"%s\".\n", value);
	  } else {
	    alaqil_error(Getfile(s), Getline(id), "CPP #error \"%s\". Use the -cpperraswarn option to continue alaqil processing.\n", value);
	  }
	}
	...
	if (((include_all) || (import_all)) && (allow)) {
	  ...
	  if (include_all && import_all) {
	    alaqil_warning(WARN_PP_INCLUDEALL_IMPORTALL, Getfile(s), Getline(id), "Both includeall and importall are defined: using includeall.\n");
	    import_all = 0;
	  }
	  Seek(value, 0, SEEK_SET);
	  ...
	  if (s1) {
	    if (include_all)
	      Printf(ns, "%%includefile \"%s\" %%beginfile\n", alaqil_filename_escape(alaqil_last_file()));
	    else if (import_all) {
	    ...
	    dirname = alaqil_file_dirname(alaqil_last_file());
	...
	if (Strncmp(value, "alaqil ", 5) == 0) {
	  ...
	  if (*c) {
	    if (strncmp(c, "nowarn=", 7) == 0) {
	      ...
	      alaqil_warnfilter(nowarn, 1);
	      Delete(nowarn);
	      Delete(val);
	    } else if (strncmp(c, "cpperraswarn=", 13) == 0) {
	      error_as_warning = atoi(c + 13);
	    } else {
              alaqil_error(Getfile(s), Getline(id), "Unknown alaqil pragma: %s\n", c);
            }
	  }
	}
      } else if (Equal(id, kpp_level)) {
	alaqil_error(Getfile(s), Getline(id), "cpp debug: level = %d, startlevel = %d\n", level, start_level);
      } else if (Equal(id, "")) {
	/* Null directive */
	...
	if (allow)
	  alaqil_error(Getfile(s), Getline(id), "Unknown alaqil preprocessor directive: %s (if this is a block of target language code, delimit it with %%{ and %%})\n", id);
      }
    ...
    case 110:
      if (!isidchar(c)) {
	Ungetc(c, s);
	/* Look for common alaqil directives  */
	if (Equal(decl, kpp_dinclude) || Equal(decl, kpp_dimport) || Equal(decl, kpp_dextern)) {
	  ...
	  if (allow) {
	    ...
	    if (Equal(decl, kpp_dextern)) {
	      alaqil_warning(WARN_DEPRECATED_EXTERN, Getfile(s), Getline(s), "%%extern is deprecated. Use %%import instead.\n");
	      Clear(decl);
	      Append(decl, "%%import");
	    }
	    ...
	    if (s1) {
	      ...
	      copy_location(s, chunk);
	      add_chunk(ns, chunk, allow);
	      Printf(ns, "%sfile%s%s%s\"%s\" %%beginfile\n", decl, options_whitespace, opt, filename_whitespace, alaqil_filename_escape(alaqil_last_file()));
  ...
  while (level > 0) {
    alaqil_error(Getfile(s), cond_lines[level - 1], "Missing #endif for conditional starting here\n");
    level--;
  }
  if (state == 120) {
    alaqil_error(Getfile(s), start_line, "Missing %%endoffile for file inclusion block starting here\n");
  }
  if (state == 150) {
    Seek(value, 0, SEEK_SET);
    alaqil_error(Getfile(s), Getline(value), "Missing %%enddef for macro starting here\n", Getline(value));
  }
  if ((state >= 105) && (state < 107)) {
    alaqil_error(Getfile(s), start_line, "Unterminated %%{ ... %%} block\n");
  }
  if ((state >= 30) && (state < 40)) {
    alaqil_error(Getfile(s), start_line, "Unterminated comment\n");
  }
