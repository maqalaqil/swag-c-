...
static void init_precedence() {
  prec[alaqil_TOKEN_NOT] = 10;
  prec[EXPR_UMINUS] = 10;
  prec[alaqil_TOKEN_STAR] = 20;
  prec[alaqil_TOKEN_SLASH] = 20;
  prec[alaqil_TOKEN_PERCENT] = 20;
  prec[alaqil_TOKEN_PLUS] = 30;
  prec[alaqil_TOKEN_MINUS] = 30;
  prec[alaqil_TOKEN_LSHIFT] = 40;
  prec[alaqil_TOKEN_RSHIFT] = 40;
  prec[alaqil_TOKEN_AND] = 50;
  prec[alaqil_TOKEN_XOR] = 60;
  prec[alaqil_TOKEN_OR] = 70;
  prec[alaqil_TOKEN_EQUALTO] = 80;
  prec[alaqil_TOKEN_NOTEQUAL] = 80;
  prec[alaqil_TOKEN_LESSTHAN] = 80;
  prec[alaqil_TOKEN_GREATERTHAN] = 80;
  prec[alaqil_TOKEN_LTEQUAL] = 80;
  prec[alaqil_TOKEN_GTEQUAL] = 80;
  prec[alaqil_TOKEN_LNOT] = 90;
  prec[alaqil_TOKEN_LAND] = 100;
  prec[alaqil_TOKEN_LOR] = 110;
  expr_init = 1;
}

#define UNARY_OP(token) (((token) == alaqil_TOKEN_NOT) || \
			 ((token) == alaqil_TOKEN_LNOT) || \
			 ((token) == EXPR_UMINUS))
...
static int expr_token(Scanner * s) {
  ...
  while (1) {
    t = Scanner_token(s);
    if (!((t == alaqil_TOKEN_BACKSLASH) || (t == alaqil_TOKEN_ENDLINE) || (t == alaqil_TOKEN_COMMENT)))
      break;
  }
  return t;
}
  ...
  while (1) {
	...
	*error = 1;
	return 0;
      ...
      if ((token == alaqil_TOKEN_INT) || (token == alaqil_TOKEN_UINT) || (token == alaqil_TOKEN_LONG) || (token == alaqil_TOKEN_ULONG)) {
	...
	stack[sp].op = EXPR_VALUE;
      } else if (token == alaqil_TOKEN_PLUS) {
      } else if ((token == alaqil_TOKEN_MINUS) || (token == alaqil_TOKEN_LNOT) || (token == alaqil_TOKEN_NOT)) {
	if (token == alaqil_TOKEN_MINUS)
	  token = EXPR_UMINUS;
	stack[sp].value = token;
	stack[sp++].op = EXPR_OP;
	stack[sp].op = EXPR_TOP;
	stack[sp].svalue = 0;
      } else if (token == alaqil_TOKEN_LPAREN) {
	stack[sp++].op = EXPR_GROUP;
	stack[sp].op = EXPR_TOP;
	stack[sp].value = 0;
	stack[sp].svalue = 0;
      } else if (token == alaqil_TOKEN_ENDLINE) {
      } else if (token == alaqil_TOKEN_STRING) {
	stack[sp].svalue = NewString(Scanner_text(scan));
	stack[sp].op = EXPR_VALUE;
      } else if (token == alaqil_TOKEN_ID) {
	stack[sp].value = 0;
	stack[sp].svalue = 0;
	stack[sp].op = EXPR_VALUE;
      } else if ((token == alaqil_TOKEN_FLOAT) || (token == alaqil_TOKEN_DOUBLE)) {
