 ...
 * deprecate.c
 *
 * The functions in this file are alaqil core functions that are deprecated 
...
/* Discussion.  This function is used to generate error messages, but take 
   ...
   parameter lists, it really isn't a core feature of alaqilparm.h or parms.c.
...
String *ParmList_errorstr(ParmList *p) {
  ...
  while (p) {
    if (Getattr(p,"hidden")) {
      ...
      String *pstr = alaqilType_str(Getattr(p, "type"), 0);
      Append(out, pstr);
      p = nextSibling(p);
      if (p) {
	Append(out, ",");
