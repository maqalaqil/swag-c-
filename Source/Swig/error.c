...
void alaqil_warning(int wnum, const_String_or_char_ptr filename, int line, const char *fmt, ...) {
  String *out;
  char *msg;
  int wrn = 1;
  va_list ap;
  if (silence)
    return;
  if (!init_fmt)
    alaqil_error_msg_format(DEFAULT_ERROR_MSG_FORMAT);
...
void alaqil_error(const_String_or_char_ptr filename, int line, const char *fmt, ...) {
  va_list ap;
...
void alaqil_error_silent(int s) {
  silence = s;
}
...
void alaqil_warnfilter(const_String_or_char_ptr wlist, int add) {
  char *c;
  char *cw;
  String *s;
  if (!filter)
  ...
  while (c) {
    if (isdigit((int) *c) || (*c == '+') || (*c == '-')) {
       ...
       * warnings (a bit odd really), eg: %warnfilter(alaqilWARN_TYPEMAP_CHARLEAK_MSG) */
      if (add) {
	Insert(filter, 0, c);
	if (isdigit((int) *c)) {
	  Insert(filter, 0, "-");
...
int alaqil_warn_count(void) {
  return nwarning;
}

/* -----------------------------------------------------------------------------
 * alaqil_error_msg_format()
 *
 * Set the type of error/warning message display
 * ----------------------------------------------------------------------------- */

void alaqil_error_msg_format(ErrorMessageFormat format) {
...
/* -----------------------------------------------------------------------------
 * alaqil_stringify_with_location()
 *
...
String *alaqil_stringify_with_location(DOH *object) {
...
void alaqil_diagnostic(const_String_or_char_ptr filename, int line, const char *fmt, ...) {
  va_list ap;
  String *formatted_filename = NULL;
