...
static void add_parms(ParmList *p, List *patchlist, List *typelist) {
  while (p) {
    ...
    alaqilType *val = Getattr(p, "value");
...
void alaqil_cparse_debug_templates(int x) {
  template_debug = x;
}
...
static void cparse_template_expand(Node *templnode, Node *n, String *tname, String *rname, String *templateargs, List *patchlist, List *typelist, List *cpatchlist) {
  ...
  if (Equal(nodeType, "template")) {
    ...
    if (checkAttribute(n, "storage", "friend")) {
      ...
      if (symname) {
	String *stripped_name = alaqilType_templateprefix(symname);
    ...
    if (!(Getattr(n, "templatetype"))) {
      String *symname;
      String *stripped_name = alaqilType_templateprefix(name);
	...
	Replaceid(name, stripped_name, tname);
      ...
      if (symname) {
	stripped_name = alaqilType_templateprefix(symname);
	if (Strstr(tname, stripped_name)) {
...
/* -----------------------------------------------------------------------------
 * alaqil_cparse_template_expand()
 * ----------------------------------------------------------------------------- */

int alaqil_cparse_template_expand(Node *n, String *rname, ParmList *tparms, Symtab *tscope) {
...

  {
    ...
    if (tparms) {
      alaqilType_add_template(tmp, tparms);
    }
  ...
  tbase = alaqil_scopename_last(tname);

  /* Look for partial specialization matching */
  if (Getattr(n, "partialargs")) {
    Parm *p, *tp;
    ParmList *ptargs = alaqilType_function_parms(Getattr(n, "partialargs"), n);
    p = ptargs;
    tp = tparms;
    while (p && tp) {
      alaqilType *ptype;
      alaqilType *tptype;
      alaqilType *partial_type;
  ...
  */
  ...
  /* Patch all of the types */
  {
	...
	if (name) {
	  ...
	  qvalue = alaqil_symbol_typedef_reduce(value, tsdecl);
	  dvalue = alaqil_symbol_type_qualify(qvalue, tsdecl);
	  if (alaqilType_istemplate(dvalue)) {
	    String *ty = alaqil_symbol_template_deftype(dvalue, tscope);
	    Delete(dvalue);
	    dvalue = ty;
	  }

	  assert(dvalue);
	  valuestr = alaqilType_str(dvalue, 0);
	  ...
	  for (i = 0; i < sz; i++) {
	    ...
	    alaqilType_typename_replace(s, name, dvalue);
	    alaqilType_typename_replace(s, tbase, iname);
	    /*      Printf(stdout,"'%s'\n", s); */
	  }
  ...
  /* Patch bases */
  {
	...
	String *qn = alaqil_symbol_type_qualify(b.item, tscope);
	Clear(b.item);
	Append(b.item, qn);
	Delete(qn);
      }
...
static EMatch does_parm_match(alaqilType *type, alaqilType *partial_parm_type, const char *partial_parm_type_base, Symtab *tscope, int *specialization_priority) {
  static const int EXACT_MATCH_PRIORITY = 99999; /* a number bigger than the length of any conceivable type */
  int matches;
  int substitutions;
  EMatch match;
  alaqilType *ty = alaqil_symbol_typedef_reduce(type, tscope);
  String *base = alaqilType_base(ty);
  alaqilType *t = Copy(partial_parm_type);
  ...
  *specialization_priority = -1;
  if (substitutions == 1) {
    ...
    alaqilType *tt = Copy(partial_parm_type);
    int len;
    /*
...
static Node *template_locate(String *name, Parm *tparms, Symtab *tscope) {
  ...
  templ = alaqil_symbol_clookup(name, 0);

  if (template_debug) {
    tname = Copy(name);
    alaqilType_add_template(tname, tparms);
    Printf(stdout, "\n");
    alaqil_diagnostic(cparse_file, cparse_line, "template_debug: Searching for match to: '%s'\n", tname);
    Delete(tname);
    tname = 0;
  }
  ...
  if (templ) {
    ...
    expandedparms = alaqil_symbol_template_defargs(parms, targs, tscope, primary_scope);
    ...
    while (p) {
      alaqilType *ty = Getattr(p, "type");
      if (ty) {
	alaqilType *nt = alaqil_symbol_type_qualify(ty, tscope);
	Setattr(p, "type", nt);
	Delete(nt);
      }
      p = nextSibling(p);
    }
    alaqilType_add_template(tname, expandedparms);
	...
	Printf(stdout, "    searching for : '%s' (explicit specialization)\n", tname);
      ...
      n = alaqil_symbol_clookup_local(tname, primary_scope);
      if (!n) {
	alaqilType *rname = alaqil_symbol_typedef_reduce(tname, tscope);
	if (!Equal(rname, tname)) {
	  if (template_debug) {
	    Printf(stdout, "    searching for : '%s' (explicit specialization with typedef reduction)\n", rname);
	  }
	  n = alaqil_symbol_clookup_local(rname, primary_scope);
	}
	Delete(rname);
      }
      if (n) {
	...
	alaqil_error(cparse_file, cparse_line, "'%s' is not defined as a template. (%s)\n", name, nodeType(n));
	Delete(tname);
	Delete(parms);
    ...
    if (posslen > 0) {
      ...
      n = alaqil_symbol_clookup_local(s, primary_scope);
      if (posslen > 1) {
	int i;
	if (n) {
	  alaqil_warning(WARN_PARSE_TEMPLATE_AMBIG, cparse_file, cparse_line, "Instantiation of template '%s' is ambiguous,\n", alaqilType_namestr(tname));
	  alaqil_warning(WARN_PARSE_TEMPLATE_AMBIG, Getfile(n), Getline(n), "  instantiation '%s' used,\n", alaqilType_namestr(Getattr(n, "name")));
	}
	for (i = 1; i < posslen; i++) {
	  String *templcsymname = Getattr(Getitem(possiblepartials, i), "templcsymname");
	  Node *ignored_node = alaqil_symbol_clookup_local(templcsymname, primary_scope);
	  assert(ignored_node);
	  alaqil_warning(WARN_PARSE_TEMPLATE_AMBIG, Getfile(ignored_node), Getline(ignored_node), "  instantiation '%s' ignored.\n", alaqilType_namestr(Getattr(ignored_node, "name")));
	}
      }
    }
  ...
  if (!n) {
    alaqil_error(cparse_file, cparse_line, "Template '%s' undefined.\n", name);
  } else if (n) {
    String *nodeType = nodeType(n);
    if (!Equal(nodeType, "template")) {
      alaqil_error(cparse_file, cparse_line, "'%s' is not defined as a template. (%s)\n", name, nodeType);
...
Node *alaqil_cparse_template_locate(String *name, Parm *tparms, Symtab *tscope) {
  ...
  if (n) {
	...
	Printf(stdout, "    Not a templated class, seeking most appropriate templated function\n");
      ...
      n = alaqil_symbol_clookup_local(name, 0);
      while (n) {
