 ...
 * parser.y
 *
 * YACC parser for alaqil.   The grammar is a somewhat broken subset of C/C++.
...
static Node *new_node(const_String_or_char_ptr tag) {
  Node *n = alaqil_cparse_new_node(tag);
...
void alaqil_typemap_lang(const char *tm_lang) {
  typemap_lang = alaqil_copy_string(tm_lang);
}

void alaqil_cparse_set_compact_default_args(int defargs) {
  compact_default_args = defargs;
}

int alaqil_cparse_template_reduce(int treduce) {
...
Hash *alaqil_cparse_features(void) {
...
static String *make_name(Node *n, String *name,alaqilType *decl) {
  String *made_name = 0;
  ...
  made_name = alaqil_name_make(n, Namespaceprefix, name, decl, add_oldname);
...
static void add_symbols(Node *n) {
  ...
  while (n) {
	...
	String *prefix = name ? alaqil_scopename_prefix(name) : 0;
	old_prefix = Namespaceprefix;
	old_scope = alaqil_symbol_popscope();
	Namespaceprefix = alaqil_symbol_qualifiedscopename(0);
	if (!prefix) {
	  ...
	  Symtab *st = alaqil_symbol_getscope(prefix);
	  String *ns = st ? Getattr(st,"name") : prefix;
	  String *base  = alaqil_scopename_last(name);
	...
	*/
	...
	if (cplus_mode == CPLUS_PROTECTED) {
	  Setattr(n,"access", "protected");
	  only_csymbol = !alaqil_need_protected(n);
	} else {
	  Setattr(n,"access", "private");
	...
	if (Cmp(storage,"typedef") == 0) {
	  ...
	  if (type) {
	    ...
	    if (decl) {
	      ty = tmp = Copy(type);
	      alaqilType_push(ty,decl);
	    } else {
	      ty = type;
	    }
	    if (!alaqilType_ismutable(ty) || (storage && Strstr(storage, "constexpr"))) {
      ...
      alaqil_features_get(alaqil_cparse_features(), Namespaceprefix, name, 0, n);
      if (makename) {
	...
	if (parsing_template_declaration)
	  SetFlag(n, "parsing_template_declaration");
	wrn = alaqil_name_warning(n, Namespaceprefix, symname,0);
	Delattr(n, "parsing_template_declaration");
      }
    } else {
      String *name = Getattr(n,"name");
      alaqilType *fdecl = Copy(decl);
      alaqilType *fun = alaqilType_pop_function(fdecl);
      if (iscdecl) {	
	Setattr(n,"kind","function");
      }
      
      alaqil_features_get(alaqil_cparse_features(),Namespaceprefix,name,fun,n);

      symname = make_name(n, name,fun);
      if (parsing_template_declaration)
	SetFlag(n, "parsing_template_declaration");
      wrn = alaqil_name_warning(n, Namespaceprefix,symname,fun);
      Delattr(n, "parsing_template_declaration");
	...
	/* C++11 deleted definition / deleted function */
      ...
      if (alaqilType_isrvalue_reference(Getattr(n, "refqualifier"))) {
	...
	if (!(Getattr(n, "feature:ignore") || Strncmp(symname, "$ignore", 7) == 0)) {
	  ...
	  alaqil_warning(WARN_TYPE_RVALUE_REF_QUALIFIER_IGNORED, Getfile(n), Getline(n),
	...
	if (!Getmeta(metaname,"already_warned")) {
      ...
      if (c != n) {
        ...
        if (Getattr(n,"sym:weak")) {
          ...
          int redefined = alaqil_need_redefined_warn(n,c,inclass);
          if (redefined) {
          ...
          if (Cmp(symname,Getattr(n,"name"))) {
            Printf(en," (Renamed from '%s')", alaqilType_namestr(Getattr(n,"name")));
          }
          Printf(en,",");
          if (Cmp(symname,Getattr(c,"name"))) {
            Printf(ec," (Renamed from '%s')", alaqilType_namestr(Getattr(c,"name")));
          }
          Printf(ec,".");
	  alaqil_WARN_NODE_BEGIN(n);
          if (redefined) {
            alaqil_warning(WARN_PARSE_REDEFINED,Getfile(n),Getline(n),"%s\n",en);
            alaqil_warning(WARN_PARSE_REDEFINED,Getfile(c),Getline(c),"%s\n",ec);
          } else if (!is_friend(n) && !is_friend(c)) {
            alaqil_warning(WARN_PARSE_REDUNDANT,Getfile(n),Getline(n),"%s\n",en);
            alaqil_warning(WARN_PARSE_REDUNDANT,Getfile(c),Getline(c),"%s\n",ec);
          }
	  alaqil_WARN_NODE_END(n);
    ...
    if (isfriend) {
      alaqil_symbol_setscope(old_scope);
      if (old_prefix) {
...
static void add_symbols_copy(Node *n) {
  ...
  while (n) {
	...
	*/
	...
	alaqil_symbol_cadd(Getattr(n,"partialargs"),n);
      }
      add_only_one = 0;
      name = Getattr(n,"name");
      if (Getattr(n,"requires_symtab")) {
	alaqil_symbol_newscope();
	alaqil_symbol_setscopename(name);
	Delete(Namespaceprefix);
	Namespaceprefix = alaqil_symbol_qualifiedscopename(0);
      }
	...
	cplus_mode = emode;
      ...
      if (Getattr(n,"requires_symtab")) {
	Setattr(n,"symtab", alaqil_symbol_popscope());
...
static String *make_class_name(String *name) {
  ...
  prefix = alaqilType_istemplate_templateprefix(nname);
  if (prefix) {
    String *args, *qargs;
    args   = alaqilType_templateargs(nname);
    qargs  = alaqil_symbol_type_qualify(args,0);
    Append(prefix,qargs);
    Delete(nname);
    Delete(args);
    Delete(qargs);
...
static void add_typedef_name(Node *n, Node *declnode, String *oldName, Symtab *cscope, String *scpname) {
  ...
  alaqilType *decl = Getattr(declnode, "decl");
  if (!decl || !Len(decl)) {
    String *cname;
    String *tdscopename;
    String *class_scope = alaqil_symbol_qualifiedscopename(cscope);
...
static Symtab *set_scope_to_global() {
  Symtab *symtab = alaqil_symbol_global_scope();
  alaqil_symbol_setscope(symtab);
  return symtab;
}
...
static String *resolve_create_node_scope(String *cname, int is_class_definition) {
  Symtab *gscope = 0;
  Node *cname_node = 0;
  String *last = alaqil_scopename_last(cname);
  nscope = 0;
  nscope_inner = 0;  
  ...
  if (Strncmp(cname,"::" ,2) != 0) {
    if (is_class_definition) {
      ...
      cname_node = alaqil_symbol_clookup_no_inherit(cname, 0);
      if (!cname_node) {
	Node *full_lookup_node = alaqil_symbol_clookup(cname, 0);
	if (full_lookup_node) {
	 /* This finds a symbol brought into scope via both a using directive and a using declaration. */
	  Node *last_node = alaqil_symbol_clookup_no_inherit(last, 0);
      ...
      cname_node = alaqil_symbol_clookup(cname, 0);
    }
  }
#if RESOLVE_DEBUG
  if (!cname_node)
  ...
  if (cname_node) {
    ...
    if ((symtab && sym_weak) || Equal(nodeType(cname_node), "template")) {
      ...
      String *current_scopename = alaqil_symbol_qualifiedscopename(0);
      String *found_scopename = alaqil_symbol_qualifiedscopename(symtab);
	...
	found_scopename = NewString("");
      ...
      {
	int fail = 1;
	List *current_scopes = alaqil_scopename_tolist(current_scopename);
	List *found_scopes = alaqil_scopename_tolist(found_scopename);
...
Printf(stdout, "comparing current: [%s] found: [%s]\n", current_scopename, found_scopename);
	...
	if (!cit.item) {
	  ...
	  if (is_class_definition) {
	    if (!fit.item) {
	      ...
	      fail = 0;
	    } else if (alaqil_scopename_check(cname)) {
	...
	if (fail) {
	  ...
	  alaqil_error(cparse_file, cparse_line, "'%s' resolves to '%s' and was incorrectly instantiated in scope '%s' instead of within scope '%s'.\n", cname, cname_resolved, current_scopename, found_scopename);
	  cname = Copy(last);
    ...
    alaqil_error(cparse_file, cparse_line, "No template found to instantiate '%s' with %%template.\n", cname);
     */
  }

  if (alaqil_scopename_check(cname)) {
    Node   *ns;
    String *prefix = alaqil_scopename_prefix(cname);
...
/* I don't think we can use :: global scope to declare classes and hence neither %template. - consider reporting error instead - wsfulton. */
    ...
    ns = alaqil_symbol_clookup(prefix,0);
    if (!ns) {
      alaqil_error(cparse_file,cparse_line,"Undefined scope '%s'\n", prefix);
    } else {
      Symtab *nstab = Getattr(ns,"symtab");
      if (!nstab) {
	alaqil_error(cparse_file,cparse_line, "'%s' is not defined as a valid scope.\n", prefix);
	ns = 0;
      } else {
	/* Check if the node scope is the current scope */
	String *tname = alaqil_symbol_qualifiedscopename(0);
	String *nname = alaqil_symbol_qualifiedscopename(nstab);
	...
	List *scopes = alaqil_scopename_tolist(prefix);
	String *sname;
	Iterator si;
	...
	for (si = First(scopes); si.item; si = Next(si)) {
	  Node *ns1,*ns2;
	  sname = si.item;
	  ns1 = alaqil_symbol_clookup(sname,0);
	  assert(ns1);
	  if (Strcmp(nodeType(ns1),"namespace") == 0) {
	    ...
	    nscope_inner = alaqil_symbol_clookup(sname,0);
	    /* set the scope to the inner class */
	    alaqil_symbol_setscope(Getattr(nscope_inner,"symtab"));
	  ...
	  add_symbols(ns2);
	  alaqil_symbol_setscope(Getattr(ns1,"symtab"));
	  Delete(Namespaceprefix);
	  Namespaceprefix = alaqil_symbol_qualifiedscopename(0);
	  if (nscope_inner) {
...
static Node *nested_forward_declaration(const char *storage, const char *kind, String *sname, String *name, Node *cpp_opt_declarators) {
  ...
  if (!currentOuterClass || !GetFlag(currentOuterClass, "nested")) {
	...
	alaqil_WARN_NODE_BEGIN(n);
	alaqil_warning(WARN_PARSE_NAMED_NESTED_CLASS, cparse_file, cparse_line,"Nested %s not currently supported (%s ignored)\n", kind, sname ? sname : name);
	alaqil_WARN_NODE_END(n);
      }
    } else {
      alaqil_warning(WARN_PARSE_UNNAMED_NESTED_CLASS, cparse_file, cparse_line, "Nested %s not currently supported (ignored).\n", kind);
    }
  }
...
static void single_new_feature(const char *featurename, String *val, Hash *featureattribs, char *declaratorid, alaqilType *type, ParmList *declaratorparms, String *qualifier) {
  String *fname;
  String *name;
  String *fixname;
  alaqilType *t = Copy(type);

  /* Printf(stdout, "single_new_feature: [%s] [%s] [%s] [%s] [%s] [%s]\n", featurename, val, declaratorid, t, ParmList_str_defaultargs(declaratorparms), qualifier); */

  /* Warn about deprecated features */
  if (strcmp(featurename, "nestedworkaround") == 0)
    alaqil_warning(WARN_DEPRECATED_NESTED_WORKAROUND, cparse_file, cparse_line, "The 'nestedworkaround' feature is deprecated.\n");
  ...
  if (!Len(t)) t = 0;
  if (t) {
    if (qualifier) alaqilType_push(t,qualifier);
    if (alaqilType_isfunction(t)) {
      alaqilType *decl = alaqilType_pop_function(t);
      if (alaqilType_ispointer(t)) {
	String *nname = NewStringf("*%s",name);
	alaqil_feature_set(alaqil_cparse_features(), nname, decl, fname, val, featureattribs);
	Delete(nname);
      } else {
	alaqil_feature_set(alaqil_cparse_features(), name, decl, fname, val, featureattribs);
      }
      Delete(decl);
    } else if (alaqilType_ispointer(t)) {
      String *nname = NewStringf("*%s",name);
      alaqil_feature_set(alaqil_cparse_features(),nname,0,fname,val, featureattribs);
      Delete(nname);
    }
  } else {
    /* Global feature, that is, feature not associated with any particular symbol */
    alaqil_feature_set(alaqil_cparse_features(),name,0,fname,val, featureattribs);
  }
  Delete(fname);
  Delete(name);
}
...
static void new_feature(const char *featurename, String *val, Hash *featureattribs, char *declaratorid, alaqilType *type, ParmList *declaratorparms, String *qualifier) {
  ...
  if (type) {
    while (declparms) {
      if (ParmList_has_defaultargs(declparms)) {
        ...
        alaqilType *newtype = Copy(type);
        Delete(alaqilType_pop_function(newtype)); /* remove the old parameter list from newtype */
        alaqilType_add_function(newtype,newparms);
...
static void default_arguments(Node *n) {
  ...
  while (function) {
    ...
    if (ParmList_has_defaultargs(parms)) {
      ...
      {
	alaqilType *ntype = Copy(nodeType(function));
	char *cntype = Char(ntype);
        Node *new_function = new_node(ntype);
        alaqilType *decl = Copy(Getattr(function,"decl"));
	...
	alaqilType *ctype = Copy(Getattr(function,"type"));
	String *cthrow = Copy(Getattr(function,"throw"));

        Delete(alaqilType_pop_function(decl)); /* remove the old parameter list from decl */
        alaqilType_add_function(decl,newparms);
        if (constqualifier)
          alaqilType_add_qualifier(decl,"const");
...
static String *add_qualifier_to_declarator(alaqilType *type, alaqilType *qualifier) {
  ...
  while (decl) {
    if (alaqilType_ismemberpointer(decl)) {
      String *memberptr = alaqilType_pop(decl);
      if (alaqilType_isfunction(decl)) {
	is_pointer_to_member_function = 1;
	alaqilType_push(decl, qualifier);
	alaqilType_push(decl, memberptr);
	...
	Append(poppedtype, memberptr);
      ...
      String *popped = alaqilType_pop(decl);
      if (!popped)
	break;
...
%token NATIVE INLINE
%token TYPEMAP EXCEPT ECHO APPLY CLEAR alaqilTEMPLATE FRAGMENT
%token WARN 
...
%type <node>     program interface declaration alaqil_directive ;
...
%type <node>     cpp_alaqil_directive cpp_template_possible cpp_opt_declarators ;
 ...
 *
 * An interface is just a sequence of declarations which may be alaqil directives
...
declaration    : alaqil_directive { $$ = $1; }
               | c_declaration { $$ = $1; } 
               | cpp_declaration { $$ = $1; }
               | SEMI { $$ = 0; }
               | error {
		  ...
		  if (cparse_unknown_directive) {
		      alaqil_error(cparse_file, cparse_line, "Unknown directive '%s'.\n", cparse_unknown_directive);
		  } else {
		      alaqil_error(cparse_file, cparse_line, "Syntax error in input(1).\n");
		  }
		  exit(1);
               }
...
alaqil_directive : extend_directive { $$ = $1; }
               | apply_directive { $$ = $1; }
 	       | clear_directive { $$ = $1; }
...
extend_directive : EXTEND options classkeyopt idcolon LBRACE {
		 ...
		 if (!cls) {
		   ...
		   Node *am = Getattr(alaqil_extend_hash(),clsname);
		   if (!am) {
		     alaqil_symbol_newscope();
		     alaqil_symbol_setscopename($4);
		     prev_symtab = 0;
		   } else {
		     prev_symtab = alaqil_symbol_setscope(Getattr(am,"symtab"));
		   }
		   current_class = 0;
		 } else {
		   ...
		   prev_symtab = alaqil_symbol_setscope(Getattr(cls, "symtab"));
		   current_class = cls;
		   alaqil_WARN_NODE_BEGIN(cls);
		   alaqil_warning(WARN_PARSE_EXTEND_NAME, cparse_file, cparse_line, "Deprecated %%extend name used - the %s name '%s' should be used instead of the typedef name '%s'.\n", Getattr(cls, "kind"), alaqilType_namestr(Getattr(cls, "name")), $4);
		   alaqil_WARN_NODE_END(cls);
		 }
	       } else {
		 ...
		 current_class = cls;
	       ...
	       extendmode = 0;
               $$ = new_node("extend");
	       Setattr($$,"symtab",alaqil_symbol_popscope());
	       if (prev_symtab) {
		 alaqil_symbol_setscope(prev_symtab);
	       }
	       Namespaceprefix = alaqil_symbol_qualifiedscopename(0);
               clsname = make_class_name($4);
		 ...
		 if (am) {
		   ...
		   appendChild($$, $7);
		   Setattr(alaqil_extend_hash(),clsname,$$);
		 }
	       }
	       current_class = 0;
	       Delete(Classprefix);
...
constant_directive :  CONSTANT identifier EQUAL definetype SEMI {
		   if (($4.type != T_ERROR) && ($4.type != T_SYMBOL)) {
		     alaqilType *type = NewalaqilType($4.type);
		     $$ = new_node("constant");
		     Setattr($$,"name",$2);
		     Setattr($$,"type",type);
		     Setattr($$,"value",$4.val);
		     ...
		     if ($4.type == T_ERROR) {
		       alaqil_warning(WARN_PARSE_UNSUPPORTED_VALUE,cparse_file,cparse_line,"Unsupported constant value (ignored)\n");
		     }
		     $$ = 0;
		   }
		 ...
		 if (($4.type != T_ERROR) && ($4.type != T_SYMBOL)) {
		   alaqilType_push($2,$3.type);
		   /* Sneaky callback function trick */
		   if (alaqilType_isfunction($2)) {
		     alaqilType_add_pointer($2);
		   }
		   $$ = new_node("constant");
		   Setattr($$,"name",$3.id);
		   Setattr($$,"type",$2);
		   ...
		   if ($4.type == T_ERROR) {
		     alaqil_warning(WARN_PARSE_UNSUPPORTED_VALUE,cparse_file,cparse_line, "Unsupported constant value\n");
		   }
		   $$ = 0;
		 }
               }
		 ...
		 if (($8.type != T_ERROR) && ($8.type != T_SYMBOL)) {
		   alaqilType_add_function($2, $5);
		   alaqilType_push($2, $7.qualifier);
		   alaqilType_push($2, $3.type);
		   /* Sneaky callback function trick */
		   if (alaqilType_isfunction($2)) {
		     alaqilType_add_pointer($2);
		   ...
		   $$ = 0;
               ...
               | CONSTANT error SEMI {
		 alaqil_warning(WARN_PARSE_BAD_VALUE,cparse_file,cparse_line,"Bad constant value (ignored).\n");
		 $$ = 0;
	       }
               ;
...
except_directive : EXCEPT LPAREN identifier RPAREN LBRACE {
                    skip_balanced('{','}');
		    $$ = 0;
		    alaqil_warning(WARN_DEPRECATED_EXCEPT,cparse_file, cparse_line, "%%except is deprecated.  Use %%exception instead.\n");
	       }
...
inline_directive : INLINE HBLOCK {
                 String *cpps;
		 if (Namespaceprefix) {
		   alaqil_error(cparse_file, cparse_start_line, "%%inline directive inside a namespace is disallowed.\n");
		   $$ = 0;
		 } else {
		   $$ = new_node("insert");
		   Setattr($$,"code",$2);
...
insert_directive : HBLOCK {
		 ...
		 Setattr($$,"section",$3);
		 Setattr($$,"code",code);
		 if (alaqil_insert_file($5,code) < 0) {
		   alaqil_error(cparse_file, cparse_line, "Couldn't find '%s'.\n", $5);
		   $$ = 0;
		 } 
               }
...
module_directive: MODULE options idstring {
		 ...
		 if ($2) {
		   ...
		   if (Getattr($2,"directors")) {
		     ...
		     if (!cparse_cplusplus) {
		       alaqil_error(cparse_file, cparse_line, "Directors are not supported for C code and require the -c++ option\n");
		     }
		   } 
...
name_directive : NAME LPAREN idstring RPAREN {
                 alaqil_warning(WARN_DEPRECATED_NAME,cparse_file,cparse_line, "%%name is deprecated.  Use %%rename instead.\n");
		 Delete(yyrename);
		 ...
		 alaqil_warning(WARN_DEPRECATED_NAME,cparse_file,cparse_line, "%%name is deprecated.  Use %%rename instead.\n");
		 $$ = 0;
		 alaqil_error(cparse_file,cparse_line,"Missing argument to %%name directive.\n");
	       }
               ;
...
native_directive : NATIVE LPAREN identifier RPAREN storage_class identifier SEMI {
		 ...
		 if (!alaqilType_isfunction($7.type)) {
		   alaqil_error(cparse_file,cparse_line,"%%native declaration '%s' is not a function.\n", $7.id);
		   $$ = 0;
		 } else {
		     Delete(alaqilType_pop_function($7.type));
		     /* Need check for function here */
		     alaqilType_push($6,$7.type);
		     $$ = new_node("native");
	             Setattr($$,"name",$3);
...
rename_directive : rename_namewarn declarator idstring SEMI {
		...
		if (t) {
		  if (alaqilType_isfunction(t)) {
		    ...
		    if (alaqilType_ispointer(t)) {
		      ...
		      if ($1) {
			alaqil_name_rename_add(Namespaceprefix, nname,decl,kws,$2.parms);
		      } else {
			alaqil_name_namewarn_add(Namespaceprefix,nname,decl,kws);
		      }
		      Delete(nname);
		    } else {
		      if ($1) {
			alaqil_name_rename_add(Namespaceprefix,(fixname),decl,kws,$2.parms);
		      } else {
			alaqil_name_namewarn_add(Namespaceprefix,(fixname),decl,kws);
		      }
		    }
		    Delete(decl);
		  } else if (alaqilType_ispointer(t)) {
		    String *nname = NewStringf("*%s",fixname);
		    if ($1) {
		      alaqil_name_rename_add(Namespaceprefix,(nname),0,kws,$2.parms);
		    } else {
		      alaqil_name_namewarn_add(Namespaceprefix,(nname),0,kws);
		    }
		    Delete(nname);
		  }
		} else {
		  if ($1) {
		    alaqil_name_rename_add(Namespaceprefix,(fixname),0,kws,$2.parms);
		  } else {
		    alaqil_name_namewarn_add(Namespaceprefix,(fixname),0,kws);
		  }
		}
                $$ = 0;
		...
		/* Special declarator check */
		if (t) {
		  if ($6.qualifier) alaqilType_push(t,$6.qualifier);
		  if (alaqilType_isfunction(t)) {
		    alaqilType *decl = alaqilType_pop_function(t);
		    if (alaqilType_ispointer(t)) {
		      String *nname = NewStringf("*%s",fixname);
		      if ($1) {
			alaqil_name_rename_add(Namespaceprefix, nname,decl,kws,$5.parms);
		      } else {
			alaqil_name_namewarn_add(Namespaceprefix,nname,decl,kws);
		      }
		      Delete(nname);
		      ...
		      if ($1) {
			alaqil_name_rename_add(Namespaceprefix,(fixname),decl,kws,$5.parms);
		      } else {
			alaqil_name_namewarn_add(Namespaceprefix,(fixname),decl,kws);
		      }
		    }
		    ...
		    String *nname = NewStringf("*%s",fixname);
		    if ($1) {
		      alaqil_name_rename_add(Namespaceprefix,(nname),0,kws,$5.parms);
		    } else {
		      alaqil_name_namewarn_add(Namespaceprefix,(nname),0,kws);
		    }
		    Delete(nname);
		  ...
		  if ($1) {
		    alaqil_name_rename_add(Namespaceprefix,(fixname),0,kws,$5.parms);
		  } else {
		    alaqil_name_namewarn_add(Namespaceprefix,(fixname),0,kws);
		  }
		}
		...
		if ($1) {
		  alaqil_name_rename_add(Namespaceprefix,$5,0,$3,0);
		} else {
		  alaqil_name_namewarn_add(Namespaceprefix,$5,0,$3);
		}
		$$ = 0;
		scanner_clear_rename();
              }
...
varargs_directive : VARARGS LPAREN varargs_parms RPAREN declarator cpp_const SEMI {
		 ...
		 if (t) {
		   ...
		   if (alaqilType_isfunction(t)) {
		     ...
		     if (alaqilType_ispointer(t)) {
		       String *nname = NewStringf("*%s",name);
		       alaqil_feature_set(alaqil_cparse_features(), nname, decl, "feature:varargs", val, 0);
		       Delete(nname);
		     } else {
		       alaqil_feature_set(alaqil_cparse_features(), name, decl, "feature:varargs", val, 0);
		     }
		     Delete(decl);
		   } else if (alaqilType_ispointer(t)) {
		     String *nname = NewStringf("*%s",name);
		     alaqil_feature_set(alaqil_cparse_features(),nname,0,"feature:varargs",val, 0);
		     Delete(nname);
		   }
		 } else {
		   alaqil_feature_set(alaqil_cparse_features(),name,0,"feature:varargs",val, 0);
		 }
		 Delete(name);
		 $$ = 0;
              };
...
varargs_parms   : parms { $$ = $1; }
		  ...
		  n = atoi(Char($1.val));
		  if (n <= 0) {
		    alaqil_error(cparse_file, cparse_line,"Argument count in %%varargs must be positive.\n");
		    $$ = 0;
		  } else {
...
typemap_type   : kwargs {
		 ...
		 if (p && (!Getattr(p,"value"))) {
 		   ...
 		   alaqil_warning(WARN_DEPRECATED_TYPEMAP_LANG,cparse_file, cparse_line,
				"Specifying the language name in %%typemap is deprecated - use #ifdef alaqil<LANG> instead.\n");
...
template_directive: alaqilTEMPLATE LPAREN idstringopt RPAREN idcolonnt LESSTHAN valparms GREATERTHAN SEMI {
                  Parm *p, *tp;
		  Node *n;
		  ...
		  tscope = alaqil_symbol_current();          /* Get the current scope */
		  ...
		  */
		  n = alaqil_cparse_template_locate($5,$7,tscope);

		  /* Patch the argument types to respect namespaces */
		  p = $7;
		  while (p) {
		    alaqilType *value = Getattr(p,"value");
		    if (!value) {
		      alaqilType *ty = Getattr(p,"type");
		      if (ty) {
			alaqilType *rty = 0;
			int reduce = template_reduce;
			if (reduce || !alaqilType_ispointer(ty)) {
			  rty = alaqil_symbol_typedef_reduce(ty,tscope);
			  if (!reduce) reduce = alaqilType_ispointer(rty);
			}
			ty = reduce ? alaqil_symbol_type_qualify(rty,tscope) : alaqil_symbol_type_qualify(ty,tscope);
			Setattr(p,"type",ty);
			Delete(ty);
			Delete(rty);
		      }
		    } else {
		      value = alaqil_symbol_type_qualify(value,tscope);
		  ...
		  /* Look for the template */
		  {
                    ...
                    while (nn) {
                      ...
                      if (Strcmp(nodeType(nn),"template") == 0) {
                        ...
                        if (nnisclass && !variadic && !specialized && (ParmList_len($7) > ParmList_len(tparms))) {
                          alaqil_error(cparse_file, cparse_line, "Too many template parameters. Maximum of %d.\n", ParmList_len(tparms));
                        } else if (nnisclass && !specialized && ((ParmList_len($7) < (ParmList_numrequired(tparms) - (variadic?1:0))))) { /* Variadic parameter is optional */
                          alaqil_error(cparse_file, cparse_line, "Not enough template parameters specified. %d required.\n", (ParmList_numrequired(tparms)-(variadic?1:0)) );
			  ...
			  String *tname = Copy($5);
                          int def_supplied = 0;
                          /* Expand the template */
			  Node *templ = alaqil_symbol_clookup($5,0);
			  Parm *targs = templ ? Getattr(templ,"templateparms") : 0;
				...
				ti = nextSibling(ti);
			      ...
			      alaqil_WARN_NODE_BEGIN(nn);
                              alaqil_warning(WARN_CPP11_VARIADIC_TEMPLATE,cparse_file, cparse_line,"Only the first variadic template argument is currently supported.\n");
			      alaqil_WARN_NODE_END(nn);
                              break;
                            }
                          }
			  ...
			  /* for now, nested %template is allowed only in the same scope as the template declaration */
			    ...
			    /*
			       ...
			       String *symname = alaqil_name_make(templnode,0,$3,0,0);
			    */
			    String *symname = NewString($3);
                            alaqil_cparse_template_expand(templnode,symname,temparms,tscope);
                            Setattr(templnode,"sym:name",symname);
                          } else {
                            static int cnt = 0;
                            String *nname = NewStringf("__dummy_%d__", cnt++);
                            alaqil_cparse_template_expand(templnode,nname,temparms,tscope);
                            Setattr(templnode,"sym:name",nname);
			    Delete(nname);
                            Setattr(templnode,"feature:onlychildren", "typemap,typemapitem,typemapcopy,typedef,types,fragment,apply");
			    if ($3) {
			      alaqil_warning(WARN_PARSE_NESTED_TEMPLATE, cparse_file, cparse_line, "Named nested template instantiations not supported. Processing as if no name was given to %%template().\n");
			    }
                          }
			  ...
			  if (outer_class && nnisclass) {
			    ...
			    Setattr(templnode, "nested:outer", outer_class);
                          ...
                          if (Strcmp(nodeType(templnode),"class") == 0) {
                            ...
                            /* Set up inheritance in symbol table */
                            {
                              ...
                              csyms = alaqil_symbol_current();
                              alaqil_symbol_setscope(Getattr(templnode,"symtab"));
                              if (baselist) {
                                List *bases = alaqil_make_inherit_list(Getattr(templnode,"name"),baselist, Namespaceprefix);
                                if (bases) {
                                  Iterator s;
			       ...
			       %extend methods at the beginning of the class */
                            {
                              ...
                              am = Getattr(alaqil_extend_hash(),clsname);
                              if (am) {
                                Symtab *st = alaqil_symbol_current();
                                alaqil_symbol_setscope(Getattr(templnode,"symtab"));
                                /*			    Printf(stdout,"%s: %s %p %p\n", Getattr(templnode,"name"), clsname, alaqil_symbol_current(), Getattr(templnode,"symtab")); */
                                alaqil_extend_merge(templnode,am);
                                alaqil_symbol_setscope(st);
				alaqil_extend_append_previous(templnode,am);
                                Delattr(alaqil_extend_hash(),clsname);
                              }
			      if (stmp) Delete(stmp);
                            }
			    ...
			    if (Namespaceprefix) {
			      ...
			      String *qs = alaqil_symbol_qualifiedscopename(templnode);
...
warn_directive : WARN string {
		  alaqil_warning(0,cparse_file, cparse_line,"%s\n", $2);
		  $$ = 0;
               }
               ;
...
/* An extern C type declaration, disable cparse_cplusplus if needed. */
		  ...
		  if (Strcmp($2,"C") == 0) {
		    ...
		    appendChild($$,n);
		    while (n) {
		      alaqilType *decl = Getattr(n,"decl");
		      if (alaqilType_isfunction(decl) && !Equal(Getattr(n, "storage"), "typedef")) {
			Setattr(n,"storage","externc");
		      }
		      n = nextSibling(n);
		    }
		  } else {
		     alaqil_warning(WARN_PARSE_UNDEFINED_EXTERN,cparse_file, cparse_line,"Unrecognized extern type \"%s\".\n", $2);
		    $$ = new_node("extern");
		    Setattr($$,"name",$2);
		    appendChild($$,firstChild($5));
                ...
                | cpp_lambda_decl {
		  $$ = $1;
		  alaqil_WARN_NODE_BEGIN($$);
		  alaqil_warning(WARN_CPP11_LAMBDA, cparse_file, cparse_line, "Lambda expressions and closures are not fully supported yet.\n");
		  alaqil_WARN_NODE_END($$);
		}
...
c_decl  : storage_class type declarator cpp_const initializer c_decl_tail {
		...
		alaqilType *valuetype = NewalaqilType($5.type);
		if (Len(valuetype) > 0)
		...
		if (Strstr($3.id, "::")) {
		  ...
		  String *p = alaqil_scopename_prefix($3.id);
		  if (p) {
		    if ((Namespaceprefix && Strcmp(p, Namespaceprefix) == 0) ||
			(Classprefix && Strcmp(p, Classprefix) == 0)) {
		      String *lstr = alaqil_scopename_last($3.id);
		      Setattr($$, "name", lstr);
		      Delete(lstr);
		      set_nextSibling($$, $6);
		    } else {
		...
		alaqil_error(cparse_file, cparse_line, "Missing symbol name for global declaration\n");
		$$ = 0;
	      }

	      if ($4.qualifier && $1 && Strstr($1, "static"))
		alaqil_error(cparse_file, cparse_line, "Static function %s cannot have a qualifier.\n", alaqil_name_decl($$));
           }
           ...
           | storage_class AUTO declarator cpp_const ARROW cpp_alternate_rettype virt_specifier_seq_opt initializer c_decl_tail {
              $$ = new_node("cdecl");
	      if ($4.qualifier) alaqilType_push($3.type, $4.qualifier);
	      Setattr($$,"refqualifier",$4.refqualifier);
	      Setattr($$,"type",$6);
	      Setattr($$,"storage",$1);
	      Setattr($$,"name",$3.id);
...
c_decl_tail    : SEMI { 
		 ...
		 $$ = new_node("cdecl");
		 if ($3.qualifier) alaqilType_push($2.type,$3.qualifier);
		 ...
		 if (!$5) {
		   ...
		   $$ = 0;
		   if (yychar == RPAREN) {
		       alaqil_error(cparse_file, cparse_line, "Unexpected ')'.\n");
		   } else {
		       alaqil_error(cparse_file, cparse_line, "Syntax error - possibly a missing semicolon.\n");
		   }
		   exit(1);
               }
              ;
...
c_constructor_decl : storage_class type LPAREN parms RPAREN ctor_end {
		    ...
		    if ((ParmList_len($4) == 1) && (!alaqil_scopename_check($2))) {
		      alaqilType *ty = Getattr($4,"type");
			...
			if ($6.have_parms) {
			  alaqilType *decl = NewStringEmpty();
			  alaqilType_add_function(decl,$6.parms);
			  Setattr($$,"decl",decl);
		    ...
		    if (err) {
		      alaqil_error(cparse_file,cparse_line,"Syntax error in input(2).\n");
		      exit(1);
		    }
                }
                ;
...
cpp_class_decl  : storage_class cpptype idcolon inherit LBRACE {
		   ...
		   Setattr($<node>$,"prev_symtab",alaqil_symbol_current());
		   ...
		   alaqil_features_get(alaqil_cparse_features(), Namespaceprefix, Getattr($<node>$, "name"), 0, $<node>$);
		   ...
		   if ($4)
		     bases = alaqil_make_inherit_list($3,Getattr($4,"public"),Namespaceprefix);
		   prefix = alaqilType_istemplate_templateprefix($3);
		   if (prefix) {
		     String *fbase, *tbase;
		     if (Namespaceprefix) {
		     ...
		     alaqil_name_inherit(tbase,fbase);
		     Delete(fbase);
		     Delete(tbase);
		   }
                   if (strcmp($2,"class") == 0) {
		   ...
		   alaqil_symbol_newscope();
		   alaqil_symbol_setscopename($3);
		   alaqil_inherit_base_symbols(bases);
		   Delete(Namespaceprefix);
		   Namespaceprefix = alaqil_symbol_qualifiedscopename(0);
		   ...
		   if (nscope_inner && Strcmp(nodeType(nscope_inner), "class") == 0) { /* actual parent class for this class */
		     Node* forward_declaration = alaqil_symbol_clookup_no_inherit(Getattr($<node>$,"name"), Getattr(nscope_inner, "symtab"));
		   ...
		   {
		     String *clsname = alaqil_symbol_qualifiedscopename(0);
		     am = Getattr(alaqil_extend_hash(), clsname);
		     if (am) {
		       alaqil_extend_merge($$, am);
		       Delattr(alaqil_extend_hash(), clsname);
		     }
		     Delete(clsname);
		   }
		   if (!classes) classes = NewHash();
		   scpname = alaqil_symbol_qualifiedscopename(0);
		   Setattr(classes, scpname, $$);

		   appendChild($$, $7);
		   
		   if (am) 
		     alaqil_extend_append_previous($$, am);
		   ...
		   if (cplus_mode == CPLUS_PRIVATE) {
		     ...
		     } else if (!alaqilType_istemplate(ty) && template_parameters == 0) { /* for template we need the class itself */
		       $$ = $9;
		     }
		   } else {
		     Delete(yyrename);
			 ...
			 outer = Getattr(outer, "nested:outer");
		       appendSibling(outer, $$);
		       alaqil_symbol_setscope(cscope); /* declaration goes in the parent scope */
		       add_symbols($9);
		       set_scope_to_global();
		       Delete(Namespaceprefix);
		       Namespaceprefix = alaqil_symbol_qualifiedscopename(0);
...
/* An unnamed struct, possibly with a typedef */
		 ...
		 set_access_mode($<node>$);
	       ...
	       alaqil_features_get(alaqil_cparse_features(), Namespaceprefix, 0, 0, $<node>$);
	       /* save yyrename to the class attribute, to be used later in add_symbols()*/
		 ...
		 if (name) {
		   ...
		   alaqil_symbol_setscopename(name);
		   if ($3)
		     bases = alaqil_make_inherit_list(name,Getattr($3,"public"),Namespaceprefix);
		   alaqil_inherit_base_symbols(bases);
		   ...
		   n = $8;
		   alaqil_symbol_setscopename("<unnamed>");
		 }
		 appendChild($$,$6);
		 /* Pop the scope */
		 Setattr($$,"symtab",alaqil_symbol_popscope());
...
cpp_template_decl : TEMPLATE LESSTHAN template_parms GREATERTHAN { 
			...
			Symtab *cscope = alaqil_symbol_current();
			Symtab *sti = 0;
			Node *ntop = $6;
			Node *ni = ntop;
			alaqilType *ntype = ni ? nodeType(ni) : 0;
			...
			if (($$) && (Strchr(tname,'<')) && (!is_operator(tname))) {
			  ...
			  Node *tempn = 0;
			  {
			    String *tbase = alaqilType_templateprefix(tname);
			    tempn = alaqil_symbol_clookup_local(tbase,0);
			    if (!tempn || (Strcmp(nodeType(tempn),"template") != 0)) {
			      alaqil_WARN_NODE_BEGIN(tempn);
			      alaqil_warning(WARN_PARSE_TEMPLATE_SP_UNDEF, Getfile($$),Getline($$),"Specialization of non-template '%s'.\n", tbase);
			      alaqil_WARN_NODE_END(tempn);
			      tempn = 0;
			      error = 1;
			    }
			    Delete(tbase);
			  ...
			  if (tempn && ($3) && ($6)) {
			    List   *tlist;
			    String *targs = alaqilType_templateargs(tname);
			    tlist = alaqilType_parmlist(targs);
			    ...
			    if (Len(tlist) != ParmList_len(Getattr(tempn,"templateparms"))) {
			      alaqil_error(Getfile($$),Getline($$),"Inconsistent argument count in template partial specialization. %d %d\n", Len(tlist), ParmList_len(Getattr(tempn,"templateparms")));
			    ...
			    */
				...
				for (i = 0; i < nargs; i++){
				  pn = Getattr(p,"name");
				  if (Strcmp(pn,alaqilType_base(Getitem(tlist,i))) == 0) {
				    int j;
				    Parm *p1 = tp;
				    for (j = 0; j < i; j++) {
				      p1 = nextSibling(p1);
				...
				Iterator tt;
				Parm *parm_current = 0;
				List *tparms = alaqilType_parmlist(fname);
				ffname = alaqilType_templateprefix(fname);
				Append(ffname,"<(");
				for (tt = First(tparms); tt.item; ) {
				  alaqilType *rtt = alaqil_symbol_typedef_reduce(tt.item,0);
				  alaqilType *ttr = alaqil_symbol_type_qualify(rtt,0);
				...
				/*			      Printf(stdout,"partial: fname = '%s', '%s'\n", fname, alaqil_symbol_typedef_reduce(fname,0)); */
			    ...
			    String *ty = alaqil_symbol_template_deftype(tname,0);
			    String *fname = alaqil_symbol_type_qualify(ty,0);
			    alaqil_symbol_cadd(fname,$$);
			    Delete(ty);
			    Delete(fname);
			  }
			}  else if ($$) {
		...
		/* Explicit template instantiation */
		  ...
		  alaqil_warning(WARN_PARSE_EXPLICIT_TEMPLATE, cparse_file, cparse_line, "Explicit template instantiation ignored.\n");
                  $$ = 0; 
		}
...
template_parms  : templateparameters {
		  ...
		  while (p) {
			...
			/*
			 alaqil_error(cparse_file, cparse_line, "Missing template parameter name\n");
			 $$.rparms = 0;
			 $$.parms = 0;
			 break; */
		      }
...
cpp_using_decl : USING idcolon SEMI {
                  String *uname = alaqil_symbol_type_qualify($2,0);
		  String *name = alaqil_scopename_last($2);
                  $$ = new_node("using");
		  Setattr($$,"uname",uname);
		  Setattr($$,"name", name);
		  Delete(uname);
		  ...
		  add_symbols($$);
	       ...
	       Node *n = alaqil_symbol_clookup($3,0);
	       if (!n) {
		 alaqil_error(cparse_file, cparse_line, "Nothing known about namespace '%s'\n", $3);
		 $$ = 0;
	       } else {
		 ...
		 if (n) {
		   if (Strcmp(nodeType(n),"namespace") == 0) {
		     Symtab *current = alaqil_symbol_current();
		     Symtab *symtab = Getattr(n,"symtab");
		     $$ = new_node("using");
		     Setattr($$,"node",n);
		     Setattr($$,"namespace", $3);
		     if (current != symtab) {
		       alaqil_symbol_inherit(symtab);
		     }
		   } else {
		     alaqil_error(cparse_file, cparse_line, "'%s' is not a namespace.\n", $3);
		     $$ = 0;
		   }
		 } else {
		   $$ = 0;
...
cpp_namespace_decl : NAMESPACE idcolon LBRACE { 
                Hash *h;
		Node *parent_ns = 0;
		List *scopes = alaqil_scopename_tolist($2);
		int ilen = Len(scopes);
		int i;
...
*/
		...
		for (i = 0; i < ilen; i++) {
		  ...
		  Symtab *current_symtab = alaqil_symbol_current();
		  ...
		  parent_ns = ns;
		  h = alaqil_symbol_clookup(scopename, 0);
...
*/
		    if (Getattr(h, "alias")) {
		      ...
		      alaqil_warning(WARN_PARSE_NAMESPACE_ALIAS, cparse_file, cparse_line, "Namespace alias '%s' not allowed here. Assuming '%s'\n",
				   scopename, Getattr(h, "name"));
		      scopename = Getattr(h, "name");
		    }
		    alaqil_symbol_setscope(Getattr(h, "symtab"));
		  } else {
/*
Printf(stdout, "  Scope %s [creating single scope C++17 style]\n", scopename);
*/
		    h = alaqil_symbol_newscope();
		    alaqil_symbol_setscopename(scopename);
		  }
		  Delete(Namespaceprefix);
		  Namespaceprefix = alaqil_symbol_qualifiedscopename(0);
		}
		...
		$$ = top_ns;
             ...
             | NAMESPACE LBRACE {
	       Hash *h;
	       $1 = alaqil_symbol_current();
	       h = alaqil_symbol_clookup("    ",0);
		 ...
		 alaqil_symbol_setscopename("    ");
	       ...
	       Setattr($$,"name",$2);
	       Setattr($$,"alias",$4);
	       n = alaqil_symbol_clookup($4,0);
	       if (!n) {
		 alaqil_error(cparse_file, cparse_line, "Unknown namespace '%s'\n", $4);
		 $$ = 0;
	       } else {
		 if (Strcmp(nodeType(n),"namespace") != 0) {
		   alaqil_error(cparse_file, cparse_line, "'%s' is not a namespace\n",$4);
		   ...
		   alaqil_symbol_alias($2,Getattr(n,"symtab"));
		 }
	       }
             }
             ;
...
cpp_members  : cpp_member cpp_members {
	       ...
	       extendmode = 1;
	       if (cplus_mode != CPLUS_PUBLIC) {
		 alaqil_error(cparse_file,cparse_line,"%%extend can only be used in a public section\n");
	       }
             } cpp_members RBRACE {
	       extendmode = 0;
	     } cpp_members {
	     ...
	     | error {
	       int start_line = cparse_line;
	       skip_decl();
	       alaqil_error(cparse_file,start_line,"Syntax error in input(3).\n");
	       exit(1);
	       } cpp_members { 
		 $$ = $3;
   	     }
...
cpp_constructor_decl : storage_class type LPAREN parms RPAREN ctor_end {
		...
		Setattr($$,"name",$2);
		Setattr($$,"parms",$4);
		alaqilType_add_function(decl,$4);
		Setattr($$,"decl",decl);
		Setattr($$,"throws",$6.throws);
		Setattr($$,"throw",$6.throwf);
		Setattr($$,"noexcept",$6.nexcept);
...
cpp_destructor_decl : NOT idtemplate LPAREN parms RPAREN cpp_end {
		 ...
		 Delete(decl);
	       ...
	       if ($6.val)
	         Setattr($$,"value",$6.val);
	       if ($6.qualifier)
		 alaqil_error(cparse_file, cparse_line, "Destructor %s %s cannot have a qualifier.\n", alaqil_name_decl($$), alaqilType_str($6.qualifier, 0));
	       add_symbols($$);
	      }
...
/* A virtual destructor */
		...
		{
		  String *decl = NewStringEmpty();
		  alaqilType_add_function(decl,$5);
		  Setattr($$,"decl",decl);
		  Delete(decl);
		}
		if ($7.qualifier)
		  alaqil_error(cparse_file, cparse_line, "Destructor %s %s cannot have a qualifier.\n", alaqil_name_decl($$), alaqilType_str($7.qualifier, 0));
		add_symbols($$);
	      }
              ;
...
cpp_conversion_operator : storage_class CONVERSIONOPERATOR type pointer LPAREN parms RPAREN cpp_vend {
		 ...
		 alaqilType_add_function($4,$6);
		 if ($8.qualifier) {
		   alaqilType_push($4,$8.qualifier);
		 }
		 ...
		 Setattr($$,"storage",$1);
		 decl = NewStringEmpty();
		 alaqilType_add_reference(decl);
		 alaqilType_add_function(decl,$6);
		 if ($8.qualifier) {
		   alaqilType_push(decl,$8.qualifier);
		 }
		 Setattr($$,"refqualifier",$8.refqualifier);
		 Setattr($$,"decl",decl);
		 Setattr($$,"parms",$6);
		 ...
		 Setattr($$,"storage",$1);
		 decl = NewStringEmpty();
		 alaqilType_add_rvalue_reference(decl);
		 alaqilType_add_function(decl,$6);
		 if ($8.qualifier) {
		   alaqilType_push(decl,$8.qualifier);
		 }
		 ...
		 Setattr($$,"storage",$1);
		 decl = NewStringEmpty();
		 alaqilType_add_pointer(decl);
		 alaqilType_add_reference(decl);
		 alaqilType_add_function(decl,$7);
		 if ($9.qualifier) {
		   alaqilType_push(decl,$9.qualifier);
		 }
		...
		Setattr($$,"name",$2);
		 Setattr($$,"storage",$1);
		alaqilType_add_function(t,$5);
		if ($7.qualifier) {
		  alaqilType_push(t,$7.qualifier);
		}
...
cpp_alaqil_directive: pragma_directive { $$ = $1; }
...
valparm        : parm {
		  $$ = $1;
		  {
		    ...
		    while (!n) {
		      type = Getattr($1,"type");
		      n = alaqil_symbol_clookup(type,0);     /* See if we can find a node that matches the typename */
...
def_args       : EQUAL definetype { 
                  $$ = $2; 
		  if ($2.type == T_ERROR) {
		    alaqil_warning(WARN_PARSE_BAD_DEFAULT,cparse_file, cparse_line, "Can't set default argument (ignored)\n");
		    $$.val = 0;
		    $$.rawval = 0;
		    $$.bitfield = 0;
		    $$.throws = 0;
...
parameter_declarator : declarator def_args {
	    ...
	    | direct_declarator LPAREN parms RPAREN cv_ref_qualifier {
	      ...
	      $$ = $1;
	      t = NewStringEmpty();
	      alaqilType_add_function(t,$3);
	      if ($5.qualifier)
	        alaqilType_push(t, $5.qualifier);
	      if (!$$.have_parms) {
		$$.parms = $3;
		$$.have_parms = 1;
	      }
...
plain_declarator : declarator {
                 $$ = $1;
		 if (alaqilType_isfunction($1.type)) {
		   Delete(alaqilType_pop_function($1.type));
		 } else if (alaqilType_isarray($1.type)) {
		   alaqilType *ta = alaqilType_pop_arrays($1.type);
		   if (alaqilType_isfunction($1.type)) {
		     Delete(alaqilType_pop_function($1.type));
		   } else {
		     $$.parms = 0;
		   }
		   alaqilType_push($1.type,ta);
		   Delete(ta);
		 } else {
		   $$.parms = 0;
		 }
...
declarator :  pointer notso_direct_declarator {
		...
		Delete($$.type);
	      ...
	      $$.type = $1;
           ...
           | pointer LAND notso_direct_declarator {
              $$ = $3;
	      alaqilType_add_rvalue_reference($1);
              if ($$.type) {
		alaqilType_push($1,$$.type);
		Delete($$.type);
	      }
	     ...
	     $$ = $2;
	     $$.type = NewStringEmpty();
	     alaqilType_add_reference($$.type);
	     if ($2.type) {
	       alaqilType_push($$.type,$2.type);
	       Delete($2.type);
	     }
           }
	     ...
	     $$ = $2;
	     $$.type = NewStringEmpty();
	     alaqilType_add_rvalue_reference($$.type);
	     if ($2.type) {
	       alaqilType_push($$.type,$2.type);
	       Delete($2.type);
	     }
	     ...
	     alaqilType *t = NewStringEmpty();

	     $$ = $3;
	     alaqilType_add_memberpointer(t,$1);
	     if ($$.type) {
	       alaqilType_push(t,$$.type);
	       Delete($$.type);
	     }
	     ...
	     alaqilType *t = NewStringEmpty();
	     $$ = $4;
	     alaqilType_add_memberpointer(t,$2);
	     alaqilType_push($1,t);
	     if ($$.type) {
	       alaqilType_push($1,$$.type);
	       Delete($$.type);
	     ...
	     $$ = $5;
	     alaqilType_add_memberpointer($1,$2);
	     alaqilType_add_reference($1);
	     if ($$.type) {
	       alaqilType_push($1,$$.type);
	       Delete($$.type);
		...
		Delete($$.type);
	     ...
	     $$ = $7;
	     alaqilType_add_memberpointer(t,$1);
	     alaqilType_add_rvalue_reference(t);
	     if ($$.type) {
	       alaqilType_push(t,$$.type);
	       Delete($$.type);
	     } 
...
abstract_declarator : pointer {
		    ...
		    alaqilType_add_rvalue_reference($$.type);
                  ...
                  | idcolon DSTAR { 
		    $$.type = NewStringEmpty();
                    alaqilType_add_memberpointer($$.type,$1);
                    $$.id = 0;
                    $$.parms = 0;
		    $$.have_parms = 0;
      	          }
...
direct_abstract_declarator : direct_abstract_declarator LBRACKET RBRACKET { 
		    ...
		    $$.parms = 0;
		    $$.have_parms = 0;
		    alaqilType_add_array($$.type,$2.val);
		  }
		    ...
		    if (!$$.have_parms) {
		      ...
		      $$.have_parms = 1;
                  ...
                  | LPAREN parms RPAREN {
                    $$.type = NewStringEmpty();
                    alaqilType_add_function($$.type,$2);
		    $$.parms = $2;
		    $$.have_parms = 1;
		    $$.id = 0;
                  }
...
cv_ref_qualifier : type_qualifier {
		  ...
		  $$.qualifier = $1;
		  $$.refqualifier = $2;
		  alaqilType_push($$.qualifier, $2);
	       }
	       | ref_qualifier {
		  $$.qualifier = NewStringEmpty();
		  $$.refqualifier = $1;
		  alaqilType_push($$.qualifier, $1);
	       }
	       ;
...
type_qualifier : type_qualifier_raw {
	          $$ = NewStringEmpty();
	          if ($1) alaqilType_add_qualifier($$,$1);
               }
...
decltype       : DECLTYPE LPAREN idcolon RPAREN {
                 Node *n = alaqil_symbol_clookup($3,0);
                 if (!n) {
		   alaqil_error(cparse_file, cparse_line, "Identifier %s not defined.\n", $3);
                   $$ = $3;
                 } else {
                   $$ = Getattr(n, "type");
                 }
...
primitive_type_list : type_specifier { 
               ...
               | type_specifier primitive_type_list {
                    if ($1.us && $2.us) {
		      alaqil_error(cparse_file, cparse_line, "Extra %s specifier.\n", $2.us);
		    }
                    $$ = $2;
                    if ($1.us) $$.us = $1.us;
		    if ($1.type) {
...
edecl          :  identifier {
		   alaqilType *type = NewalaqilType(T_INT);
		   $$ = new_node("enumitem");
		   Setattr($$,"name",$1);
		   Setattr($$,"type",type);
		   SetFlag($$,"feature:immutable");
		   ...
		   alaqilType *type = NewalaqilType($3.type == T_BOOL ? T_BOOL : ($3.type == T_CHAR ? T_CHAR : T_INT));
		   $$ = new_node("enumitem");
		   Setattr($$,"name",$1);
		   Setattr($$,"type",type);
		   SetFlag($$,"feature:immutable");
...
etype            : expr {
		   ...
		   if (($$.type != T_INT) && ($$.type != T_UINT) &&
		     ...
		     alaqil_error(cparse_file,cparse_line,"Type error. Expecting an integral type\n");
		   }
                }
               ;
...
expr           : valexpr { $$ = $1; }
		 ...
		 n = alaqil_symbol_clookup($1,0);
		 if (n) {
                   /* A band-aid for enum values used in expressions. */
                   if (Strcmp(nodeType(n),"enumitem") == 0) {
                     String *q = alaqil_symbol_qualified(n);
                     if (q) {
...
valexpr        : exprnum {
		  ...
		  alaqilType_push($3,$4.type);
		  $$.val = NewStringf("sizeof(%s)",alaqilType_str($3,0));
		  $$.type = T_ULONG;
               }
               | SIZEOF PERIOD PERIOD PERIOD LPAREN type parameter_declarator RPAREN {
		  alaqilType_push($6,$7.type);
		  $$.val = NewStringf("sizeof...(%s)",alaqilType_str($6,0));
		  $$.type = T_ULONG;
               }
               | exprcompound { $$ = $1; }
	       | wstring {
...
/* A few common casting operations */
		 ...
		 if ($4.type != T_STRING) {
		   switch ($2.type) {
		     ...
		     case T_FLTCPLX:
		     case T_DBLCPLX:
		       $$.val = NewStringf("(%s)%s", $2.val, $4.val); /* alaqilType_str and decimal points don't mix! */
		       break;
		     default:
		       $$.val = NewStringf("(%s) %s", alaqilType_str($2.val,0), $4.val);
		       break;
		   }
		 }
		 $$.type = promote($2.type, $4.type);
               ...
               | LPAREN expr pointer RPAREN expr %prec CAST {
                 $$ = $5;
		 if ($5.type != T_STRING) {
		   alaqilType_push($2.val,$3);
		   $$.val = NewStringf("(%s) %s", alaqilType_str($2.val,0), $5.val);
		 }
 	       }
               | LPAREN expr AND RPAREN expr %prec CAST {
                 $$ = $5;
		 if ($5.type != T_STRING) {
		   alaqilType_add_reference($2.val);
		   $$.val = NewStringf("(%s) %s", alaqilType_str($2.val,0), $5.val);
		 }
 	       }
               | LPAREN expr LAND RPAREN expr %prec CAST {
                 $$ = $5;
		 if ($5.type != T_STRING) {
		   alaqilType_add_rvalue_reference($2.val);
		   $$.val = NewStringf("(%s) %s", alaqilType_str($2.val,0), $5.val);
		 }
 	       }
		 ...
		 if ($6.type != T_STRING) {
		   alaqilType_push($2.val,$3);
		   alaqilType_add_reference($2.val);
		   $$.val = NewStringf("(%s) %s", alaqilType_str($2.val,0), $6.val);
		 }
 	       }
...
*/
		 ...
		 String *qty;
                 skip_balanced('(',')');
		 qty = alaqil_symbol_type_qualify($1,0);
		 if (alaqilType_istemplate(qty)) {
		   String *nstr = alaqilType_namestr(qty);
		   Delete(qty);
		   qty = nstr;
		 }
...
base_specifier : opt_virtual {
		 ...
		 Setline($3,$<intvalue>2);
		   ...
		   alaqil_warning(WARN_PARSE_NO_ACCESS, Getfile($$), Getline($$), "No access specifier given for base class '%s' (ignored).\n", alaqilType_namestr($3));
                 } else {
		 ...
		 Setattr($$,"access",$2);
		   ...
		   alaqil_warning(WARN_PARSE_PRIVATE_INHERIT, Getfile($$), Getline($$), "%s inheritance from base '%s' (ignored).\n", $2, alaqilType_namestr($5));
		 }
		 if ($6)
		   SetFlag($$, "variadic");
               }
...
ctor_end       : cpp_const ctor_initializer SEMI { 
		    ...
		    $$.final = $1.final;
                    if ($1.qualifier)
                      alaqil_error(cparse_file, cparse_line, "Constructor cannot have a qualifier.\n");
               }
...
identifier     : ID { $$ = $1; }
	       | OVERRIDE { $$ = alaqil_copy_string("override"); }
	       | FINAL { $$ = alaqil_copy_string("final"); }
	       ;
...
alaqilType *alaqil_cparse_type(String *s) {
   String *ns;
   ns = NewStringf("%s;",s);
   Seek(ns,0,SEEK_SET);
   scanner_file(ns);
...
Parm *alaqil_cparse_parm(String *s) {
   String *ns;
   ns = NewStringf("%s;",s);
   Seek(ns,0,SEEK_SET);
   scanner_file(ns);
...
ParmList *alaqil_cparse_parms(String *s, Node *file_line_node) {
   String *ns;
   char *cs = Char(s);
   if (cs && cs[0] != '(') {
     ns = NewStringf("(%s);",s);
