 ...
 * scanner.c
 *
 * alaqil tokenizer.  This file is a wrapper around the generic C scanner
 * found in alaqil/scanner.c.   Extra logic is added both to accommodate the
 ...
 * C identifiers up into keywords and alaqil directives.
 * ----------------------------------------------------------------------------- */
...
/* -----------------------------------------------------------------------------
 * alaqil_cparse_cplusplus()
 * ----------------------------------------------------------------------------- */

void alaqil_cparse_cplusplus(int v) {
  cparse_cplusplus = v;
}

/* -----------------------------------------------------------------------------
 * alaqil_cparse_cplusplusout()
 * ----------------------------------------------------------------------------- */

void alaqil_cparse_cplusplusout(int v) {
...
void skip_balanced(int startchar, int endchar) {
  ...
  if (Scanner_skip_balanced(scan,startchar,endchar) < 0) {
    alaqil_error(cparse_file, start_line, "Missing '%c'. Reached end of input.\n", endchar);
    return;
  }
...
void skip_decl(void) {
  ...
  while (!done) {
	...
	alaqil_error(cparse_file, start_line, "Missing semicolon. Reached end of input.\n");
      }
      return;
    }
    if (tok == alaqil_TOKEN_LBRACE) {
      if (Scanner_skip_balanced(scan,'{','}') < 0) {
	alaqil_error(cparse_file, start_line, "Missing '}'. Reached end of input.\n");
      }
      break;
    }
    if (tok == alaqil_TOKEN_SEMI) {
      done = 1;
    }
  }
  cparse_file = Scanner_file(scan);
...
static int yylook(void) {
  ...
  while (1) {
    ...
    switch(tok) {
    case alaqil_TOKEN_ID:
      return ID;
    case alaqil_TOKEN_LPAREN: 
      return LPAREN;
    case alaqil_TOKEN_RPAREN: 
      return RPAREN;
    case alaqil_TOKEN_SEMI:
      return SEMI;
    case alaqil_TOKEN_COMMA:
      return COMMA;
    case alaqil_TOKEN_STAR:
      return STAR;
    case alaqil_TOKEN_RBRACE:
      num_brace--;
      if (num_brace < 0) {
	...
	return RBRACE;
      ...
      break;
    case alaqil_TOKEN_LBRACE:
      last_brace = num_brace;
      num_brace++;
      return LBRACE;
    case alaqil_TOKEN_EQUAL:
      return EQUAL;
    case alaqil_TOKEN_EQUALTO:
      return EQUALTO;
    case alaqil_TOKEN_PLUS:
      return PLUS;
    case alaqil_TOKEN_MINUS:
      return MINUS;
    case alaqil_TOKEN_SLASH:
      return SLASH;
    case alaqil_TOKEN_AND:
      return AND;
    case alaqil_TOKEN_LAND:
      return LAND;
    case alaqil_TOKEN_OR:
      return OR;
    case alaqil_TOKEN_LOR:
      return LOR;
    case alaqil_TOKEN_XOR:
      return XOR;
    case alaqil_TOKEN_NOT:
      return NOT;
    case alaqil_TOKEN_LNOT:
      return LNOT;
    case alaqil_TOKEN_NOTEQUAL:
      return NOTEQUALTO;
    case alaqil_TOKEN_LBRACKET:
      return LBRACKET;
    case alaqil_TOKEN_RBRACKET:
      return RBRACKET;
    case alaqil_TOKEN_QUESTION:
      return QUESTIONMARK;
    case alaqil_TOKEN_LESSTHAN:
      return LESSTHAN;
    case alaqil_TOKEN_LTEQUAL:
      return LESSTHANOREQUALTO;
    case alaqil_TOKEN_LSHIFT:
      return LSHIFT;
    case alaqil_TOKEN_GREATERTHAN:
      return GREATERTHAN;
    case alaqil_TOKEN_GTEQUAL:
      return GREATERTHANOREQUALTO;
    case alaqil_TOKEN_RSHIFT:
      return RSHIFT;
    case alaqil_TOKEN_ARROW:
      return ARROW;
    case alaqil_TOKEN_PERIOD:
      return PERIOD;
    case alaqil_TOKEN_MODULO:
      return MODULO;
    case alaqil_TOKEN_COLON:
      return COLON;
    case alaqil_TOKEN_DCOLONSTAR:
      return DSTAR;
      
    case alaqil_TOKEN_DCOLON:
      {
	int nexttok = Scanner_token(scan);
	if (nexttok == alaqil_TOKEN_STAR) {
	  return DSTAR;
	} else if (nexttok == alaqil_TOKEN_NOT) {
	  return DCNOT;
	} else {
    ...
    case alaqil_TOKEN_RSTRING:
      yylval.type = NewString(Scanner_text(scan));
      return TYPE_RAW;
      
    case alaqil_TOKEN_STRING:
      yylval.id = alaqil_copy_string(Char(Scanner_text(scan)));
      return STRING;

    case alaqil_TOKEN_WSTRING:
      yylval.id = alaqil_copy_string(Char(Scanner_text(scan)));
      return WSTRING;
      
    case alaqil_TOKEN_CHAR:
      yylval.str = NewString(Scanner_text(scan));
      if (Len(yylval.str) == 0) {
	alaqil_error(cparse_file, cparse_line, "Empty character constant\n");
      }
      return CHARCONST;

    case alaqil_TOKEN_WCHAR:
      yylval.str = NewString(Scanner_text(scan));
      if (Len(yylval.str) == 0) {
	alaqil_error(cparse_file, cparse_line, "Empty character constant\n");
      }
    ...
    case alaqil_TOKEN_INT:
      return NUM_INT;
      
    case alaqil_TOKEN_UINT:
      return NUM_UNSIGNED;
      
    case alaqil_TOKEN_LONG:
      return NUM_LONG;
      
    case alaqil_TOKEN_ULONG:
      return NUM_ULONG;
      
    case alaqil_TOKEN_LONGLONG:
      return NUM_LONGLONG;
      
    case alaqil_TOKEN_ULONGLONG:
      return NUM_ULONGLONG;
      
    case alaqil_TOKEN_DOUBLE:
    case alaqil_TOKEN_FLOAT:
      return NUM_FLOAT;
      
    case alaqil_TOKEN_BOOL:
      return NUM_BOOL;
      
    case alaqil_TOKEN_POUND:
      Scanner_skip_line(scan);
      yylval.id = alaqil_copy_string(Char(Scanner_text(scan)));
      return POUND;
      break;
      
    case alaqil_TOKEN_CODEBLOCK:
      yylval.str = NewString(Scanner_text(scan));
      return HBLOCK;
      
    case alaqil_TOKEN_COMMENT:
      {
	typedef enum {
	  DOX_COMMENT_PRE = -1,
	  DOX_COMMENT_NONE,
	...
	do {
	  ...
	  char *loc = Char(cmt);
	  if ((strncmp(loc, "/*@alaqil", 7) == 0) && (loc[Len(cmt)-3] == '@')) {
	  ...
	  do {
	    tok = Scanner_token(scan);
	  } while (tok == alaqil_TOKEN_ENDLINE);
	} while (tok == alaqil_TOKEN_COMMENT);
	...
	switch (existing_comment) {
      ...
      break;
    case alaqil_TOKEN_ENDLINE:
      break;
    case alaqil_TOKEN_BACKSLASH:
      break;
    default:
      alaqil_error(cparse_file, cparse_line, "Illegal token '%s'.\n", Scanner_text(scan));
      return (ILLEGAL);
    }
  }
}
...
int yylex(void) {
  ...
  /*   alaqil_diagnostic(cparse_file, cparse_line, ":::%d: '%s'\n", l, Scanner_text(scan)); */
  ...
  case ID:
	...
	yylval.type = NewalaqilType(T_INT);
	return (TYPE_INT);
      }
      if (strcmp(yytext, "double") == 0) {
	yylval.type = NewalaqilType(T_DOUBLE);
	return (TYPE_DOUBLE);
      }
      if (strcmp(yytext, "void") == 0) {
	yylval.type = NewalaqilType(T_VOID);
	return (TYPE_VOID);
      }
      if (strcmp(yytext, "char") == 0) {
	yylval.type = NewalaqilType(T_CHAR);
	return (TYPE_CHAR);
      }
      if (strcmp(yytext, "wchar_t") == 0) {
	yylval.type = NewalaqilType(T_WCHAR);
	return (TYPE_WCHAR);
      }
      if (strcmp(yytext, "short") == 0) {
	yylval.type = NewalaqilType(T_SHORT);
	return (TYPE_SHORT);
      }
      if (strcmp(yytext, "long") == 0) {
	yylval.type = NewalaqilType(T_LONG);
	return (TYPE_LONG);
      }
      if (strcmp(yytext, "float") == 0) {
	yylval.type = NewalaqilType(T_FLOAT);
	return (TYPE_FLOAT);
      }
	...
	yylval.type = NewalaqilType(T_UINT);
	return (TYPE_UNSIGNED);
      }
      if (strcmp(yytext, "bool") == 0) {
	yylval.type = NewalaqilType(T_BOOL);
	return (TYPE_BOOL);
      }
	...
	if (strcmp(yytext, "operator") == 0) {
	  ...
	  */
	  ...
	  } while (nexttok == alaqil_TOKEN_ENDLINE || nexttok == alaqil_TOKEN_COMMENT);
	  ...
	  if (Scanner_isoperator(nexttok)) {
	    ...
	    yylval.str = s;
	    return OPERATOR;
	  } else if (nexttok == alaqil_TOKEN_LPAREN) {
	    /* Function call operator.  The next token MUST be a RPAREN */
	    nexttok = Scanner_token(scan);
	    if (nexttok != alaqil_TOKEN_RPAREN) {
	      alaqil_error(Scanner_file(scan),Scanner_line(scan),"Syntax error. Bad operator name.\n");
	    } else {
	      Append(s,"()");
	      yylval.str = s;
	      return OPERATOR;
	    }
	  } else if (nexttok == alaqil_TOKEN_LBRACKET) {
	    /* Array access operator.  The next token MUST be a RBRACKET */
	    nexttok = Scanner_token(scan);
	    if (nexttok != alaqil_TOKEN_RBRACKET) {
	      alaqil_error(Scanner_file(scan),Scanner_line(scan),"Syntax error. Bad operator name.\n");	      
	    } else {
	      Append(s,"[]");
	      yylval.str = s;
	  ...
	  } else if (nexttok == alaqil_TOKEN_STRING) {
	    /* Operator "" or user-defined string literal ""_suffix */
	    Append(s,"\"\"");
	    yylval.str = s;
	    return OPERATOR;
	  } else if (nexttok == alaqil_TOKEN_ID) {
		...
		alaqil_error(Scanner_file(scan),Scanner_line(scan),"Syntax error. Bad operator name.\n");	      
	      ...
	      if (nexttok == alaqil_TOKEN_LPAREN) {
		termtoken = alaqil_TOKEN_LPAREN;
		termvalue = "(";
		break;
              } else if (nexttok == alaqil_TOKEN_CODEBLOCK) {
                termtoken = alaqil_TOKEN_CODEBLOCK;
                termvalue = Char(Scanner_text(scan));
                break;
              } else if (nexttok == alaqil_TOKEN_LBRACE) {
                termtoken = alaqil_TOKEN_LBRACE;
                termvalue = "{";
                break;
              } else if (nexttok == alaqil_TOKEN_SEMI) {
		termtoken = alaqil_TOKEN_SEMI;
		termvalue = ";";
		break;
              } else if (nexttok == alaqil_TOKEN_STRING) {
		termtoken = alaqil_TOKEN_STRING;
                termvalue = alaqil_copy_string(Char(Scanner_text(scan)));
		break;
	      } else if (nexttok == alaqil_TOKEN_ID) {
		if (needspace) {
		  Append(s," ");
		...
		Append(s,Scanner_text(scan));
	      } else if (nexttok == alaqil_TOKEN_ENDLINE) {
	      } else if (nexttok == alaqil_TOKEN_COMMENT) {
	      } else {
		Append(s,Scanner_text(scan));
		needspace = 0;
	      }
	...
	if (strcmp(yytext, "class") == 0) {
	  alaqil_warning(WARN_PARSE_CLASS_KEYWORD, cparse_file, cparse_line, "class keyword used, but not in C++ mode.\n");
	}
	if (strcmp(yytext, "complex") == 0) {
	  yylval.type = NewalaqilType(T_COMPLEX);
	  return (TYPE_COMPLEX);
	}
	...
	alaqil_warning(WARN_DEPRECATED_VAL, cparse_file, cparse_line, "%%val directive deprecated (ignored).\n");
	return (yylex());
      }
      if (strcmp(yytext, "%out") == 0) {
	alaqil_warning(WARN_DEPRECATED_OUT, cparse_file, cparse_line, "%%out directive deprecated (ignored).\n");
	return (yylex());
      }
    ...
    /* Need to fix this */
    if (check_typedef) {
      if (alaqilType_istypedef(yytext)) {
	yylval.type = NewString(yytext);
	return (TYPE_TYPEDEF);
      }
    }
    yylval.id = alaqil_copy_string(yytext);
    last_id = 1;
    return (ID);
  case POUND:
    return yylex();
